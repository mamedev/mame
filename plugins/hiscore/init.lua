...
function hiscore.startplugin()
    ...
    local function read_config()
        ...
        if file then
            local parsed_settings = json.decode(file:read('*all'))
            file:close()
    ...
    local function parse_table(dsting)
        ...
        for line in string.gmatch(dsting, '([^\n]+)') do
            ...
            if delay and #delay > 0 then
                ...
                if not cpu then error(cputag .. " device not found") end
                ...
                if not mem then error(space .. " space not found") end
                _table[#_table + 1] = {
                    mem = mem,
            ...
            repeat
                ...
                if line then
                    ...
                    elseif string.find(line, '^[a-z0-9_,]+:') then --- some game
                        if current_is_match and #cluster > 0 then
                            break -- we're done
                        end
    ...
    local function check_mem(posdata)
        if #posdata < 1 then return false end
        for _, row in ipairs(posdata) do
			-- must pass mem check
            if row.c_start ~= row.mem:read_u8(row.addr) then return false end
            if row.c_end ~= row.mem:read_u8(row.addr + row.size - 1) then return false end
        end
        return true
    end
    ...
    local function write_scores(posdata)
		-- check if game is on the exclude list
    ...
    reset_subscription = emu.add_machine_reset_notifier(function()
        ...
        if dat and dat ~= "" then
            ...
            local res, pos = pcall(parse_table, dat)
            if not res then
                emu.print_error("hiscore: hiscore.dat parse error " .. pos)
                return
            end
            positions = pos
            for _, row in pairs(positions) do
                if row.fill then
                    for i=0, row.size - 1 do

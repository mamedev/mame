...
class UnidasmTests(SelfExeTests):
    ...
    For unidasm the strategy is to point at the asm/asm_bin folder and
    derive unit tests from there.
    To compose a new test for a given arch:
    
    1. craft a new binary file and save it in asm_bin subfolder, name it <test_arch>.bin,
    ...
    2. run unidasm $(regtests)/(asm_bin)/<test_arch>.bin -arch <test_arch> > $(regtests)/(asm)/<test_arch>.asm
    ...
    def _collect_tests(self):
        ...
        return {
            __asm.split(".")[0]: UnidasmFileMapper(asm_filename=__asm, bin_filename=__bin) for __asm, __bin in __static_tests
        }

    def _subprocess_args(self, test_handle: str, test_params: UnidasmFileMapper):
        return [os.path.join(self._bin_test_folder, test_params.bin_filename), "-arch", test_handle]

    def _assert_test_result(self, launch_result: CompletedProcess, test_handle: str, test_params: UnidasmFileMapper):

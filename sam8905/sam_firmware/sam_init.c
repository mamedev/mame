 ...
 *
 * Computes 128-note pitch frequency table.
 * Three 128-byte banks form 24-bit phase increment per MIDI note.
 *
 * Note: This is a simplified implementation. The original uses
 * fixed-point math with successive octave divisions.
 *
 * Original approach:
 * - Start with base frequency constant for note 0
 * - For each octave, divide by 2 (right shift)
 * - Within octave, interpolate or use pre-computed ratios
 *============================================================================*/

void sam_pitch_table_init(void)
{
    /*
     * TODO: Port exact computation from firmware.
     *
     * For now, this is a placeholder that should be filled
     * by either:
     * 1. Porting the exact algorithm from CODE:9B16
     * 2. Dumping the computed values from MAME and using as lookup
     *
     * The pitch values are 24-bit phase increments for the SAM8905
     * DDS oscillators. Format: (hi << 16) | (mid << 8) | lo
     */

    uint8_t note;

    /* Placeholder: clear tables */
    for (note = 0; note < 128; note++) {
        g_extmem.pitch_table_lo[note] = 0x00;
        g_extmem.pitch_table_mid[note] = 0x00;
        g_extmem.pitch_table_hi[note] = 0x00;
    }

    /* TODO: Implement actual pitch computation */
}

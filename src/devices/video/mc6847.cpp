...
ROM_START( mc6847_rom ) // extracted from decap imagery
...
mc6847_friend_device::mc6847_friend_device(const machine_config &mconfig, device_type type, const char *tag, device_t *owner, uint32_t clock,
	...
	, m_font_rom_region(*this, "chargen")
	, m_write_hsync(*this)
	, m_write_fsync(*this)
	, m_charrom_cb(*this)
...
void mc6847_friend_device::device_start()
{
	...
	/* save states */
	save_item(NAME(m_top_border_scanlines));
	save_item(NAME(m_body_scanlines));
	save_item(NAME(m_wide));
	save_item(NAME(m_recording_scanline));
...
mc6847_base_device::mc6847_base_device(const machine_config &mconfig, device_type type, const char *tag, device_t *owner, uint32_t clock, double tpfs, bool pal)
	: mc6847_friend_device(mconfig, type, tag, owner, clock, type == MC6847T1, tpfs, LINES_TOP_BORDER + LINES_ACTIVE_VIDEO - 1, 1, true, pal)
	, m_input_cb(*this, 0)
...
mc6847_device::mc6847_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock, bool pal)
	: mc6847_base_device(mconfig, MC6847, tag, owner, clock, pal ? 312.0 : 262.0, pal)
{
...
mc6847y_device::mc6847y_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock, bool pal)
	: mc6847_base_device(mconfig, MC6847Y, tag, owner, clock, pal ? 312.0 : 262.5, pal)
{
	m_artifacter.set_pal_artifacting(pal);
}
...
const tiny_rom_entry *mc6847t1_device::device_rom_region() const
{
	return ROM_NAME( mc6847t1_rom );
}
...
m5c6847p1_device::m5c6847p1_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock, bool pal)
	: mc6847_base_device(mconfig, M5C6847P1, tag, owner, clock, pal ? 312.0 : 262.5, pal)
{
	m_artifacter.set_pal_artifacting(pal);
}

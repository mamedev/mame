...
void sis630_svga_device::seq_reg_write(uint8_t index, uint8_t data)
{
	...
	else
	{
		...
		else
		{
			if (!m_unlock_reg)
			{
				LOG("Attempt to write to extended SVGA while locked [$%02x] -> %02x\n", index, data);
				return;
			}
			...
			switch(index)
			{
				...
				case 0x06:
					...
					break;
				case 0x07:
					LOG("Extended Misc. Control register 0 (%02x) %02x\n", index, data);
					m_ext_misc_ctrl_0 = data;
					std::tie(svga.rgb24_en, svga.rgb32_en) = flush_true_color_mode();
					break;
				case 0x0a:
					LOG("Extended vertical Overflow register (%02x) %02x\n", index, data);
				...
				case 0x0b:
					...
					LOG("Extended horizontal Overflow 1 (%02x) %02x\n", index, data);
					...
					break;
				case 0x0c:
					LOG("Extended horizontal Overflow 2 (%02x) %02x\n", index, data);
					...
					break;
				case 0x0d:
					LOG("Extended starting address register (%02x) %02x\n", index, data);
					vga.crtc.start_addr_latch &= ~0xff0000;
					vga.crtc.start_addr_latch |= data << 16;
					break;
				case 0x0e:
					LOG("Extended pitch register (%02x) %02x\n", index, data);

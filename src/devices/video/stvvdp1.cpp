...
static constexpr inline u8 RGB_R(u16 color){ return color & 0x1f; }
static constexpr inline u8 RGB_G(u16 color){ return (color >> 5) & 0x1f; }
static constexpr inline u8 RGB_B(u16 color){ return (color >> 10) & 0x1f; }
...
{
//  int mode;
	bool const mesh = stv2_current_sprite.CMDPMOD & 0x100, spd = stv2_current_sprite.CMDPMOD & 0x40;
	u16 pix = 0, raw = 0, transpen = 0, endcode = 0xffff;

	if (((stv2_current_sprite.CMDPMOD & 0x600) == 0x600) &&
		(((x >= m_vdp1.user_cliprect.left() && x <= m_vdp1.user_cliprect.right())) ||
		((y >= m_vdp1.user_cliprect.top() && y <= m_vdp1.user_cliprect.bottom())))) // Outside draw mode
		return;

	/*
	if ((stv2_current_sprite.CMDPMOD & 0x600) == 0x200) // Invaild setting, Currently ignored
		return
	*/
	...
	if (stv2_current_sprite.ispoly)
	{
		...
		transpen = 0xffff; /* TODO : Isn't transparent? */
		#if 0
		if (pix & 0x8000)
		{
			mode = 5;
	...
	else
	{
		switch (stv2_current_sprite.CMDPMOD&0x0038)
		{
			...
			case 0x0010: // mode 2 64 colour bank mode (8bits) (character select portraits on hanagumi)
				raw = m_vdp1.gfx_decode[(patterndata+offsetcnt) & 0xfffff];
				//mode = 2;
				pix = (raw & 0x3f)+(stv2_current_sprite.CMDCOLR&0xffc0);
			...
			case 0x0018: // mode 3 128 colour bank mode (8bits) (little characters on hanagumi use this mode)
				raw = m_vdp1.gfx_decode[(patterndata+offsetcnt) & 0xfffff];
				pix = (raw & 0x7f)+(stv2_current_sprite.CMDCOLR&0xff80);
				endcode = 0xff;
				//mode = 3;
				break;
...
void saturn_state::stv_vdp1_set_drawpixel(void)
{
	...
	bool const cmod = stv2_current_sprite.CMDPMOD & 0x200;

	if (cmod || mesh || !ecd || ((stv2_current_sprite.CMDPMOD & 0x7) != 0))
	{
...
void saturn_state::stv_vdp1_process_list(void)
{
	...
	// TODO: sprite / polygon limit is related to texture size / special functions?
	while (spritecount < 16384) // Max 16384 single color polygon, Max 16383 Texutre-mapped polygon or sprites (per frame)
	{
...
//              case 0x000b: // mirror? Bug 2
	...
	// if spritecount = 16384 don't send a vdp1 draw end
//  if (spritecount < 16384)

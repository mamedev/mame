...
//  int mode;
	...
	u16 raw = 0, transpen = 0, endcode = 0xffff;
	...
	if ( stv2_current_sprite.ispoly )
	{
		...
		transpen = 0xffff; // TODO : Isn't transparented?
		#if 0
		if ( pix & 0x8000 )
		{
			mode = 5;
	...
	else
	{
		switch (stv2_current_sprite.CMDPMOD&0x0038)
		{
			...
			case 0x0028: // mode 5 32,768 colour RGB mode (16bits)
				...
				pix = raw; // invaild when MSB is offed?
				//mode = 5;
				endcode = 0x7fff;
				break;
			case 0x0038: // invalid
				...
				raw = m_vdp1.gfx_decode[1] | (m_vdp1.gfx_decode[0]<<8);
				pix = raw; // invaild when MSB is offed?
				endcode = 0x7fff;
				break;
			default: // other settings illegal
				raw = pix = machine().rand();
				//mode = 0;
				endcode = 0xff;
		...
		if (((stv2_current_sprite.CMDPMOD & 0x80) == 0) && (raw == endcode))
			return;

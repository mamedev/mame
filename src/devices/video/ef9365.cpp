...
void ef9365_device::device_timer(timer_instance const &timer, device_timer_id id, int param, void *ptr)
{
	switch(id)
	{
		case BUSY_TIMER:
...
void ef9365_device::set_video_mode(void)
{
	...
	if (screen().width() != new_width)
	{
		...
		screen().configure(new_width, screen().height(), visarea, screen().frame_period().as_subseconds());
	}

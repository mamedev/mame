...
void v99x8_device::configure_pal_ntsc()
{
	if (m_pal_ntsc)
	{
		...
		screen().configure(HTOTAL, VTOTAL_PAL * 2, visible, attotime::from_hz(50.158974));
	}
	else
	{
		// NTSC
		...
		screen().configure(HTOTAL, VTOTAL_NTSC * 2, visible, attotime::from_hz(59.922743));
	}
...
void v99x8_device::mode_graphic23(uint32_t *ln, int line)
{
	...
	int pixels_to_mask = BIT(m_cont_reg[25], 1) ? 8 - dot_scroll : 0;
	...
	do
	{
		...
		if (BIT(m_cont_reg[25], 0) && !nametbl_offset)
			nametbl_base ^= 0x8000;
	}

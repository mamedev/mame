...
uint8_t h8s2357_device::syscr_r(address_space &space, offs_t offset, uint8_t mem_mask)
{
	return syscr;
}

void h8s2357_device::syscr_w(address_space &space, offs_t offset, uint8_t data, uint8_t mem_mask)
{
	syscr = data;
	update_irq_filter();
	logerror("syscr = %02x\n", data);

 ...
 */
std::string address (offs_t pc, const device_disasm_interface::data_buffer &opcodes)
{
	switch (ADDR_MODE)
	{
...
CPU_DISASSEMBLE(clipper)
{
	...
	case 0x12: util::stream_format(stream, "calls   $%d", opcodes.r16(pc) & 0x7F); bytes = 2; flags |= DASMFLAG_STEP_OVER; break;
	...
	case 0x45: util::stream_format(stream, "call    r%d,%s", ADDR_R2, address(pc, opcodes)); bytes = 2 + ADDR_SIZE; flags |= DASMFLAG_STEP_OVER; break;
	...
	case 0xb7:
		...
		case 0x04: util::stream_format(stream, "reti    r%d", (opcodes.r16(pc+2) & 0xf0) >> 4); flags |= DASMFLAG_STEP_OUT; break;

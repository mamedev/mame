...
{
	uint8_t res = m_program.read_interruptible(m_base->adr_in_bank_i(adr));
	if(adr == 0x0000)
		res = m_base->bank_i_r();
	else if(adr == 0x0001)
		res = m_base->bank_y_r();
...
{
	uint8_t res = m_csprogram.read_interruptible(m_base->adr_in_bank_i(adr));
	if(adr == 0x0000)
		res = m_base->bank_i_r();
	else if(adr == 0x0001)
		res = m_base->bank_y_r();
...
{
	uint8_t res = m_cprogram.read_interruptible(m_base->adr_in_bank_i(adr));
	if(adr == 0x0000)
		res = m_base->bank_i_r();
	else if(adr == 0x0001)
		res = m_base->bank_y_r();
...
{
	uint8_t res = m_program.read_interruptible(m_base->adr_in_bank_y(adr));
	if(adr == 0x0000)
		res = m_base->bank_i_r();
	else if(adr == 0x0001)
		res = m_base->bank_y_r();
...
{
	m_program.write_interruptible(m_base->adr_in_bank_i(adr), val);
	if(adr == 0x0000)
		m_base->bank_i_w(val);
	else if(adr == 0x0001)
		m_base->bank_y_w(val);
...
{
	m_program.write_interruptible(m_base->adr_in_bank_y(adr), val);
	if(adr == 0x0000)
		m_base->bank_i_w(val);
	else if(adr == 0x0001)
		m_base->bank_y_w(val);

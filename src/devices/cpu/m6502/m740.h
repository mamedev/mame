...
#define O(o) void o ## _full(); void o ## _partial()
	...
	O(adc_m_aba); O(adc_m_abx); O(adc_m_aby); O(adc_m_idx); O(adc_m_idy); O(adc_m_zpx);
	O(and_m_aba); O(and_m_abx); O(and_m_aby); O(and_m_idx); O(and_m_idy); O(and_m_zpx);
	...
	O(bvc_m_rel);
	O(bvs_m_rel);
	O(cmp_m_aba); O(cmp_m_abx); O(cmp_m_aby); O(cmp_m_idx); O(cmp_m_idy); O(cmp_m_zpx);
	O(cpx_m_aba);
	O(cpy_m_aba);
	O(dec_m_aba); O(dec_m_abx); O(dec_m_zpg); O(dec_m_zpx);
	O(eor_m_aba); O(eor_m_abx); O(eor_m_aby); O(eor_m_idx); O(eor_m_idy); O(eor_m_zpx);
	O(inc_m_aba); O(inc_m_abx); O(inc_m_zpg); O(inc_m_zpx);
	O(jmp_m_ind);
	O(jsr_m_adr);
	O(lda_m_aba); O(lda_m_abx); O(lda_m_aby); O(lda_m_idx); O(lda_m_idy); O(lda_m_zpx);
	O(ldx_m_aba); O(ldx_m_aby); O(ldx_m_zpg); O(ldx_m_zpy);
	O(ldy_m_aba); O(ldy_m_abx); O(ldy_m_zpg); O(ldy_m_zpx);
	O(lsr_m_aba); O(lsr_m_abx); O(lsr_m_zpg); O(lsr_m_zpx);
	O(ora_m_aba); O(ora_m_abx); O(ora_m_aby); O(ora_m_idx); O(ora_m_idy); O(ora_m_zpx);
	O(pla_m_imp);
	O(plp_m_imp);
	...
	O(rti_m_imp);
	O(rts_m_imp);
	O(sbc_m_aba); O(sbc_m_abx); O(sbc_m_aby); O(sbc_m_idx); O(sbc_m_idy); O(sbc_m_zpx);

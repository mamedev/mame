...
static offs_t immediate(offs_t &pc, const device_disasm_interface::data_buffer &params)
{
	return params.r8(pc++);
}

static offs_t short_branch(offs_t base_pc, offs_t &pc, const device_disasm_interface::data_buffer &params)
{
	return (base_pc & 0xff00) | immediate(pc, params);
}

static offs_t long_branch(offs_t &pc, const device_disasm_interface::data_buffer &params)
{
...
static uint32_t disassemble(device_t *device, std::ostream &stream, offs_t pc, const device_disasm_interface::data_buffer &opcodes, const device_disasm_interface::data_buffer &params, uint32_t variant)
{
	offs_t base_pc = pc;
	uint32_t flags = 0;
	...
	return (pc - base_pc) | flags | DASMFLAG_SUPPORTED;
}


CPU_DISASSEMBLE( cdp1801 )
{
	return disassemble(device, stream, pc, opcodes, params, TYPE_1801);
}


CPU_DISASSEMBLE( cdp1802 )
{
	return disassemble(device, stream, pc, opcodes, params, TYPE_1802);
}

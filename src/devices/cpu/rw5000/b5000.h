...
{
public:
	b5000_cpu_device(const machine_config &mconfig, const char *tag, device_t *owner, const XTAL &clock);

protected:
	b5000_cpu_device(const machine_config &mconfig, device_type type, const char *tag, device_t *owner, const XTAL &clock, int prgwidth, address_map_constructor program, int datawidth, address_map_constructor data);

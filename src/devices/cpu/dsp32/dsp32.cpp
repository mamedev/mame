...
uint32_t dsp32c_device::opcode_alignment() const
{
	return 4;
}
...
offs_t dsp32c_device::disassemble(std::ostream &stream, offs_t pc, const data_buffer &opcodes, const data_buffer &params, uint32_t options)
{
	extern CPU_DISASSEMBLE( dsp32c );
	return CPU_DISASSEMBLE_NAME(dsp32c)(this, stream, pc, opcodes, params, options);
}

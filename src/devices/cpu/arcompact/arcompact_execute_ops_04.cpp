...
uint32_t arcompact_device::handleop32_ADD_do_op(uint32_t src1, uint32_t src2, uint8_t set_flags)
{
...
uint32_t arcompact_device::handleop32_SUB_do_op(uint32_t src1, uint32_t src2, uint8_t set_flags)
{
	uint32_t result = src1 - src2;
	if (set_flags)
...
uint32_t arcompact_device::handleop32_AND_do_op(uint32_t src1, uint32_t src2, uint8_t set_flags)
{
	uint32_t result = src1 & src2;
	if (set_flags)
		do_flags_nz(result);
...
uint32_t arcompact_device::handleop32_OR_do_op(uint32_t src1, uint32_t src2, uint8_t set_flags)
{
	uint32_t result = src1 | src2;
	if (set_flags)
...
uint32_t arcompact_device::handleop32_BIC_do_op(uint32_t src1, uint32_t src2, uint8_t set_flags)
{
	uint32_t result = src1 & (~src2);
	if (set_flags)
		do_flags_nz(result);
...
uint32_t arcompact_device::handleop32_XOR_do_op(uint32_t src1, uint32_t src2, uint8_t set_flags)
{
	uint32_t result = src1 ^ src2;
	if (set_flags)
...
void arcompact_device::handleop32_TST_do_op(uint32_t src1, uint32_t src2)
{
	uint32_t result = src1 & src2;
	do_flags_nz(result);
}
...
void arcompact_device::handleop32_CMP_do_op(uint32_t src1, uint32_t src2)
{
	uint32_t result = src1 - src2;
	do_flags_sub(result, src1, src2);
}
...
uint32_t arcompact_device::handleop32_RSUB_do_op(uint32_t src1, uint32_t src2, uint8_t set_flags)
{
...
uint32_t arcompact_device::handleop32_RSUB_f_a_b_c(uint32_t op)
{
	...
	uint8_t creg = common32_get_creg(op);
	int size = check_b_c_limm(breg, creg);
	m_regs[common32_get_areg(op)] = handleop32_RSUB_do_op(m_regs[breg], m_regs[creg], common32_get_F(op));
	return m_pc + size;
}
...
uint32_t arcompact_device::handleop32_RSUB_f_a_b_u6(uint32_t op)
{
	uint8_t breg = common32_get_breg(op);
	int size = check_b_limm(breg);
	m_regs[common32_get_areg(op)] = handleop32_RSUB_do_op(m_regs[breg], common32_get_u6(op), common32_get_F(op));
	return m_pc + size;
}
...
uint32_t arcompact_device::handleop32_RSUB_f_b_b_s12(uint32_t op)
{
	uint8_t breg = common32_get_breg(op);
	int size = check_b_limm(breg);
	m_regs[breg] = handleop32_RSUB_do_op(m_regs[breg], common32_get_s12(op), common32_get_F(op));
	return m_pc + size;
}
...
uint32_t arcompact_device::handleop32_RSUB_cc_f_b_b_c(uint32_t op)
{
	...
	int size = check_b_c_limm(breg, creg);
	if (check_condition(common32_get_condition(op)))
		m_regs[breg] = handleop32_RSUB_do_op(m_regs[breg], m_regs[creg], common32_get_F(op));
	return m_pc + size;
}
...
uint32_t arcompact_device::handleop32_RSUB_cc_f_b_b_u6(uint32_t op)
{
	...
	int size = check_b_limm(breg);
	if (check_condition(common32_get_condition(op)))
		m_regs[breg] = handleop32_RSUB_do_op(m_regs[breg], common32_get_u6(op), common32_get_F(op));
	return m_pc + size;
}
...
uint32_t arcompact_device::handleop32_BSET_do_op(uint32_t src1, uint32_t src2, uint8_t set_flags)
{
...
uint32_t arcompact_device::handleop32_BSET_f_a_b_c(uint32_t op)
{
	...
	uint8_t creg = common32_get_creg(op);
	int size = check_b_c_limm(breg, creg);
	m_regs[common32_get_areg(op)] = handleop32_BSET_do_op(m_regs[breg], m_regs[creg], common32_get_F(op));
	return m_pc + size;
}
...
uint32_t arcompact_device::handleop32_BSET_f_a_b_u6(uint32_t op)
{
	uint8_t breg = common32_get_breg(op);
	int size = check_b_limm(breg);
	m_regs[common32_get_areg(op)] = handleop32_BSET_do_op(m_regs[breg], common32_get_u6(op), common32_get_F(op));
	return m_pc + size;
}
...
uint32_t arcompact_device::handleop32_BSET_f_b_b_s12(uint32_t op)
{
	uint8_t breg = common32_get_breg(op);
	int size = check_b_limm(breg);
	m_regs[breg] = handleop32_BSET_do_op(m_regs[breg], common32_get_s12(op), common32_get_F(op));
	return m_pc + size;
}
...
uint32_t arcompact_device::handleop32_BSET_cc_f_b_b_c(uint32_t op)
{
	...
	int size = check_b_c_limm(breg, creg);
	if (check_condition(common32_get_condition(op)))
		m_regs[breg] = handleop32_BSET_do_op(m_regs[breg], m_regs[creg], common32_get_F(op));
	return m_pc + size;
}
...
uint32_t arcompact_device::handleop32_BSET_cc_f_b_b_u6(uint32_t op)
{
	...
	int size = check_b_limm(breg);
	if (check_condition(common32_get_condition(op)))
		m_regs[breg] = handleop32_BSET_do_op(m_regs[breg], common32_get_u6(op), common32_get_F(op));
	return m_pc + size;
}
...
uint32_t arcompact_device::handleop32_BCLR_do_op(uint32_t src1, uint32_t src2, uint8_t set_flags)
{
	uint32_t result = src1 & ~(1 << (src2 & 0x1f));
	if (set_flags)
		do_flags_nz(result);
...
uint32_t arcompact_device::handleop32_BCLR_f_a_b_c(uint32_t op)
{
	...
	uint8_t creg = common32_get_creg(op);
	int size = check_b_c_limm(breg, creg);
	m_regs[common32_get_areg(op)] = handleop32_BCLR_do_op(m_regs[breg], m_regs[creg], common32_get_F(op));
	return m_pc + size;
}
...
uint32_t arcompact_device::handleop32_BCLR_f_a_b_u6(uint32_t op)
{
	uint8_t breg = common32_get_breg(op);
	int size = check_b_limm(breg);
	m_regs[common32_get_areg(op)] = handleop32_BCLR_do_op(m_regs[breg], common32_get_u6(op), common32_get_F(op));
	return m_pc + size;
}
...
uint32_t arcompact_device::handleop32_BCLR_f_b_b_s12(uint32_t op)
{
	uint8_t breg = common32_get_breg(op);
	int size = check_b_limm(breg);
	m_regs[breg] = handleop32_BCLR_do_op(m_regs[breg], common32_get_s12(op), common32_get_F(op));
	return m_pc + size;
}
...
uint32_t arcompact_device::handleop32_BCLR_cc_f_b_b_c(uint32_t op)
{
	...
	int size = check_b_c_limm(breg, creg);
	if (check_condition(common32_get_condition(op)))
		m_regs[breg] = handleop32_BCLR_do_op(m_regs[breg], m_regs[creg], common32_get_F(op));
	return m_pc + size;
}
...
uint32_t arcompact_device::handleop32_BCLR_cc_f_b_b_u6(uint32_t op)
{
	...
	int size = check_b_limm(breg);
	if (check_condition(common32_get_condition(op)))
		m_regs[breg] = handleop32_BCLR_do_op(m_regs[breg], common32_get_u6(op), common32_get_F(op));
	return m_pc + size;
}
...
uint32_t arcompact_device::handleop32_BMSK_do_op(uint32_t src1, uint32_t src2, uint8_t set_flags)
{
...
uint32_t arcompact_device::handleop32_BMSK_f_a_b_c(uint32_t op)
{
	...
	uint8_t creg = common32_get_creg(op);
	int size = check_b_c_limm(breg, creg);
	m_regs[common32_get_areg(op)] = handleop32_BMSK_do_op(m_regs[breg], m_regs[creg], common32_get_F(op));
	return m_pc + size;
}
...
uint32_t arcompact_device::handleop32_BMSK_f_a_b_u6(uint32_t op)
{
	uint8_t breg = common32_get_breg(op);
	int size = check_b_limm(breg);
	m_regs[common32_get_areg(op)] = handleop32_BMSK_do_op(m_regs[breg], common32_get_u6(op), common32_get_F(op));
	return m_pc + size;
}
...
uint32_t arcompact_device::handleop32_BMSK_f_b_b_s12(uint32_t op)
{
	uint8_t breg = common32_get_breg(op);
	int size = check_b_limm(breg);
	m_regs[breg] = handleop32_BMSK_do_op(m_regs[breg], common32_get_s12(op), common32_get_F(op));
	return m_pc + size;
}
...
uint32_t arcompact_device::handleop32_BMSK_cc_f_b_b_c(uint32_t op)
{
	...
	int size = check_b_c_limm(breg, creg);
	if (check_condition(common32_get_condition(op)))
		m_regs[breg] = handleop32_BMSK_do_op(m_regs[breg], m_regs[creg], common32_get_F(op));
	return m_pc + size;
}
...
uint32_t arcompact_device::handleop32_BMSK_cc_f_b_b_u6(uint32_t op)
{
	...
	int size = check_b_limm(breg);
	if (check_condition(common32_get_condition(op)))
		m_regs[breg] = handleop32_BMSK_do_op(m_regs[breg], common32_get_u6(op), common32_get_F(op));
	return m_pc + size;
}

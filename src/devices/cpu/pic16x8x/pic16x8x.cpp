...
/****************************************************************************************
 ...
 ToDo:
    ...
    - Choose wich is the best option to set the config word, if via set instruction or from dump data
      or some sort of combination via default options.
...
void pic16x8x_device::nvram_default()
{
	...
	memset(&m_tmp_buff_16[0], 0xffff, m_internal_eeprom_size * 2);
	memset(&m_eeprom_data[0], 0xff, m_internal_eeprom_size);
	...
	if (m_region.found())
	{
		...
		memcpy(&m_tmp_buff_16[0], m_region->base() + eeprom_dump, m_internal_eeprom_size * 2);

		// Data width conversion
		for (u8 i=0; i < m_internal_eeprom_size; i++)
			m_eeprom_data[i] = m_tmp_buff_16[i] & 0x00ff;
	}
...
{
	size_t actual;
	return !file.read( &m_eeprom_data[0], m_internal_eeprom_size, actual ) && actual == m_internal_eeprom_size;
}

bool pic16x8x_device::nvram_write(util::write_stream &file)
{
	size_t actual;
	return !file.write( &m_eeprom_data[0], m_internal_eeprom_size, actual ) && actual == m_internal_eeprom_size;
}
...
/*
EEDATA - Read from internal EEPROM data memory
...
{
	//m_EEDATA = M_RDEEPROM(m_EEADR);
	return m_EEDATA;
}

/*
 EEDATA: Write to internal EEPROM data memory
...
void pic16x8x_device::device_start()
{
	...
	m_eeprom_data = std::make_unique<uint8_t []>(m_internal_eeprom_size);
	m_tmp_buff_16 = std::make_unique<uint16_t []>(m_internal_eeprom_size);

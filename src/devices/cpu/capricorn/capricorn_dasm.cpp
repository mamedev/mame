...
typedef offs_t (*fn_dis_param)(std::ostream &stream, offs_t pc, const device_disasm_interface::data_buffer &opcodes);
...
static void direct_addr(std::ostream &stream, offs_t pc, const device_disasm_interface::data_buffer &opcodes)
{
	util::stream_format(stream, "$%04x", opcodes.r16(pc));
}

static offs_t param_arp_drp(std::ostream &stream, offs_t pc, const device_disasm_interface::data_buffer &opcodes)
{
	stream << "R";
...
static offs_t param_dr(std::ostream &stream, offs_t pc, const device_disasm_interface::data_buffer &opcodes)
{
	stream << "DR";
	return 0;
}

static offs_t param_dr_ar(std::ostream &stream, offs_t pc, const device_disasm_interface::data_buffer &opcodes)
{
	stream << "DR,AR";
	return 0;
}

static offs_t param_dr_lit(std::ostream &stream, offs_t pc, const device_disasm_interface::data_buffer &opcodes)
{
	stream << "DR,=";
...
static offs_t param_dr_lit_dir(std::ostream &stream, offs_t pc,const device_disasm_interface::data_buffer &opcodes)
{
	stream << "DR,=";
...
static offs_t param_dr_idx_dir(std::ostream &stream, offs_t pc,const device_disasm_interface::data_buffer &opcodes)
{
	stream << "DR,XAR,";
	direct_addr(stream, pc+1, opcodes);
	return 2;
...
static offs_t param_xr_lit(std::ostream &stream, offs_t pc,const device_disasm_interface::data_buffer &opcodes)
{
	stream << "XR,";
	direct_addr(stream, pc+1, opcodes);
	return 2;
...
static offs_t param_lit_dir(std::ostream &stream, offs_t pc,const device_disasm_interface::data_buffer &opcodes)
{
	stream << "=";
	direct_addr(stream, pc+1, opcodes);
	return 2;
...
static offs_t param_dr_id_ar(std::ostream &stream, offs_t pc, const device_disasm_interface::data_buffer &opcodes)
{
	stream << "DR," << (BIT(opcodes.r8(pc), 1) ? '-' : '+') << "AR";
	return 0;
}

static offs_t param_jmp_off(std::ostream &stream, offs_t pc, const device_disasm_interface::data_buffer &opcodes)
{
	...
	for (p = dis_table; p->m_op_mask; p++) {
		if ((opcode & p->m_op_mask) == p->m_opcode) {
			...
			if (p->m_param_fn != nullptr) {
				stream << " ";
				res += p->m_param_fn(stream, pc, opcodes);
			}
			return res;
		}
	}

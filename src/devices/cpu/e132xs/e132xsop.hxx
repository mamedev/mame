...
void hyperstone_device::op05() // movd, local src, global dst
{
	check_delay_PC();
...
void hyperstone_device::op06()
{
	...
	DREG = m_local_regs[(decode.dst + GET_FP) % 64]; /* registers offset by frame pointer */
	DREGF = m_local_regs[(decode.dst + 1 + GET_FP) % 64];
...
void hyperstone_device::op07() // movd local,local
{
	check_delay_PC();
...
void hyperstone_device::op09()
{
	...
	SREG = m_local_regs[(decode.src + GET_FP) % 64];
	SREGF = m_local_regs[(decode.src + 1 + GET_FP) % 64];
...
void hyperstone_device::op0b()
{
	...
	decode.same_src_dst = (SRC_CODE == DST_CODE);
	decode.same_src_dstf = (SRC_CODE == ((DST_CODE + 1) % 64));
	decode.same_srcf_dst = (((SRC_CODE + 1) % 64) == DST_CODE);
	hyperstone_divu(decode);
}
...
void hyperstone_device::op17() // mask local,local
{
	const uint32_t extra_u = decode_const();
...
void hyperstone_device::op1b() // sum local,local
{
	const uint32_t extra_u = decode_const();
...
void hyperstone_device::op22() // cmp local,global
{
	check_delay_PC();
...
void hyperstone_device::op23() // cmp local,local
{
	check_delay_PC();
...
void hyperstone_device::op26()
{
	...
	{
		src_code += 16;
		sreg = get_global_register(src_code);
		if ((WRITE_ONLY_REGMASK >> src_code) & 1)
...
void hyperstone_device::op29() // add global,local
{
	check_delay_PC();
...
void hyperstone_device::op2a() // add local,global
{
	check_delay_PC();
...
void hyperstone_device::op30() // cmpb
{
	check_delay_PC();
...
void hyperstone_device::op31() // cmpb
{
	check_delay_PC();
...
void hyperstone_device::op32() // cmpb
{
	check_delay_PC();

	uint32_t sreg = m_global_regs[SRC_CODE];
	uint32_t dreg = m_local_regs[(DST_CODE + GET_FP) % 64]; /* registers offset by frame pointer */
...
void hyperstone_device::op33() // cmpb
{
	check_delay_PC();
...
void hyperstone_device::op34() // andn
{
	check_delay_PC();
...
void hyperstone_device::op35() // andn
{
	check_delay_PC();
	...
	dreg &= ~m_local_regs[(SRC_CODE + GET_FP) % 64];
...
void hyperstone_device::op36() // andn
{
	check_delay_PC();

	uint32_t dst_code = (DST_CODE + GET_FP) % 64;
	m_local_regs[dst_code] &= ~m_global_regs[SRC_CODE];
	SET_Z(m_local_regs[dst_code] == 0 ? 1 : 0);

	m_icount -= m_clock_cycles_1;
}

void hyperstone_device::op37() // andn
{
	check_delay_PC();

	uint32_t dst_code = (DST_CODE + GET_FP) % 64;
	m_local_regs[dst_code] &= ~m_local_regs[(SRC_CODE + GET_FP) % 64];
	SET_Z(m_local_regs[dst_code] == 0 ? 1 : 0);

	m_icount -= m_clock_cycles_1;
}

void hyperstone_device::op38() // or global,global
{
	check_delay_PC();
...
void hyperstone_device::op39() // or global,local
{
	check_delay_PC();
...
void hyperstone_device::op3a() // or local,global
{
	check_delay_PC();
...
void hyperstone_device::op3b() // or local,local
{
	check_delay_PC();
...
void hyperstone_device::op3c() // xor global,global
{
	check_delay_PC();
...
void hyperstone_device::op3d() // xor
{
	check_delay_PC();
...
void hyperstone_device::op3e() // xor
{
	check_delay_PC();
...
void hyperstone_device::op3f() // xor
{
	check_delay_PC();
...
void hyperstone_device::op44() // not
{
	check_delay_PC();

	uint32_t dreg = ~m_global_regs[SRC_CODE];
	set_global_register(DST_CODE, dreg);
	SET_Z(dreg == 0 ? 1 : 0);
...
void hyperstone_device::op45() // not
{
	check_delay_PC();

	uint32_t dreg = ~m_local_regs[(SRC_CODE + GET_FP) % 64];
	set_global_register(DST_CODE, dreg);
	SET_Z(dreg == 0 ? 1 : 0);
...
void hyperstone_device::op46() // not
{
	check_delay_PC();
...
void hyperstone_device::op47() // not
{
	check_delay_PC();
...
void hyperstone_device::op4e()
{
	...
	decode.dst_is_local = 1;
	DREG = m_local_regs[(decode.dst + GET_FP) % 64];
...
void hyperstone_device::op53() // addc local,local
{
	...
	const uint32_t c = (SR & C_MASK) ? 1 : 0;
...
void hyperstone_device::op66() // movi local,short imm
{
	check_delay_PC();

	const uint32_t imm = immediate_values[OP & 0x0f];
...
void hyperstone_device::op67() // movi local,long imm
{
...
void hyperstone_device::op68() // addi global,short imm
{
	regs_decode decode;
	decode_immediate_u(decode);
	check_delay_PC();
...
void hyperstone_device::op69() // addi global,long imm
{
	const uint32_t extra_u = decode_immediate_s();
	check_delay_PC();
...
void hyperstone_device::op6a() // addi local,short imm
{
	check_delay_PC();
...
void hyperstone_device::op6b() // addi local,long imm
{
	uint32_t imm = decode_immediate_s();
	check_delay_PC();
...
void hyperstone_device::op71() // cmpbi global,long imm
{
	const uint32_t dreg = m_global_regs[DST_CODE];
...
void hyperstone_device::op72() // cmpbi local,short imm
{
	check_delay_PC();
...
void hyperstone_device::op73() // cmpbi local,long imm
{
	const uint32_t dreg = m_local_regs[(DST_CODE + GET_FP) & 0x3f];
...
void hyperstone_device::op91() // ldxx1 global,local
{
...
void hyperstone_device::hyperstone_shri_local() // shri local,imm
{
	check_delay_PC();

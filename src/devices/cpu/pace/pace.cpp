...
/***************************************************************************
    ...
    IMP-16 microcode). The two clock inputs with non-overlapping low
    phases required by PACE may be generated by the DP8302 STE (System
    Timing Element), which divides its oscillator input by 2.

    Like the Data General Nova, PACE and IMP-16 have four 16-bit
    accumulators, two of which can be used as index registers. The program
    counter can also be used as a base for indexing, and instructions may
    also directly access 256 words of memory. The upper 128 words of the
    direct region may be mapped to either 0080–00FF or FF80–FFFF,
    depending on the state of the BPS input (which National suggests
    connecting to one of the flag outputs). The on-chip 10-level LIFO
    stack (IMP-16 had a 16-level stack) may hold both return addresses
    and register data. The flag register may be transferred to or from
    the stack or one of the accumulators.
    ...
    this). Six interrupts are available, one triggered only by internal
    stack full/empty conditions; the nonmaskable level 0 interrupt is
    intended primarily for debugging. Each instruction is one word.

...
private:
	...
	uint32_t crom_cram_r(address_space &space, offs_t offset, uint32_t mem_mask = 0xffffffff);

	//! write microcode CRAM, depending on m_ucode_ram_base (ignore writes to CROM)
	void crom_cram_w(address_space &space, offs_t offset, uint32_t data, uint32_t mem_mask = 0xffffffff);

	//! read constants PROM
	uint16_t const_r(address_space &space, offs_t offset, uint16_t mem_mask = 0xffff);

	//! read i/o space RAM
	uint16_t ioram_r(address_space &space, offs_t offset, uint16_t mem_mask = 0xffff);

	//!< write i/o space RAM
	void ioram_w(address_space &space, offs_t offset, uint16_t data, uint16_t mem_mask = 0xffff);
	...
	uint16_t noop_r(address_space &space, offs_t offset, uint16_t mem_mask = 0xffff);                //!< read open bus (0177777)
	void noop_w(address_space &space, offs_t offset, uint16_t data, uint16_t mem_mask = 0xffff);               //!< write open bus

	uint16_t bank_reg_r(address_space &space, offs_t offset, uint16_t mem_mask = 0xffff);            //!< read bank register in memory mapped I/O range
	void bank_reg_w(address_space &space, offs_t offset, uint16_t data, uint16_t mem_mask = 0xffff);           //!< write bank register in memory mapped I/O range

...
{
	u8 res = m_data.read_byte(tranlsate_memory_address(addr));
	T(MTM);
	return res;
}
...
void z80_device::wm(uint16_t addr, uint8_t value)
{
	...
	m_data.write_byte(tranlsate_memory_address(addr), value);
	T(MTM);
}
...
uint8_t z80_device::rop()
{
	...
	uint8_t res = m_opcodes.read_byte(tranlsate_memory_address(PCD));
...
{
	u8 res = m_args.read_byte(tranlsate_memory_address(PCD));
	T(MTM);
	PC++;
...
OP(illegal,1) {
	logerror("Z80 ill. opcode $%02x $%02x ($%04x)\n",
			m_opcodes.read_byte(tranlsate_memory_address((PCD-1)&0xffff)), m_opcodes.read_byte(tranlsate_memory_address(PCD)), PCD-1);
}
...
OP(illegal,2)
{
	logerror("Z80 ill. opcode $ed $%02x\n",
			m_opcodes.read_byte(tranlsate_memory_address((PCD-1)&0xffff)));
}

...
void adsp21xx_device::device_start()
{
	...
	for (int ireg = 0; ireg < 8; ireg++)
		state_add(ADSP2100_I0 + ireg, string_format("I%d", ireg), m_i[ireg]).mask(0x3fff).callimport();

	for (int lreg = 0; lreg < 8; lreg++)
		state_add(ADSP2100_L0 + lreg, string_format("L%d", lreg), m_l[lreg]).mask(0x3fff).callimport();

	for (int mreg = 0; mreg < 8; mreg++)
		state_add(ADSP2100_M0 + mreg, string_format("M%d", mreg), m_m[mreg]).signed_mask(0x3fff);
	...
	for (int irqnum = 0; irqnum < 4; irqnum++)
		state_add(ADSP2100_IRQSTATE0 + irqnum, string_format("IRQ%d", irqnum), m_irq_state[irqnum]).mask(1).callimport();

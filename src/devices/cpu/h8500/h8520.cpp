...
void h8520_device::device_add_mconfig(machine_config &config)
{
	H8520_INTC(config, m_intc, *this);
...
void h8520_device::internal_map(address_map &map)
{
	...
	map(0xff84, 0xff84).rw(m_port3, FUNC(h8_port_device::ff_r), FUNC(h8_port_device::ddr_w));
	...
	map(0xffc0, 0xffc0).rw(m_sci[1], FUNC(h8_sci_device::smr_r), FUNC(h8_sci_device::smr_w));
	map(0xffc1, 0xffc1).rw(m_sci[1], FUNC(h8_sci_device::brr_r), FUNC(h8_sci_device::brr_w));
	map(0xffc2, 0xffc2).rw(m_sci[1], FUNC(h8_sci_device::scr_r), FUNC(h8_sci_device::scr_w));
	map(0xffc3, 0xffc3).rw(m_sci[1], FUNC(h8_sci_device::tdr_r), FUNC(h8_sci_device::tdr_w));
	map(0xffc4, 0xffc4).rw(m_sci[1], FUNC(h8_sci_device::ssr_r), FUNC(h8_sci_device::ssr_w));
	map(0xffc5, 0xffc5).r(m_sci[1], FUNC(h8_sci_device::rdr_r));
	...
	map(0xfffc, 0xfffc).rw(m_intc, FUNC(h8500_intc_device::nmicr_r), FUNC(h8500_intc_device::nmicr_w));
	map(0xfffd, 0xfffd).rw(m_intc, FUNC(h8500_intc_device::irqcr_r), FUNC(h8500_intc_device::irqcr_w));
...
void h8520_device::internal_update(u64 current_time)
{
	u64 event_time = 0;
...
void h8520_device::notify_standby(int state)
{
...
void h8520_device::execute_set_input(int inputnum, int state)
{
	m_intc->set_input(inputnum, state);
}

void h8520_device::interrupt_taken()
{
	standard_irq_callback(m_intc->interrupt_taken(m_taken_irq_vector), m_npc);
}

void h8520_device::update_irq_filter()
{
	m_intc->set_filter((m_sr & (SR_I2 | SR_I1 | SR_I0)) >> 8);
}

void h8520_device::irq_setup()
{

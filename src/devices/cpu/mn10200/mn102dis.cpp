...
static u32 r24(const device_disasm_interface::data_buffer &opcodes, offs_t pc)
{
	return opcodes.r16(pc) | (opcodes.r8(pc+2) << 16);
}

static const char *i8str(s8 v)
{
...
static const char *i24str(u32 v)
{	
	static char res[0x10];
	if(!(v & 0x800000))
		sprintf(res, "$%x", v & 0xffffff);
	else
		sprintf(res, "-$%x", (-v) & 0xffffff);
	return res;
}
...
static int mn102_disassemble(std::ostream &stream, offs_t pc, const device_disasm_interface::data_buffer &opcodes)
{
	u8 opcode;
...
{
	return mn102_disassemble(stream, pc, opcodes);
}

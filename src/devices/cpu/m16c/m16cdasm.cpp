...
const char *const f_regs[2][6] =
{
	{ "R0L", "R0H", "R1L", "R1H", "A0", "A1" },
	{ "R0", "R1", "R2", "R3", "A0", "A1" }
};

const char *const f_cregs[8] =
{
	"", "INTBL", "INTBH", "FLG", "ISP", "SP", "SB", "FB"
};

const char *const f_byte_ops[8] =
{
	"mov.b", "mov.b",
	"and.b", "or.b",
	"add.b", "sub.b",
...
void format_imm16(std::ostream &stream, u16 imm)
{
	util::stream_format(stream, "#%0*Xh", (imm >= 0xa000) ? 5 : 4, imm);
}

void format_label(std::ostream &stream, u32 label)
{
	label &= 0xfffff;
	util::stream_format(stream, "%0*Xh", (label >= 0xa0000) ? 6 : 5, label);
}

void format_imm_signed(std::ostream &stream, s16 imm)
{
	stream << '#';
	if (imm < 0)
	{
...
void format_relative(std::ostream &stream, const char *reg, s32 disp)
{
	if (disp < 0)
	{
		stream << '-';
...
constexpr int shift_count(u8 imm4)
{
...
{
	if (mode < 6)
		stream << f_regs[size][mode];
	else if (mode < 8)
		util::stream_format(stream, "[%s]", f_regs[1][mode - 2]);
	else if (mode < 0xc)
		format_relative(stream, mode < 0xa ? f_regs[1][mode - 4] : f_cregs[mode - 4], mode == 0xb ? s8(opcodes.r8(pc++)) : opcodes.r8(pc++));
	else
	{
		if (mode == 0xf)
			format_label(stream, opcodes.r16(pc));
		else
			format_relative(stream, mode < 0xe ? f_regs[1][mode - 8] : f_cregs[mode - 8], opcodes.r16(pc));
		pc += 2;
	}
}
...
void m16c_disassembler::dasm_74(std::ostream &stream, offs_t &pc, const data_buffer &opcodes, bool size) const
{
	...
	case 0x0: case 0x1:
	{
		...
		if (BIT(op2, 4))
			util::stream_format(stream, "[%s]", f_regs[1][4]);
		pc += 3;
		break;
	}
	...
	case 0x2:
		util::stream_format(stream, "%-8s", size ? "ste.w" : "ste.b");
		dasm_ea(stream, pc, opcodes, op2 & 0x0f, size);
		util::stream_format(stream, ", [%s%s]", f_regs[1][5], f_regs[1][4]);
		break;
	...
	case 0x3:
		...
		dasm_ea(stream, pc, opcodes, op2 & 0x0f, size);
		stream << ", ";
		format_relative(stream, f_cregs[5], s8(opcodes.r8(pc++)));
		break;
	...
	case 0x6:
		util::stream_format(stream, "%-8s%s, ", size ? "rot.w" : "rot.b", f_regs[0][3]);
		dasm_ea(stream, pc, opcodes, op2 & 0x0f, size);
		break;
	...
	case 0xa:
		util::stream_format(stream, "%-8s[%s%s], ", size ? "lde.w" : "lde.b", f_regs[1][5], f_regs[1][4]);
		dasm_ea(stream, pc, opcodes, op2 & 0x0f, size);
		break;
	...
	case 0xb:
	{
		const int offset = BIT(op2, 3) ? (BIT(op2, 2) ? 2 : 1) : 0;
		util::stream_format(stream, "%-8s", size ? "mov.w" : "mov.b");
		format_relative(stream, f_cregs[5], s8(opcodes.r8(pc + offset)));
		stream << ", ";
		dasm_ea(stream, pc, opcodes, op2 & 0x0f, size);
		++pc;
		break;
	...
	case 0xe:
		util::stream_format(stream, "%-8s%s, ", size ? "shl.w" : "shl.b", f_regs[0][3]);
		dasm_ea(stream, pc, opcodes, op2 & 0x0f, size);
		break;

	case 0xf:
		util::stream_format(stream, "%-8s%s, ", size ? "sha.w" : "sha.b", f_regs[0][3]);
		dasm_ea(stream, pc, opcodes, op2 & 0x0f, size);
		break;
...
void m16c_disassembler::dasm_76(std::ostream &stream, offs_t &pc, const data_buffer &opcodes, bool size) const
{
	...
	{
		const int offset = BIT(op2, 3) ? (BIT(op2, 2) ? 2 : 1) : 0;
		util::stream_format(stream, "%-8s", f_imm76_ops[size][op2 >> 4]);
		if (size)
		{
			if (op2 < 0x40)
				format_imm16(stream, opcodes.r16(pc + offset));
...
void m16c_disassembler::dasm_7a(std::ostream &stream, offs_t &pc, const data_buffer &opcodes, bool size) const
{
	...
	if (op2 < 0x40)
	{
		util::stream_format(stream, "%-8s%s, ", size ? "xchg.w" : "xchg.b", f_regs[size][op2 >> 4]);
		dasm_ea(stream, pc, opcodes, op2 & 0x0f, size);
	}
	else if (op2 >= 0x90)
	{
		if (size)
			util::stream_format(stream, "%-8s%s, ", "stc", f_cregs[BIT(op2, 4, 3)]);
		else
			util::stream_format(stream, "%-8s", "ldc");
		dasm_ea(stream, pc, opcodes, op2 & 0x0f, true);
		if (!size)
			util::stream_format(stream, ", %s", f_cregs[BIT(op2, 4, 3)]);
	}
	else
	{
		--pc;
...
void m16c_disassembler::dasm_7c(std::ostream &stream, offs_t &pc, const data_buffer &opcodes) const
{
	...
	if (!BIT(op2, 6))
	{
		util::stream_format(stream, "%-8s", f_nibmov_ops[BIT(op2, 4, 2)]);
		if (BIT(op2, 7))
			util::stream_format(stream, "%s, ", f_regs[0][0]);
	...
	{
		util::stream_format(stream, "%-8s", f_decimal_ops[0][op2 & 0x03]);
		if (BIT(op2, 3))
	...
	case 0xeb:
		...
		format_imm_signed(stream, s8(opcodes.r8(pc++)));
		util::stream_format(stream, ", %s", f_cregs[5]);
		break;
	...
	case 0xf3:
		util::stream_format(stream, "%-8s%s", "exts.w", f_regs[1][0]);
		break;
...
void m16c_disassembler::dasm_7d(std::ostream &stream, offs_t &pc, offs_t &flags, const data_buffer &opcodes) const
{
	...
	if (op2 < 0x40)
	{
		...
		{
			format_label(stream, u32(opcodes.r8(pc + 2)) << 16 | opcodes.r16(pc));
			util::stream_format(stream, "[%s]", f_regs[1][op2 & 0x07]);
			pc += 3;
		}
		else
		{
	...
	else if ((op2 & 0xf8) == 0x98)
	{
		...
		util::stream_format(stream, "%-8s#%d, %s", "add.w", util::sext(op2, 4), f_cregs[5]);
	else if ((op2 & 0xf8) == 0xc8 && f_cnds[op2 & 0x0f][0] != '\0')
	{
		util::stream_format(stream, "j%-7s", f_cnds[op2 & 0x0f]);
		format_label(stream, pc + s8(opcodes.r8(pc)));
		++pc;
		flags |= STEP_COND;
	}
	else if ((op2 & 0xf0) == 0xd0 && f_cnds[op2 & 0x0f][0] != '\0')
		util::stream_format(stream, "bm%-6sC", f_cnds[op2 & 0x0f]);
	else if ((op2 & 0xf4) == 0xe4)
	{
		util::stream_format(stream, "%-8s", f_decimal_ops[1][op2 & 0x03]);
		if (BIT(op2, 3))
		{
			...
			stream << f_regs[1][1];
		util::stream_format(stream, ", %s", f_regs[1][0]);
	}
	else switch (op2)
	{
	case 0xe0:
...
void m16c_disassembler::dasm_7e(std::ostream &stream, offs_t &pc, const data_buffer &opcodes) const
{
	...
	if (op2 < 0xe0)
	{
		if ((op2 & 0xf0) == 0x20)
		{
			...
			if ((cnd < 0x08 || cnd >= 0xf8) && f_bmcnd[cnd & 0x0f][0] != '\0')
				util::stream_format(stream, "bm%-6s", f_bmcnd[cnd & 0x0f]);
			else
			{
				--pc;
			...
			util::stream_format(stream, "%-8s", f_bit_ops[op2 >> 4]);
		if (!BIT(op2, 3))
		{
			if ((op2 & 0x0f) < 6)
			{
				const u8 pos = opcodes.r8(pc++);
				util::stream_format(stream, "%u, %s", pos, f_regs[1][op2 & 0x0f]);
			}
			else
				util::stream_format(stream, "[%s]", f_regs[1][4 + BIT(op2, 0)]);
		}
		else if (BIT(op2, 2))
		{
			if ((op2 & 0x0f) >= 0x0e)
			{
				...
				if ((op2 & 0x0f) == 0x0f)
					...
					format_relative(stream, f_cregs[6], abs >> 3);
			}
			else
				format_relative(stream, f_regs[1][op2 & 0x07], opcodes.r16(pc));
			pc += 2;
		}
		else if (BIT(op2, 1))
		{
			...
			format_relative(stream, f_cregs[6 + BIT(op2, 0)], BIT(op2, 0) ? (s8(dsp8) >> 3) : (dsp8 >> 3));
		}
		else
			format_relative(stream, f_regs[1][4 + BIT(op2, 0)], opcodes.r8(pc++));
		if ((op2 & 0xf0) == 0x20)
			++pc;
	}
	else
...
void m16c_disassembler::dasm_eb(std::ostream &stream, offs_t &pc, offs_t &flags, const data_buffer &opcodes) const
{
	...
	else if (op2 >= 0x80)
	{
		util::stream_format(stream, "%-8s#%d, %s%s", BIT(op2, 5) ? "sha.l" : "shl.l",
				...
				f_regs[1][2 + BIT(op2, 4)], f_regs[1][BIT(op2, 4)]);
	}
	else if ((op2 & 0x4f) == 0x01)
	{
		util::stream_format(stream, "%-8s%s, %s%s", BIT(op2, 5) ? "sha.l" : "shl.l",
				f_regs[0][3], f_regs[1][2 + BIT(op2, 4)], f_regs[1][BIT(op2, 4)]);
	}
	...
	else if (op2 < 0x60 && BIT(op2, 3))
	{
		...
		dasm_ea(stream, pc, opcodes, op2 & 0x0f, false);
		util::stream_format(stream, ", %s", f_regs[1][op2 >> 4]);
	}
	else if ((op2 & 0x0f) == 0 && op2 >= 0x10)
	{
		util::stream_format(stream, "%-8s", "ldc");
		format_imm16(stream, opcodes.r16(pc));
		util::stream_format(stream, ", %s", f_cregs[op2 >> 4]);
		pc += 2;
	}
	else if ((op2 & 0x0e) == 0x02 && op2 >= 0x10)
	{
		util::stream_format(stream, "%-8s%s", BIT(op2, 0) ? "popc" : "pushc", f_cregs[op2 >> 4]);
	}
	else
	{
		--pc;
...
offs_t m16c_disassembler::disassemble(std::ostream &stream, offs_t pc, const m16c_disassembler::data_buffer &opcodes, const m16c_disassembler::data_buffer &params)
{
	...
	if (op1 < 0x40)
	{
		...
		{
			util::stream_format(stream, "%-8s", f_byte_ops[BIT(op1, 3, 3)]);
			if (op1 < 0x08)
				util::stream_format(stream, "%s, ", f_regs[0][BIT(op1, 2)]);
			switch (op1 & 0x03)
			{
			case 0:
				util::stream_format(stream, "%s", f_regs[0][BIT(~op1, 2)]);
			...
			default:
				format_relative(stream, f_cregs[5 + (op1 & 3)], BIT(op1, 1) ? s8(opcodes.r8(pc++)): opcodes.r8(pc++));
				break;
			}
			if (op1 >= 0x08)
				util::stream_format(stream, ", %s", f_regs[0][((BIT(op1, 3, 3) == 6) ? 4 : 0) + BIT(op1, 2)]);
		}
	}
	else if (op1 < 0x60)
	{
		util::stream_format(stream, "%-8s%d, ", f_bit_ops[op1 >> 3], op1 & 0x07);
		format_relative(stream, f_cregs[6], opcodes.r8(pc++));
	}
	else if (op1 < 0x68)
	{
	...
	else if (op1 < 0x70)
	{
		...
		util::stream_format(stream, "b%-7s", f_cnds[op1 & 0x07]);
	...
	case 0x82: case 0x8a: case 0x92: case 0x9a:
		util::stream_format(stream, "%-8s%s", BIT(op1, 4) ? "pop.b" : "push.b", f_regs[0][BIT(op1, 3)]);
		break;
	...
	case 0x8b: case 0x8c: case 0x8d: case 0x8e: case 0x8f:
		...
		case 6:
			format_relative(stream, f_cregs[7], s8(opcodes.r8(pc++)));
			break;
	...
	case 0xa2: case 0xaa:
		...
		format_imm16(stream, opcodes.r16(pc));
		pc += 2;
		util::stream_format(stream, ", %s", f_regs[1][4 + BIT(op1, 3)]);
		break;
	...
	case 0xb2: case 0xba: case 0xf2: case 0xfa:
		util::stream_format(stream, "%-8s%s", BIT(op1, 6) ? "dec.w" : "inc.w", f_regs[1][4 + BIT(op1, 3)]);
		break;
	...
	case 0xc2: case 0xca: case 0xd2: case 0xda:
		util::stream_format(stream, "%-8s%s", BIT(op1, 4) ? "pop.w" : "push.w", f_regs[1][4 + BIT(op1, 3)]);
		break;
	...
	case 0xdb: case 0xdc: case 0xdd: case 0xde: case 0xdf:
		...
		case 3: case 4:
			format_imm8(stream, opcodes.r8(pc++));
			util::stream_format(stream, ", %s", f_regs[0][BIT(op1, 0)]);
			break;
	...
	case 0xec:
	{
		...
		for (int i = 7; i >= 0; i--)
		{
			if (BIT(op2, i))
			{
				stream << ((i < 2) ? f_cregs[7 - i] : f_regs[1][7 - i]);
				if (op2 & ((1 << i) - 1))
					stream << ", ";
			}
		}
	...
	case 0xed:
	{
		...
		for (int i = 0; i < 8; i++)
		{
			if (BIT(op2, i))
			{
				stream << ((i > 5) ? f_cregs[i] : f_regs[1][i]);
				if ((op2 >> i) != 1)
					stream << ", ";
			}
		}

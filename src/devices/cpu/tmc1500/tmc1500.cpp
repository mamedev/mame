...
tmc1500_base_device::tmc1500_base_device(const machine_config &mconfig,
                                         device_type type, const char *tag,
                                         device_t *owner, u32 clock)
    : cpu_device(mconfig, type, tag, owner, clock),
      m_program_config("program", ENDIANNESS_LITTLE, 16, 11, -1),
      m_read_k(*this, 0), m_write_o(*this), m_write_r(*this) {}
...
tmc1500_base_device::memory_space_config() const {
...
void tmc1500_base_device::execute_instruction() {
  ...
  // PF Group: Miscellaneous Operations
    ...
    switch (pf) {
    case 0x00:
      reg_store(m_regsO[0], m_regsY[m_rab]);
      break; // STYA
    case 0x01:
      m_rab = (op >> 4) & 0x07;
      break; // RABI
    ...
    case 0x03:
      ...
      break; // RET
    case 0x04:
      reg_store(m_regsX[m_rab], m_regsO[0]);
      break; // STAX
    case 0x05:
      reg_store(m_regsO[0], m_regsX[m_rab]);
      break; // STXA
    case 0x06:
      reg_store(m_regsY[m_rab], m_regsO[0]);
      break; // STAY
    case 0x07:
      opDISP();
      break; // DISP
  ...
  case 3:         // XCHG or MOVE
    if (n == 0) { // XCHG
      if (j == 0 && k < 4)
        reg_xchg(m_regsO[0], m_regsO[k], lo, hi);
    } else { // MOVE
      if (k != 5)
        reg_move(m_regsO[j], src2, lo, hi, base);
    }
    return;
  }
  ...
  if (k == 5) { // Shift
    if (n == 0)
      reg_shl(p_dst, m_regsO[j], lo, hi);
    else
      reg_shr(p_dst, m_regsO[j], lo, hi);
  } else { // Arithmetic
    if (n == 0)
      reg_add(p_dst, m_regsO[j], src2, lo, hi, base);
    else
      reg_sub(p_dst, m_regsO[j], src2, lo, hi, base);
  }
}
...
void tmc1500_base_device::reg_add(uint8_t *dst, const uint8_t *src1,
                                  const uint8_t *src2, int lo, int hi,
                                  int base) {
  int carry = 0;
  for (int i = lo; i <= hi; i++) {
...
void tmc1500_base_device::reg_sub(uint8_t *dst, const uint8_t *src1,
                                  const uint8_t *src2, int lo, int hi,
                                  int base) {
  int carry = 0;
  for (int i = lo; i <= hi; i++) {
...
void tmc1500_base_device::reg_move(uint8_t *dst, const uint8_t *src, int lo,
                                   int hi, int base) {
  int carry = 0;
  for (int i = lo; i <= hi; i++) {
    int r = src[i] + carry;
...
// Shift operations
void tmc1500_base_device::reg_shl(uint8_t *dst, const uint8_t *src, int lo,
                                  int hi) {
  for (int i = hi; i > lo; i--) {
    dst[i] = src[i - 1];
  }
...
void tmc1500_base_device::reg_shr(uint8_t *dst, const uint8_t *src, int lo,
                                  int hi) {
  for (int i = lo; i < hi; i++) {
    dst[i] = src[i + 1];
  }
...
void tmc1500_base_device::opDISP() {
  ...
  // Phase 1: Output registers A and B to the Display Decoder
  // The display is rendered digit-by-digit (matching PCjs opDISP behavior).
  // Register B controls blanking/minus/DP, Register A provides BCD digit.
  for (int i = 11; i >= 0; i--) {
    m_scan_idx = i;

    uint16_t data = 0x0f; // Blank by default

    uint8_t a = m_regsO[0][i];
    uint8_t b = m_regsO[1][i];

    if (b & 0x08)
      data = 0x0f; // Blank (Table III: 1XXX)
    else if (b & 0x01)
      data = 0x0e; // Minus sign (Table III: 0XX1)
    else
      data = a & 0x0f; // Digit (Table III: 0XX0)
    if (b & 0x02)
      data |= 0x10; // Decimal point (Table III: XX1X)

    m_write_o(11 - i, data, 0xffff);
  }

  // Phase 2: Keyboard scan — detect which key is pressed.
  // During the 6.4ms DISP cycle, the segment scan lines double as keyboard
  // row strobes. We scan all 8 rows (digits 0-7 map to segment scan lines)
  // and build the R5 value using the PCjs encoding:
  //   bits 0-3 = row (0-based, derived from segment scan index)
  //   bits 4-7 = col (1-based, derived from K1-K5 input)
  // Per PCjs: if a key is detected, R5 is loaded and COND is set.
  for (int row = 0; row < 8; row++) {
    ...
    if (k != 0) {
      ...
      if (col != 0) {
        m_regR5 = (7 - row) | (col << 4);
        m_cond = true;
        break; // First key detected wins (no n-key rollover)
      }
    }
  }

  // The DISP instruction stretches the clock by a factor of 32.
  // Since execute_run() already decrements m_icount by 1 after this,
  // we subtract 31 more here for a total of 32 instruction cycles (6.4 ms).
  m_icount -= 31;
}

std::unique_ptr<util::disasm_interface>
tmc1500_base_device::create_disassembler() {
  return std::make_unique<tmc1500_disassembler>();
}

void tmc1500_base_device::state_string_export(const device_state_entry &entry,
                                              std::string &str) const {
  switch (entry.index()) {
  case STATE_GENPC:
    str = util::string_format("%03X", m_pc);
    break;
  }
}

tmc1501_cpu_device::tmc1501_cpu_device(const machine_config &mconfig,
                                       const char *tag, device_t *owner,
                                       u32 clock)
    : tmc1500_base_device(mconfig, TMC1501, tag, owner, clock) {}
tmc1502_cpu_device::tmc1502_cpu_device(const machine_config &mconfig,
                                       const char *tag, device_t *owner,
                                       u32 clock)
    : tmc1500_base_device(mconfig, TMC1502, tag, owner, clock) {}
tmc1503_cpu_device::tmc1503_cpu_device(const machine_config &mconfig,
                                       const char *tag, device_t *owner,
                                       u32 clock)
    : tmc1500_base_device(mconfig, TMC1503, tag, owner, clock) {}

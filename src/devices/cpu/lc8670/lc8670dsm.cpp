...
void lc8670_cpu_device::dasm_arg(uint8_t op, char *buffer, offs_t pc, int arg, const device_disasm_interface::data_buffer &opcodes, offs_t &pos)
{
	switch( arg )
	{
		case OP_NULL:
...
offs_t lc8670_cpu_device::disassemble(std::ostream &stream, offs_t pc, const data_buffer &opcodes, const data_buffer &params, uint32_t options)
{
	offs_t pos = pc;

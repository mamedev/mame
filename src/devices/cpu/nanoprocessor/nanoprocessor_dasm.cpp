...
typedef void (*fn_dis_param)(std::ostream& stream, uint8_t opcode, offs_t pc, const device_disasm_interface::data_buffer &params);
...
static void param_bitno(std::ostream& stream, uint8_t opcode, offs_t pc, const device_disasm_interface::data_buffer &params)
{
	stream << (char)('0' + (opcode & 7));
}

static void param_ds(std::ostream& stream, uint8_t opcode, offs_t pc, const device_disasm_interface::data_buffer &params)
{
	util::stream_format(stream, "DS%u", opcode & 0xf);
}

static void param_reg(std::ostream& stream, uint8_t opcode, offs_t pc, const device_disasm_interface::data_buffer &params)
{
	util::stream_format(stream, "R%u", opcode & 0xf);
}

static void param_11bit(std::ostream& stream, uint8_t opcode, offs_t pc, const device_disasm_interface::data_buffer &params)
{
	unsigned tmp = ((unsigned)(opcode & 7) << 8) | params.r8(pc);
	util::stream_format(stream, "$%03x", tmp);
}

static void param_page_no(std::ostream& stream, uint8_t opcode, offs_t pc, const device_disasm_interface::data_buffer &params)
{
	stream << (char)('0' + (opcode & 7));
}

static void param_byte(std::ostream& stream, uint8_t opcode, offs_t pc, const device_disasm_interface::data_buffer &params)
{
	util::stream_format(stream, "$%02x", params.r8(pc));
}

static void param_ds_byte(std::ostream& stream, uint8_t opcode, offs_t pc, const device_disasm_interface::data_buffer &params)
{
	util::stream_format(stream, "DS%u,$%02x", opcode & 0xf, params.r8(pc));
}

static void param_reg_byte(std::ostream& stream, uint8_t opcode, offs_t pc, const device_disasm_interface::data_buffer &params)
{
	...
	for (const dis_entry_t& ent : dis_table) {
		if ((opcode & ent.m_op_mask) == ent.m_opcode) {
			...
			if (ent.m_param_fn != nullptr) {
				ent.m_param_fn(stream, opcode, pc+1, params);
			}

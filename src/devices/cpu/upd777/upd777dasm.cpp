...
offs_t upd777_disassembler::disassemble(std::ostream &stream, offs_t pc, const upd777_disassembler::data_buffer &opcodes, const upd777_disassembler::data_buffer &params)
{
	...
	if (inst >= 0x080 && inst <= 0x0ff)
	{
		...
		util::stream_format(stream, "%04x: M-0x%02x", get_table(pc), inst & 0x7f);
	}
	else if (inst >= 0x100 && inst <= 0x17f)
	{
		// M[H[5:1],L[2:1]][7:1]+K[7:1]->M[H[5:1],L[2:1]][7:1], Skip if carry, N->L[2:1]
		util::stream_format(stream, "%04x: M+0x%02x->M, 0x%d->L", get_table(pc), inst & 0x1f, (inst >> 5) & 0x3);
	}
	...
	else if (inst >= 0x480 && inst <= 0x4bf) // 480 - 4bf
	{
		...
		util::stream_format(stream, "%04x: H-0x%02x->H BOJ", get_table(pc), inst & 0x1f);
	}
	else if (inst >= 0x4c0 && inst <= 0x4ff) // 4c0 - 4ff
	{
		// H[5:1]+K[5:1]H[5:1], Skip if carry
		util::stream_format(stream, "%04x: H+0x%02x->H CAJ", get_table(pc), inst & 0x1f);
	}
	else if (inst >= 0x500 && inst <= 0x57f) // 500 - 57f
	{
		...
		util::stream_format(stream, "%04x: 0x%02x->M", get_table(pc), inst & 0x7f);
	}
	else if (inst >= 0x580 && inst <= 0x5ff) // 580 - 5ff
	{
		// Store K[7:6] to L[2:1] and K[5:1] to H[5:1]
		util::stream_format(stream, "%04x: 0x%02x->L,H", get_table(pc), inst & 0x7f);
	}
	else if (inst >= 0x600 && inst <= 0x67f) // 600 - 67f
	{
		// Store K[7:1] to A1[7:1]
		util::stream_format(stream, "%04x: 0x%02x->A1", get_table(pc), inst & 0x7f);
	}
	else if (inst >= 0x680 && inst <= 0x6ff) // 680 - 6ff
	{
		// Store K[7:1] to A2[7:1]
		util::stream_format(stream, "%04x: 0x%02x->A2", get_table(pc), inst & 0x7f);
	}
	else if (inst >= 0x700 && inst <= 0x77f) // 680 - 6ff
	{
		// Store K[7:1] to A3[7:1]
		util::stream_format(stream, "%04x: 0x%02x->A3", get_table(pc), inst & 0x7f);
	}
	else if (inst >= 0x780 && inst <= 0x7ff)  // 780 - 7ff
	{
		// Store K[7:1] to A4[7:1]
		util::stream_format(stream, "%04x: 0x%02x->A4", get_table(pc), inst & 0x7f);
	}
	else if (inst < 0xc00) // 800 - bff
	{
		// Move K[10:1] to A[10:1], Jump to A[11:1]
		util::stream_format(stream, "%04x: JP 0x%03x", get_table(pc), inst & 0x3ff);
	}
	else if (inst < 0x1000) // c00 - fff
	{
		// Move K[10:1] to A[10:1], 0 to A11, Jump to A[11:1], Push next A[11:1] up to ROM address stack
		util::stream_format(stream, "%04x: JS 0x%03x", get_table(pc), inst & 0x3ff);
	}
	else
	{
		switch (inst)
		...
		{ // Shift STB[4:1], N->STB[1]
			util::stream_format(stream, "%04x: 0x%d->STB", get_table(pc), inst & 1);
			break;
		}
		...
		{ // Skip if (A1[7:1]·A1[7:1]) makes zero, N->L[2:1]
			util::stream_format(stream, "%04x: A1·A1, 0x%d->L EQJ", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // Skip if (A1[7:1]-A1[7:1]) makes borrow, N->L[2:1]
			util::stream_format(stream, "%04x: A1-A1, 0x%d->L BOJ", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // Skip if (A1[7:1]·A2[7:1]) makes zero, N->L[2:1]
			util::stream_format(stream, "%04x: A1·A2, 0x%d->L", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // Skip if (A1[7:1]-A2[7:1]) makes zero, N->L[2:1]  (typo on description?, should be =?)
			util::stream_format(stream, "%04x: A1=A2, 0x%d->L EQJ", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // Skip if (A1[7:1]-A2[7:1]) makes borrow, N->L[2:1]
			util::stream_format(stream, "%04x: A1-A2, 0x%d->L BOJ", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // Skip if (A2[7:1]·A1[7:1]) makes zero, N->L[2:1]
			util::stream_format(stream, "%04x: A2·A1, 0x%d->L EQJ", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // Skip if (A2[7:1]-A1[7:1]) makes borrow, N->L[2:1]
			util::stream_format(stream, "%04x: A2-A1, 0x%d->L BOJ", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // Skip if (A2[7:1]·A2[7:1]) makes zero, N->L[2:1]
			util::stream_format(stream, "%04x: A2·A2, 0x%d->L EQJ", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // Skip if (A2[7:1]-A2[7:1]) makes borrow, N->L[2:1]
			util::stream_format(stream, "%04x: A2-A2, 0x%d->L BOJ", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // Skip if (M[H[5:1],L[2:1]][7:1]·A1[7:1]) makes zero, N->L[2:1]
			util::stream_format(stream, "%04x: M·A1, 0x%d->L EQJ", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // Skip if (M[H[5:1],L[2:1]][7:1]-A1[7:1]) makes borrow, N->L[2:1]
			util::stream_format(stream, "%04x: M-A1, 0x%d->L BOJ", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // Skip if (M[H[5:1],L[2:1]][7:1]·A2[7:1]) makes zero, N->L[2:1]
			util::stream_format(stream, "%04x: M·A2, 0x%d->L EQJ", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // Skip if (M[H[5:1],L[2:1]][7:1]-A2[7:1]) makes borrow, N->L[2:1]
			util::stream_format(stream, "%04x: M-A2, 0x%d->L BOJ", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // Skip if (H[5:1]·A1[5:1]) makes zero, N->L[2:1]
			util::stream_format(stream, "%04x: H·A1, 0x%d->L EQJ", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // Skip if (H[5:1]·A2[5:1]) makes zero, N->L[2:1]
			util::stream_format(stream, "%04x: H·A2, 0x%d->L EQJ", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // Skip if (H[5:1]-A2[5:1]) makes borrow, N->L[2:1]
			util::stream_format(stream, "%04x: H-A2, 0x%d->L BOJ", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // Skip if (H[5:1]-A1[5:1]) makes non borrow, N->L[2:1]
			util::stream_format(stream, "%04x: H-A1, 0x%d->L BOJ/", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // N->L[2:1]
			util::stream_format(stream, "%04x: 0x%d->L", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // Move A1[7:1] to MODE[7:1], 1N->L[2:1]
			util::stream_format(stream, "%04x: A1->MODE, 0x%d->L", get_table(pc), (inst & 0x1) + 2);
			break;
		}
		...
		{ // Move A2[7:1] to A1[7:1], N->L[2:1
			util::stream_format(stream, "%04x: A2->A1, 0x%d->L", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // Right shift A1[7:1], 0A1[7], N->L[2:1]
			util::stream_format(stream, "%04x: A1->RS, 0x%d->L", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // Subtract A1[7:1] and A2[7:1], store to A2[7:1], Skip if borrow, N->L[2:1]
			util::stream_format(stream, "%04x: A1-A2->A2, 0x%d->L", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // AND A1[7:1] and A1[7:1], store to A1[7:1], N->L[2:1]
			util::stream_format(stream, "%04x: A1·A1->A1, 0x%d->L", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // OR A1[7:1] and A1[7:1], store to A1[7:1], N->L[2:1]
			util::stream_format(stream, "%04x: A1vA1->A1, 0x%d->L", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // AND A1[7:1] and A2[7:1], store to A1[7:1], N->L[2:1]
			util::stream_format(stream, "%04x: A1·A2->A1, 0x%d->L", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // OR A1[7:1] and A2[7:1], store to A1[7:1], N->L[2:1]
			util::stream_format(stream, "%04x: A1vA2->A1, 0x%d->L", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // Move A2[7:1] to MODE[7:1], 1N->L[2:1]
			util::stream_format(stream, "%04x: A2->MODE, 0x%d->L", get_table(pc), (inst & 0x1) + 2);
			break;
		}
		...
		{ // Subtract A2[7:1] and A1[7:1], store to A1[7:1], Skip if borrow, N->L[2:1]
			util::stream_format(stream, "%04x: A2-A1->A1, 0x%d->L", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // Right shift A2[7:1], 0A2[7], N->L[2:1]
			util::stream_format(stream, "%04x: A2->RS, 0x%d->L", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // AND A2[7:1] and A1[7:1], store to A2[7:1], N->L[2:1]
			util::stream_format(stream, "%04x: A2·A1->A2, 0x%d->L", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // OR A2[7:1] and A1[7:1], store to A2[7:1], N->L[2:1]
			util::stream_format(stream, "%04x: A2vA1->A2, 0x%d->L", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // AND A2[7:1] and A2[7:1], store to A2[7:1], N->L[2:1]
			util::stream_format(stream, "%04x: A2·A2->A2, 0x%d->L", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // OR A2[7:1] and A2[7:1], store to A2[7:1], N->L[2:1]
			util::stream_format(stream, "%04x: A2vA2->A2, 0x%d->L", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // Move A1[7:1] to M[H[5:1],L[2:1]][7:1], N->L[2:1]
			util::stream_format(stream, "%04x: A1->M, 0x%d->L", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // Exchange M[H[5:1],L[2:1]][7:1] and A1[7:1], N->L[2:1]
			util::stream_format(stream, "%04x: M<->A1, 0x%d->L", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // Move M[H[5:1],L[2:1]][7:1] to MODE[7:1], 1N->L[2:1]
			util::stream_format(stream, "%04x: M->MODE, 0x%d->L", get_table(pc), (inst & 0x1) + 2);
			break;
		}
		...
		{ // Move A2[7:1] to M[H[5:1],L[2:1]][7:1], N->L[2:1]
			util::stream_format(stream, "%04x: A2->M, 0x%d->L", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // Exchange M[H[5:1],L[2:1]][7:1] and A2[7:1], N->L[2:1]
			util::stream_format(stream, "%04x: M<->A2, 0x%d->L", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // Right shift M[H[5:1],L[2:1]][7:1], 0M[H[5:1],L[2:1]][7], N->L[2:1]
			util::stream_format(stream, "%04x: M->RS, 0x%d->L", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // AND M[H[5:1],L[2:1]][7:1] and A1[7:1], store to M[H[5:1],L[2:1]][7:1], N->L[2:1]
			util::stream_format(stream, "%04x: M·A1->M, 0x%d->L", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // OR M[H[5:1],L[2:1]][7:1] and A1[7:1], store to M[H[5:1],L[2:1]][7:1], N->L[2:1]
			util::stream_format(stream, "%04x: MvA1->M, 0x%d->L", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // AND M[H[5:1],L[2:1]][7:1] and A2[7:1], store to M[H[5:1],L[2:1]][7:1], N->L[2:1]
			util::stream_format(stream, "%04x: M·A2->M, 0x%d->L", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // OR M[H[5:1],L[2:1]][7:1] and A2[7:1], store to M[H[5:1],L[2:1]][7:1], N->L[2:1]
			...
			util::stream_format(stream, "%04x: MvA2->M, 0x%d->L", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // Move A1[5:1] to H[5:1], N->L[2:1]
			util::stream_format(stream, "%04x: A1->H, 0x%d->L", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // Move H[5:1] to A1[5:1], 0->A1[7:6], N->L[2:1]
			util::stream_format(stream, "%04x: H->A1, 0x%d->L", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // Move A2[5:1] to H[5:1], N->L[2:1]
			util::stream_format(stream, "%04x: A2->H, 0x%d->L", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // Move H[5:1] to A2[5:1], 0->A2[7:6], N->L[2:1]
			util::stream_format(stream, "%04x: H->A2, 0x%d->L", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // AND H[5:1] and A1[5:1], store to H[5:1], N->L[2:1]
			util::stream_format(stream, "%04x: H·A1->H, 0x%d->L", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // OR H[5:1] and A1[5:1], store to H[5:1], N->L[2:1]
			util::stream_format(stream, "%04x: HvA1->H, 0x%d->L", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // AND H[5:1] and A2[5:1], store to H[5:1], N->L[2:1]
			util::stream_format(stream, "%04x: H·A2->H, 0x%d->L", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // OR H[5:1] and A2[5:1], store to H[5:1], N->L[2:1]
			util::stream_format(stream, "%04x: HvA2->H, 0x%d->L", get_table(pc), inst & 0x3);
			break;
		}
		...
		{ // Set D to DISP, G to GPE, K to KIE, S to SME, NA[11]
			util::stream_format(stream, "%04x: %d->D, %d->G, %d->K, %d->S, %d->A11", get_table(pc), (inst >> 5) & 0x1, (inst >> 4) & 0x1, (inst >> 3) & 0x1, (inst >> 2) & 0x1, inst & 0x1);
			break;
		}

...
void adsp21062_device::static_generate_pop_loop()
{
	...
	UML_LOAD(block, LCNTR, m_core->lcstack, I0, SIZE_DWORD, SCALE_x4);
	...
	UML_LABEL(block, empty);
	UML_LOAD(block, LCNTR, m_core->lcstack, I0, SIZE_DWORD, SCALE_x4);
	UML_MOV(block, CURLCNTR, 0xffffffff);
	UML_MOV(block, mem(&m_core->laddr.addr), 0x00ffffff);
	UML_MOV(block, mem(&m_core->laddr.code), 0x1f);
	UML_MOV(block, mem(&m_core->laddr.loop_type), 0x3);
...
void adsp21062_device::static_generate_pop_status()
{
	...
	// DAG1 regs 4-7
	skip = label++;
	UML_TEST(block, I2, 0x8);
	UML_JMPc(block, COND_Z, skip);        // don't swap if the bits are same
	UML_CALLH(block, *m_swap_dag1_4_7);
	UML_LABEL(block, skip);

	// DAG1 regs 0-3
	skip = label++;
	UML_TEST(block, I2, 0x10);
	UML_JMPc(block, COND_Z, skip);        // don't swap if the bits are same
...
void adsp21062_device::generate_jump(drcuml_block &block, compiler_state &compiler, const opcode_desc *desc, bool delayslot, bool loopabort, bool clearint)
{
	...
	if (clearint)
	{
		...
		UML_CALLHc(block, COND_NZ, *m_pop_status);                              // callh   m_pop_status,nz
	}
		...
		case 0:             // subops
		{
			...
			switch (subop)
			{
				...
				case 0x0b:          // return from interrupt / compute      |000|01011|
				{
					...
					UML_MOV(block, mem(&m_core->interrupt_active), 0);          // mov     [interrupt_active],0
					UML_SHL(block, I0, 1, mem(&m_core->active_irq_num));        // shl     i0,1,[active_irq_num]
				...
				case 0x0c:          // do until counter expired             |000|01100|
				{
					...
					UML_MOV(block, mem(&m_core->laddr.loop_type), 0x3); // TODO
					return true;
				}
				...
				case 0x17:          // push|pop stacks / flush cache        |000|10111|
				{
					...
					{
						UML_CALLH(block, *m_push_pc);
						UML_LOAD(block, PCSTK, m_core->pcstack - 1, PCSTKP, SIZE_DWORD, SCALE_x4);
					}

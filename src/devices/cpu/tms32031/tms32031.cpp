...
uint32_t tms3203x_device::opcode_alignment() const
{
	return 1;
}
...
offs_t tms3203x_device::disassemble(std::ostream &stream, offs_t pc, const data_buffer &opcodes, const data_buffer &params, uint32_t options)
{
	extern CPU_DISASSEMBLE( tms3203x );
	return CPU_DISASSEMBLE_NAME(tms3203x)(this, stream, pc, opcodes, params, options);
}

...
void psxdma_device::write(address_space &space, offs_t offset, uint32_t data, uint32_t mem_mask)
{
	int index = offset / 4;
	psx_dma_channel *dma = &m_channel[ index ];
...
uint32_t psxdma_device::read(address_space &space, offs_t offset, uint32_t mem_mask)
{
	int index = offset / 4;
	psx_dma_channel *dma = &m_channel[ index ];

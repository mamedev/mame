...
uint32_t pdp8_device::opcode_alignment() const
{
	return 2;
}
...
offs_t pdp8_device::disassemble(std::ostream &stream, offs_t pc, const data_buffer &opcodes, const data_buffer &params, uint32_t options)
{
	extern CPU_DISASSEMBLE( pdp8 );
	return CPU_DISASSEMBLE_NAME(pdp8)(this, stream, pc, opcodes, params, options);
}

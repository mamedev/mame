...
void mk3835_device::nvram_default()
{
	if (m_region.found())
	{
		if (m_region->bytes() != m_ram.size() + m_rtc.size())
			fatalerror("mk3835 region '%s' wrong size (expected size = 0x20)\n", tag());

		std::copy_n(m_region->base(), m_ram.size(), m_ram.begin());
		std::copy_n(m_region->base() + m_ram.size(), m_rtc.size(), m_rtc.begin());
...
bool mk3835_device::nvram_write(util::write_stream &file)
{
	...
	if (err || actual != m_ram.size())
		return false;
	auto const [clock_err, clock_actual] = write(file, m_rtc.data(), m_rtc.size());
	return !clock_err && clock_actual == m_rtc.size();
}

void mk3835_device::rtc_clock_updated(int year, int month, int day, int day_of_week, int hour, int minute, int second)
{
	if (m_time_initialized && BIT(m_rtc[0], 7))
	{
		restore_clock();
		return;
	}
	m_time_initialized = true;
	m_rtc[0] = (m_rtc[0] & 0x80) | convert_to_bcd(second);
	m_rtc[1] = convert_to_bcd(minute);
	m_rtc[2] = BIT(m_rtc[2], 7)
		? 0x80 | (hour >= 12 ? 0x20 : 0) | convert_to_bcd(hour % 12 ? hour % 12 : 12)
		: convert_to_bcd(hour);
	m_rtc[3] = (m_rtc[3] & 0x80) | convert_to_bcd(day);
	m_rtc[4] = convert_to_bcd(month);
	m_rtc[5] = (m_rtc[5] & 0x80) | convert_to_bcd(day_of_week);
	m_rtc[6] = convert_to_bcd(year % 100);
}

void mk3835_device::restore_clock()
{
	const int hour = BIT(m_rtc[2], 7)
		? bcd_to_integer(m_rtc[2] & 0x1f) % 12 + (BIT(m_rtc[2], 5) ? 12 : 0)
		: bcd_to_integer(m_rtc[2] & 0x3f);
	set_time(false, bcd_to_integer(m_rtc[6]), bcd_to_integer(m_rtc[4]),
		bcd_to_integer(m_rtc[3] & 0x3f), m_rtc[5] & 7, hour,
		bcd_to_integer(m_rtc[1]), bcd_to_integer(m_rtc[0] & 0x7f));
}
...
void mk3835_device::advance_output()
{
	...
	{
		if (m_burst)
			m_address = (m_address + 1) % (m_ram_selected ? m_ram.size() : m_rtc.size());
		m_shift = read_byte();
		m_bits = 0;
	}
...
void mk3835_device::write_byte(u8 data)
{
	if (BIT(m_rtc[7], 7))
	{
		if (!m_ram_selected && m_address == 7)
			m_rtc[7] = (m_rtc[7] & 0x7f) | (data & 0x80);
	}
	...
	else if (m_address < m_rtc.size())
	{
		...
		m_rtc[m_address] = data & masks[m_address];
		if (m_address < 7)
		{
			int value = bcd_to_integer(m_rtc[m_address] & (m_address == 6 ? 0xff : 0x7f));
	...
	m_transfer_done = !m_burst || ++m_address >= (m_ram_selected ? m_ram.size() : m_rtc.size());
}
...
u8 mk3835_device::read_byte() const
{
	...
	return (m_address < m_rtc.size()) ? m_rtc[m_address] : 0xff;
}

...
public:
	...
	void start()
	{
		...
		m_thread = std::thread(
				[this] ()
				{
					LOG("MB89372: network thread started\n");
					try {
						m_ioctx.run();
					} catch (const std::exception& e) {
						LOG("MB89372: Exception in network thread: %s\n", e.what());
					} catch (...) { // Catch any other unknown exceptions
						LOG("MB89372: Unknown exception in network thread\n");
					}
					LOG("MB89372: network thread completed\n");
				});
	}

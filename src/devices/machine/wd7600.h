...
public:
	...
	wd7600_device(const machine_config &mconfig, const char *tag, device_t *owner, u32 clock);
	...
	u16 refresh_r();
	void refresh_w(u16 data);
	u16 chipsel_r();
	void chipsel_w(u16 data);
	u16 mem_ctrl_r();
	void mem_ctrl_w(u16 data);
	u16 bank_01_start_r(offs_t offset, u16 mem_mask);
	void bank_01_start_w(offs_t offset, u16 data, u16 mem_mask);
	u16 bank_23_start_r(offs_t offset, u16 mem_mask);
	void bank_23_start_w(offs_t offset, u16 data, u16 mem_mask);
	u16 split_addr_r();
	void split_addr_w(u16 data);
	u16 diag_r();
	void diag_w(u16 data);
...
private:
	...
	u8 pic1_slave_ack_r(offs_t offset);
	void ctc_out1_w(int state);
	void ctc_out2_w(int state);
	void rtc_nmi_w(u8 data);
	void keyb_cmd_w(u8 data);
	void keyb_data_w(u8 data);
	u8 keyb_data_r();
	u8 keyb_status_r();
	void a20_reset_w(u8 data);
	u8 a20_reset_r();
	u8 portb_r();
	void portb_w(u8 data);
	void dma_page_w(offs_t offset, u8 data) { m_dma_page[offset & 0x0f] = data; }
	u8 dma_page_r(offs_t offset) { return m_dma_page[offset & 0x0f]; }
	...
	u8 dma2_ior1_r() { u16 result = m_read_ior(5); m_dma_high_byte = result >> 8; return result; }
	u8 dma2_ior2_r() { u16 result = m_read_ior(6); m_dma_high_byte = result >> 8; return result; }
	u8 dma2_ior3_r() { u16 result = m_read_ior(7); m_dma_high_byte = result >> 8; return result; }

...
//  const uint16_t bgcol = 0x0000; // black
	...
	for (uint32_t scanline = (uint32_t)cliprect.min_y; scanline <= (uint32_t)cliprect.max_y; scanline++)
	{
		...
		for (int i = 0; i < 4; i++)
		{
			...
			m_renderer->draw_sprites(true, m_use_legacy_mode ? false : true, m_alt_extrasprite_hack ? true : false, m_703a_palettebank, highres, cliprect, scanline, i, sprites_addr, mem, m_paletteram, m_spriteram, -1);
		}

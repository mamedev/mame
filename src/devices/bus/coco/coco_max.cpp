...
INPUT_PORTS_START( cocomax_mouse )
	...
	PORT_BIT( 0xff, 0x80,  IPT_MOUSE_X) PORT_NAME("CoCo Max Mouse X") PORT_SENSITIVITY(MOUSE_SENSITIVITY) PORT_KEYDELTA(MOUSE_DELTA) PORT_MINMAX(0x00,0xff) PORT_CODE_DEC(KEYCODE_4_PAD) PORT_CODE_INC(KEYCODE_6_PAD) PORT_CODE_DEC(JOYCODE_X_LEFT_SWITCH) PORT_CODE_INC(JOYCODE_X_RIGHT_SWITCH) PORT_PLAYER(1)
	PORT_START(COCOMAX_Y_TAG)
	PORT_BIT( 0xff, 0x80,  IPT_MOUSE_Y) PORT_NAME("CoCo Max Mouse Y") PORT_SENSITIVITY(MOUSE_SENSITIVITY) PORT_KEYDELTA(MOUSE_DELTA) PORT_MINMAX(0x00,0xff) PORT_CODE_DEC(KEYCODE_8_PAD) PORT_CODE_INC(KEYCODE_2_PAD) PORT_CODE_DEC(JOYCODE_Y_UP_SWITCH) PORT_CODE_INC(JOYCODE_Y_DOWN_SWITCH) PORT_PLAYER(1)
...
u8 coco_pak_max_device::ff60_read(offs_t offset)
{

...
#ifndef MAME_BUS_A2BUS_A2FROBTIA_H
#define MAME_BUS_A2BUS_A2FROBTIA_H
...
/***************************************************************************
  ...
  (copied from of mame/atari/tia.h for use with the frob card)
...
// ======================> a2frobtia_video_device

class a2frobtia_video_device : public device_t, public device_video_interface, public device_palette_interface
{
public:
...
protected:
	...
	a2frobtia_video_device(const machine_config &mconfig, device_type type, const char *tag, device_t *owner, uint32_t clock);

	template <typename T> void set_a2frobtia_tag(T &&tag) { m_tia.set_tag(std::forward<T>(tag)); }
...
class a2frobtia_pal_video_device : public a2frobtia_video_device
{
public:
	template <typename T> a2frobtia_pal_video_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock, T &&a2frobtia_tag)
		: a2frobtia_pal_video_device(mconfig, tag, owner, clock)
	{
		set_a2frobtia_tag(std::forward<T>(a2frobtia_tag));
	}

	a2frobtia_pal_video_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock);

protected:
	virtual void init_palette() override;
};

class a2frobtia_ntsc_video_device : public a2frobtia_video_device
{
public:
	template <typename T> a2frobtia_ntsc_video_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock, T &&a2frobtia_tag)
		: a2frobtia_ntsc_video_device(mconfig, tag, owner, clock)
	{
		set_a2frobtia_tag(std::forward<T>(a2frobtia_tag));
	}

	a2frobtia_ntsc_video_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock);
...
DECLARE_DEVICE_TYPE(A2FROBTIA_PAL_VIDEO, a2frobtia_pal_video_device)
DECLARE_DEVICE_TYPE(A2FROBTIA_NTSC_VIDEO, a2frobtia_ntsc_video_device)

#endif // MAME_BUS_A2BUS_A2FROBTIA_H

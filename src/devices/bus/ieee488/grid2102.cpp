...
#define GRID2102_DEV_ADDR 5

#define GRID2102_GPIB_STATE_IDLE 0
#define GRID2102_GPIB_STATE_WAIT_DAV_FALSE 1
#define GRID2102_GPIB_STATE_SEND_DATA_START 2
#define GRID2102_GPIB_STATE_WAIT_NDAC_FALSE 3

#define GRID2102_STATE_IDLE 0
#define GRID2102_STATE_READING_DATA 1
#define GRID2102_STATE_WRITING_DATA 2
#define GRID2102_STATE_WRITING_DATA_WAIT 3

static uint8_t grid2102_identify_response[0x34] = {0x00, 0x02, 0xf8, 0x01, 0xD0, 0x02, 0x01, 0x20, 0x01, 0x21, 0x01, 0x01, 0x00, 0x00,
...
grid2102_device::grid2102_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock)
	: device_t(mconfig, GRID2102, tag, owner, clock),
      ...
      m_gpib_loop_state(GRID2102_GPIB_STATE_IDLE),
...
void grid2102_device::device_timer(emu_timer &timer, device_timer_id id, int param, void *ptr) {
    if (m_floppy_loop_state == GRID2102_STATE_READING_DATA) {
        uint8_t data[512];
...
void grid2102_device::ieee488_eoi(int state) {
    // logerror("grid2102_device eoi state set to %d\n", state);
}
...
void grid2102_device::ieee488_dav(int state) {
    if(state == 0 && m_gpib_loop_state == GRID2102_GPIB_STATE_IDLE) {
        ...
        m_last_recv_eoi = eoi;
        m_bus->ndac_w(this, 1);
        m_gpib_loop_state = GRID2102_GPIB_STATE_WAIT_DAV_FALSE;
    } else if (state == 1 && m_gpib_loop_state == GRID2102_GPIB_STATE_WAIT_DAV_FALSE) {
        ...
        if (talking) {
            if (serial_polling) {
                ...
                serial_poll_byte = 0;
                m_send_eoi = 1;
                m_gpib_loop_state = GRID2102_GPIB_STATE_SEND_DATA_START;
...
void grid2102_device::ieee488_nrfd(int state) {
    if (state == 1 && m_gpib_loop_state == GRID2102_GPIB_STATE_SEND_DATA_START) {
...
void grid2102_device::ieee488_ndac(int state) {
    if (state == 1 && m_gpib_loop_state == GRID2102_GPIB_STATE_WAIT_NDAC_FALSE) {
        // restore initial state
        // logerror("grid2102_device restore ndac nrfd dav eoi\n");
        m_bus->nrfd_w(this, 1);
        m_bus->dav_w(this, 1);
        m_bus->eoi_w(this, 1);
        m_gpib_loop_state = GRID2102_GPIB_STATE_IDLE;
    ...
    // logerror("grid2102_device ndac state set to %d\n", state);
}

void grid2102_device::ieee488_ifc(int state) {
    // logerror("grid2102_device ifc state set to %d\n", state);
}
...
void grid2102_device::ieee488_atn(int state) {
    // logerror("grid2102_device atn state set to %d\n", state);
    update_ndac(state ^ 1);
}

void grid2102_device::update_ndac(int atn) {
    if (m_gpib_loop_state == GRID2102_GPIB_STATE_IDLE) {
        if (atn) {
            // pull NDAC low
            m_bus->ndac_w(this, 0);
        } else {
...
void grid2102_device::ieee488_ren(int state) {
    logerror("grid2102_device ren state set to %d\n", state);
}

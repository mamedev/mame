...
//**************************************************************************
//  GBCK003 board soldered inside some units
//**************************************************************************
/*
 Seems to be a kind of MBC1 emulator, like MMM01 but less sophisticated.

 Base page for game right shifted by one bit position is written to 0x7B00.
 Fine bank mask right shifted by one bit position is written to 0x7B01.
 It always seems to write 0xF0 to 0x7B02 before jumping to the game.
 */

class gbck003_device : public flat_ram_device_base<mbc_dual_uniform_device_base>
{
public:
	gbck003_device(machine_config const &mconfig, char const *tag, device_t *owner, uint32_t clock) :
	...
	virtual image_init_result load(std::string &message) override ATTR_COLD
	{
		...
		// install bank switch handlers
		cart_space()->install_write_handler(0x2000, 0x3fff, write8smo_delegate(*this, FUNC(gbck003_device::bank_switch_inner)));
		cart_space()->install_write_handler(0x7b00, 0x7b00, write8smo_delegate(*this, FUNC(gbck003_device::bank_switch_outer)));
		cart_space()->install_write_handler(0x7b01, 0x7b01, write8smo_delegate(*this, FUNC(gbck003_device::bank_set_mux)));
		cart_space()->install_write_handler(0x7b02, 0x7b02, write8smo_delegate(*this, FUNC(gbck003_device::exit_config_mode)));
...
DEFINE_DEVICE_TYPE_PRIVATE(GB_ROM_GBCK003,      device_gb_cart_interface, bus::gameboy::gbck003_device,         "gb_rom_gbck003",      "Game Boy GBCK003 188 in 1 Game Board")

...
public:
	...
	static void or_bits(u8 const *source, unsigned sbitoff, u8 *dest, int numbit)
	{
		sbitoff--;
	...
	static void deweave(std::array<std::array<u8, PACKET_LEN>, 10> &pipes, u8 const *data)
	{
		unsigned cursor = 0;
	...
	static void get_data(u8 const *in, u8 *out)
	{
...
#endif // MAME_BUS_MEGADRIVE_CART_SEGACH_IMG_H

...
uint8_t msx_cart_dooly::read_cart(address_space &space, offs_t offset, uint8_t mem_mask)
{
...
void msx_cart_dooly::write_cart(address_space &space, offs_t offset, uint8_t data, uint8_t mem_mask)
{
	if (offset >= 0x4000 && offset < 0xc000)
	{
		m_prot = data & 0x07;

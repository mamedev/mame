...
class multibus_sun1_device
	: public device_t
	, public device_multibus_interface
	, public m68000_device::mmu
{
public:
	multibus_sun1_device(machine_config const &mconfig, char const *tag, device_t *owner, u32 clock)
		: multibus_sun1_device(mconfig, MULTIBUS_SUN1, tag, owner, clock)
	{
	}

protected:
	multibus_sun1_device(machine_config const &mconfig, device_type type, char const *tag, device_t *owner, u32 clock)
	...
	required_device<sun1_mmu_device> m_mmu;
...
void multibus_sun1_device::device_reset()
{
...
void multibus_sun1_device::device_add_mconfig(machine_config &config)
{
	...
	int_callback<1>().set(FUNC(multibus_sun1_device::irq_w<M68K_IRQ_1>)).invert();
	int_callback<2>().set(FUNC(multibus_sun1_device::irq_w<M68K_IRQ_2>)).invert();
	int_callback<3>().set(FUNC(multibus_sun1_device::irq_w<M68K_IRQ_3>)).invert();
	int_callback<4>().set(FUNC(multibus_sun1_device::irq_w<M68K_IRQ_4>)).invert();

	SUN1_MMU(config, m_mmu);
	m_mmu->error().set(
		[this](int state)
		{
			// check multibus access pending
			if (state == sun1_mmu_device::MMU_DEFER)
			{
	...
	m_stc->out3_cb().set(FUNC(multibus_sun1_device::irq_w<M68K_IRQ_7>));
	...
	if (false)
	{
		...
		m_stc->out5_cb().set(FUNC(multibus_sun1_device::irq_w<M68K_IRQ_7>));
	...
	m_duart->out_int_callback().set(FUNC(multibus_sun1_device::irq_w<M68K_IRQ_5>));
...
void multibus_sun1_device::device_config_complete()
{
...
void multibus_sun1_device::cpu_mem(address_map &map)
{
	...
	map(0x20'0000, 0x20'3fff).mirror(0x03'c000).rom().region("prom0", 0).w(FUNC(multibus_sun1_device::prom0_w));
	...
	map(0xa0'0000, 0xbf'ffff).rw(m_mmu, FUNC(sun1_mmu_device::page_r), FUNC(sun1_mmu_device::page_w));
	map(0xc0'0000, 0xdf'ffff).rw(m_mmu, FUNC(sun1_mmu_device::segment_r), FUNC(sun1_mmu_device::segment_w));
	map(0xe0'0000, 0xe0'0001).mirror(0x1f'fffe).r(FUNC(multibus_sun1_device::parallel_r));
	map(0xe0'0000, 0xe0'0001).mirror(0x1f'fffe).w(m_mmu, FUNC(sun1_mmu_device::context_w));
}

void multibus_sun1_device::watchdog_w(int state)
{
	if (state)
		m_irq |= WATCHDOG;
	else
...
template <unsigned N> void multibus_sun1_device::irq_w(int state)
{
	if (state)
...
void multibus_sun1_device::prom0_w(u16 data)
{
	m_parity = BIT(data, 0);
...
const tiny_rom_entry *multibus_sun1_device::device_rom_region() const
{
...
ioport_constructor multibus_sun1_device::device_input_ports() const
{
	return INPUT_PORTS_NAME(sun1);
}

class multibus_sgi_pm1_device : public multibus_sun1_device
{
public:
	multibus_sgi_pm1_device(machine_config const &mconfig, char const *tag, device_t *owner, u32 clock)
		: multibus_sun1_device(mconfig, MULTIBUS_SGI_PM1, tag, owner, clock)
	{
	}
...
void multibus_sgi_pm1_device::device_reset()
{
	...
	m_cpu->set_clock(16_MHz_XTAL / 2);
	m_stc->set_clock(16_MHz_XTAL / 2);
	m_duart->set_clock(16_MHz_XTAL / 2);
}

void multibus_sgi_pm1_device::device_add_mconfig(machine_config &config)
{
	multibus_sun1_device::device_add_mconfig(config);

	// TODO: mouse
}

ROM_START(sgi_pm1)
	ROM_SYSTEM_BIOS(0, "v13", "SGI Monitor, Version 1.3")

	ROM_REGION16_BE(0x4000, "prom0", 0)
	ROMX_LOAD("sgi1400_sun13.101.bin", 0x0001, 0x2000, CRC(3c17b4a2) SHA1(f8bb8e6236fd1592a0d77004af3c3749bb770448), ROM_BIOS(0) | ROM_SKIP(1))
	ROMX_LOAD("sgi1400_sun13.103.bin", 0x0000, 0x2000, CRC(0c67bb14) SHA1(cfcc1f27433158fe26ecab9ed776c47e9a9c0f28), ROM_BIOS(0) | ROM_SKIP(1))

	ROM_REGION16_BE(0x4000, "prom1", 0)
	ROMX_LOAD("sgi1400_sun13.102.bin", 0x0001, 0x2000, CRC(45c74d1c) SHA1(380094f0283f9cb81b3ba59faa2028d359bcc589), ROM_BIOS(0) | ROM_SKIP(1))
	ROMX_LOAD("sgi1400_sun13.104.bin", 0x0000, 0x2000, CRC(3ad7773e) SHA1(ec2827ec430af78446ec1c11a99742ae3be59521), ROM_BIOS(0) | ROM_SKIP(1))
ROM_END

const tiny_rom_entry *multibus_sgi_pm1_device::device_rom_region() const
{
	return ROM_NAME(sgi_pm1);
}

static INPUT_PORTS_START(sgi_pm1)
INPUT_PORTS_END

ioport_constructor multibus_sgi_pm1_device::device_input_ports() const
{

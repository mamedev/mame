...
static INPUT_PORTS_START(am100kbd)
	...
	PORT_START("KEY3")
	PORT_BIT(0x01, IP_ACTIVE_LOW, IPT_KEYBOARD) PORT_NAME("Retrn") PORT_CODE(KEYCODE_ENTER) PORT_CODE(KEYCODE_ENTER_PAD) PORT_CHAR(0x0d) // also Enter on keypad?
	...
	PORT_BIT(0x40, IP_ACTIVE_LOW, IPT_KEYBOARD) PORT_NAME("Break") PORT_CODE(KEYCODE_DEL) PORT_CHAR(0x03) // to right of ]
	...
	PORT_BIT(0x08, IP_ACTIVE_LOW, IPT_KEYBOARD) PORT_NAME("Reset") PORT_CODE(KEYCODE_ASTERISK) PORT_CHANGED_MEMBER(DEVICE_SELF, FUNC(am100kbd_device::reset_changed), 0)
...
DEFINE_DEVICE_TYPE_PRIVATE(AM100_KEYBOARD, device_a2kbd_interface, am100kbd_device, "am100kbd", "AM 100 Keyboard Unit (MAK-II)")

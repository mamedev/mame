...
uint16_t kof2k3bl_prot_device::protection_r(address_space &space, offs_t offset, uint16_t mem_mask)
{
	return m_cartridge_ram[offset];
}

uint16_t kof2k3bl_prot_device::overlay_r(address_space &space, offs_t offset, uint16_t mem_mask) // hack?
{
	return m_overlay;
}

void kof2k3bl_prot_device::kof2003_w(address_space &space, offs_t offset, uint16_t data, uint16_t mem_mask)
{
...
void kof2k3bl_prot_device::kof2003p_w(address_space &space, offs_t offset, uint16_t data, uint16_t mem_mask)
{
	data = COMBINE_DATA(&m_cartridge_ram[offset]);
	if (offset == 0x1ff0/2 || offset == 0x1ff2/2)
	{

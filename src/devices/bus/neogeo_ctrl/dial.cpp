...
uint8_t neogeo_dial_device::in0_r(address_space &space, offs_t offset, uint8_t mem_mask)
{
	uint8_t res = 0;
	if (m_ctrl_sel & 0x01)
		res = m_joy1->read();
...
uint8_t neogeo_dial_device::in1_r(address_space &space, offs_t offset, uint8_t mem_mask)
{
	uint8_t res = 0;
	if (m_ctrl_sel & 0x01)
		res = m_joy2->read();

...
sp400_device::sp400_device(const machine_config &mconfig, const char *tag, device_t *owner, u32 clock):
...
void sp400_device::device_start()
{
}

void sp400_device::dser_map(address_map &map)
{
	map(0x0000, 0x0fff).rom();
}

void sp400_device::dser_p1_w(uint8_t v)
{
	m_dserdata = v;
}

void sp400_device::dser_p2_w(uint8_t v)
{
	m_busy = BIT(v, 5);
...
uint8_t sp400_device::dser_p2_r()
{
	return
		(m_motbusy<<7)|
		(m_dserstrobe<<6)|
...
void sp400_device::mot_pb_w(uint8_t v)
{
	m_plotter->update_motors(v&0x0F, (v>>4)&0x0F);
}


uint8_t sp400_device::mot_pc_r()
{
	return
		(m_motPenUp<<0)|
		(m_motPenDown<<1)|
...
void sp400_device::mot_pc_w(uint8_t v)
{
	m_motPenUp = BIT(v, 0);
...
uint8_t sp400_device::mot_pd_r()
{
	return sp400_device::m_dserdata;
}

void sp400_device::update_pen_state()
{
	if (!m_motPenDown)
	{
		m_plotter->pen_down();
...
void sp400_device::device_add_mconfig(machine_config &config)
{
	I8035(config, m_dsercpu, 4_MHz_XTAL);
	m_dsercpu->p1_out_cb().set(FUNC(sp400_device::dser_p1_w));
	m_dsercpu->p2_in_cb().set(FUNC(sp400_device::dser_p2_r));
	m_dsercpu->p2_out_cb().set(FUNC(sp400_device::dser_p2_w));
	m_dsercpu->set_addrmap(AS_PROGRAM, &sp400_device::dser_map);
	M6805U3(config, m_motcpu, 4_MHz_XTAL);
	m_motcpu->porta_r().set(FUNC(sp400_device::mot_pa_r));
	m_motcpu->portb_w().set(FUNC(sp400_device::mot_pb_w));
	m_motcpu->portc_r().set(FUNC(sp400_device::mot_pc_r));
	m_motcpu->portc_w().set(FUNC(sp400_device::mot_pc_w));
	m_motcpu->portd_r().set(FUNC(sp400_device::mot_pd_r));
	ALPS_DPG1302(config, m_plotter);
	m_plotter->set_panel_update(FUNC(sp400_device::update_panel));
}

uint32_t sp400_device::update_panel(screen_device &screen, bitmap_rgb32 &bitmap, const rectangle &cliprect)
{
...
INPUT_CHANGED_MEMBER(sp400_device::misc_interaction)
{
...
const tiny_rom_entry * sp400_device::device_rom_region() const
{
...
static INPUT_PORTS_START( sp400_ipt )
	...
	PORT_START("MISC")
	PORT_BIT(0x01, IP_ACTIVE_LOW, IPT_KEYBOARD) PORT_CHANGED_MEMBER(DEVICE_SELF, sp400_device, misc_interaction, 0 )  PORT_NAME("Save snapshot") PORT_CODE(KEYCODE_5_PAD)
	PORT_BIT(0x02, IP_ACTIVE_LOW, IPT_KEYBOARD) PORT_CHANGED_MEMBER(DEVICE_SELF, sp400_device, misc_interaction, 0 )  PORT_NAME("Change paper") PORT_CODE(KEYCODE_8_PAD)
INPUT_PORTS_END

ioport_constructor sp400_device::device_input_ports() const
{
	return INPUT_PORTS_NAME(sp400_ipt);
}

DEFINE_DEVICE_TYPE(SP400_PRINTER, sp400_device, "sp_400", "SP-400 Plotter")

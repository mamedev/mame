...
uint32_t p2000_hires_device::screen_update(screen_device &screen, bitmap_rgb32 &bitmap, const rectangle &cliprect)
{
    ...
    */
        ...
        BIT(m_hires_image_select, HIRES_IMAGE_GREEN_ENABLE_BIT))
    {
        ...
        // Each row has 256 bits => 32 bytes so ofset needs a correction of 32 bytes
        ...
        {
            // Y-lines are stored reversed in memory also take scroll reg into account (255 = no scroll., 0 = max scroll)
            ypos = (255 - m_hires_scroll_reg + yposcnt) % 256;

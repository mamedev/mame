...
void buddha_device::update_interrupts()
{
	int2_w(m_ide_interrupts_enabled && (m_ide_0_interrupt || m_ide_1_interrupt));
}
...
void buddha_device::autoconfig_base_address(offs_t address)
{
	...
	zorro_space().unmap_readwrite(0xe80000, 0xe8ffff);

	// install buddha memory access to final location
	zorro_space().install_device(address, address + 0xffff, *this, &buddha_device::mmio_map);
...
void buddha_device::cfgin_w(int state)
{
	...
	if (state == 0)
	{
		...
		zorro_space().install_device(0xe80000, 0xe8ffff, *this, &buddha_device::mmio_map);

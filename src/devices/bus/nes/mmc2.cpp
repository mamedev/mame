...
void nes_pxrom_device::ppu_latch(offs_t offset)
{
	...
	{
		LOGMASKED(LOG_GENERAL, "mmc2 vrom latch switch (bank 0 low): %02x\n", m_reg[0]);
		m_latch1 = 0xfd;
		chr4_0(m_reg[0], CHRROM);
	}
	else if ((offset & 0x3ff0) == 0x0fe0)
	{
		LOGMASKED(LOG_GENERAL, "mmc2 vrom latch switch (bank 0 high): %02x\n", m_reg[1]);
		m_latch1 = 0xfe;
		chr4_0(m_reg[1], CHRROM);
	}
	else if ((offset & 0x3ff0) == 0x1fd0)
	{
		LOGMASKED(LOG_GENERAL, "mmc2 vrom latch switch (bank 1 low): %02x\n", m_reg[2]);
		m_latch2 = 0xfd;
...
{
	LOGMASKED(LOG_GENERAL, "pxrom write_h, offset: %04x, data: %02x\n", offset, data);
	switch (offset & 0x7000)
	{
		...
		case 0x7000:
			...
			break;
		default:
			LOGMASKED(LOG_GENERAL, "MMC2 uncaught w: %04x:%02x\n", offset, data);
			break;
	}
}
...
{
	LOGMASKED(LOG_GENERAL, "fxrom write_h, offset: %04x, data: %02x\n", offset, data);
	switch (offset & 0x7000)
	{
		case 0x2000:
			prg16_89ab(data);

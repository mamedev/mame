...
DEFINE_DEVICE_TYPE(ISA16_AVGA1_CIRRUS_GD5401, isa16_svga_cirrus_gd5401_device, "clgd5401",    "Cirrus Logic GD5401 AVGA1 Graphics Card")
DEFINE_DEVICE_TYPE(ISA16_SVGA_CIRRUS_GD542X,  isa16_svga_cirrus_gd542x_device, "clgd542x",    "Generic Cirrus Logic GD542X Graphics Card (BIOS v1.20)")
DEFINE_DEVICE_TYPE(ISA16_SVGA_CIRRUS_GD5430,  isa16_svga_cirrus_gd5430_device, "dm_clgd5430", "Diamond Speedstar Pro SE ISA Graphics Card (BIOS v1.00)")
...
isa16_svga_cirrus_gd5401_device::isa16_svga_cirrus_gd5401_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock) :
	device_t(mconfig, ISA16_AVGA1_CIRRUS_GD5401, tag, owner, clock),
...
void isa16_svga_cirrus_gd5401_device::device_add_mconfig(machine_config &config)
{
	screen_device &screen(SCREEN(config, "screen", SCREEN_TYPE_RASTER));
	screen.set_raw(25.175_MHz_XTAL, 800, 0, 640, 524, 0, 480);
	screen.set_screen_update("vga", FUNC(cirrus_gd5401_vga_device::screen_update));

	CIRRUS_GD5401_VGA(config, m_vga, 0);
	m_vga->set_screen("screen");
	m_vga->set_vram_size(256*1024);
}

const tiny_rom_entry *isa16_svga_cirrus_gd5401_device::device_rom_region() const
{
	return ROM_NAME( clgd5401 );
}


void isa16_svga_cirrus_gd5401_device::device_start()
{
	set_isa_device();
}


void isa16_svga_cirrus_gd5401_device::device_reset()
{
	remap(AS_PROGRAM, 0, 0xfffff);
	remap(AS_IO, 0, 0xffff);
}

void isa16_svga_cirrus_gd5401_device::remap(int space_id, offs_t start, offs_t end)
{
	if (space_id == AS_PROGRAM)
	{
		m_isa->install_memory(0xa0000, 0xbffff, read8sm_delegate(*m_vga, FUNC(cirrus_gd5401_vga_device::mem_r)), write8sm_delegate(*m_vga, FUNC(cirrus_gd5401_vga_device::mem_w)));
		m_isa->install_rom(this, 0xc0000, 0xc7fff, "bios");
	}
	else if (space_id == AS_IO)
		m_isa->install_device(0x0000, 0xffff, *this, &isa16_svga_cirrus_gd5401_device::io_isa_map);
}

void isa16_svga_cirrus_gd5401_device::io_isa_map(address_map &map)
{
...
ROM_START( clgd542x )
	...
	// TODO: these two are their own thing and needs separating in own file
	...
	ROM_SYSTEM_BIOS(13, "octek_vlcombo3.2", "Octek VL-Combo rev.3.2")
	...
	ROM_SYSTEM_BIOS(14, "octek_vlcombo2.1", "Octek VL-Combo rev.2.1")
...
void isa16_svga_cirrus_gd542x_device::remap(int space_id, offs_t start, offs_t end)
{
	if (space_id == AS_PROGRAM)
	{
		...
		m_isa->install_device(0x0000, 0xffff, *this, &isa16_svga_cirrus_gd542x_device::io_isa_map);
}
...
isa16_svga_cirrus_gd5430_device::isa16_svga_cirrus_gd5430_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock) :
	device_t(mconfig, ISA16_SVGA_CIRRUS_GD5430, tag, owner, clock),
	device_isa16_card_interface(mconfig, *this),
	m_vga(*this, "vga")
{
}
...
void isa16_svga_cirrus_gd5430_device::device_add_mconfig(machine_config &config)
{
	screen_device &screen(SCREEN(config, "screen", SCREEN_TYPE_RASTER));
	screen.set_raw(25.175_MHz_XTAL, 800, 0, 640, 524, 0, 480);
...
const tiny_rom_entry *isa16_svga_cirrus_gd5430_device::device_rom_region() const
{
...
//  device_start - device-specific startup
//-------------------------------------------------
uint8_t isa16_svga_cirrus_gd5430_device::input_port_0_r() { return 0xff; } //return machine().root_device().ioport("IN0")->read(); }

void isa16_svga_cirrus_gd5430_device::device_start()
{
	set_isa_device();
}
...
void isa16_svga_cirrus_gd5430_device::device_reset()
{
	remap(AS_PROGRAM, 0, 0xfffff);
	remap(AS_IO, 0, 0xffff);
}

void isa16_svga_cirrus_gd5430_device::remap(int space_id, offs_t start, offs_t end)
{
	if (space_id == AS_PROGRAM)
	{
		...
		m_isa->install_device(0x0000, 0xffff, *this, &isa16_svga_cirrus_gd5430_device::io_isa_map);
}


void isa16_svga_cirrus_gd5430_device::io_isa_map(address_map &map)
{

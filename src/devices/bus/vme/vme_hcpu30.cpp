...
void vme_hcpu30_card_device::hcpu30_mem(address_map &map)
{
	...
	map(0xff020000, 0xff027fff).ram().region("mailbox", 0).mirror(0x8000); // SRAM 32KB -- shared with iocpu
	map(0xffff8000, 0xffff9fff).ram().region("mailbox", 0);
...
ROM_START(hcpu30)
	...
	ROM_REGION32_BE(0x8000, "mailbox", ROMREGION_ERASEFF)
ROM_END
...
//	m_dusccterm->out_rtsb_callback().set(m_dusccterm, FUNC(duscc68562_device::ctsb_w));
	...
	// schematics connect INT to IPL1, not DRQ
	...
	// aka ACKNOWL-CENTDS BUSY-CENTD3 PE-CENTD1 SLCT-CENTD5 ERROR-CENTD7
...
void vme_hcpu30_card_device::device_start()
{
	...
	m_mailbox = (uint16_t*)(memregion ("mailbox")->base());

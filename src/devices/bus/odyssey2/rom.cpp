...
uint8_t o2_rom_device::read_rom04(address_space &space, offs_t offset, uint8_t mem_mask)
{
	return m_rom[(offset + (m_bank_base & 0x03) * 0x800) & (m_rom_size - 1)];
}
uint8_t o2_rom_device::read_rom0c(address_space &space, offs_t offset, uint8_t mem_mask)
{
	return m_rom[(offset + (m_bank_base & 0x03) * 0x800) & (m_rom_size - 1)];
}

uint8_t o2_rom12_device::read_rom04(address_space &space, offs_t offset, uint8_t mem_mask)
{
	return m_rom[offset + (m_bank_base & 0x03) * 0xc00];
}
uint8_t o2_rom12_device::read_rom0c(address_space &space, offs_t offset, uint8_t mem_mask)
{
	return m_rom[offset + 0x800 + (m_bank_base & 0x03) * 0xc00];
}

uint8_t o2_rom16_device::read_rom04(address_space &space, offs_t offset, uint8_t mem_mask)
{
	return m_rom[offset + 0x400 + (m_bank_base & 0x03) * 0x1000];
}
uint8_t o2_rom16_device::read_rom0c(address_space &space, offs_t offset, uint8_t mem_mask)
{

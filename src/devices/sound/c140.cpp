...
{
	memset(m_c219bank, 0, sizeof(uint8_t)*4);
}
...
void c219_device::device_start()
{
	...
	memset(m_c219bank, 0, sizeof(m_c219bank));
			...
			if (v->mode&8)
			{
				...
				for(j=0;j<samples;j++)
				{
					...
					//for(;cnt>0;cnt--)
					{
						...
						dt=(int8_t)read_byte((bank<<16)+pos);
			...
			else
			{
				...
				for(j=0;j<samples;j++)
				{
					...
					if( cnt )
					{
						...
						lastdt=(int8_t)read_byte((bank<<16)+pos);
...
void c219_device::voice_update()
{
	...
	int32_t   frequency,delta,offset,pos,bank,bank2;
	int32_t   cnt;
	int32_t   lastdt,prevdt,dltdt;
	float   pbase=(float)m_baserate*2.0f / (float)m_sample_rate;
		...
		if( v->key )
		{
			...
			for(j=0;j<samples;j++)
			{
				...
				if( cnt )
				{
					...
					lastdt = (int8_t)read_byte(BYTE_XOR_BE(bank2+(bank<<16)+pos));
...
void c140_device::sound_stream_update(sound_stream &stream, stream_sample_t **inputs, stream_sample_t **outputs, int samples)
{
	...
	memset(&m_mixer_buffer_left[0], 0, samples * sizeof(int16_t));
	memset(&m_mixer_buffer_right[0], 0, samples * sizeof(int16_t));

...
void sn76477_device::log_enable_line()
{
	...
	LOGMASKED(LOG_GENERAL, "Enable line (9): %d [%s]\n", m_enable, desc[m_enable]);
}
...
void sn76477_device::log_mixer_mode()
{
	...
	LOGMASKED(LOG_GENERAL, "Mixer mode (25-27): %d [%s]\n", m_mixer_mode, desc[m_mixer_mode]);
}
...
void sn76477_device::log_envelope_mode()
{
	...
	LOGMASKED(LOG_GENERAL, "Envelope mode (1,28): %d [%s]\n", m_envelope_mode, desc[m_envelope_mode]);
}
...
void sn76477_device::log_vco_mode()
{
	...
	LOGMASKED(LOG_GENERAL, "VCO mode (22): %d [%s]\n", m_vco_mode, desc[m_vco_mode]);
}
...
void sn76477_device::log_one_shot_time()
{
	if (!m_one_shot_cap_voltage_ext)
	{
		...
		{
			LOGMASKED(LOG_GENERAL, "One-shot time (23,24): %.4f sec\n", ONE_SHOT_CAP_VOLTAGE_RANGE * (1 / compute_one_shot_cap_charging_rate()));
		}
		else
		{
			LOGMASKED(LOG_GENERAL, "One-shot time (23,24): N/A\n");
		}
	}
	else
	{
		LOGMASKED(LOG_GENERAL, "One-shot time (23,24): External (cap = %.2fV)\n", m_one_shot_cap_voltage);
	}
}
...
void sn76477_device::log_slf_freq()
{
	if (!m_slf_cap_voltage_ext)
	{
		if (compute_slf_cap_charging_rate() > 0)
		{
			...
			LOGMASKED(LOG_GENERAL, "SLF frequency (20,21): %.2f Hz\n", 1 / (charging_time + discharging_time));
		}
		else
		{
			LOGMASKED(LOG_GENERAL, "SLF frequency (20,21): N/A\n");
		}
	}
	else
	{
		LOGMASKED(LOG_GENERAL, "SLF frequency (20,21): External (cap = %.2fV)\n", m_slf_cap_voltage);
	}
}
...
{
	LOGMASKED(LOG_GENERAL, "VCO pitch voltage (19): %.2fV\n", m_pitch_voltage);
}


void sn76477_device::log_vco_duty_cycle()
{
	LOGMASKED(LOG_GENERAL, "VCO duty cycle (16,19): %.0f%%\n", compute_vco_duty_cycle() * 100.0);
}
...
void sn76477_device::log_vco_freq()
{
	if (!m_vco_cap_voltage_ext)
	{
		if (compute_vco_cap_charging_discharging_rate() > 0)
		{
			...
			LOGMASKED(LOG_GENERAL, "VCO frequency (17,18): %.2f Hz - %.1f Hz\n", min_freq, max_freq);
		}
		else
		{
			LOGMASKED(LOG_GENERAL, "VCO frequency (17,18): N/A\n");
		}
	}
	else
	{
		LOGMASKED(LOG_GENERAL, "VCO frequency (17,18): External (cap = %.2fV)\n", m_vco_cap_voltage);
	}
}
...
void sn76477_device::log_vco_ext_voltage()
{
	if (m_vco_voltage <= VCO_MAX_EXT_VOLTAGE)
	{
		...
		LOGMASKED(LOG_GENERAL, "VCO ext. voltage (16): %.2fV (%.2f Hz)\n",
				m_vco_voltage,
				min_freq + ((max_freq - min_freq) * m_vco_voltage / VCO_MAX_EXT_VOLTAGE));
	}
	else
	{
		LOGMASKED(LOG_GENERAL, "VCO ext. voltage (16): %.2fV (saturated, no output)\n", m_vco_voltage);
	}
}
...
void sn76477_device::log_noise_gen_freq()
{
	...
	{
		LOGMASKED(LOG_GENERAL, "Noise gen frequency (4): External\n");
	}
	else
	{
		if (compute_noise_gen_freq() > 0)
		{
			LOGMASKED(LOG_GENERAL, "Noise gen frequency (4): %d Hz\n", compute_noise_gen_freq());
		}
		else
		{
			LOGMASKED(LOG_GENERAL, "Noise gen frequency (4): N/A\n");
		}
	}
}
	...
	if (!m_noise_filter_cap_voltage_ext)
	{
		...
		if (charging_rate > 0)
		{
			if (charging_rate < 1000000.0)
			{
				...
				LOGMASKED(LOG_GENERAL, "Noise filter frequency (5,6): %.0f Hz\n", 1 / (charging_time + discharging_time));
			}
			else
			{
				LOGMASKED(LOG_GENERAL, "Noise filter frequency (5,6): Very Large (Filtering Disabled)\n");
			}
		}
		else
		{
			LOGMASKED(LOG_GENERAL, "Noise filter frequency (5,6): N/A\n");
		}
	}
	else
	{
		LOGMASKED(LOG_GENERAL, "Noise filter frequency (5,6): External (cap = %.2fV)\n", m_noise_filter_cap);
	}
}
...
void sn76477_device::log_attack_time()
{
	if (!m_attack_decay_cap_voltage_ext)
	{
		...
		{
			LOGMASKED(LOG_GENERAL, "Attack time (8,10): %.4f sec\n", AD_CAP_VOLTAGE_RANGE * (1 / compute_attack_decay_cap_charging_rate()));
		}
		else
		{
			LOGMASKED(LOG_GENERAL, "Attack time (8,10): N/A\n");
		}
	}
	else
	{
		LOGMASKED(LOG_GENERAL, "Attack time (8,10): External (cap = %.2fV)\n", m_attack_decay_cap_voltage);
	}
}
...
void sn76477_device::log_decay_time()
{
	if (!m_attack_decay_cap_voltage_ext)
	{
		...
		{
			LOGMASKED(LOG_GENERAL, "Decay time (7,8): %.4f sec\n", AD_CAP_VOLTAGE_RANGE * (1 / compute_attack_decay_cap_discharging_rate()));
		}
		else
		{
			LOGMASKED(LOG_GENERAL, "Decay time (8,10): N/A\n");
		}
	}
	else
	{
		LOGMASKED(LOG_GENERAL, "Decay time (7, 8): External (cap = %.2fV)\n", m_attack_decay_cap_voltage);
	}
}
...
{
	LOGMASKED(LOG_GENERAL, "Voltage OUT range (11,12): %.2fV - %.2fV (clips above %.2fV)\n",
...
void sn76477_device::open_wav_file()
{
	...
	LOGMASKED(LOG_GENERAL, "Logging output: %s\n", wav_file_name);
}

...
protected:
	...
	TIMER_CALLBACK_MEMBER(update_sample_rate) { m_stream->set_sample_rate(m_sclock); }
	...
	uint8_t get_rom_byte(uint32_t offset) { return m_rom[offset % m_rom.bytes()]; } // need optimise or it's good as is?
	...
	struct ymz_channel
	{
		ymz_voice_params current;
		ymz_voice_params latch;
		void apply_params() { current = latch; }

...
{
	util::xml::data_node mamelayout = layout.child("mamelayout");
	if (mamelayout)
	{
		util::xml::data_node script = mamelayout.child("script");
		if(script)
			mame_machine_manager::instance()->lua()->call_plugin_set("layout", script.text().get());
	}
}

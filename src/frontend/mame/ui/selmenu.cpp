...
void menu_select_launch::custom_render(void *selectedref, float top, float bottom, float origx1, float origy1, float origx2, float origy2)
{
	...
	bool isstar = false;
	rgb_t color = ui().options().background_color();
	...
	draw_text_box(
			...
			ui::text_layout::CENTER, ui::text_layout::NEVER, true,
			ui().options().text_color(), color, 1.0f);
...
void menu_select_launch::draw_common_arrow(float origx1, float origy1, float origx2, float origy2, int current, int dmin, int dmax, float title_size)
{
	...
	fgcolor_right = fgcolor_left = ui().options().text_color();

	// set hover
	if (mouse_in_rect(ar_x0, ar_y0, ar_x1, ar_y1) && current != dmax)
	{
		ui().draw_textured_box(container(), ar_x0 + 0.01f, ar_y0, ar_x1 - 0.01f, ar_y1, ui().options().mouseover_bg_color(), rgb_t(43, 43, 43),
				hilight_main_texture(), PRIMFLAG_BLENDMODE(BLENDMODE_ALPHA) | PRIMFLAG_TEXWRAP(1));
		set_hover(HOVER_UI_RIGHT);
		fgcolor_right = ui().options().mouseover_color();
	}
	else if (mouse_in_rect(al_x0, al_y0, al_x1, al_y1) && current != dmin)
	{
		ui().draw_textured_box(container(), al_x0 + 0.01f, al_y0, al_x1 - 0.01f, al_y1, ui().options().mouseover_bg_color(), rgb_t(43, 43, 43),
				hilight_main_texture(), PRIMFLAG_BLENDMODE(BLENDMODE_ALPHA) | PRIMFLAG_TEXWRAP(1));
		set_hover(HOVER_UI_LEFT);
		fgcolor_left = ui().options().mouseover_color();
	}
...
void menu_select_launch::draw_info_arrow(int ub, float origx1, float origx2, float oy1, float line_height, float text_size, float ud_arrow_width)
{
	...
	{
		ui().draw_textured_box(container(), origx1 + 0.01f, oy1, origx2 - 0.01f, oy1 + (line_height * text_size), ui().options().mouseover_bg_color(),
	...
	for (int z = 0; z < UI_TOOLBAR_BUTTONS; ++z)
	{
		if (t_bitmap[z].valid())
		{
			...
			if (mouse_in_rect(x1, y1, x2, y2))
			{
				...
				ui().draw_text_box(container(), _(hover_msg[z]), ui::text_layout::CENTER, 0.5f, ypos, ui().options().background_color());
			}
...
float menu_select_launch::draw_right_box_title(float x1, float y1, float x2, float y2)
{
	...
	// add separator line
	container().add_line(x1 + midl, y1, x1 + midl, y1 + line_height, UI_LINE_WIDTH, ui().options().border_color(), PRIMFLAG_BLENDMODE(BLENDMODE_ALPHA));
	...
	for (int cells = RP_FIRST; cells <= RP_LAST; ++cells)
	{
		...
		if (ui_globals::rpanel != cells)
		{
			container().add_line(x1, y1 + line_height, x1 + midl, y1 + line_height, UI_LINE_WIDTH,
					ui().options().border_color(), PRIMFLAG_BLENDMODE(BLENDMODE_ALPHA));
			if (fgcolor != ui().options().mouseover_color())
				fgcolor = ui().options().clone_color();
		}
		...
		else if (bgcolor == ui().options().mouseover_bg_color())
		{
...
std::string menu_select_launch::arts_render_common(float origx1, float origy1, float origx2, float origy2)
{
	...
	rgb_t const fgcolor = (m_focus == focused_menu::RIGHTBOTTOM) ? rgb_t(0xff, 0xff, 0x00) : ui().options().text_color();
	rgb_t const bgcolor = (m_focus == focused_menu::RIGHTBOTTOM) ? rgb_t(0xff, 0xff, 0xff) : ui().options().text_bg_color();
...
void menu_select_launch::infos_render(float origx1, float origy1, float origx2, float origy2)
{
	...
	for (std::size_t x = 0; total > x; ++x)
	{
		...
		ui().draw_text_full(
				...
				mame_ui_manager::NONE, ui().options().text_color(), ui().options().text_bg_color(),
				&txt_length, nullptr);
		txt_length += 0.01f;
	...
	for (int r = 0; r < r_visible_lines; ++r)
	{
		...
		else if (justify == '2') // two-column layout
		{
			...
			ui().draw_text_full(
					...
					ui::text_layout::LEFT, ui::text_layout::TRUNCATE,
					mame_ui_manager::NORMAL, ui().options().text_color(), ui().options().text_bg_color(),
					nullptr, nullptr,
					tmp_size3);
			ui().draw_text_full(

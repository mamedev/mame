...
void gfxset_draw_item(running_machine &machine, gfx_element &gfx, int index, bitmap_rgb32 &bitmap, int dstx, int dsty, int color, int rotate, device_palette_interface *dpalette)
{
	int const width = (rotate & ORIENTATION_SWAP_XY) ? gfx.height() : gfx.width();
	int const height = (rotate & ORIENTATION_SWAP_XY) ? gfx.width() : gfx.height();
...
private:
	...
	struct tilemap
	{
		bool handle_keys(running_machine &machine, float pixelscale)
		{
			...
			if (input.pressed(IPT_UI_PAGE_UP) && m_flags != TILEMAP_DRAW_ALL_CATEGORIES)
			{
				if (m_flags > 0)
				{
					m_flags--;
			...
			if (input.pressed(IPT_UI_PAGE_DOWN) && (m_flags < TILEMAP_DRAW_CATEGORY_MASK || (m_flags == TILEMAP_DRAW_ALL_CATEGORIES)))
			{
				if (m_flags == TILEMAP_DRAW_ALL_CATEGORIES)
					m_flags = 0;
				else
					m_flags++;
				result = true;
	...
	{
		device_palette_interface &palette = *m_palette.m_interface;
		palette_device *const paldev = dynamic_cast<palette_device *>(&palette.device());

		bool const indirect = palette::subset::INDIRECT == m_palette.m_which;
		...
		float const cellwidth = (cellboxbounds.x1 - cellboxbounds.x0) / float(m_palette.m_columns);
		float const cellheight = (cellboxbounds.y1 - cellboxbounds.y0) / float(m_palette.m_columns);
		...
		for (int x = 0; x < m_palette.m_columns; x += 1 + rowskip)
		{
		...
		for (int y = 0; y < m_palette.m_columns; y += 1 + colskip)
		{
			// only display if there is data to show
			if (m_palette.m_offset + y * m_palette.m_columns < total)
			{
				...
				// draw the row header
				char buffer[10];
				sprintf(buffer, "%5X", m_palette.m_offset + (y * m_palette.m_columns));
				for (int x = 4; x >= 0; x--)
				{
		...
		for (int y = 0; y < m_palette.m_columns; y++)
		{
			for (int x = 0; x < m_palette.m_columns; x++)
			{
				int const index = m_palette.m_offset + (y * m_palette.m_columns) + x;
				if (index < total)
				{
				...
				if (index < gfx.elements())
				{
					...
					for (int x = 0; x < xcells; x++, index++)
					{
						...
						if (index < gfx.elements()) // only render if there is data
							gfxset_draw_item(m_machine, gfx, index, m_bitmap, cellbounds.min_x, cellbounds.min_y, set.m_color, set.m_rotate, set.m_palette);

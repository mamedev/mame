...
{
	// a headless window stays off screen
	if(_fullscreenWindow || _headless)
	{
		return;
	}
...
// convert a point in window base coordinates to content view backing pixels,
// top-left origin; returns whether the point is inside the view
static bool WindowPointToView(NSWindow *window, NSPoint location, int *outx, int *outy)
{
...
static bool EventLocation(NSEvent *event, int *outx, int *outy)
{
	...
	return WindowPointToView(window, [event locationInWindow], outx, outy);
}

// get the current pointer position in content view backing pixels, top-left
// origin, without waiting for an event; returns whether it's inside the view
bool MacGetPointerPosition(void *wincontroller, int *outx, int *outy)
{
	MAMEWindowController *wc = (MAMEWindowController *)wincontroller;
	NSWindow *window = [wc getWindow];

	return WindowPointToView(window, [window mouseLocationOutsideOfEventStream], outx, outy);
}
...
void MacOrderWindowFront(void *wincontroller)
{
	...
	// a headless window stays off screen even when the app is activated
	if ([wc headless])
	{
		return;
	}
...
void *CreateMAMEWindow(const char *title, int x, int y, int w, int h, bool isFullscreen, uint32_t displayID, bool headless)
{
	...
	// a headless (-video none) window backs the render target and gives the
	// menu bar and the Dock a way to quit MAME, but is never ordered in so
	// the app doesn't take focus from whatever the user is doing

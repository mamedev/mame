...
void main()
{
	...
	if (u_phosphor_mode == 0) {
		...
		prev.r *= tau.r == 0 ? 0 : exp(-delta_time / tau.r);
		prev.g *= tau.g == 0 ? 0 : exp(-delta_time / tau.g);
		prev.b *= tau.b == 0 ? 0 : exp(-delta_time / tau.b);
	} else {
		vec3 gamma = 1.0 / (u_phosphor_time * GAMMA_INV_FACTOR);

		if (prev.r != 0.0f)
			prev.r = pow(gamma.r * delta_time + pow(1 / r,
			         1 / u_phosphor_beta.r), -u_phosphor_beta.r);
		if (prev.g != 0.0f)
			prev.g = pow(gamma.g * delta_time + pow(1 / g,
			         1 / u_phosphor_beta.g), -u_phosphor_beta.g);
		if (prev.b != 0.0f)
			prev.b = pow(gamma.b * delta_time + pow(1 / b,

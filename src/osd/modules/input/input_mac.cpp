...
extern bool MacGetPointerPosition(void *wincontroller, int *outx, int *outy);
...
//============================================================
//  event records queued from the AppKit side
//============================================================
...
//============================================================
//  state shared by the pointer-driven devices
//============================================================

constexpr unsigned MOUSE_MAX_BUTTONS = 5;

struct mac_mouse_state
{
	int32_t lX, lY, lV, lH;
	uint8_t buttons[MOUSE_MAX_BUTTONS];
};
...
public:
	...
	virtual void configure(input_device &device) override
	{
		...
		for (unsigned button = 0; button < MOUSE_MAX_BUTTONS; button++)
		{
			device.add_item(
...
protected:
	virtual void process_event(const mac_mouse_event &event) override
	{
		...
		case mac_mouse_event::kind::BUTTON:
			if (event.button >= 0 && event.button < int(MOUSE_MAX_BUTTONS))
			{
...
private:
	mac_mouse_state m_mouse;
	int32_t m_x, m_y, m_v, m_h;
};


//============================================================
//  mac_lightgun_device_base
//============================================================

class mac_lightgun_device_base : public event_based_device<mac_mouse_event>
{
public:
	virtual void reset() override
	{
		event_based_device::reset();
		memset(&m_gun, 0, sizeof(m_gun));
	}

protected:
	mac_lightgun_device_base(std::string &&name, std::string &&id, input_module &module) :
		event_based_device(std::move(name), std::move(id), module),
		m_gun({ 0 })
	{
	}
	...
	void do_configure(input_device &device, unsigned buttons)
	{
		...
		for (unsigned button = 0; button < buttons; button++)
		{
			device.add_item(
					...
					input_item_id(ITEM_ID_BUTTON1 + button),
					generic_button_get_state<std::uint8_t>,
					&m_gun.buttons[button]);
		}
	}

	// sample the system pointer over the first window as absolute axes
	void update_position()
	{
		...
		{
			auto &window = static_cast<mac_window_info &>(*osd_common_t::window_list().front());
			if (window.platform_window() != nullptr)
			{
				// the pointer is reported relative to the content view, in the
				// same backing pixels as the view size; off-window positions
				// clamp to the nearest edge
				int x, y;
				MacGetPointerPosition(window.platform_window(), &x, &y);

				const osd_dim size = window.get_size_pixels();
				if ((size.width() > 0) && (size.height() > 0))
				{
					xpos = normalize_absolute_axis(x, 0, size.width() - 1);
					ypos = normalize_absolute_axis(y, 0, size.height() - 1);
				}
			}
		}
	...
	mac_mouse_state m_gun;
};


//============================================================
//  mac_lightgun_device - the system pointer as a lightgun
//============================================================

class mac_lightgun_device : public mac_lightgun_device_base
{
public:
	mac_lightgun_device(std::string &&name, std::string &&id, input_module &module) :
		mac_lightgun_device_base(std::move(name), std::move(id), module),
		m_v(0),
		m_h(0)
	{
	}
	...
	virtual void poll(bool relative_reset) override
	{
		mac_lightgun_device_base::poll(relative_reset);

		update_position();

		if (relative_reset)
		{
			m_gun.lV = std::exchange(m_v, 0);
			m_gun.lH = std::exchange(m_h, 0);
		}
	}

	virtual void reset() override
	{
		mac_lightgun_device_base::reset();
		m_v = m_h = 0;
	}

	virtual void configure(input_device &device) override
	{
		do_configure(device, MOUSE_MAX_BUTTONS);
...
protected:
	virtual void process_event(const mac_mouse_event &event) override
	{
		...
		case mac_mouse_event::kind::BUTTON:
			if (event.button >= 0 && event.button < int(MOUSE_MAX_BUTTONS))
			{
				m_gun.buttons[event.button] = event.down ? 0x80 : 0x00;
			}
			break;

		case mac_mouse_event::kind::MOTION:
			// the position is sampled from the system pointer when polled
			break;
...
//============================================================
//  lightgun_input_mac - Mac lightgun input module
//============================================================

class lightgun_input_mac : public input_module_impl<mac_lightgun_device_base, mac_osd_interface>
{
public:
	lightgun_input_mac() : input_module_impl<mac_lightgun_device_base, mac_osd_interface>(OSD_LIGHTGUNINPUT_PROVIDER, "mac")
	{
	}
...
lightgun_input_mac *g_lightgun_input = nullptr;
...
void lightgun_input_mac::input_init(running_machine &machine)
{
	input_module_impl<mac_lightgun_device_base, mac_osd_interface>::input_init(machine);

	create_device<mac_lightgun_device>(DEVICE_CLASS_LIGHTGUN, "Mac Gun 1", "Mac Gun 1");

	g_lightgun_input = this;
}

void lightgun_input_mac::exit()
{
	g_lightgun_input = nullptr;

	input_module_impl<mac_lightgun_device_base, mac_osd_interface>::exit();
}
...
extern "C" void MacMouseButtonEvent(int button, int down)
{
	...
	if (osd::g_lightgun_input != nullptr)
	{
		osd::g_lightgun_input->queue_mouse_event(event);
	}
}
...
MODULE_NOT_SUPPORTED(lightgun_input_mac, OSD_LIGHTGUNINPUT_PROVIDER, "mac")
...
MODULE_DEFINITION(LIGHTGUNINPUT_MAC, osd::lightgun_input_mac)

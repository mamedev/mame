...
public:
	win32_keyboard_device(running_machine& machine, const TCHAR *name, input_module &module)
		: keyboard_device(machine, name, module)
	{
	}

	void on_key(bool down, UINT8 scancode)
	{
		std::lock_guard<std::mutex> scope_lock(m_device_lock);

		keyboard.state[scancode] = down ? 0x80 : 0x00;
	}
};
...
public:
	...
	virtual void input_init(running_machine &machine)
	{
		...
		win32_keyboard_device *devinfo = devicelist()->create_device<win32_keyboard_device>(machine, TEXT("Win32 Keyboard 1"), *this);
		...
		switch (eventid)
		{
			...
			case INPUT_EVENT_KEYUP:
				...
				for (int i = 0; i < devicelist()->size(); i++)
					downcast<win32_keyboard_device*>(devicelist()->at(i))->on_key(eventid == INPUT_EVENT_KEYDOWN, args->scancode);
...
public:
	...
	win32_mouse_device(running_machine& machine, const TCHAR *name, input_module &module)
		: mouse_device(machine, name, module),
			win32_mouse({0})
	{
	}

	void on_mouse_button(MouseButtonEventArgs *args)
	{
		std::lock_guard<std::recursive_mutex> scope_lock(device_lock);
...
public:
	...
	virtual void input_init(running_machine &machine)
	{
		...
		// allocate a device
		devinfo = devicelist()->create_device<win32_mouse_device>(machine, TEXT("Win32 Mouse 1"), *this);
		if (devinfo == NULL)
			return;
	...
	BOOL handle_input_event(input_event eventid, void *eventdata) override
	{
		...
		for (int i = 0; i < devicelist()->size(); i++)
			downcast<win32_mouse_device*>(devicelist()->at(i))->on_mouse_button(args);
...
class win32_lightgun_device : public lightgun_device
{
private:
	std::recursive_mutex device_lock;
...
public:
	win32_lightgun_device(running_machine& machine, const TCHAR *name, input_module &module)
		: lightgun_device(machine, name, module),
...
public:
	...
	virtual void input_init(running_machine &machine)
	{
		...
		{
			static const TCHAR *const gun_names[] = { TEXT("Win32 Gun 1"), TEXT("Win32 Gun 2") };
	...
	BOOL handle_input_event(input_event eventid, void* eventdata) override
	{
		if (!input_enabled() || eventid != INPUT_EVENT_MOUSE_BUTTON)
			return FALSE;

		for (int i = 0; i < devicelist()->size(); i++)
			downcast<win32_lightgun_device*>(devicelist()->at(i))->on_mouse_button(static_cast<MouseButtonEventArgs*>(eventdata));

...
/****************************************************************************
	...
	- leap year bug in imgtool (https://github.com/mamedev/mame/pull/2263)
	- bootblock bug in imgtool (commit aca90520)
...
static const char32_t koi8_r_code_page[128] =
{
	// 0x80 - 0x8F
...
static imgtool::simple_charconverter charconverter_koi8_r(koi8_r_code_page);


static util::arbitrary_datetime _rt11_crack_time(uint16_t rt11_time)
{
...
static imgtool::datetime rt11_crack_time(uint16_t rt11_time)
{
	util::arbitrary_datetime dt;
	imgtool::datetime it;
...
static int is_file_storagetype(uint16_t status)
{
	return !(status & (E_MPTY|E_EOS|E_TENT));
}


static rt11_diskinfo *get_rt11_info(imgtool::image &image)
{
	rt11_diskinfo *info;
	info = (rt11_diskinfo *) imgtool_floppy_extrabytes(image);
	return info;
}
	...
	uint32_t *track, uint32_t *head, uint32_t *sector)
{
	...
	*head = (sector_index / sectors) % heads;
	...
	if (*track != di->cached_track || *head != di->cached_head)
	{
		...
		{
			ferr = floppy_get_indexed_sector_info(imgtool_floppy(image), *head, *track, i, NULL, NULL, &sector_id, NULL, NULL);
			if (ferr)
				continue;
...
static imgtoolerr_t rt11_image_readblock(imgtool::image &image, void *buffer, uint64_t block)
{
	...
	{
		err = rt11_get_sector_position(image, block * 2, &track, &head, &sector);
		if (err)
			return err;
		...
		err = rt11_get_sector_position(image, (block * 2) + 1, &track, &head, &sector);
		if (err)
			return err;
	...
	{
		err = rt11_get_sector_position(image, block, &track, &head, &sector);
		if (err)
			return err;
...
static imgtoolerr_t rt11_image_open(imgtool::image &image, imgtool::stream::ptr &&dummy)
{
	...
	// real-world images seem to never have a valid checksum
#if 0
	uint16_t tmp, cksum;
	...
	rt11_direnum *rt11enum, rt11_dirent &rt_ent)
{
	...
	rt_ent.filesize = BLOCK_SIZE * pick_integer_le(rt11enum->segment_data, offset + 8, 2);
...
static imgtoolerr_t rt11_image_beginenum(imgtool::directory &enumeration, const char *path)
{
	...
	rt11_direnum *rt11enum;

	rt11enum = (rt11_direnum *) enumeration.extra_bytes();

	/* seek initial block */
	err = rt11_enum_seek(image, rt11enum, 1, 0);
	if (err)
		return err;
	...
	creation_policy_t create, rt11_direnum *direnum, rt11_dirent *rt_ent)
{
	...
	err = rt11_enum_seek(image, direnum, segment, 0);
	if (err)
		goto done;
...
static imgtoolerr_t rt11_image_readfile(imgtool::partition &partition, const char *filename, const char *fork, imgtool::stream &destf)
{
	...
	for (i = rt_ent.data; i < rt_ent.data + (rt_ent.filesize / BLOCK_SIZE); i++)
	{
...
void rt11_get_info(const imgtool_class *imgclass, uint32_t state, union imgtoolinfo *info)
{
	switch (state)
	{
		...
		case IMGTOOLINFO_PTR_CHARCONVERTER:                 info->charconverter = &charconverter_koi8_r; break;
	}
}

...
std::pair<std::error_condition, std::vector<dir_entry>> prodos_impl::directory_contents(const std::vector<std::string> &path)
{
	...
	if(path.empty())
		...
		if(!off || !dir)
			return std::make_pair(std::errc::no_such_file_or_directory, std::vector<dir_entry>());
...
std::pair<std::error_condition, meta_data> prodos_impl::metadata(const std::vector<std::string> &path)
{
	...
	if(!off)
		return std::make_pair(std::errc::no_such_file_or_directory, meta_data());
	...
	if(dir) {
		...
		if(type == 5) {
			...
			return std::make_pair(err_t::UNSUPPORTED, meta_data());
	}
...
std::pair<std::error_condition, std::vector<u8>> prodos_impl::file_read(const std::vector<std::string> &path)
{
	auto [blk, off, dir] = path_find(path);
	if(!off || dir)
		return std::make_pair(std::errc::no_such_file_or_directory, std::vector<u8>());
	...
	if(type >= 1 && type <= 3)
		...
		return std::make_pair(err_t::UNSUPPORTED, std::vector<u8>());
}

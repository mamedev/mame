...
/*********************************************************************
    ...
    HP 9825 & 9845 filesystem handling

    HP9825 and HP9845 have very similar filesystems for disk storage.
    (On the contrary, files are stored on tape cartridges in quite a
    different way.)
...
namespace {
	...
	public:
		hp98x5_impl(fsblk_t& blockdev, bool is_9825);
	...
	private:
		...
		static constexpr unsigned FMT_INTERLEAVE_9825 = 2;
		static constexpr unsigned FMT_INTERLEAVE_9845 = 7;
		...
		// 9825 FS (true) or 9845 FS (false)
		const bool m_is_9825;
		...
		// Mapping of file types for 9825
		// Key is type code
		// Value is virtual file extension
		static const std::map<u8, std::string_view> m_9825_types;
		// Mapping of file types for 9845
		static const std::map<u8, std::string_view> m_9845_types;
	};
}

hp98x5_impl::hp98x5_impl(fsblk_t& blockdev, bool is_9825)
...
err_t hp98x5_impl::file_create(const std::vector<std::string> &path, const meta_data &meta)
{
	...
	it2->m_in_use = true;
	it2->m_name = basename;
	it2->m_type_code = (file_type != TEXT_TYPE) ? file_type : (m_is_9825 ? DATA_TYPE_9825 : DATA_TYPE_9845);
	it2->m_size = 0;
	it2->m_sectors = 0;
	it2->m_bpr = bpr > 0 ? unsigned(bpr) : DEF_BPR;
	it2->m_1st_sect = 0;
	it2->m_extra_w = 0;
	it2->m_attrs = m_is_9825 ? 0 : (it2->m_type_code == DATA_TYPE_9845 ? SYS_FLAG_DATA : SYS_FLAG_NON_DATA);
...
err_t hp98x5_impl::format(const meta_data &meta)
{
	...
	// Interleave factor
	w16b(out_ins, m_is_9825 ? FMT_INTERLEAVE_9825 : FMT_INTERLEAVE_9845);

	m_blockdev.get(0).copy(0, sys_rec.data(), sys_rec.size());

	// 9825 has a backup copy of system record in spare track but 9845 hasn't, go figure
	if (m_is_9825) {
...
err_t hp98x5_impl::parse_filename(const std::string& name, std::string& basename, int& bpr, u8& file_type) const
{
	...
	if (p1 != std::string::npos) {
		...
		if (ext == TEXT_EXT) {
			...
			for (const auto& e : (m_is_9825 ? m_9825_types : m_9845_types)) {
				if (e.second == ext) {
					file_type = e.first;
					break;
				}
...
std::vector<u8> hp98x5_impl::encode_dir() const
{
	...
	if (!m_is_9825 && dir_len - res.size() >= DIR_ENTRY_SIZE) {
		w16b(out_ins, 0);
		w16b(out_ins, 0xffff);
		w16b(out_ins, 0);
		w16b(out_ins, 0);
...
hp98x5_impl::dir_t::iterator hp98x5_impl::find_file(const std::string& basename, int bpr, u8 file_type)
{
	...
	if (it != m_dir.end() &&
		(file_type == NO_FILE_TYPE ||
		 (file_type == TEXT_TYPE && it->m_type_code == (m_is_9825 ? DATA_TYPE_9825 : DATA_TYPE_9845)) ||
...
meta_data hp98x5_impl::get_metadata(const entry& e)
{
	...
	if (m_is_9825) {
		...
		// * 9825 *
		// ********
		auto it = m_9825_types.find(e.m_type_code);
		if (it != m_9825_types.end()) {
		...
		// * 9845 *
		// ********
		auto it = m_9845_types.find(e.m_type_code);
		if (it != m_9845_types.end()) {
			meta.set(meta_name::file_type , it->second);
		} else {
			meta.set(meta_name::file_type , util::string_format("?%02x", e.m_type_code));
		}
...
const std::map<u8, std::string_view> hp98x5_impl::m_9825_types = {
	{ 1, "DATA"sv },
	{ 2, "PROG"sv },
	{ 3, "KEYS"sv },
	{ 5, "ALL"sv },
...
const std::map<u8, std::string_view> hp98x5_impl::m_9845_types = {
	{ 0, "BKUP"sv },
	{ 8, "DATA"sv },
	{ 16,"PROG"sv },
	{ 24,"KEYS"sv },
...
{
	return std::make_unique<hp98x5_impl>(blockdev, true);
}
...
{
	return std::make_unique<hp98x5_impl>(blockdev, false);
}

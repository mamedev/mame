...
/*********************************************************************
	...
	This format was introduced in the CCS64 emulator by Per Hakan Sundell, and use
	the .CRT file extension.
	
	It was designed to help Commodore 64 emulators handle the various different C64
	cartridge types by using a header that documents the special features that the
	specific cartridge types use.
	
	Documentation based on the VICE manual chapter 15.11, which is based on
	CRT.txt rev 1.14 compiled by Peter Schepers, with additional contributions
	by Per Hakan Sundell, Markus Brenner, and Marco Van Den Heuvel.
	...
	Bytes $0000-000F - 16-byte cartridge signature "C64 CARTRIDGE" (padded with spaces)
		  ...
		  $0016-0017 - Cartridge hardware type ($0000, high/low), see lines xx to xx for more details
	...
	CHIP Packet Contents (starting from $0040; there can be multiple CHIP packets in a single CRT file):

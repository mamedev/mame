...
void write_escaped(std::ostream &file, std::string_view str)
{
	while (!str.empty())
	{
		auto const found = str.find_first_of("\"&<>");
		if (found == std::string_view::npos)
		{
			file << str;
			break;
		}
		file.write(str.data(), found);
		switch (str[found])
		{
		case '"': file << "&quot;"; break;
		case '&': file << "&amp;";  break;
...
bool file::write(write_stream &file) const
{
	// ensure this is a root node
	assert(!get_name());

	owritestream str(file, owritestream::UTF_8, false);
	str.imbue(std::locale::classic());

	// output a simple header
	str << "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n";
	str << "<!-- This file is autogenerated; comments and unknown tags will be stripped -->\n";

	// loop over children of the root and output
	write_recursive(0, str);
	str << std::flush;
...
void data_node::write_recursive(int indent, std::ostream &file) const
{
	if (!get_name())
	{
	...
	{
		// output this tag
		stream_format(file, "%*s<%s", indent, "", get_name());

		// output any attributes, escaping as necessary
		for (attribute_node const &anode : m_attributes)
		{
			stream_format(file, " %s=\"", anode.name);

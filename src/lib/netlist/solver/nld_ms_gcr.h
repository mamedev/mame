...
namespace netlist::solver {
	...
	{
	public:
		using mat_type = plib::pGEmatrix_cr<
			plib::pmatrix_cr<arena_type, FT, SIZE>>;
			...
			, m_proc()
		{
			...
			for (mat_index_type k = 0; k < iN; k++)
			{
				...
				this->m_mat_ptr[k][this->m_terms[k].rail_start()]
					= &mat.A[mat.diagonal[k]];
			}

			this->state().log().verbose("maximum fill: {1}", gr.first);
			this->state().log().verbose(
			...
			if (this->state().static_solver_lib().isLoaded())
			{
				...
				{
					this->state().log().info(
						"External static solver {1} found ...", symname);
				}
				else
				{
					this->state().log().warning(
						"External static solver {1} not found ...", symname);
				}
			}
		}
		...
		std::pair<pstring, pstring>
		create_solver_code(static_compile_target target) override;
...
#define COMPRESSED 0
	...
	template <typename FT, int SIZE>
	void
	matrix_solver_GCR_t<FT, SIZE>::generate_code(plib::putf8_fmt_writer &strm)
	{
		const std::size_t    iN = this->size();
		const pstring        fptype(fp_constants<FT>::name());
		...
		for (std::size_t i = 0; i < iN - 1; i++)
		{
			...
			if (nzbd_count > 0)
			{
				...
				if ((!COMPRESSED) || nzbd_count > 1) // keep code comparable to
													 // previous versions
				{
				...
				for (std::size_t jj = 0; jj < nzbd_count; jj++)
				{
					...
					for (std::size_t pii = pi; pii < pi_end;)
					{
	...
	template <typename FT, int SIZE>
	std::pair<pstring, pstring>
	matrix_solver_GCR_t<FT, SIZE>::create_solver_code(
		static_compile_target target)
	{
		std::stringstream t;
		t.imbue(std::locale::classic());
		...
		strm.write_line(plib::pfmt(
			"{1} void {2}({3} * __restrict V, "

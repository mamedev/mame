...
///
/// The approach is describes in "Numerical Recipes in C", Second edition, Page
/// 75ff
///
...
///
/// One fact omitted in the book is the fact that actually the matrix Z which
/// contains in it's columns the solutions of
///
///      A * zk = uk
///
...
/// inverse of A.
///
/// The approach performs relatively well for matrices up to n ~ 40 (`kidniki`
/// using frontiers). `Kidniki` without frontiers has n==88. Here, the average
/// number of Newton-Raphson loops increase to 20. It looks like that the
/// approach for larger matrices introduces numerical instability.
///
...
namespace netlist::solver {
	...
	protected:
		...
		// access to the inverted matrix for fixed columns over row, values
		// stored transposed
		template <typename T1, typename T2>
	...
	template <typename FT, int SIZE>
	template <typename T>
	void matrix_solver_w_t<FT, SIZE>::LE_compute_x(T &x)
	{
		const std::size_t kN = this->size();

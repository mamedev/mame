...
namespace netlist::solver {
	...
	void matrix_solver_SOR_t<FT, SIZE>::upstream_solve_non_dynamic()
	{
		...
		for (std::size_t k = 0; k < iN; k++)
		{
			...
			using fp_aggregation_type = std::remove_reference_t<
				std::remove_cv_t<decltype(this->m_gtn[0][0])>>;

			fp_aggregation_type gtot_t = nlconst_base<
				fp_aggregation_type>::zero();
			fp_aggregation_type gabs_t = nlconst_base<
				fp_aggregation_type>::zero();
			fp_aggregation_type RHS_t = nlconst_base<
				fp_aggregation_type>::zero();
			...
			for (std::size_t i = this->m_terms[k].rail_start(); i < term_count;
				 i++)
		...
		do
		{
			...
			for (std::size_t k = 0; k < iN; k++)
			{
				const int *net_other = this->m_terms[k]
										   .m_connected_net_idx.data();
				...
				for (std::size_t i = 0; i < rail_start; i++)
					Idrive = Idrive
							 - static_cast<float_type>(go[i])
								   * this->m_new_V[static_cast<std::size_t>(
									   net_other[i])];

				const float_type new_val = this->m_new_V[k] * one_m_w[k]
										   + (Idrive + this->m_RHS[k]) * w[k];

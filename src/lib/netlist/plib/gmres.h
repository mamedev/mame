...
namespace plib {
	...
	struct mat_precondition_ILU
	{
		...
		void precondition() { m_LU.incomplete_LU_factorization(m_mat); }
	...
	struct mat_precondition_diagonal
	{
		...
		void precondition()
		{
			for (std::size_t i = 0; i < m_diagonal.size(); i++)
			{
				...
				for (std::size_t j = m_mat.row_idx[i]; j < m_mat.row_idx[i + 1];
					 j++)
					v += m_mat.A[j] * m_mat.A[j];
				...
				while ((j = nz[nzcolp++])
					   != narrow_cast<std::size_t>(
						   -1)) // NOLINT(bugprone-infinite-loop)
				{
					v += m_mat.A[j] * m_mat.A[j];
				}
				m_diagonal[i] = m_mat.A[m_mat.diagonal[i]] / v;
	...
	public:
		...
		{
			return (SIZE <= 0) ? m_size : narrow_cast<std::size_t>(SIZE);
		}
			...
			const std::size_t itr_max, float_type accuracy)
		{
			...
			//
			// The code below was completely written from scratch based on the
			...
			if (m_use_more_precise_stop_condition)
			{
				...
				// based `|xnew - xold|` pretty precisely. But it is slow, or
				// expressed differently: The invest doesn't pay off.
				//
				...
				const float_type rho_to_accuracy = plib::sqrt(vec_mult2<FT>(Ax))
												   / accuracy;
	...
	private:
			...
			[[maybe_unused]] bool dummy)
		{
			...
			const float_type mu = reciprocal(
				plib::hypot(m_ht[k][k], m_ht[kp1][k]));
		...
		template <int k, typename OPS, typename VT>
		bool
		do_k(OPS &ops, VT &x, std::size_t &itr_used, FT rho_delta, float dummy)
		{

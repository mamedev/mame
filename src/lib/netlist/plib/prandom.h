...
namespace plib {
	...
	private:
		...
		{
			const T lowest_w(~T(0) >> (sizeof(T) * 8 - w));
			const T lower_mask((T(1) << r) - 1); // That is, the binary number
												 // of r 1's
		...
		void fill(P &p) noexcept
		{
			for (std::size_t i = 0; i < m_buf.size(); i += 2)
			{
				...
				else
				{
					...
					// float: 9.999999e-37
					// FIXME: with 128 bit randoms log(s)/w will fail 1/(2^128)
					// ~ 2.9e-39
					const FT m(
						m_stddev
						* plib::sqrt(-constants<FT>::two() * plib::log(s) / s));
					m_buf[i] = /*mean+*/ m * v1;
					m_buf[i + 1] = /*mean+*/ m * v2;
				}
			}

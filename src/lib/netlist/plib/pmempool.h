...
namespace plib {
	...
	public:
		...
		bool operator==(const mempool_arena &rhs) const noexcept
		{
			return this == &rhs;
		}
	...
	private:
		...
		struct info
		{
			info(block *b, size_type p, size_type orig_size)
				: m_block(b)
				, m_pos(p)
	...
	mempool_arena<BASEARENA, MINALIGN>::~mempool_arena()
	{
		...
		if (!m_info.empty())
			plib::perrlogger(
				"Still found {} info blocks after mempool deleted\n",
				m_info.size());
	}

	template <typename BASEARENA, std::size_t MINALIGN>
	void *
	mempool_arena<BASEARENA, MINALIGN>::allocate(size_t align, size_t size)
	{
		block *b = nullptr;
		...
		void *ret = reinterpret_cast<void *>(
			b->m_data
			+ b->m_cur); // NOLINT(cppcoreguidelines-pro-type-reinterpret
		auto capacity(rs);

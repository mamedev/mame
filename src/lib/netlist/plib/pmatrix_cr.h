...
namespace plib {
	...
	struct pmatrix_cr
	{
		...
		pmatrix_cr(pmatrix_cr &&) noexcept(
			std::is_nothrow_move_constructible<parray<value_type, NSQ>>::value)
			= default;
		pmatrix_cr &operator=(pmatrix_cr &&) noexcept(
			std::is_nothrow_move_assignable<parray<value_type, NSQ>>::value
				&&std::is_nothrow_move_assignable<
					pmatrix2d_vrl<ARENA, index_type>>::value)
			= default;
		...
		// NOLINTNEXTLINE
		parray<index_type, NSQ> col_idx; // column index array nz_num, initially
										 // (n * n)
		// NOLINTNEXTLINE
		...
		{
			return (N > 0) ? narrow_cast<std::size_t>(N) : m_size;
		}
			...
			std::size_t band_width = FILL_INFINITY) noexcept(false)
		{
			...
			if (nz_num != 0)
				throw pexception(
					"build_from_mat only allowed on empty CR matrix");
			for (std::size_t k = 0; k < size(); k++)
			{
				...
				for (std::size_t j = 0; j < size(); j++)
					if (f[k][j] <= max_fill
						&& plib::abs(narrow_cast<int>(k) - narrow_cast<int>(j))
							   <= narrow_cast<int>(band_width))
					{
		...
		constexpr index_type *nzbd(std::size_t row) noexcept
		{
			return m_nzbd[row];
		}
		constexpr std::size_t nzbd_count(std::size_t row) noexcept
		{
			return m_nzbd.col_count(row) - 1;
		}

	protected:
		// FIXME: this should be private
		// NOLINTNEXTLINE
		// parray<std::vector<index_type>, N > m_nzbd;    // Support for
		// gaussian elimination
		pmatrix2d_vrl<ARENA, index_type> m_nzbd; // Support for gaussian
												 // elimination
	private:
		// parray<C, N < 0 ? -N * (N-1) / 2 : N * (N+1) / 2 > nzbd;    //
	...
	struct pGEmatrix_cr : public B
	{
		...
		pGEmatrix_cr(pGEmatrix_cr &&) noexcept(
			std::is_nothrow_move_constructible<base_type>::value)
			= default;
		pGEmatrix_cr &operator=(pGEmatrix_cr &&) noexcept(
		...
		gaussian_extend_fill_mat(M &fill) noexcept
		{
			std::size_t ops = 0;
			...
			for (std::size_t k = 0; k < fill.size(); k++)
			{
				...
				for (std::size_t row = k + 1; row < fill.size(); row++)
				{
					...
					{
						ops++;
						for (std::size_t col = k + 1; col < fill[row].size();
							 col++)
		...
		void gaussian_elimination(V &RHS) noexcept
		{
			...
			for (std::size_t i = 0; i < iN - 1; i++)
			{
				...
				while (auto j = nz[nzbdp++]) // NOLINT(bugprone-infinite-loop)
				{
					...
					const typename base_type::value_type f1 = -base_type::A
																  [pj++]
															  * f;
					...
					[this, &RHS, &l](unsigned ll)
					{
						auto &i = m_ge_par[l][ll];
						{
							...
							while (auto j = nz[nzbdp++])
							{
								...
								// subtract row i from j
								// fill-in available assumed, i.e. matrix was
								// prepared
								for (std::size_t pii = pi; pii < piie; pii++)
								{
									while (base_type::col_idx[pj]
										   < base_type::col_idx[pii])
										pj++;
									if (base_type::col_idx[pj]
										== base_type::col_idx[pii])
										base_type::A[pj++] += base_type::A[pii]
															  * f1;
								}
								RHS[j] += f1 * RHS[i];
							}
	...
	private:
		...
		std::vector<std::vector<std::size_t>> m_ge_par; // parallel execution
														// support for Gauss
	};
	...
	struct pLUmatrix_cr : public B
	{
		...
		pLUmatrix_cr(pLUmatrix_cr &&) noexcept(
			std::is_nothrow_move_constructible<base_type>::value)
			= default;
		pLUmatrix_cr &operator=(pLUmatrix_cr &&) noexcept(
			std::is_nothrow_move_assignable<base_type>::value)
			= default;
		...
		void build(M &fill, std::size_t ilup) noexcept(false)
		{
			...
			for (decltype(fill.size()) i = 1; i < fill.size(); i++)
			{
				...
				for (decltype(fill.size()) k = 0; k < i; k++)
				{
					...
					{
						// assume A[k][k]!=0
						for (decltype(fill.size()) j = k + 1; j < fill.size();
							 j++)
						{
		...
		void incomplete_LU_factorization(const base_type &mat) noexcept
		{
			...
			while (auto i = ilu_rows[p++]) // NOLINT(bugprone-infinite-loop)
			{
				...
				//  printf("occ %d\n", (int)i);
				for (auto i_k = base_type::row_idx[i];
					 i_k < base_type::diagonal[i]; i_k++)
				{
					const index_type k(base_type::col_idx[i_k]);
					const index_type p_k_end(base_type::row_idx[k + 1]);
					const typename base_type::value_type LUp_i_k
						= base_type::A[i_k] = base_type::A[i_k]
											  / base_type::A
												  [base_type::diagonal[k]];
					...
					{
						// we can assume that within a row ja increases
						// continuously
						const index_type c_i_j(
							base_type::col_idx[i_j]); // row
													  // i,
													  // column
													  // j
						const index_type c_k_j(
							base_type::col_idx[k_j]); // row
													  // k,
													  // column
													  // j

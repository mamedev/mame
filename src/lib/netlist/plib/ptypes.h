...
namespace plib {
	...
	struct compile_info
	{
		...
		static constexpr int128_type int128_max()
		{
			return (~static_cast<uint128_type>(0)) >> 1;
		}
		static constexpr uint128_type uint128_max()
		{
	...
	|| defined(__mips64) || defined(_M_AMD64) || defined(_M_ARM64)
...
#elif defined(__arm__) || defined(__ARMEL__) || defined(__aarch64__)           \
	|| defined(_M_ARM)
...
namespace plib {
	...
	{
	} // NOLINT(readability-named-parameter) // FIXME: remove unused var completely
	...
	struct has_ostream_operator_impl
	{
		template <class V>
		static auto
		test(V *) -> decltype(std::declval<LEFT &>() << std::declval<V>());
		template <typename>
...
#define PDEFINE_HAS_MEMBER(name, member)                                       \
	...
	{                                                                          \
		template <typename U>                                                  \
		static long test(decltype(&U::member));                                \
		template <typename U>                                                  \
		static char test(...);                                                 \
																			   \
	public:                                                                    \
		static constexpr const bool value = sizeof(test<T>(nullptr))           \
											== sizeof(long);                   \
	}

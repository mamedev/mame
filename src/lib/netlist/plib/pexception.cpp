...
namespace plib {
	...
	fp_signal_enabler::fp_signal_enabler(unsigned fp_exceptions)
	{
		...
		{
			int b = 0;
			if (fp_exceptions & plib::FP_INEXACT)
				b = b | FE_INEXACT;
			if (fp_exceptions & plib::FP_DIVBYZERO)
				b = b | FE_DIVBYZERO;
			if (fp_exceptions & plib::FP_UNDERFLOW)
				b = b | FE_UNDERFLOW;
			if (fp_exceptions & plib::FP_OVERFLOW)
				b = b | FE_OVERFLOW;
			if (fp_exceptions & plib::FP_INVALID)
				b = b | FE_INVALID;
		...
		if (m_enable)
		{
			int b = _EM_DENORMAL | _EM_INEXACT | _EM_ZERODIVIDE | _EM_UNDERFLOW
					| _EM_OVERFLOW | _EM_INVALID;
			if (fp_exceptions & plib::FP_INEXACT)
				b &= ~_EM_INEXACT;
			if (fp_exceptions & plib::FP_DIVBYZERO)
				b &= ~_EM_ZERODIVIDE;
	...
	fp_signal_enabler::~fp_signal_enabler()
	{
		...
		if (m_enable)
		{
			fedisableexcept(FE_ALL_EXCEPT); // Enable all floating point
											// exceptions but FE_INEXACT
			feenableexcept(m_last_enabled); // Enable all floating point
											// exceptions but FE_INEXACT
		}
#elif defined(_WIN32) && defined(_EM_INEXACT)
		if (m_enable)

...
namespace plib {
	...
	public:
		...
		static constexpr const size_type stride_size = align_traits<
			allocator_type>::stride_size;
		...
		pointer data() noexcept { return m_v; }

		const_pointer data() const noexcept { return m_v; }

		size_type data_index(size_type r, size_type c) const noexcept
		{
			return m_stride * r + c;
		}
	...
	public:
		...
		pmatrix2d_vrl &operator=(pmatrix2d_vrl &&) noexcept(
			!compile_info::clang_noexcept_issue::value)
			= default;
		...
		void set(size_type r, size_type c, const T &v) noexcept
		{
			...
			{
				m_v.insert(
					m_v.begin() + narrow_cast<std::ptrdiff_t>(m_row[r + 1]), v);
		...
		T *data() noexcept { return m_v.data(); }

		const T *data() const noexcept { return m_v.data(); }

		// FIXME: no check!
		constexpr size_type data_index(size_type r, size_type c) const noexcept
		{
			return m_row[r] + c;
		}

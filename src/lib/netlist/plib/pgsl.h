...
#include <utility>
		...
		#define gsl_Expects(e)                                                 \
			((e) ? static_cast<void>(0) : __builtin_unreachable())
	#else
		#define gsl_Expects(e)                                                 \
			((e) ? static_cast<void>(0) : static_cast<void>(0))
	#endif
	...
	#define gsl_Expects(e)                                                     \
		(__builtin_expect(!!(e), 1) ? static_cast<void>(0)                     \
...
#if 1
	// FIXME: __builtin_unreachable contrary to google sources does not seem to
	// be of any use and decreases performance slightly. Convert gsl_Expects to
	// a noop and revisit in the future.
	#undef gsl_Expects
	#define gsl_Expects(e)                                                     \
		do                                                                     \
		{                                                                      \
...
namespace plib {
	namespace pgsl {
		...
		///
		/// The c++ core guidelines require the narrow function to raise an
		/// error This will make narrow noexcept(false). This has shown to have
		/// a measurable impact on performance and thus we deviate from the
		/// standard here.
		///
		template <typename T, typename O>
	...
	/// cast wrapped into a std::optional type forcing the caller to examine if
	...
	std::enable_if_t<
		std::is_pointer_v<D> && std::is_pointer_v<std::remove_reference_t<B>>,
		std::optional<D>>
	...
	{
		static_assert(
			(std::is_pointer<D>::value && std::is_pointer<B>::value)
				|| (std::is_reference<D>::value && std::is_reference<B>::value),
			"downcast only supports pointers or reference for derived");
	...
	/// \brief cast to void *
	///
	/// The purpose of void_ptr_cast is to help identify casts to void in the
	/// code.
	///
	template <typename T>
	constexpr void *void_ptr_cast(T *ptr) noexcept
	{

...
namespace plib {
	...
	public:
		struct datatype_t
		{
			datatype_t(std::size_t size, bool is_integral, bool is_float)
				: m_size(size)
			...
			explicit datatype_t(bool custom)
				: m_size(0)
				, m_is_integral(false)
				, m_is_float(false)
				, m_is_custom(custom)
		...
		public:
			...
			virtual void
			register_state(state_manager_t &manager, const pstring &module)
				= 0;
		...
		{
			save_state_ptr(owner, item_name, datatype<C>(), N, &(state[0]));
		}

...
namespace plib {
	...
	{
		static constexpr std::size_t ABS() noexcept
		{
			return (SIZE < 0) ? narrow_cast<std::size_t>(0 - SIZE)
	...
	/// SIZE = 0 is pure dynamic allocation, the actual array size is passed to
	/// the constructor. SIZE < 0 reserves abs(SIZE) elements statically in
	/// place allocated. The actual size is passed in by the constructor. This
	/// array is purely intended for HPC application where depending on the
	/// architecture a preference dynamic/static has to be made.
	///
	/// This struct is not intended to be a full replacement to std::array.
	/// It is a subset to enable switching between dynamic and static
	/// allocation. I consider > 10% performance difference to be a use case.
	///
	...
	{
	public:
		static constexpr std::size_t SIZEABS() noexcept
		{
			return parray_traits<FT, SIZE, ARENA>::ABS();
		}
			...
			: m_size(size)
		{
			if ((SIZE < 0 && size > SIZEABS())
				|| (SIZE > 0 && size != SIZEABS()))
				throw pexception(
					pfmt("parray: size error: {1} > {2}")(size, SIZE));
		}
		...
		parray(const parray &rhs)
			: m_a(rhs.m_a)
			, m_size(rhs.m_size)
		{
		}
		parray(parray &&rhs) noexcept
		...
		parray &operator=(parray &&rhs) noexcept
		{
		...
		constexpr reference operator[](size_type i) noexcept { return m_a[i]; }
	...
	public:
		...
		parray2D(parray2D &&) noexcept(
			std::is_nothrow_move_constructible<base_type>::value)
			= default;
		parray2D &operator=(parray2D &&) noexcept(

...
namespace plib {
	...
	public:
		...
		///
		/// FIXME: this is sub-optimal. Refactor input_context into
		/// `pinput`_context and pass this to `ppreprocessor`.
		///
		template <typename T>
		pstring process(T &&in_stream, const pstring &filename)
		{
			m_outbuf.clear();
		...
		pstring   resolve_macros(const pstring &line);

...
namespace plib {
	...
	static const std::map<pstring, operator_t> &operator_table()
	{
		// clang-format off
		static const std::map<pstring, operator_t> static_local_operators =
		{
			{ "^",     { POW,   1, 30 } },
			{ "neg",   { NEG,   0, 25 } },
			{ "+",     { ADD,   1, 10 } },
  ...
  //{ "da", narrow_cast<F>(1e1) }, // NOLINT: Deca
		...
		const std::vector<pstring> &cmds, const pstring &expr) noexcept(false)
	{
		...
		{
			rpn_inst rc;
			auto     p = operator_table().find(cmd);
			if (p != operator_table().end())
			{
			...
			else
			{
				...
				if (rc.cmd() != PUSH_INPUT)
				{
					...
					if (r == units_si<NT>().end())
						...
						rc = rpn_inst(
							plib::pstonum_ne<NT>(
								plib::left(cmd, cmd.length() - 1), err)
							* r->second);
					if (err)
	...
	static pstring
	pop_check(std::stack<pstring> &stk, const pstring &expr) noexcept(false)
	{
		if (stk.empty())
		...
		const inputs_container                      &inputs)
	{
		...
		std::vector<pstring> step2_v;
		std::vector<pstring> function_v;
		...
		for (std::size_t i = 0; i < step1_v.size();)
		{
			if (i + 2 < step1_v.size() && step1_v[i].length() > 1)
			{
				auto r(plib::right(step1_v[i], 1));
				auto ne(step1_v[i + 1]);
				if ((is_number(step1_v[i])) && (r == "e" || r == "E")
					&& (ne == "-" || ne == "+"))
				{
					step2_v.push_back(step1_v[i] + ne + step1_v[i + 2]);
					i += 3;
				}
				else
					step2_v.push_back(step1_v[i++]);
			}
			else
				step2_v.push_back(step1_v[i++]);
		}
		// Fix numbers with unary minus/plus
		for (std::size_t i = 0; i < step2_v.size();)
		{
			if (step2_v[i] == "-" && (i + 1 < step2_v.size())
				&& is_number(step2_v[i + 1]))
			{
				if (i == 0
					|| !(is_number(step2_v[i - 1]) || step2_v[i - 1] == ")"
						 || is_id(step2_v[i - 1])))
				{
					function_v.push_back("-" + step2_v[i + 1]);
					i += 2;
				}
				else
					function_v.push_back(step2_v[i++]);
			}
			else if (step2_v[i] == "-" && (i + 1 < step2_v.size())
					 && (is_id(step2_v[i + 1]) || step2_v[i + 1] == "("))
			{
				if (i == 0
					|| !(is_number(step2_v[i - 1]) || step2_v[i - 1] == ")"
						 || is_id(step2_v[i - 1])))
				{
					function_v.emplace_back("neg");
					function_v.push_back(step2_v[i + 1]);
					i += 2;
				}
				else
		...
		for (std::size_t i = 0; i < function_v.size(); i++)
		{
			pstring &s = function_v[i];
			if (s == "(")
				op_stack.push(s);
			else if (s == ")")
			{
				pstring x = pop_check(op_stack, expr);
				while (x != "(")
				{
					postfix.push_back(x);
					x = pop_check(op_stack, expr);
				}
				if (!op_stack.empty() && get_prio(op_stack.top()) == 0)
					postfix.push_back(pop_check(op_stack, expr));
			}
			else if (s == ",")
			{
				pstring x = pop_check(op_stack, expr);
			...
			else
			{
				...
				{
					if ((i + 1 < function_v.size()) && function_v[i + 1] == "(")
						op_stack.push(s);
					else
						postfix.push_back(s);
				}
...
#define OP(OP, ADJ, EXPR)                                                      \
	case OP:                                                                   \
		if (ADJ == 2)                                                          \
		{                                                                      \
			if (m_precompiled[ptr - 3].cmd() == PUSH_CONST                     \
				&& m_precompiled[ptr - 2].cmd() == PUSH_CONST                  \
				&& m_precompiled[ptr - 1].cmd() == PUSH_CONST)                 \
			{                                                                  \
				ptr--;                                                         \
		...
		{                                                                      \
			if (m_precompiled[ptr - 2].cmd() == PUSH_CONST                     \
				&& m_precompiled[ptr - 1].cmd() == PUSH_CONST)                 \
			{                                                                  \
				m_precompiled[ptr - 2] = rpn_inst(EXPR);                       \
				n -= 2;                                                        \

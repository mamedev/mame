...
namespace plib {
	...
	struct queue_entry_t final
	{
		...
		constexpr queue_entry_t() noexcept
			: m_exec_time()
			, m_object(nullptr)
		{
		}
		constexpr queue_entry_t(const Time &t, const Element &o) noexcept
			: m_exec_time(t)
			, m_object(o)
		{
		}
		...
		static constexpr queue_entry_t never() noexcept
		{
			return queue_entry_t(Time::never(), nullptr);
		}
	...
	public:
		...
		constexpr bool
		exists(const typename T::element_type &elem) const noexcept;
	...
	timed_queue_linear<A, T>::emplace(Args &&...args) noexcept
	{
		T *i(m_end);
	...
	template <bool KEEPSTAT>
	inline constexpr void
	timed_queue_linear<A, T>::remove(const T &elem) noexcept
	{
		if constexpr (KEEPSTAT)
			m_prof_remove.inc();
	...
	template <class A, class T>
	template <bool KEEPSTAT>
	inline constexpr void timed_queue_linear<A, T>::remove(
		const typename T::element_type &elem) noexcept
	{
		if constexpr (KEEPSTAT)
			m_prof_remove.inc();
		for (T *i = m_end - 1; i > &m_list[0]; --i)
	...
	template <class A, class T>
	inline constexpr bool timed_queue_linear<A, T>::exists(
		const typename T::element_type &elem) const noexcept
	{

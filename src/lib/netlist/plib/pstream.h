...
namespace plib {
	...
	static S &istream_read(S &is, T *data, size_t len)
	{
		...
		static_assert((sizeof(T) % sizeof(ct)) == 0,
					  "istream_read sizeof issue");
		// NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
		return is.read(reinterpret_cast<ct *>(data),
					   gsl::narrow<std::streamsize>(len * sizeof(T)));
	}
	...
	static S &ostream_write(S &os, const T *data, size_t len)
	{
		using ct = typename S::char_type;
		static_assert((sizeof(T) % sizeof(ct)) == 0,
					  "ostream_write sizeof issue");
		// NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
		return os.write(reinterpret_cast<const ct *>(data),
						gsl::narrow<std::streamsize>(len * sizeof(T)));
	}
	...
	struct istream_uptr
	{
		...
		istream_uptr(std::unique_ptr<std::istream> &&strm,
	...
	public:
		...
		bool read_code(putf8string::traits_type::code_t &c)
		{
			...
			if (m_strm->eof())
				return false;
			// NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
			const std::size_t l = putf8string::traits_type::codelen(
				reinterpret_cast<putf8string::traits_type::mem_t *>(&b));
			...
			// NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
			c = putf8string::traits_type::code(
				reinterpret_cast<putf8string::traits_type::mem_t *>(&b));
			return true;
		}
	...
	{
	public:
		explicit putf8_writer(std::ostream *strm)
			: m_strm(strm)
		{
		}

		putf8_writer(putf8_writer &&src) noexcept
			: m_strm(src.m_strm)
		{
		}
		...
		void write(const pstring &text) const
		{
			...
			// m_strm->write(conv_utf8.c_str(),
			// static_cast<std::streamsize>(plib::strlen(conv_utf8.c_str()  )));
	...
	class putf8_fmt_writer
		: public pfmt_writer_t<putf8_fmt_writer>
		, public putf8_writer
	{
	public:
		...
		// protected:
		void upstream_write(const pstring &s) const { write(s); }
	...
	{
	public:
		explicit pbinary_writer(std::ostream &strm)
			: m_strm(strm)
		{
		}
		pbinary_writer(pbinary_writer &&src) noexcept
			: m_strm(src.m_strm)
		{
		}
	...
	{
	public:
		explicit pbinary_reader(std::istream &strm)
			: m_strm(strm)
		{
		}
		pbinary_reader(pbinary_reader &&src) noexcept
			: m_strm(src.m_strm)
		{
		}
	...
	{
		// FIXME: optimize
		std::array<std::ostream::char_type, 1024>
	...
	{
	public:
		using filename_type = std::conditional<
			compile_info::win32()
				&& (!compile_info::mingw()
					|| compile_info::version::vmajor() >= 9),
			pstring_t<pwchar_traits>, pstring_t<putf8_traits>>::type;

		template <typename T>
		explicit ifstream(const pstring_t<T> &name,
						  ios_base::openmode  mode = ios_base::in)
			: std::ifstream(filename_type(name).c_str(), mode)
		{
		}

		explicit ifstream(const std::string &name,
						  ios_base::openmode mode = ios_base::in)
	...
	public:
		...
		template <typename T>
		explicit ofstream(const pstring_t<T> &name,
		...
		explicit ofstream(const std::string &name,
						  ios_base::openmode mode = ios_base::out
													| ios_base::trunc)
		...
		template <typename T>
		void open(const pstring_t<T> &name,

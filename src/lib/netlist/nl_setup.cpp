...
namespace netlist
{
	...
	void nlparse_t::register_param_fp(const pstring &param,
		const nl_fptype                              value)
	{
	...
	void nlparse_t::register_default_param(const pstring &name,
		const pstring &                                   def)
	{
		// strip " from stringified strings
		pstring val(def);
	...
	void nlparse_t::register_alias_no_fqn(const pstring &alias,
		const pstring &                                  out)
	{
	...
	bool nlparse_t::parse_stream(plib::istream_uptr &&in_stream,
		const pstring &                               name)
	{
#if 0
		...
		const std::array<terminal_t *, 2> &splitter_terms)
	{
		...
		m_connected_terminals.insert({&term,
	...
	std::vector<pstring> setup_t::get_terminals_for_device_name(
		const pstring &devname) const
	{
	...
	devices::nld_base_proxy *setup_t::get_d_a_proxy(
		const detail::core_terminal_t &out)
	{
	...
	// NOLINTNEXTLINE(misc-no-recursion)
	devices::nld_base_proxy *setup_t::get_a_d_proxy(
		detail::core_terminal_t &inp)
	{
	...
	detail::core_terminal_t &setup_t::resolve_proxy(
		detail::core_terminal_t &term)
	{
		if (term.is_logic())
		{
	...
	plib::istream_uptr source_string_t::stream(
	...
	plib::istream_uptr source_mem_t::stream(
		[[maybe_unused]] const pstring &name)
	{
		plib::istream_uptr ret(
	...
	plib::istream_uptr source_file_t::stream(
		[[maybe_unused]] const pstring &name)
	{
	...
	plib::istream_uptr source_pattern_t::stream(
		[[maybe_unused]] const pstring &name)
	{
	...
	plib::istream_uptr source_proc_t::stream(
		[[maybe_unused]] const pstring &name)
	{
		return plib::istream_uptr();
	}

...
// FIXME: These are either defined in other parts or have more complex
// consequences
...
public:
	...
	~wav_t()
	{
		...
		{
			m_fh.file_len = m_data.len + sizeof(m_data) + sizeof(m_fh)
							+ sizeof(m_fmt) - 8;
			m_f.seekp(0);
			write(m_fh);
			write(m_fmt);
	...
	void write_sample_int(double sample)
	{
		constexpr auto type_max(
			static_cast<double>(plib::numeric_limits<T>::max()));
		constexpr auto type_min(
			static_cast<double>(plib::numeric_limits<T>::min()));
	...
	void write_samples(double *sample)
	{
		...
		for (std::size_t i = 0; i < channels(); i++)
		{
			switch (m_format)
			{
				case s16: write_sample_int<int16_t>(sample[i]); break;
				case s32: write_sample_int<int32_t>(sample[i]); break;
				case f32:
...
private:
	struct riff_chunk_t
	{
		std::array<uint8_t, 4> group_id = {
			{'R', 'I', 'F', 'F'}
		};
		uint32_t               file_len = 0;
		std::array<uint8_t, 4> riff_type = {
			{'W', 'A', 'V', 'E'}
		};
	};
	...
	struct riff_data_t
	{
		explicit riff_data_t(uint32_t alen)
			: len(alen)
		{
		}
		std::array<uint8_t, 4> signature = {
...
struct filter_hp
{
	...
	filter_hp(double freq, bool boost, std::size_t channels, callback_type cb)
		: m_cb(cb)
		, m_hp_omega(plib::constants<double>::two()
					 * plib::constants<double>::pi() * freq)
		, m_boost(boost)
		, m_lt(channels, 0.0)
		, m_in(channels, 0.0)
		, m_cap(channels, 0.0)
	...
	void process(std::size_t chan, double time, double val)
	{
		...
		double omega = ((m_boost && (time < 1.0 / m_hp_omega)) ? 1e12
															   : m_hp_omega);
...
struct filter_lp
{
	...
	filter_lp(double freq, std::size_t channels, callback_type cb)
		: m_cb(cb)
		, m_lp_omega(plib::constants<double>::two()
					 * plib::constants<double>::pi() * freq)
		, m_lt(channels, 0.0)
...
public:
	...
	void process(std::size_t chan, double time, double sample)
	{
		if (time > m_last_time)
		{
			write(
				"#"
				+ plib::to_string(static_cast<std::int64_t>(m_last_time * 1e9))
				+ " ");
			write(m_buf + "\n");
			m_buf = "";
			m_last_time = time;
		...
		if (time > m_last_time)
		{
			if (m_n < m_samples)
			{
				while (m_next_time < time && m_n < m_samples)
				{
					...
					for (auto &e : m_buf)
					{
						o += pstring(",") + plib::to_string(e); // FIXME:
																// locale!!
					}
					write(o.substr(1) + "\n");
					m_n++;
...
public:
	...
	, opt_ex1(*this, "./nlwav -f vcdd -o x.vcd log_V*", "convert all files starting with \"log_V\" into a digital vcd file")
	, opt_ex2(*this, "./nlwav -f wav16s -o x.wav log_V*", "convert all files starting with \"log_V\" into a multichannel wav file (16bit, signed)")
...
void nlwav_app::convert_vcd(std::ostream &output, vcd_writer::format_e format)
{
	...
	arena::unique_ptr<vcd_writer> wo = plib::make_unique<vcd_writer, arena>(
		output, opt_args(), format, opt_high(), opt_low());
	log_processor::callback_type lp_cb(&vcd_writer::process, wo.get());
...
int nlwav_app::execute()
{
	...
	try
	{
		for (const auto &oi : opt_args())
		{
			...
			{
				fin = std::make_unique<plib::ifstream>(
					plib::filesystem::u8path(oi), std::ios::in);
				if (fin->fail())

...
// FIXME: "NamingConvention", "FunctionTable:", "Pinalias" need to be renamed
// here
...
public:
	...
	, opt_pre_pro(*this,  "",  "preprocessor",            "output preprocessor output to stderr")
	...
	, opt_grp6(*this,     "Options for header command",   "These options are only used by the header command.")
...
public:
	...
	plib::istream_uptr stream(const pstring &file) override
	{
		...
		plib::istream_uptr strm(
			std::make_unique<plib::ifstream>(plib::filesystem::u8path(name)),
			plib::filesystem::u8path(name));
		if (strm->fail())
...
public:
		...
		: netlist::netlist_state_t(name, logger)
	{
		if (boost_lib == "builtin")
			set_static_solver_lib(
				std::make_unique<plib::static_library>(nl_static_solver_syms));
		else if (boost_lib == "generic")
			set_static_solver_lib(
				std::make_unique<plib::static_library>(nullptr));
		else if (NL_DISABLE_DYNAMIC_LOAD)
			throw netlist::nl_exception(
				"Dynamic library loading not supported due to project security concerns.");
		else
			//#pstring libpath = plib::util::environment("NL_BOOSTLIB", plib::util::buildpath({".", "nlboost.so"}));
			set_static_solver_lib(
				std::make_unique<plib::dynamic_library>(boost_lib));
	}
...
struct input_t
{
	...
	{
		std::array<char, 400>
			   buf; // NOLINT(cppcoreguidelines-pro-type-member-init)
		double t(0);
		double val(0);
	...
	void setparam() const
	{
		switch (m_param.param().param_type())
		{
			...
			case netlist::param_t::POINTER:
				throw netlist::nl_exception(plib::pfmt(
					"param {1} is not numeric\n")(m_param.param().name()));
			case netlist::param_t::DOUBLE:
				plib::downcast<netlist::param_fp_t &>(m_param.param())
					.set(m_value);
				break;
			case netlist::param_t::INTEGER:
				plib::downcast<netlist::param_int_t &>(m_param.param())
					.set(static_cast<int>(m_value));
				break;
			case netlist::param_t::LOGIC:
				plib::downcast<netlist::param_logic_t &>(m_param.param())
					.set(static_cast<bool>(m_value));
				break;
		}
	}
...
static std::vector<input_t>
read_input(const netlist::setup_t &setup, const pstring &fname)
{
	std::vector<input_t> ret;
	if (!fname.empty())
	{
		plib::putf8_reader r = plib::putf8_reader(
			std::make_unique<plib::ifstream>(plib::filesystem::u8path(fname)));
		if (r.stream().fail())
	...
	netlist::netlist_time_ext start, netlist::netlist_time_ext duration)
{
	...
	else
	{
		...
		while (now < end)
		{
			...
			auto next_sec = start
							+ netlist::netlist_time_ext::from_sec(elapsed_sec);
			if (end < next_sec)
			{
...
void tool_app_t::run()
{
	...
	if (opt_load_state.was_specified())
	{
		...
		if (strm.fail())
			throw netlist::nl_exception(
				netlist::MF_FILE_OPEN_ERROR(opt_load_state()));
		strm.imbue(std::locale::classic());
	...
	netlist::netlist_time_ext start = nt.exec().time();
	{
		...
		while (pos < inputs.size() && inputs[pos].m_time < duration
...
void tool_app_t::static_compile()
{
	...
	if (!opt_static_include()
		&& ((opt_dir.was_specified() ^ opt_out.was_specified()) == 0))
		throw netlist::nl_exception("either --dir or --output option needed");

	if (opt_static_include()
		&& (!opt_dir.was_specified() || !opt_out.was_specified()))
		throw netlist::nl_exception(
			"--split requires both --dir and --output option");

	if (!opt_static_include() && opt_dir.was_specified())
	{
		if (opt_files().size() != 1)
			throw netlist::nl_exception(
				"nltool: static_compile needs exactly one file");
	...
	else
	{
		...
		for (const auto &f : opt_files())
		{
			...
			{
				plib::putf8_reader r = plib::putf8_reader(
					std::make_unique<plib::ifstream>(
						plib::filesystem::u8path(f)));
				if (r.stream().fail())
				...
				while (r.read_line(line))
				{
					...
					{
						auto sp = plib::psplit(
							pstring(plib::trim(line.substr(13))), ' ', true);
						for (auto &e : sp)
							names.push_back(e);
					}
				}
		...
		for (auto &se : sort_map)
		{
			...
			if (opt_static_include())
			{
				...
				if (last != fpath)
				{
					...
					if (output_stream.fail())
						throw netlist::nl_exception(
							netlist::MF_FILE_OPEN_ERROR(fpath));
					output_stream << "// spell-checker: disable\n\n";
		...
		for (auto &se : sort_map)
		{
			...
			for (auto &s : e->second.m_modules)
				mods += s + ",";
			output_stream << "// "
						  << putf8string(plib::left(mods, mods.length() - 1))
						  << "\n";
			output_stream << "\t{\"" << putf8string(e->first)
						  << "\", reinterpret_cast<void *>(&"
						  << putf8string(e->first) << ")}, // NOLINT\n";
		}
		output_stream << "#endif\n\n";
...
static doc_ext
read_documentation_from_source(const pstring &fname, const pstring &id)
{
	plib::putf8_reader r = plib::putf8_reader(
		std::make_unique<plib::ifstream>(plib::filesystem::u8path(fname)));
	if (r.stream().fail())
				...
				else
				{
					...
					else if (n == "Example")
					{
						...
						if (ret.example.size() != 2 && !ret.example.empty())
							throw netlist::nl_exception(
								"Example requires 2 parameters, but found {1}",
								ret.example.size());
					}
					else
...
void tool_app_t::header_entry(const netlist::factory::element_t *e)
{
	...
	for (const auto &s : v)
		if (!plib::startsWith(s, "@"))
			vs += ", p"
				  + plib::replace_all(plib::replace_all(s, "+", ""), ".", "_");
		else
			avs += ", " + s.substr(1);
...
void tool_app_t::create_header()
{
	if (!opt_files().empty())
		throw netlist::nl_exception(
			"Header doesn't support input files, but {1} where given",
			opt_files().size());
...
void tool_app_t::create_doc_header()
{
	...
	std_out("///\n");
	std_out(
		"/// Below is a list of all the devices currently known to the system ...\n");
	std_out("///\n");
	...
	{
		auto d(
			read_documentation_from_source(e->source().file_name(), e->name()));
	...
	output_with_prefix("///", " @page ''"); // FIXME: snippets and pages need to
											// be separate files
	output_with_prefix("", "");

	for (auto &d : de_cache)
	{
		// auto d(read_documentation_from_source(e->source().file_name(),
		// e->name()));
...
void tool_app_t::list_devices()
{
	...
	std::vector<netlist::device_arena::unique_ptr<netlist::core_device_t>>
		devices;
	...
	for (auto &fl : list)
	{
		...
		std::vector<pstring> terms(
			nt.setup().get_terminals_for_device_name(d->name()));
...
void tool_app_t::convert()
{
	...
	else
	{
		...
		if (strm.fail())
			throw netlist::nl_exception(
				netlist::MF_FILE_OPEN_ERROR(opt_files()[0]));
		strm.imbue(std::locale::classic());
...
int tool_app_t::execute()
{
	...
	try
	{
		...
		plib::fp_signal_enabler fp_protect(
			plib::FP_DIVBYZERO | plib::FP_UNDERFLOW | plib::FP_OVERFLOW
			| plib::FP_INVALID);

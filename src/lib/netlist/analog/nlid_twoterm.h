...
namespace netlist::analog {
	...
	public:
		...
		{
			//               GT, GO,        I
			m_P.set_gt_go_I(G, -G, (V)*G - I);
			m_N.set_gt_go_I(G, -G, (-V) * G + I);
		}
		...
		{
			//               GT,  GO,     I
			m_P.set_gt_go_I(a11, a12, rhs1);
			m_N.set_gt_go_I(a22, a21, rhs2);
		}

		void
		set_mat(const std::array<std::array<nl_fptype, 3>, 2> &a) const noexcept
		{
			//                 GT,      GO,       I
			m_P.set_gt_go_I(a[0][0], a[0][1], a[0][2]);
			m_N.set_gt_go_I(a[1][1], a[1][0], a[1][2]);
		}

		void clear_mat() const noexcept
		{
			static constexpr const auto z = nlconst::zero();
			//               GT,  GO,     I
			m_P.set_gt_go_I(z, z, z);
			m_N.set_gt_go_I(z, z, z);
		}

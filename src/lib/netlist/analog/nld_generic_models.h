...
namespace netlist::analog {
	...
	public:
		...
		static capacitor_e type() noexcept
		{
			return capacitor_e::VARIABLE_CAPACITY;
		}
		...
		nl_fptype
		Ieq(nl_fptype cap, [[maybe_unused]] nl_fptype v) const noexcept
		{
	...
	public:
		...
		static capacitor_e type() noexcept
		{
			return capacitor_e::CONSTANT_CAPACITY;
		}
	...
	public:
		...
		// Returns { G, Ieq }
		std::pair<nl_fptype, nl_fptype>
		time_step(nl_fptype cap, nl_fptype v, nl_fptype step) const noexcept
		{
	...
	public:
		...
		// Returns { G, Ieq }
		std::pair<nl_fptype, nl_fptype>
		time_step(nl_fptype cap, nl_fptype v, nl_fptype step) noexcept
		{
			const nl_fptype h(plib::reciprocal(step));
			if (m_trn == 0.0)
			{
	...
	public:
		...
		void update_diode(nl_fptype nVd) noexcept
		{
			if (TYPE == diode_e::BIPOLAR)
			{
				...
				if (nVd > m_Vcrit)
				{
					...
					const nl_fptype d = std::min(
						+fp_constants<nl_fptype>::DIODE_MAXDIFF(), nVd - old);
				...
				if (nVd > m_Vcrit)
				{
					m_Id = m_Icrit_p_Is - m_Is
						   + (m_Vd - m_Vcrit) * m_Icrit_p_Is * m_VtInv;
			...
			nl_fptype temp) noexcept
		{
			...
			// Vcrit : f(V) has smallest radius of curvature rho(V) ==
			// min(rho(v))
			...
			// m_Imin = m_gmin * m_Vt - m_Is;
			//  Fixme: calculate max dissipation voltage - use use 0.5 (500mW)
			//  here for typical diode P = V * I = V * (Is*exp(V/Vt) - Is) P ~=
			//  V * I = V * Is*exp(V/Vt) ln(P/Is) = ln(V)+V/Vt ~= V - 1 + V/vt
			//  V = (1+ln(P/Is))/(1 + 1/Vt)

			m_Vcrit = (nlconst::one() + plib::log(nlconst::half() / m_Is))
					  / (nlconst::one() + m_VtInv);
			// printf("Vcrit: %f\n", m_Vcrit);

...
namespace {
    ...
    private:
        ...
        u8 scu_read(offs_t offs) {
            int res = 0;

            if (offs == 0) {
                static int i = 0;
                static u8 pd[] = { 0x66, 0x31 };
                res = pd[i++ % 2];
            }
            ...
            printf("scu_read: %08x -> %02x\n", offs, res);
            return res;
        }

        void scu_write(offs_t offs, u8 data) {
            printf("scu_write: %08x %02x\n", offs, data);
        }
    };
    ...
    void korgtriton_state::map(address_map &map)
    {
        ...
        map(0xf00000, 0xf00001).rw(FUNC(korgtriton_state::scu_read), FUNC(korgtriton_state::scu_write));
    ...
    {
        printf("upd71051_txrdy_w: %d\n", state);
    }

    void korgtriton_state::scu_rxrdy_w(int state)
    {
        printf("upd71051_rxrdy_w: %d\n", state);
    }

...
#define LOG_ALL             (LOG_UNKNOWN | LOG_CSR | LOG_CTRLBUS | LOG_SYS_CTRL | LOG_FDC_CTRL | LOG_FDC_PORT | LOG_FDC_CMD | LOG_FDC_MECH | LOG_BRUSH_ADDR | \
							 LOG_STORE_ADDR | LOG_COMBINER | LOG_SIZE_CARD | LOG_FILTER_CARD | LOG_TABLET | LOG_COMMANDS)

#define VERBOSE             (LOG_CSR | LOG_CTRLBUS | LOG_STORE_ADDR | LOG_COMBINER | LOG_SIZE_CARD | LOG_FILTER_CARD | LOG_BRUSH_ADDR | LOG_COMMANDS)
#include "logmacro.h"
...
static INPUT_PORTS_START( dpb7000 )
	...
	PORT_START("PENX")
	PORT_BIT( 0xffff, 3800, IPT_LIGHTGUN_X) PORT_NAME("Pen X") PORT_MINMAX(1300, 6299) PORT_SENSITIVITY(50) PORT_CROSSHAIR(X, 1.0, 0.0, 0)

	PORT_START("PENY")
	PORT_BIT( 0xffff, 2500, IPT_LIGHTGUN_Y) PORT_NAME("Pen Y") PORT_MINMAX(0, 4999) PORT_SENSITIVITY(50) PORT_CROSSHAIR(Y, 1.0, 0.0, 0)
...
{
	static const char* const s_const_names[16] = { "Y0", "CY", "CV", "CU", "ES", "EI", "EII", "Y7", "IS", "IY", "IU", "IV", "IIS", "IIY", "IIU", "IIV" };
	...
	switch ((data >> 10) & 0xf)
	{
		...
		case 7: // Output Timing CP Flags
			m_output_cpflags = (uint8_t)(data & 0x000f);
			break;
		case 8: // IS
...
void dpb7000_state::handle_command(uint16_t data)
{
	...
	case 8: // Draw
	{
		...
		uint16_t fcs = BIT(data, 13) << 8;
		uint16_t prom_addr = bitswap<4>(data, 1, 2, 3, 4) | sel_2 | sel_ext | 0x80 | fcs | sel_1;
		...
		uint16_t pal_addr_extra = (BIT(prom_data, 5) << 8) | (BIT(~data, 11) << 9);
		...
		if (data == 0x3991 || data == 0x39b1)
		{
			printf("Drawing %d,%d at %d,%d\n", 0x1000 - m_bxlen, 0x1000 - m_bylen, m_cxpos[1], m_cypos[1]);
			printf("/S1:%d /S2:%d LUM:%d CHR:%d EXT:%d BZ:%d /BI:%d FCS:%d\n", s1_disable, s2_disable, lum_en, chr_en, BIT(data, 9), brush_zero, BIT(data, 11), fcs);
			...
			for (uint16_t x = m_cxpos[1], blx = bxlen; blx != 0x1000; blx++, x = (x + 1) & 0xfff)
			{
				...
				for (int ch = 0; ch < 2; ch++)
				{
					...
					if (comp_a > comp_b)
					{
						if (data == 0x3991 || data == 0x39b1)
						{
							printf("        comp_a %02x > comp_b %02x, final ALU pass should add\n", comp_a, comp_b);
						}
						alu_out = comp_b - comp_a;
						final_add = true;
					}
					else if(comp_b > comp_a)
					{
						if (data == 0x3991 || data == 0x39b1)
						{
							printf("        comp_a %02x < comp_b %02x, final ALU pass should subtract\n", comp_a, comp_b);
						}
						alu_out = comp_a - comp_b;
					}
					...
					if (data == 0x3991 || data == 0x39b1)
					{
						printf("        ALU out %02x, out product %04x, PALSel:%d, PALInv:%d\n", alu_out, out_product, pal_sel, pal_invert);
					}
					uint8_t final_msb = 0;
					uint8_t final_lsb = 0;
					if (!word_width)
					{
						...
						if (data == 0x3991 || data == 0x39b1)
						{
							if (final_add)
							{
								printf("        8bpp, ALUA %02x, ALUB %02x, ALU final %02x+%02x=%02x, MSB %02x\n", alu_a_val, alu_b_val, alu_a_val, alu_b_val, final_alu_msb_out, final_msb);
							}
							else
							{
								printf("        8bpp, ALUA %02x, ALUB %02x, ALU final %02x-%02x=%02x, MSB %02x\n", alu_a_val, alu_b_val, alu_a_val, alu_b_val, final_alu_msb_out, final_msb);
...
void dpb7000_state::cpu_ctrlbus_w(uint16_t data)
{
	...
	case 1: // Disk Sequencer Card, disc access
	{
		...
		else if (hi_nybble == 2)
		{
			...
			req_b_w(nullptr, 0); // Flag ourselves as in-use
...
TIMER_CALLBACK_MEMBER(dpb7000_state::tds_adc_tick)
{
	...
	{
		value += (0xaf >> m_tds_pressure_shift);
	}
...
TIMER_CALLBACK_MEMBER(dpb7000_state::tablet_hle_tick)
{
	if (m_tds_press_state != STATE_IDLE || (m_pen_switches->read() == 1))
	{
		...
		m_tablet_pen_y = 4999 - m_pen_y->read();
...
{
	const bool blank_1_or = m_select_matte[0] /*|| m_blank_1*/;
	const bool blank_2_or = m_select_matte[1] /*|| m_blank_2*/;
	const bool palette = false;
	const bool use_store1_matte = (blank_1_or && palette);
	const bool use_store2_matte = (blank_2_or && palette);
	const bool use_ext_store1_matte = (blank_1_or && !BIT(m_invert_mask, 0));
	const bool use_ext_store2_matte = (blank_2_or && !BIT(m_invert_mask, 1));

	for (int32_t y = 128; y < 128 + 625; y++)
	{
		uint32_t y_line = y * 800;

		uint32_t *d = &bitmap.pix(y - 128);
		for (int32_t x = 0; x < 800; x++)
		{
			const uint8_t uv_sel = (x & 1);

			const uint32_t pix_idx = y_line + x;

			const uint16_t lum1 = use_store1_matte ? m_matte_y[0] : m_framestore_lum[0][pix_idx];
			const uint16_t chr1 = use_store1_matte ? (uv_sel ? m_matte_v[0] : m_matte_u[0]) : m_framestore_chr[0][pix_idx];
			const uint16_t ext1 = use_ext_store1_matte ? m_matte_ext[0] : m_framestore_ext[0][pix_idx];

			const uint16_t lum2 = use_store2_matte ? m_matte_y[1] : m_framestore_lum[1][pix_idx];
			const uint16_t chr2 = use_store2_matte ? (uv_sel ? m_matte_v[1] : m_matte_u[1]) : m_framestore_chr[1][pix_idx];
			const uint16_t ext2 = use_ext_store2_matte ? m_matte_ext[1] : m_framestore_ext[1][pix_idx];

			const uint16_t ext_product = (ext1 * ext2) + 0x0080;
			const uint8_t ext_a_mask = (BIT(m_invert_mask, 2) ? 0xff : 0x00);
			const uint8_t ext_b_mask = (BIT(m_invert_mask, 2) ? 0xff : 0x00);
	...
	{
		//const uint8_t vlines = m_output_vlines[v & 0x3ff] & 0x0f;
...
void dpb7000_state::dpb7000(machine_config &config)
{
	...
	combined_screen.set_visarea(0, 799, 0, 624);
...
ROM_START( dpb7000 )
	...
	ROM_SYSTEM_BIOS( 0, "v411", "V4.11A" )
	ROMX_LOAD("01993c-naa-0608.bin", 0x00001, 0x10000, CRC(9e4cfd96) SHA1(faae2ba849d30c894d9f929bae371ae76e05bdac), ROM_BIOS(0) | ROM_SKIP(1))
	ROMX_LOAD("01993c-nca-3964.bin", 0x00000, 0x10000, CRC(04d75a15) SHA1(79f706296ba0c399b4e2d3acf947cae7706f9d63), ROM_BIOS(0) | ROM_SKIP(1))
	ROMX_LOAD("01993c-maa-fa58.bin", 0x20001, 0x10000, CRC(88dc8ee0) SHA1(a2f6b80022ddb5adf687b678c621c8f5db1f3575), ROM_BIOS(0) | ROM_SKIP(1))
	ROMX_LOAD("01993c-mca-b2b0.bin", 0x20000, 0x10000, CRC(16bf9db4) SHA1(a4696ee5a68b8a304c2433f16a6d57d7108f6406), ROM_BIOS(0) | ROM_SKIP(1))
	ROMX_LOAD("01993c-laa-6be3.bin", 0x40001, 0x10000, CRC(56e693cc) SHA1(b4f3e0f8b635fe2b3fa975ed57f966e8ade7fbb1), ROM_BIOS(0) | ROM_SKIP(1))
	ROMX_LOAD("01993c-lca-7c0d.bin", 0x40000, 0x10000, CRC(9540fa68) SHA1(79e46c4b6e86f6e53e3f5fea72e3be8c64de03af), ROM_BIOS(0) | ROM_SKIP(1))
	ROMX_LOAD("01993c-kaa-3b12.bin", 0x60001, 0x10000, CRC(41e2ec71) SHA1(37112f878b3db24b198e878f5dba90fcbb4175d2), ROM_BIOS(0) | ROM_SKIP(1))
	ROMX_LOAD("01993c-kca-f705.bin", 0x60000, 0x10000, CRC(00bfbd62) SHA1(17a5f2cbc91cabf1113c7d6e26124b67e5ece848), ROM_BIOS(0) | ROM_SKIP(1))
	ROMX_LOAD("01993c-jaa-3cc9.bin", 0x80001, 0x10000, CRC(9e477845) SHA1(b89e5d2b69a6043452a56685a1227df86c83a328), ROM_BIOS(0) | ROM_SKIP(1))
	ROMX_LOAD("01993c-jca-0b41.bin", 0x80000, 0x10000, CRC(58e8f302) SHA1(b37953046d81027b069b7a7a1834cd8c94fd9dca), ROM_BIOS(0) | ROM_SKIP(1))
	ROM_SYSTEM_BIOS( 1, "v406", "V4.06A" )
	ROMX_LOAD("01616a-nad-4af2.bin", 0x00001,  0x8000, CRC(a42eace6) SHA1(78c629a8afb48a95fc0a86ca762cc5b84bd9929b), ROM_BIOS(1) | ROM_SKIP(1))
	ROMX_LOAD("01616a-ncd-58de.bin", 0x00000,  0x8000, CRC(f70fff2a) SHA1(a6f85d086a0c53d156eeeb157184ebcad4adecb3), ROM_BIOS(1) | ROM_SKIP(1))
	ROMX_LOAD("01616a-mad-f512.bin", 0x10001,  0x8000, CRC(4c3e39f6) SHA1(443095c56481fbcadd4dcec1757d889c8f78805d), ROM_BIOS(1) | ROM_SKIP(1))
	ROMX_LOAD("01616a-mcd-91f2.bin", 0x10000,  0x8000, CRC(4b6b6eb3) SHA1(1bef443d78197d33e44c708ead9604020881f67f), ROM_BIOS(1) | ROM_SKIP(1))
	ROMX_LOAD("01616a-lad-0059.bin", 0x20001,  0x8000, CRC(0daf670d) SHA1(2342a43054ed141de298a1c1a6867949297bb52a), ROM_BIOS(1) | ROM_SKIP(1))
	ROMX_LOAD("01616a-lcd-5639.bin", 0x20000,  0x8000, CRC(c8977d3f) SHA1(4ee9f3a883400b4771e6ae33c6e4edcd5c0b49e7), ROM_BIOS(1) | ROM_SKIP(1))
	ROMX_LOAD("01616a-kad-1d9b.bin", 0x30001,  0x8000, CRC(bda7e309) SHA1(377edf2675a6736fe7ec775894858967b0e9247e), ROM_BIOS(1) | ROM_SKIP(1))
	ROMX_LOAD("01616a-kcd-e51c.bin", 0x30000,  0x8000, CRC(aa05a5cc) SHA1(85dce335a72643f7640524b18cfe480a3c299f23), ROM_BIOS(1) | ROM_SKIP(1))
	ROMX_LOAD("01616a-jad-47a8.bin", 0x40001,  0x8000, CRC(60fff4c9) SHA1(ba60281c0dd8627dffe07e7ea66f4eb688e74001), ROM_BIOS(1) | ROM_SKIP(1))
	ROMX_LOAD("01616a-jcd-7825.bin", 0x40000,  0x8000, CRC(bb258ede) SHA1(ab8042391cd361bcd874b2f9d8fcaf20d4b2ebe7), ROM_BIOS(1) | ROM_SKIP(1))
	ROMX_LOAD("01616a-iad-73a8.bin", 0x50001,  0x8000, CRC(98709fd2) SHA1(bd0f4689600e9fc49dbd8f2f326e18f8d602825e), ROM_BIOS(1) | ROM_SKIP(1))
	ROMX_LOAD("01616a-icd-0562.bin", 0x50000,  0x8000, CRC(bab5274e) SHA1(3e51977da3dfe8fda089b9d2c3199acb4fed3212), ROM_BIOS(1) | ROM_SKIP(1))
	ROMX_LOAD("01616a-had-d6fa.bin", 0x60001,  0x8000, CRC(70d791c5) SHA1(c281e4f27404e58ad5a80d6de1c5583cd9f3fe0e), ROM_BIOS(1) | ROM_SKIP(1))
	ROMX_LOAD("01616a-hcd-9c0e.bin", 0x60000,  0x8000, CRC(938cb614) SHA1(ea7ea8a13e0ab1497691bab53090296ba51d271f), ROM_BIOS(1) | ROM_SKIP(1))
	ROMX_LOAD("01616a-gad-397d.bin", 0x70001,  0x8000, CRC(0b95f9ed) SHA1(77126ee6c1f3dcdb8aa669ab74ff112e3f01918a), ROM_BIOS(1) | ROM_SKIP(1))
	ROMX_LOAD("01616a-gcd-3ab8.bin", 0x70000,  0x8000, CRC(e9c21438) SHA1(1784ab2de1bb6023565b2e27872a0fcda25e1b1f), ROM_BIOS(1) | ROM_SKIP(1))

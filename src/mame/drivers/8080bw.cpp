...
***********************************************************************************************************************************/
void _8080bw_state::cane_map(address_map &map)
{
  map(0x0000, 0x1fff).rom().nopw();
  map(0x2000, 0x3fff).ram().share("main_ram");
}

void _8080bw_state::cane_io_map(address_map &map)
{
/*********************************************************************************************************************************
  -----------
  I/O mapping
...
**********************************************************************************************************************************/  
  map(0x00, 0x00).w(FUNC(_8080bw_state::cane_unknown_port0_w));

  map(0x01, 0x01).portr("IN1").w(m_mb14241, FUNC(mb14241_device::shift_count_w));
  map(0x02, 0x02).w(m_mb14241, FUNC(mb14241_device::shift_data_w));
  map(0x03, 0x03).r(m_mb14241, FUNC(mb14241_device::shift_result_r)).w(FUNC(_8080bw_state::cane_sh_port_1_w));

  map(0x04, 0x04).w(m_watchdog, FUNC(watchdog_timer_device::reset_w));

  map(0x05, 0x05).w(FUNC(_8080bw_state::cane_music_w));
}
...
void _8080bw_state::cane(machine_config &config)
{
  mw8080bw_root(config);

  // Basic machine hardware
  I8080(config.replace(), m_maincpu, 1996800); /* 19.968MHz / 10 */
  m_maincpu->set_addrmap(AS_PROGRAM, &_8080bw_state::cane_map);
  m_maincpu->set_addrmap(AS_IO, &_8080bw_state::cane_io_map);
  m_maincpu->set_irq_acknowledge_callback(FUNC(_8080bw_state::interrupt_vector));

  WATCHDOG_TIMER(config, m_watchdog).set_vblank_count("screen", 255);

  MCFG_MACHINE_START_OVERRIDE(_8080bw_state, cane)
  MCFG_MACHINE_RESET_OVERRIDE(_8080bw_state, cane)
...
WRITE8_MEMBER( _8080bw_state::cane_unknown_port0_w)
{
  logerror("Unmapped io memory write to 00 = 00 %u\n", data);
}

MACHINE_START_MEMBER(_8080bw_state, cane)
{
  mw8080bw_state::machine_start();
}

MACHINE_RESET_MEMBER(_8080bw_state, cane)
{
  MACHINE_RESET_CALL_MEMBER(mw8080bw);
}

...
class megapc486_state : public driver_device // used to be at_vrom_fix_state in at.cpp
{
public:
	public:
	megapc486_state(const machine_config &mconfig, device_type type, const char *tag)
		: driver_device(mconfig, type, tag),
		m_maincpu(*this, "maincpu"),
		m_mb(*this, "mb"),
		m_ram(*this, RAM_TAG)
...
protected:
	...
	void init_megapc_common(int xmsbase);

	void megapc486_io(address_map &map);
	void megapc486_map(address_map &map);

protected:
	virtual void machine_start() override;
};

void megapc486_state::megapc486_map(address_map &map)
{
...
void megapc486_state::megapc486_io(address_map &map)
{
	map.unmap_value_high();
...
void megapc486_state::init_megapcpla()
{
	uint8_t* ROM = memregion("bios")->base();

	init_megapc_common(0xa0000);
...
void megapc486_state::init_megapc_common(int xmsbase)
{
...
void megapc486_state::machine_start()
{
	// megapc486_state::machine_start();
...
void megapc486_state::megapcpla(machine_config &config)
{
	i486_device &maincpu(I486(config, m_maincpu, 66'000'000 / 2));  // 486SLC
	maincpu.set_addrmap(AS_PROGRAM, &megapc486_state::megapc486_map);
	maincpu.set_addrmap(AS_IO, &megapc486_state::megapc486_io);
...
COMP( 1993, megapc,    0,       ibm5150, megapc,    0,     megapc_state,    init_megapc,    "Amstrad plc", "MegaPC",                       MACHINE_NOT_WORKING )
COMP( 199?, megapcpl,  megapc,  0,       megapcpl,  0,     megapc_state,    init_megapcpl,  "Amstrad plc", "MegaPC Plus",                  MACHINE_NOT_WORKING )
COMP( 199?, megapcpla, megapc,  0,       megapcpla, 0,     megapc486_state, init_megapcpla, "Amstrad plc", "MegaPC Plus (WINBUS chipset)", MACHINE_NOT_WORKING )

...
void pc4_state::machine_start()
{
	...
	m_busy_timer = machine().scheduler().timer_alloc(timer_expired_delegate(FUNC(pc4_state::clear_busy_flag), this));
	m_blink_timer = machine().scheduler().timer_alloc(timer_expired_delegate(FUNC(pc4_state::blink_tick), this));

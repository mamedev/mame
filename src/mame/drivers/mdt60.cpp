...
private:
	...
	void mdt60_map(address_map &map);
	void z22_map(address_map &map);
...
u8 mdt60_state::z22_status_r()
{
	return m_crtc->hsync_r() ? 0x02 : 0x00; // total guess
}

void mdt60_state::mdt60_map(address_map &map)
{
...
void mdt60_state::z22_map(address_map &map)
{
	map(0x0000, 0x07ff).ram();
	map(0x2000, 0x27ff).ram().share("charram");
	map(0x4000, 0x4001).rw(m_uart[0], FUNC(i8251_device::read), FUNC(i8251_device::write));
	map(0x6000, 0x67ff).ram().share("attrram");
	map(0x8000, 0x8001).rw(m_uart[1], FUNC(i8251_device::read), FUNC(i8251_device::write));
	...
	map(0xa000, 0xa000).r(FUNC(mdt60_state::z22_status_r));
	map(0xc000, 0xdfff).rom().region("charrom", 0);
	map(0xe000, 0xffff).rom().region("coderom", 0);
}

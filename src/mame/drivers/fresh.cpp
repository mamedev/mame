...
public:
	...
	void d30000_write(address_space &space, offs_t offset, uint16_t data, uint16_t mem_mask = 0xffff)
	{
		m_d30000_value = data;
	}

	void c71000_write(address_space &space, offs_t offset, uint16_t data, uint16_t mem_mask = 0xffff)
	{
		logerror("c74000_write (scroll 0) %04x (m_d30000_value = %04x)\n", data, m_d30000_value);
	}
	void c74000_write(address_space &space, offs_t offset, uint16_t data, uint16_t mem_mask = 0xffff)
	{
		logerror("c74000_write (scroll 1) %04x (m_d30000_value = %04x)\n", data, m_d30000_value);
	}
	void c75000_write(address_space &space, offs_t offset, uint16_t data, uint16_t mem_mask = 0xffff)
	{
		logerror("c75000_write (scroll 2) %04x (m_d30000_value = %04x)\n", data, m_d30000_value);
	}
	void c76000_write(address_space &space, offs_t offset, uint16_t data, uint16_t mem_mask = 0xffff)
	{
	...
	uint16_t unk2_r(address_space &space, offs_t offset, uint16_t mem_mask = 0xffff)
	{
		return 0x10;
	}
...
void fresh_state::fresh_bg_videoram_w(address_space &space, offs_t offset, uint16_t data, uint16_t mem_mask)
{
	COMBINE_DATA(&m_bg_videoram[offset]);
	m_bg_tilemap->mark_tile_dirty(offset);
}

void fresh_state::fresh_attr_videoram_w(address_space &space, offs_t offset, uint16_t data, uint16_t mem_mask)
{
...
void fresh_state::fresh_bg_2_videoram_w(address_space &space, offs_t offset, uint16_t data, uint16_t mem_mask)
{
	COMBINE_DATA(&m_bg_2_videoram[offset]);
	m_bg_2_tilemap->mark_tile_dirty(offset);
}

void fresh_state::fresh_attr_2_videoram_w(address_space &space, offs_t offset, uint16_t data, uint16_t mem_mask)
{

...
ROM_START( galivan )
	ROM_REGION( 0x14000, RGNCLASS_CPU, "main", 0 )	/* main cpu code */
	...
	ROM_REGION( 0x0100, RGNCLASS_USER, "user1", 0 )
...
ROM_START( ninjemak )
	ROM_REGION( 0x18000, RGNCLASS_CPU, "main", 0 )	/* main cpu code */
	...
	ROM_REGION( 0x20000, RGNCLASS_GFX, "gfx3", ROMREGION_DISPOSE )
	...
	ROM_REGION( 0x4000, RGNCLASS_GFX, "gfx5", 0 )	/* data for mcu/blitter? */
	ROM_LOAD( "ninjemak.5",   0x0000, 0x4000, CRC(5f91dd30) SHA1(3513c0a2e4ca83f602cacad6af9c07fe9e4b16a1) )	/* text layer data */

	ROM_REGION( 0x0400, RGNCLASS_PROMS, "proms", 0 )	/* Region 3 - color data */
...
ROM_START( youmab )
	...
	ROM_REGION( 0x10000, RGNCLASS_USER, "user2", 0 )	/* main cpu code */
...
static WRITE8_HANDLER( youmab_extra_bank_w )
{
	if (data==0xff)
	{
		memory_set_bankptr( 2, memory_region(machine, RGNCLASS_USER, "user2")+0x4000 );
	}
	else if (data==0x00)
	{
		memory_set_bankptr( 2, memory_region(machine, RGNCLASS_USER, "user2") );
	}
	else
	{
		printf("data %03x\n",data);

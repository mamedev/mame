	...
	if(ACCESSING_BITS_0_7) {
		...
		case 0:
			...
			case 0x9:
				...
				fdc_pt = memory_region(machine, RGNCLASS_USER, "user2") + track_size*(2*fdc_phys_track+(data & 8 ? 1 : 0));
				fdc_span = track_size;
				fdc_status = 3;
				fdc_drq = 1;
				fdc_data = *fdc_pt;
...
static NVRAM_HANDLER(system24)
{
	if(!track_size || !file)
		return;
	if(read_or_write)
		mame_fwrite(file, memory_region(machine, RGNCLASS_USER, "user2"), 2*track_size);
	else
		mame_fread(file, memory_region(machine, RGNCLASS_USER, "user2"), 2*track_size);
}
...
ROM_START( hotrod )
	...
	ROM_REGION( 0x1d6000, RGNCLASS_USER, "user2", 0)
...
ROM_START( qgh )
	...
	ROM_REGION16_BE( 0x400000, RGNCLASS_USER, "user1", 0)
...
ROM_START( sspirtfc )
	...
	ROM_REGION( 0x2000, RGNCLASS_USER, "user3", 0 )	/* decryption key */
...
ROM_START( roughrac )
	ROM_REGION( 0x40000, RGNCLASS_CPU, "main", 0 ) /* FD1094 code */
	ROM_LOAD16_BYTE( "epr-12187.ic2", 0x000000, 0x20000, CRC(e83783f3) SHA1(4b3b32df7de85aef9cd77c8a4ffc17e10466b638) )
	ROM_LOAD16_BYTE( "epr-12186.ic1", 0x000001, 0x20000, CRC(ce76319d) SHA1(0ede61f0700f9161285c768fa97636f0e42b96f8) )

...
static WRITE16_HANDLER( seta2_sound_bank_w )
{
	if (ACCESSING_BITS_0_7)
	{
		...
		int banks = (memory_region_length( machine, RGNCLASS_SOUND, "x1" ) - 0x100000) / 0x20000;
		if (data >= banks)
		{
...
static READ16_HANDLER( pzlbowl_protection_r )
{
	...
	return memory_region(machine, RGNCLASS_CPU, "main")[address - 2];
}
...
ROM_START( gundamex )
	ROM_REGION( 0x600000, RGNCLASS_CPU, "main", 0 )		/* TMP68301 Code */
	...
	ROM_REGION( 0x2000000, RGNCLASS_GFX, "gfx1", ROMREGION_DISPOSE|ROMREGION_ERASE)	/* Sprites */
...
ROM_START( grdians )
	...
	ROM_REGION( 0x200000, RGNCLASS_SOUND, "x1", 0 )	/* Samples */
	/* Leave 1MB empty (addressable by the chip) */
...
ROM_START( mj4simai )
	...
	ROM_REGION( 0x500000, RGNCLASS_SOUND, "x1", 0 )	/* Samples */
	/* Leave 1MB empty (addressable by the chip) */

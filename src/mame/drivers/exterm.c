...
static ADDRESS_MAP_START( master_map, ADDRESS_SPACE_PROGRAM, 16 )
	...
	AM_RANGE(0x03000000, 0x03ffffff) AM_MIRROR(0xfc000000) AM_ROM AM_REGION(RGNCLASS_USER, "user1", 0)
ADDRESS_MAP_END
...
ROM_START( exterm )
	ROM_REGION( 0x10000, RGNCLASS_CPU, "audio", 0 )		/* 64k for YM2151 code */
	ROM_LOAD( "v101y1", 0x8000, 0x8000, CRC(cbeaa837) SHA1(87d8a258f059512dbf9bc0e7cfff728ef9e616f1) )

	ROM_REGION( 0x10000, RGNCLASS_CPU, "audioslave", 0 )		/* 64k for DAC code */
	ROM_LOAD( "v101d1", 0x8000, 0x8000, CRC(83268b7d) SHA1(a9139e80e2382122e9919c0555937e120d4414cf) )

	ROM_REGION16_LE( 0x200000, RGNCLASS_USER, "user1", 0 )	/* 2MB for 34010 code */

...
static MACHINE_DRIVER_START( segac )
	...
	MDRV_SOUND_ADD("ym", YM3438, XL2_CLOCK/7)
	MDRV_SOUND_CONFIG(ym3438_intf)
	MDRV_SOUND_ROUTE(0, "mono", 0.50)
	/* right channel not connected */

	MDRV_SOUND_ADD("sn", SN76496, XL2_CLOCK/15)
...
ROM_START( ribbit ) /* Ribbit  (c)1991 Sega */
	...
	ROM_COPY( "main", 0x000000, 0x080000, 0x080000 )
...
static DRIVER_INIT( tfrceacb )
{
	...
	segac2_common_init(NULL);
	memory_install_write_handler(machine, 0, ADDRESS_SPACE_PROGRAM, 0x800000, 0x800001, 0, 0, SMH_NOP);
}
...
static DRIVER_INIT( pclub )
{
	...
	memory_install_read_handler(machine, 0, ADDRESS_SPACE_PROGRAM, 0x880120, 0x880121, 0, 0, printer_r );/*Print Club Vol.1*/
	memory_install_read_handler(machine, 0, ADDRESS_SPACE_PROGRAM, 0x880124, 0x880125, 0, 0, printer_r );/*Print Club Vol.2*/
	memory_install_write_handler(machine, 0, ADDRESS_SPACE_PROGRAM, 0x880124, 0x880125, 0, 0, print_club_camera_w);
}

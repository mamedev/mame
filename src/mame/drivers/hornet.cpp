...
/*  Konami Hornet System
    ...
    was also developed for a "JAMMA to JVS converter" using the same H8/3644 MCU.

	Notes:
          058232 - Konami Custom Ceramic Package (SIL14)
         H8/3644 - Hitachi H8/3644 Microcontroller
         X1 - Crystal @7.3728MHz
    ...
    GQ830 PWB(J) I/O PCB
    ------------------------------------------
    Note: This is simply another ADC12138. It uses two 5k potentiometers for X/Y analog inputs. Silent Scope EX would
    later use this pcb.
    
    PPC Data Line 3 = CONV (Conversion)
    PPC Data Line 4 = CS (Chip Select)
    PPC Data Line 5 = DIN (Data In/Receive)
    PPC Data Line 7 = SCLK (Serial Data Clock Input)
    Player 2 Button 1 = EOC (End of Conversion)
    Player 2 Button 2 = DOR (Data Output Ready)
    Player 2 Button 3 = DOUT (Data Out/Send)
    
    ADCH0 --> Analog 0 (Gun X Axis pot)
    ADCH1 --> Analog 1 (Gun Y Axis pot)
...
// (vsync and hsync status for each of the two CCD cameras?)
...
static INPUT_PORTS_START(nbapbp) //Player 3 and 4's inputs require JVS support

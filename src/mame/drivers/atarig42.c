...
ROM_START( guardian )
	...
	ROM_REGION( 0x1500, "plds", ROMREGION_DISPOSE )
...
static DRIVER_INIT( roadriot )
{
	...
	sloop_base = memory_install_readwrite_handler(machine, 0, ADDRESS_SPACE_PROGRAM, 0x000000, 0x07ffff, 0, 0, roadriot_sloop_data_r, roadriot_sloop_data_w);
...
static DRIVER_INIT( guardian )
{
	...
	*(UINT16 *)&memory_region(machine, "main")[0x80000] = 0x4E75;
	...
	sloop_base = memory_install_readwrite_handler(machine, 0, ADDRESS_SPACE_PROGRAM, 0x000000, 0x07ffff, 0, 0, guardians_sloop_data_r, guardians_sloop_data_w);
	memory_set_opbase_handler(0, sloop_opbase_handler);

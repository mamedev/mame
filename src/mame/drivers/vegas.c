...
static void remap_dynamic_addresses(running_machine *machine)
{
	...
	for (addr = 0; addr < dynamic_count; addr++)
		memory_install_readwrite_handler(machine, 0, ADDRESS_SPACE_PROGRAM, dynamic[addr].start, dynamic[addr].end, 0, 0, SMH_UNMAP, SMH_UNMAP);
	...
	for (addr = 0; addr < dynamic_count; addr++)
	{
		...
		if (dynamic[addr].mread != NULL || dynamic[addr].mwrite != NULL)
			_memory_install_handler(machine, 0, ADDRESS_SPACE_PROGRAM, dynamic[addr].start, dynamic[addr].end, 0, 0, (genf *)dynamic[addr].mread, (genf *)dynamic[addr].mwrite, dynamic[addr].rdname, dynamic[addr].wrname);
		if (dynamic[addr].dread != NULL || dynamic[addr].dwrite != NULL)
			_memory_install_device_handler(dynamic[addr].device, 0, ADDRESS_SPACE_PROGRAM, dynamic[addr].start, dynamic[addr].end, 0, 0, (genf *)dynamic[addr].dread, (genf *)dynamic[addr].dwrite, dynamic[addr].rdname, dynamic[addr].wrname);
	}

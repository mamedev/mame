...
{
	if (cpu_get_pc(space->cpu)==0x60154b2) cpu_spinuntil_int(space->cpu); // bios menus..
...
{
	if (cpu_get_pc(space->cpu)==0x6013aee) cpu_spinuntil_int(space->cpu); // for use in japan
...
{
 if (cpu_get_pc(space->cpu)==0x0600440e)
  cpu_spinuntil_time(space->cpu, ATTOTIME_IN_USEC(20)); // is this safe... we can't skip till vbl because its not a vbl wait loop
...
{
	if (cpu_get_pc(space->cpu)==0x060041C6) cpu_spinuntil_int(space->cpu); // after you enable the sound cpu ...
...
{
	if (cpu_get_pc(space->cpu)==0x6018640) cpu_spinuntil_int(space->cpu); // after you enable the sound cpu ...
...
{
	if (cpu_get_pc(space->cpu)==0x06010160) cpu_spinuntil_int(space->cpu); // title logos
...
{
	if (cpu_get_pc(space->cpu)==0x6021CF0) cpu_spinuntil_time(space->cpu, ATTOTIME_IN_USEC(400)); // spinuntilint breaks controls again .. urgh
...
{
	if (cpu_get_pc(space->cpu)==0x060461A0) cpu_spinuntil_time(space->cpu, ATTOTIME_IN_USEC(20)); // spinuntilint breaks controls again .. urgh
...
{
	if (cpu_get_pc(space->cpu)==0x06030EE2) cpu_spinuntil_time(space->cpu, ATTOTIME_IN_USEC(20)); // spinuntilint breaks lots of things
...
{
	if (cpu_get_pc(space->cpu)==0x06031c7a) cpu_spinuntil_time(space->cpu, ATTOTIME_IN_USEC(20)); // spinuntilint breaks lots of things
...
{
	if (cpu_get_pc(space->cpu)==0x6027c90) cpu_spinuntil_int(space->cpu);//cpu_spinuntil_time(space->cpu, ATTOTIME_IN_USEC(20));
...
static WRITE32_HANDLER(dnmtdeka_cmd_write)
{
	...
	if ( (cpunum_get_active() == 0) && (cpu_get_pc(space->cpu) == 0x00000d04) )
		return;

	if ( data != 0 ) dnmtdeka_pending_commands++;
	//logerror( "CMD: Written by cpu=%d, at = %08X, offset = %08X, data = %08X, commands = %d\n", cpunum_get_active(), cpu_get_pc(space->cpu), offset, data, dnmtdeka_pending_commands );
	cpuexec_trigger(space->machine, 1000);
}

static READ32_HANDLER(dnmtdeka_cmd_read)
{
	//logerror( "CMD: Read by cpu=%d, at = %08X, offset = %08X, data = %08X, commands = %d\n", cpunum_get_active(), cpu_get_pc(space->cpu), offset, stv_workram_h[0xe0bd0/4 + offset], dnmtdeka_pending_commands );
	if ( cpu_get_pc(space->cpu) == 0x060051f2 )
	{
...
{
	if ( cpu_get_pc(space->cpu) == 0x06027c98 ) cpu_spinuntil_int(space->cpu);
...
static WRITE32_HANDLER(diehard_cmd_write)
{
	...
	//logerror( "CMD: Written by cpu=%d, at = %08X, offset = %08X, data = %08X, commands = %d\n", cpunum_get_active(), cpu_get_pc(space->cpu), offset, data, diehard_pending_commands );
	cpuexec_trigger(space->machine, 1000);
}

static READ32_HANDLER(diehard_cmd_read)
{
	//logerror( "CMD: Read by cpu=%d, at = %08X, offset = %08X, data = %08X, commands = %d\n", cpunum_get_active(), cpu_get_pc(space->cpu), offset, stv_workram_h[0xe0bd0/4 + offset], diehard_pending_commands );
	if ( cpu_get_pc(space->cpu) == 0x060051f2 )
	{
...
{
	//logerror( "CMDACK: Read by cpu=%d, at = %08X, offset = %08X, data = %08X, commands = %d\n", cpunum_get_active(), cpu_get_pc(space->cpu), offset, stv_workram_h[0x000e0dd8/4], diehard_pending_commands );
...
{
	if (cpu_get_pc(space->cpu)==0x060041c2) cpu_spinuntil_time(space->cpu, ATTOTIME_IN_USEC(20));
...
{
	if (cpu_get_pc(space->cpu)==0x0600bb0a) cpu_spinuntil_time(space->cpu, ATTOTIME_IN_USEC(20));
...
{
	if (cpu_get_pc(space->cpu)==0x0600b31e )
		cpu_spinuntil_int(space->cpu);
...
//  logerror ("groove speedup \n");
	if (cpu_get_pc(space->cpu)==0x060a4970)
	{
		cpu_spinuntil_int(space->cpu); // title logos
...
{
    if (cpu_get_pc(space->cpu)==0x060060c2)     cpu_set_input_line(space->machine->cpu[1], INPUT_LINE_HALT, ASSERT_LINE);
    return 0;
}
#endif
...
{
	if (cpu_get_pc(space->cpu)==0x06028c8e) cpu_spinuntil_time(space->cpu, ATTOTIME_IN_USEC(20));
	return stv_workram_h[0x0ffc10/4];
}
...
{
	if(cpu_get_pc(space->cpu) == 0x0605b9da )
...
{
	if (cpu_get_pc(space->cpu) == 0x0602A024) cpu_spinuntil_int(space->cpu);
	return stv_workram_h[0x00031424/4];
}

static READ32_HANDLER(thunt_speedup2_r)
{
	if (cpu_get_pc(space->cpu) == 0x06013EEA) cpu_spinuntil_int(space->cpu);
...
{
	if (cpu_get_pc(space->cpu) == 0x0602a0f8) cpu_spinuntil_int(space->cpu);
	return stv_workram_h[0x000314f8/4];
}

static READ32_HANDLER(sandor_speedup2_r)
{
	if (cpu_get_pc(space->cpu) == 0x06013fbe) cpu_spinuntil_int(space->cpu);
...
{
	if ( cpu_get_pc(space->cpu) == 0x06041E32 ) cpu_spinuntil_time(space->cpu, ATTOTIME_IN_USEC(20));
...
{
	//logerror( "batmanfr speedup: pc = %08x, mem = %08x\n", cpu_get_pc(space->cpu), stv_workram_h[0x0002acf0/4] );
	if ( cpu_get_pc(space->cpu) != 0x060121c0 )
		cpu_spinuntil_int(space->cpu);
...
{
	if ( cpu_get_pc(space->cpu) == 0x06098aea ) cpu_spinuntil_time(space->cpu, ATTOTIME_IN_USEC(20));//cpu_spinuntil_int(space->cpu);
	return stv_workram_h[0x000ffc10/4];
}
...
{
	if ( cpu_get_pc(space->cpu) == 0x0602cbfa ) cpu_spinuntil_time(space->cpu, ATTOTIME_IN_USEC(20));
	return stv_workram_h[0x000ffc10/4];
}
...
{
	if ( cpu_get_pc(space->cpu) == 0x0602c30c ) cpu_spinuntil_int(space->cpu);
...
{
	if ( cpu_get_pc(space->cpu) == 0x06026398 ) cpu_spinuntil_int(space->cpu);
	return stv_workram_h[0x000ffc10/4];
}
...
{
	if ( cpu_get_pc(space->cpu) == 0x0602bcbe ) cpu_spinuntil_time(space->cpu, ATTOTIME_IN_USEC(20));
	return stv_workram_h[0x000ffc10/4];
}
...
{
	if ( cpu_get_pc(space->cpu) == 0x06012ee4 ) cpu_spinuntil_int(space->cpu);
...
{
	if ( cpu_get_pc(space->cpu) == 0x06013f7a ) cpu_spinuntil_int(space->cpu);
...
{
	if (cpu_get_pc(space->cpu)==0x6012ec2) cpu_spinuntil_time(space->cpu, ATTOTIME_IN_USEC(20));
	return stv_workram_h[0x0ffc10/4];
}
...
{
	if (cpu_get_pc(space->cpu)==0x605edde) cpu_spinuntil_time(space->cpu, ATTOTIME_IN_USEC(20));
	return stv_workram_h[0x0ffc10/4];
}
...
{
	if (cpu_get_pc(space->cpu)==0x6063bf4) cpu_spinuntil_time(space->cpu, ATTOTIME_IN_USEC(20));
	return stv_workram_h[0x0ffc10/4];
}
...
{
	if (cpu_get_pc(space->cpu)==0x6064d60) cpu_spinuntil_int(space->cpu);
	return stv_workram_h[0x0ffc10/4];
}
#endif
...
//	memory_install_read32_handler(machine, 0, ADDRESS_SPACE_PROGRAM, 0x60ffc10, 0x60ffc13, 0, 0, finlarch_speedup_r ); //breaks test mode
...
{
	if (cpu_get_pc(space->cpu)==0x06012a52) cpu_spinuntil_time(space->cpu, ATTOTIME_IN_USEC(20));
	return stv_workram_h[0x0ffc10/4];
}
...
{
	if (cpu_get_pc(space->cpu)==0x0605eb78)
...
{
	if (cpu_get_pc(space->cpu)==0x06020c5c)
			cpu_spinuntil_int(space->cpu);
...
{
	if (cpu_get_pc(space->cpu)==0x0604eac0) cpu_spinuntil_time(space->cpu, ATTOTIME_IN_USEC(20));
	return stv_workram_h[0x0ffc10/4];
}
...
{
	if (cpu_get_pc(space->cpu)==0x06034d04) cpu_spinuntil_time(space->cpu, ATTOTIME_IN_USEC(20));
	return stv_workram_h[0x0ffc10/4];
}
...
{
	if (cpu_get_pc(space->cpu)==0x601eb4c) cpu_spinuntil_time(space->cpu, ATTOTIME_IN_USEC(30));

...
static ADDRESS_MAP_START( nova2001_map, ADDRESS_SPACE_PROGRAM, 8 )
	...
	AM_RANGE(0xa000, 0xa7ff) AM_RAM AM_WRITE(nova2001_fg_videoram_w) AM_BASE(&nova2001_fg_videoram)
	AM_RANGE(0xa800, 0xafff) AM_RAM AM_WRITE(nova2001_bg_videoram_w) AM_BASE(&nova2001_bg_videoram)
...
static ADDRESS_MAP_START( ninjakun_cpu1_map, ADDRESS_SPACE_PROGRAM, 8 )
	...
	AM_RANGE(0xc000, 0xc7ff) AM_READWRITE(SMH_RAM, nova2001_fg_videoram_w) AM_BASE(&nova2001_fg_videoram) AM_SHARE(1)
	...
	AM_RANGE(0xd800, 0xd9ff) AM_READWRITE(SMH_RAM, ninjakun_paletteram_w) AM_BASE(&paletteram) AM_SHARE(4)
...
static ADDRESS_MAP_START( ninjakun_cpu2_map, ADDRESS_SPACE_PROGRAM, 8 )
	...
	AM_RANGE(0xc000, 0xc7ff) AM_READWRITE(SMH_RAM, nova2001_fg_videoram_w) AM_SHARE(1)
	...
	AM_RANGE(0xd800, 0xd9ff) AM_READWRITE(SMH_RAM, ninjakun_paletteram_w) AM_SHARE(4)
...
static ADDRESS_MAP_START( pkunwar_map, ADDRESS_SPACE_PROGRAM, 8 )
	...
	AM_RANGE(0x8800, 0x8fff) AM_RAM AM_WRITE(nova2001_bg_videoram_w) AM_BASE(&nova2001_bg_videoram)
...
static ADDRESS_MAP_START( raiders5_cpu1_map, ADDRESS_SPACE_PROGRAM, 8 )
	AM_RANGE(0x0000, 0x7fff) AM_ROM
	AM_RANGE(0x8000, 0x87ff) AM_RAM AM_BASE(&spriteram)
	AM_RANGE(0x8800, 0x8fff) AM_RAM AM_WRITE(nova2001_fg_videoram_w) AM_BASE(&nova2001_fg_videoram)
	...
	AM_RANGE(0xd000, 0xd1ff) AM_READWRITE(SMH_RAM, ninjakun_paletteram_w) AM_BASE(&paletteram)
...
static INPUT_PORTS_START( ninjakun )
	...
	PORT_DIPSETTING(    0x40, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x80, 0x80, "Infinite Lives (If Free Play)" )
	PORT_DIPSETTING(    0x80, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
INPUT_PORTS_END
...
static GFXDECODE_START( nova2001 )
	GFXDECODE_ENTRY( REGION_GFX1, 0x0000, layout16x16,    0x000, 16 )	// sprites
	GFXDECODE_ENTRY( REGION_GFX1, 0x0000, layout8x8_part, 0x000, 16 )	// fg tiles (using only 1/4th of the ROM space)
	GFXDECODE_ENTRY( REGION_GFX1, 0x4000, layout8x8_part, 0x100, 16 )	// bg tiles (using only 1/4th of the ROM space)
GFXDECODE_END

static GFXDECODE_START( ninjakun )
	GFXDECODE_ENTRY( REGION_GFX1, 0, layout16x16, 0x200, 16 )	// sprites
	GFXDECODE_ENTRY( REGION_GFX1, 0, layout8x8,   0x000, 16 )	// fg tiles
	GFXDECODE_ENTRY( REGION_GFX2, 0, layout8x8,   0x100, 16 )	// bg tiles
GFXDECODE_END

static GFXDECODE_START( pkunwar )
	GFXDECODE_ENTRY( REGION_GFX1, 0, layout16x16, 0x000, 16 )	// sprites
	GFXDECODE_ENTRY( REGION_GFX1, 0, layout8x8,   0x100, 16 )	// bg tiles
GFXDECODE_END

static GFXDECODE_START( raiders5 )
	GFXDECODE_ENTRY( REGION_GFX1, 0, layout16x16,    0x200, 16 )	// sprites
	GFXDECODE_ENTRY( REGION_GFX1, 0, layout8x8_part, 0x000, 16 )	// fg tiles (using only 1/4th of the ROM space)
	GFXDECODE_ENTRY( REGION_GFX2, 0, layout8x8,      0x100, 16 )	// bg tiles
GFXDECODE_END
...
static const struct AY8910interface nova2001_ay8910_interface_1 =
{
	0,
	0,
	nova2001_scroll_x_w,
...
static const struct AY8910interface nova2001_ay8910_interface_2 =
{
	input_port_3_r,
	input_port_4_r
};

static const struct AY8910interface pkunwar_ay8910_interface_1 =
{
	input_port_0_r,
	input_port_1_r
};

static const struct AY8910interface pkunwar_ay8910_interface_2 =
{
	input_port_2_r,
	input_port_3_r
};
...
ROM_START( pkunwar )
	...
	ROM_REGION( 0x10000, REGION_GFX1, ROMREGION_DISPOSE )	// (need lineswapping)
...
ROM_START( raiders5 )
	...
	ROM_REGION( 0x8000, REGION_GFX1, ROMREGION_DISPOSE )	// (need lineswapping)
	...
	ROM_REGION( 0x4000, REGION_GFX2, ROMREGION_DISPOSE )	// (need lineswapping)

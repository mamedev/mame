...
static void tecmo_adpcm_int(running_machine *machine, int num)
{
	...
	if (adpcm_pos >= adpcm_end ||
				adpcm_pos >= memory_region_length(machine, RGNCLASS_SOUND, "adpcm"))
		MSM5205_reset_w(0,1);
	...
	{
		UINT8 *ROM = memory_region(machine, RGNCLASS_SOUND, "adpcm");
...
ROM_START( rygar )
	...
	ROM_REGION( 0x4000, RGNCLASS_SOUND, "adpcm", 0 )	/* ADPCM samples */
...
ROM_START( silkworm )
	...
	ROM_REGION( 0x20000, RGNCLASS_CPU, "sound", 0 )

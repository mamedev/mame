...
static DRIVER_INIT( backfirt )
{
	...
	/* no MSM */
	memory_nop_write(machine->device("soundcpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0xc000, 0xc000, 0, 0);
	memory_nop_write(machine->device("soundcpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0xd000, 0xd000, 0, 0);
	memory_nop_write(machine->device("soundcpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0xe000, 0xe000, 0, 0);
}

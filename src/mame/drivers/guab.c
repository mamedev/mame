...
{
	UINT8* disk = (UINT8*)memory_region(machine, RGNCLASS_USER, "user1");
	int more_data = 0;
...
static ADDRESS_MAP_START( guab_map, ADDRESS_SPACE_PROGRAM, 16 )
	...
	AM_RANGE(0x040000, 0x04ffff) AM_ROM AM_REGION(RGNCLASS_CPU, "main", 0x10000)
...
ROM_START( guab )
	...
	ROM_REGION( 0xb4000, RGNCLASS_USER, "user1", 0 )

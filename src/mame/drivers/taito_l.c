...
static void machine_init(running_machine *machine)
{
	...
	memory_set_bankptr(1, memory_region(machine, RGNCLASS_CPU, "cpu1") + 0x10000);
...
//      logerror("robs %d, %02x (%04x)\n", offset, data, activecpu_get_pc());
		cur_rombank = data;
		memory_set_bankptr(1, memory_region(machine, RGNCLASS_CPU, "main")+0x10000+0x2000*cur_rombank);
	}
}
...
//      logerror("robs2 %02x (%04x)\n", data, activecpu_get_pc());
		...
		cur_rombank2 = data;
		memory_set_bankptr(6, memory_region(machine, RGNCLASS_CPU, "cpu2")+0x10000+0x4000*cur_rombank2);
	}
}
...
static MACHINE_DRIVER_START( kurikint )
	...
	MDRV_CPU_VBLANK_INT("cpu1", irq0_line_hold)
...
static MACHINE_DRIVER_START( evilston )
	...
	/* basic machine hardware */
	MDRV_CPU_ADD("cpu1", Z80, 13330560/2) 	/* not verfied */
...
ROM_START( raimais )
	ROM_REGION( 0xb0000, RGNCLASS_CPU, "cpu1", 0 )
	...
	ROM_REGION( 0x1c000, RGNCLASS_CPU, "audio", 0 )	/* sound (audio/rastan.c wants it as #2 */
	...
	ROM_REGION( 0x80000, RGNCLASS_SOUND, "ym", 0 )
...
ROM_START( fhawk )
	...
	ROM_REGION( 0x30000, RGNCLASS_CPU, "cpu2", 0 )
...
ROM_START( champwr )
	ROM_REGION( 0xf0000, RGNCLASS_CPU, "cpu1", 0 )
...
ROM_START( plotting ) /* Likely B96-10 or higher by Taito's rom numbering system, demo mode is 1 player */
	ROM_REGION( 0x20000, RGNCLASS_CPU, "cpu1", 0 )
...
ROM_START( puzznic )
	ROM_REGION( 0x30000, RGNCLASS_CPU, "cpu1", 0 )
...
ROM_START( cubybop )
	ROM_REGION( 0x50000, RGNCLASS_CPU, "cpu1", 0 )
...
ROM_START( evilston )
	...
	ROM_REGION( 0x80000, RGNCLASS_CPU, "audio", 0 )
...
static DRIVER_INIT( plottina )
{
	...
	p = memory_region(machine, RGNCLASS_CPU, "main");
	for(i=0;i<0x20000;i++)
	{
		*p = tab[*p];
		p++;

...
{
	int vol_button_state = m_digitalvol->read();
...
READ16_MEMBER(cps2_state::joy_or_paddle_ecofghtr_r)
{
	...
	{
		int dial0 = (m_dial[0]->read());
		int dial1 = (m_dial[1]->read());
...
template<int Xor>
READ16_MEMBER(cps2_state::cps2_objram_r)
{
	return m_objram[m_objram_bank ^ Xor][offset];
}

template<int Xor>
WRITE16_MEMBER(cps2_state::cps2_objram_w)
{
...
void cps2_state::cps2_map(address_map &map)
{
	...
	map(0x700000, 0x701fff).w(FUNC(cps2_state::cps2_objram_w<0>)).share("objram1");                           /* Object RAM, no game seems to use it directly */
	map(0x708000, 0x709fff).mirror(0x006000).rw(FUNC(cps2_state::cps2_objram_r<1>), FUNC(cps2_state::cps2_objram_w<1>)).share("objram2");           /* Object RAM */
	...
	map(0x8040e1, 0x8040e1).w(FUNC(cps2_state::cps2_objram_bank_w));                                                   /* bit 0 = Object ram bank swap */
...
MACHINE_START_MEMBER(cps2_state,cps2)
{
	if (m_audiocpu != nullptr) // gigaman2 has an AT89C4051 (8051) MCU as an audio cpu, no qsound.
		m_audiobank->configure_entries(0, (QSOUND_SIZE - 0x8000) / 0x4000, m_audioregion + 0x8000, 0x4000);
}

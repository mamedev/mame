...
static ADDRESS_MAP_START( amiga_map, ADDRESS_SPACE_PROGRAM, 16 )
	...
	AM_RANGE(0xf80000, 0xffffff) AM_ROM AM_REGION(RGNCLASS_USER, "user1", 0)		/* Kickstart BIOS */

	AM_RANGE(0x800000, 0x97ffff) AM_ROMBANK(2) AM_REGION(RGNCLASS_USER, "user3", 0)
	AM_RANGE(0x980000, 0x9fbfff) AM_ROM AM_REGION(RGNCLASS_USER, "user2", 0)
...
#define ARCADIA_BIOS \
	ROM_REGION16_BE(0x80000, RGNCLASS_USER, "user1", 0 ) \
	...
	\
	ROM_REGION16_BE( 0x80000, RGNCLASS_USER, "user2", 0 ) \
...
ROM_START( ar_bios )
	...
	ROM_REGION16_BE(0x180000, RGNCLASS_USER, "user3", ROMREGION_ERASEFF)
ROM_END
...
ROM_START( ar_ninj )
	...
	ROM_REGION16_BE(0x200000, RGNCLASS_USER, "user3", 0)
...
{
	UINT16 *rom = (UINT16 *)memory_region(Machine, RGNCLASS_USER, tag);
	int i;
...
static void arcadia_init(running_machine *machine)
{
	...
	biosrom = (UINT16 *)memory_region(Machine, RGNCLASS_USER, "user2");
	if (biosrom[0] != 0x4afc)

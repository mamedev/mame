...
WRITE32_MEMBER(djmain_state::pcmram_bank_w)
{
	if (ACCESSING_BITS_16_31)
	{
		m_pcmram_bank = (data >> 16) & 0x1f;
	}
}

READ32_MEMBER(djmain_state::pcmram_r)
{
	uint32_t data = 0;

	offset += (m_pcmram_bank * 0x80000);

	if (ACCESSING_BITS_24_31)
		data |= m_pcmram[offset * 4] << 24;

	if (ACCESSING_BITS_16_23)
		data |= m_pcmram[offset * 4 + 1] << 16;

	if (ACCESSING_BITS_8_15)
		data |= m_pcmram[offset * 4 + 2] << 8;

	if (ACCESSING_BITS_0_7)
		data |= m_pcmram[offset * 4 + 3];

	return data;
}

WRITE32_MEMBER(djmain_state::pcmram_w)
{
	offset += (m_pcmram_bank * 0x80000);

	if (ACCESSING_BITS_24_31)
		m_pcmram[offset * 4] = data >> 24;

	if (ACCESSING_BITS_16_23)
		m_pcmram[offset * 4 + 1] = data >> 16;

	if (ACCESSING_BITS_8_15)
		m_pcmram[offset * 4 + 2] = data >> 8;

	if (ACCESSING_BITS_0_7)
		m_pcmram[offset * 4 + 3] = data;
}
...
static ADDRESS_MAP_START( maincpu_djmain, AS_PROGRAM, 32, djmain_state )
	...
	AM_RANGE(0x500000, 0x57ffff) AM_READWRITE(pcmram_r, pcmram_w)               // SOUND RAM
	...
	AM_RANGE(0x5d6000, 0x5d6003) AM_WRITE(pcmram_bank_w)                    // SOUND RAM bank
...
static ADDRESS_MAP_START( k054539_map, AS_0, 8, djmain_state )
	AM_RANGE(0x000000, 0xffffff) AM_RAM AM_SHARE("pcmram")
ADDRESS_MAP_END

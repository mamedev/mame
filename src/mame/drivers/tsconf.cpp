...
void tsconf_state::video_start()
{
	...
	m_frame_irq_timer = machine().scheduler().timer_alloc(timer_expired_delegate(FUNC(tsconf_state::irq_frame), this));
	m_line_irq_timer = machine().scheduler().timer_alloc(timer_expired_delegate(FUNC(tsconf_state::irq_scanline), this));
}

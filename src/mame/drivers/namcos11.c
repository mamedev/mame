...
static DRIVER_INIT( namcos11 )
{
	...
	while( namcos11_config_table[ n_game ].s_name != NULL )
	{
		if( strcmp( machine->gamedrv->name, namcos11_config_table[ n_game ].s_name ) == 0 )
		{
			...
			{
				int bank;
				UINT32 len = memory_region_length( machine, RGNCLASS_USER, "user2" );
				UINT8 *rgn = memory_region( machine, RGNCLASS_USER, "user2" );
...
ROM_START( danceyes )
	ROM_REGION32_LE( 0x0400000, RGNCLASS_USER, "user1", 0 ) /* main prg */
...
ROM_START( starswep )
	...
	ROM_REGION32_LE( 0x0100000, RGNCLASS_USER, "user2", ROMREGION_ERASE00 ) /* main data */
...
ROM_START( tekkena )
	...
	ROM_REGION( 0x0200000, RGNCLASS_SOUND, "c352", 0 ) /* samples */
	ROM_LOAD( "te1wave.8k",   0x0000000, 0x200000, CRC(fce6c57a) SHA1(7fb8c69452c92c59a940a2b69d0d73ef7aefcb82) )
ROM_END

...
static ADDRESS_MAP_START( jantotsu_io, AS_IO, 8, jantotsu_state )
	...
	AM_RANGE(0x01, 0x01) AM_READ_LEGACY(jantotsu_dsw2_r) AM_DEVWRITE_LEGACY("sn2", sn76496_w)
	AM_RANGE(0x02, 0x03) AM_DEVWRITE_LEGACY("adpcm", jan_adpcm_w)
	AM_RANGE(0x04, 0x04) AM_READWRITE_LEGACY(jantotsu_mux_r, jantotsu_mux_w)
	AM_RANGE(0x07, 0x07) AM_WRITE_LEGACY(bankaddr_w)
ADDRESS_MAP_END

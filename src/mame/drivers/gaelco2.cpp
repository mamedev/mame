...
static GFXDECODE_START( gfx_gaelco2 )
	GFXDECODE_ENTRY( "gfx1", 0x0000000, tilelayout16, 0, 128 )
GFXDECODE_END
...
void gaelco2_state::maniacsq(machine_config &config)
{
	...
	gaelco_gae1_device &gaelco(GAELCO_GAE1(config, "gaelco", XTAL(26'000'000) / 26));
...
void gaelco2_state::saltcrdi(machine_config &config)
{
	...
	gaelco_gae1_device &gaelco(GAELCO_GAE1(config, "gaelco", XTAL(24'000'000) / 24)); /* unused? ROMs contain no sound data */
	gaelco.set_device_rom_tag("gfx1");
	gaelco.set_bank_offsets(0 * 0x0040000, 1 * 0x0040000, 0, 0);
	gaelco.add_route(0, "lspeaker", 1.0);
	gaelco.add_route(1, "rspeaker", 1.0);
}
...
ROM_START( saltcrdi ) // REF 970410
	...
	ROM_LOAD( "bi-37-bank0-27c1001.u37",   0x00c0000, 0x0020000, CRC(779fca47) SHA1(fce95893a5bcf0c6f26c223491c95154f072c92b) )
	ROM_FILL(                              0x0100000, 0x0020000, 0x00 )         /* to decode GFX as 5bpp */
	ROM_LOAD( "bi-44-bank1-27c1001.u44",   0x0020000, 0x0020000, CRC(171d2f88) SHA1(e2b406dad78e3ab5bebb673ed03db5d27879283e) )
	ROM_LOAD( "bi-43-bank1-27c1001.u43",   0x0060000, 0x0020000, CRC(69b35d81) SHA1(d9657e3d592079071df019cd75d676fa4b1bcba9) )
	ROM_LOAD( "bi-42-bank1-27c1001.u42",   0x00a0000, 0x0020000, CRC(eaef0565) SHA1(4214b05f1df3062eaeea91505b61816725556ed5) )
	ROM_LOAD( "bi-41-bank1-27c1001.u41",   0x00e0000, 0x0020000, CRC(c4d24254) SHA1(e6ff7624e628dc6ace11a50b6ff89812844b52c5) )
	ROM_FILL(                              0x0120000, 0x0020000, 0x00 )         /* to decode GFX as 5bpp */
...
void bang_state::bang_map(address_map &map)
{
	...
	map(0x218004, 0x218007).writeonly().w(FUNC(bang_state::vregs_w)).share("vregs");                   /* Video Registers */
...
void gaelco2_state::alighunt(machine_config &config)
{
	...
	gaelco_gae1_device &gaelco(GAELCO_GAE1(config, "gaelco", XTAL(24'000'000) / 24));
	gaelco.set_device_rom_tag("gfx1");
...
void gaelco2_state::touchgo(machine_config &config)
{
	...
	gaelco_gae1_device &gaelco(GAELCO_GAE1(config, "gaelco", XTAL(32'000'000) / 32));
	gaelco.set_device_rom_tag("gfx1");

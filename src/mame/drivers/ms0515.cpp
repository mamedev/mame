...
public:
	...
	DECLARE_WRITE16_MEMBER(ms0515_vram_w);
...
static ADDRESS_MAP_START(ms0515_mem, AS_PROGRAM, 16, ms0515_state)
	...
	AM_RANGE(0160000, 0177377) AM_ROM // AM_WRITE(ms0515_vram_w)
	...
	AM_RANGE(0177500, 0177507) AM_DEVREAD8("pit8253", pit8253_device, read, 0x00ff)
...
WRITE16_MEMBER(ms0515_state::ms0515_bank_w)
{
	...
	DBG_LOG(2,"Bank",("<- %04x & %04x (vblank %d timer %d)\n", machine().describe_context(),
		data, mem_mask, BIT(data, 8), BIT(data, 9)));

	if (BIT(data ^ m_bankreg, 8))
		irq2_w(BIT(data, 8) ? ASSERT_LINE : CLEAR_LINE);
...
WRITE16_MEMBER(ms0515_state::ms0515_vram_w)
{
	m_video_ram[offset*2]   = data;
	m_video_ram[offset*2+1] = data >> 8;
}

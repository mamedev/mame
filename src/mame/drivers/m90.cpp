...
WRITE_LINE_MEMBER(m90_state::fake_nmi)
{
	if (state)
	{
...
WRITE_LINE_MEMBER(m90_state::bomblord_fake_nmi)
{
	if (state)
	{
		address_space &space = m_maincpu->space(AS_PROGRAM);
...
MACHINE_CONFIG_START(m90_state::m90)
	...
	IREM_M72_AUDIO(config, m_audio, 128*60);    /* clocked by V1? (Vigilante) */
...
MACHINE_CONFIG_START(m90_state::bbmanw)
	...
	m_audio->sample_update().set(FUNC(m90_state::fake_nmi));
MACHINE_CONFIG_END
...
MACHINE_CONFIG_START(m90_state::bomblord)
	...
	m_audio->sample_update().set(FUNC(m90_state::bomblord_fake_nmi));
...
GAME( 1992, bbmanw,   0,        bbmanw,   bbmanw,   m90_state, empty_init,    ROT0, "Irem (licensed from Hudson Soft)", "Bomber Man World / New Dyna Blaster - Global Quest", MACHINE_IMPERFECT_GRAPHICS | MACHINE_NO_COCKTAIL | MACHINE_SUPPORTS_SAVE )
GAME( 1992, bbmanwj,  bbmanw,   bbmanw,   bbmanwj,  m90_state, empty_init,    ROT0, "Irem (licensed from Hudson Soft)", "Bomber Man World (Japan)", MACHINE_IMPERFECT_GRAPHICS | MACHINE_NO_COCKTAIL | MACHINE_SUPPORTS_SAVE )
GAME( 1992, bbmanwja, bbmanw,   bbmanwj,  bbmanwj,  m90_state, empty_init,    ROT0, "Irem (licensed from Hudson Soft)", "Bomber Man World (Japan, revised sound hardware)", MACHINE_IMPERFECT_GRAPHICS | MACHINE_NO_COCKTAIL | MACHINE_SUPPORTS_SAVE )
GAME( 1992, newapunk, bbmanw,   bbmanw,   newapunk, m90_state, empty_init,    ROT0, "Irem America (licensed from Hudson Soft)", "New Atomic Punk - Global Quest (US)", MACHINE_IMPERFECT_GRAPHICS | MACHINE_NO_COCKTAIL | MACHINE_SUPPORTS_SAVE )

...
WRITE_LINE_MEMBER(m72_state::fake_nmi)
{
	if (state)
	{
...
MACHINE_CONFIG_START(m72_state::m72_audio_chips)
	...
	IREM_M72_AUDIO(config, m_audio, 128*55);   /* clocked by V1? (Vigilante) */
	m_audio->set_screen_tag(m_screen);
	m_audio->set_dac_tag(m_dac);
	m_audio->set_samples_tag(m_sampleregion);
...
{
	m72_base(config);
	m_audio->sample_update().set(FUNC(m72_state::fake_nmi));
}
...
MACHINE_CONFIG_START(m72_state::m72_dbreed)
	...
	m_audio->sample_update().set_inputline(m_soundcpu, INPUT_LINE_NMI);
MACHINE_CONFIG_END

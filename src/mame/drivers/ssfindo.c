...
{
	UINT16 *FLASH = (UINT16 *)memory_region(machine, RGNCLASS_USER, "user2"); //16 bit - WORD access
...
static ADDRESS_MAP_START( ssfindo_map, ADDRESS_SPACE_PROGRAM, 32 )
	AM_RANGE(0x00000000, 0x000fffff) AM_ROM AM_REGION(RGNCLASS_USER, "user1", 0)
...
ROM_START( ssfindo )
	ROM_REGION(0x100000, RGNCLASS_USER, "user1", 0 ) /* ARM 32 bit code */
	...
	ROM_REGION(0x1000000, RGNCLASS_USER, "user2", 0 ) /* flash roms */
...
ROM_START( ppcar )
	...
	ROM_REGION(0x10000, RGNCLASS_USER, "user4", ROMREGION_ERASE00 ) /* qdsp code */
	/* none */

	ROM_REGION(0x100000, RGNCLASS_USER, "user5", 0 ) /* HWASS 1008S-1  qdsp samples */
	ROM_LOAD( "nasn9289.u9",	0x000000, 0x100000, CRC(9aef9545) SHA1(f23ef72c3e3667923768dfdd0c5b4951b23dcbcf) )

	ROM_REGION(0x100000, RGNCLASS_USER, "user6", ROMREGION_ERASE00 ) /* samples - same internal structure as qdsp samples  */
	/* none */
ROM_END

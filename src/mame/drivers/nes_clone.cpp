...
void nes_clone_state::nes_clone(machine_config &config)
{
	...
	m_screen->set_vblank_time(subseconds::from_usec((113.66/(NTSC_APU_CLOCK.dvalue()/1000000)) * (ppu2c0x_device::VBLANK_LAST_SCANLINE_NTSC-ppu2c0x_device::VBLANK_FIRST_SCANLINE+1+2)));
...
void nes_clone_state::nes_clone_pal(machine_config &config)
{
	...
	m_screen->set_vblank_time(subseconds::from_usec((113.66 / (PALC_APU_CLOCK.dvalue() / 1000000)) * (ppu2c0x_device::VBLANK_LAST_SCANLINE_PAL - ppu2c0x_device::VBLANK_FIRST_SCANLINE_PALC + 1 + 2)));

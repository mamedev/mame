...
static MACHINE_CONFIG_START( spiders, spiders_state )
	...
	MCFG_PIA6821_IRQA_HANDLER(WRITELINE(spiders_state,main_cpu_irq))
	MCFG_PIA6821_IRQB_HANDLER(WRITELINE(spiders_state,main_cpu_irq))

	MCFG_DEVICE_ADD("pia2", PIA6821, 0)
	MCFG_PIA6821_READPA_HANDLER(READ8(spiders_state,gfx_rom_r))
	MCFG_PIA6821_WRITEPB_HANDLER(WRITE8(spiders_state,gfx_rom_intf_w))
	MCFG_PIA6821_CB2_HANDLER(WRITELINE(spiders_state,flipscreen_w))
	MCFG_PIA6821_IRQA_HANDLER(WRITELINE(spiders_state,main_cpu_firq))
	MCFG_PIA6821_IRQB_HANDLER(WRITELINE(spiders_state,main_cpu_irq))

	MCFG_DEVICE_ADD("pia3", PIA6821, 0)
	MCFG_PIA6821_WRITEPA_HANDLER(WRITE8(spiders_state, spiders_audio_ctrl_w))
	MCFG_PIA6821_WRITEPB_HANDLER(WRITE8(spiders_state, spiders_audio_command_w))
	MCFG_PIA6821_IRQA_HANDLER(WRITELINE(spiders_state,main_cpu_irq))
	MCFG_PIA6821_IRQB_HANDLER(WRITELINE(spiders_state,main_cpu_irq))

	MCFG_DEVICE_ADD("pia4", PIA6821, 0)
	MCFG_PIA6821_WRITEPA_HANDLER(WRITE8(spiders_state, spiders_audio_a_w))
	MCFG_PIA6821_WRITEPB_HANDLER(WRITE8(spiders_state, spiders_audio_b_w))
	MCFG_PIA6821_IRQA_HANDLER(WRITELINE(spiders_state, audio_cpu_irq))

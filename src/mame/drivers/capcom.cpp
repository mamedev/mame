...
These could be turned into redemption machines with the flick of a switch in the setup menu.

Main CPU is a MC68306, which has a 68EC000 core, plus MC2681 DUART, 2 parallel ports,
 DMA controller, PIC, JTAG port, 8 chip select lines, and extra instructions to control it all.
 All ports can have their addresses configured by software.

U16: Custom ULA ACTEL 1020. Interfaces the CPU to the RAM and DMD.

Sound hardware: 87C52 CPU. X9241U Quad Electronic Potentiometer.
 74LS165 shift register + TMS320AV120 MPEG audio decoder (1 or 2 sets depending on the game).
...
- Pool Player (by Illinois Pinball, unreleased)
...
{
	// The map is defined by the CPU CS lines, which can be programmed as needed.
	// The programmed settings are:
	// CS1: 0x00000000, 0x0007ffff, RW, RAM
	// CS0: 0x10000000, 0x10ffffff, R, ROM
	// CS3: 0x30000000, 0x3001ffff, RW, NVRAM
	// CS2: 0x40000000, 0x40ffffff, RW, I/O
	//  CS2 is further split by A22,A23 to 4 ranges
	//      0x40000000, 0x403fffff, AUX I/O
...
/-------------------------------------------------------------------*/
ROM_START(bsv103)
	ROM_REGION16_BE(0x00100000, "maincpu", ROMREGION_ERASEFF)
...
/-------------------------------------------------------------------*/
ROM_START(ffv104)
	ROM_REGION16_BE(0x00d00000, "maincpu", ROMREGION_ERASEFF)
...
/*-------------------------------------------------------------------
/ Goofy Hoops (Romstar) (04/94)
  Upright, no flippers, guide the ball to the basketball hoops.
/-------------------------------------------------------------------*/
ROM_START(ghv101)
	ROM_REGION16_BE(0x00500000, "maincpu", ROMREGION_ERASEFF)
...
GAME(1994,  ghv101,  0,      capcom, capcom, capcom_state, empty_init, ROT0, "Romstar",          "Goofy Hoops",                MACHINE_IS_SKELETON_MECHANICAL)

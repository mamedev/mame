...
static DRIVER_INIT( kf2k3pcb )
{
	...
	state->fixed_layer_bank_type = 2;
	install_pvc_protection(machine);
	memory_install_read_bank(machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0xc00000, 0xc7ffff, 0, 0, "bios" );  // 512k bios
}
...
static DRIVER_INIT( jockeygp )
{
	...
	memory_install_ram(machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x200000, 0x201fff, 0, 0, NULL);

//  memory_install_read_port(machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x280000, 0x280001, 0, 0, "IN5");
//  memory_install_read_port(machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x2c0000, 0x2c0001, 0, 0, "IN6");
}
...
static DRIVER_INIT( kog )
{
	...
	memory_install_read_port(machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x0ffffe, 0x0fffff, 0, 0, "JUMPER");

...
static NVRAM_HANDLER( konamigv_93C46 )
{
	...
	else
	{
		...
		{
			eeprom_set_data( memory_region( machine, RGNCLASS_USER, "user2" ), memory_region_length( machine, RGNCLASS_USER, "user2" ) );
		}
	}
}
...
ROM_START( konamigv )
	...
	ROM_REGION( 0x0000080, RGNCLASS_USER, "user2", ROMREGION_ERASE00 ) /* default eeprom */
ROM_END

ROM_START( susume )
	GV_BIOS

	ROM_REGION( 0x0000080, RGNCLASS_USER, "user2", 0 ) /* default eeprom */

...
class nes_vt09new_base_state : public driver_device
{
public:
	nes_vt09new_base_state(const machine_config& mconfig, device_type type, const char* tag) :
...
protected:
	...
	void nes_vt09new_map(address_map& map);
...
class nes_vt09new_state : public nes_vt09new_base_state
{
public:
	nes_vt09new_state(const machine_config& mconfig, device_type type, const char* tag) :
		nes_vt09new_base_state(mconfig, type, tag),
		m_soc(*this, "soc")
	{ }
...
void nes_vt09new_base_state::vtspace_w(offs_t offset, uint8_t data)
{
...
void nes_vt09new_state::vt_external_space_map_32mbyte(address_map &map)
{
	map(0x0000000, 0x1ffffff).r(FUNC(nes_vt09new_state::vt_rom_r));
}

void nes_vt09new_state::vt_external_space_map_16mbyte(address_map &map)
{
	map(0x0000000, 0x0ffffff).mirror(0x1000000).r(FUNC(nes_vt09new_state::vt_rom_r));
}

void nes_vt09new_state::vt_external_space_map_8mbyte(address_map &map)
{
	map(0x0000000, 0x07fffff).mirror(0x1800000).r(FUNC(nes_vt09new_state::vt_rom_r));
}

void nes_vt09new_state::vt_external_space_map_4mbyte(address_map &map)
{
	map(0x0000000, 0x03fffff).mirror(0x1c00000).r(FUNC(nes_vt09new_state::vt_rom_r));
}

void nes_vt09new_state::vt_external_space_map_2mbyte(address_map &map)
{
	map(0x0000000, 0x01fffff).mirror(0x1e00000).r(FUNC(nes_vt09new_state::vt_rom_r));
}

void nes_vt09new_state::vt_external_space_map_1mbyte(address_map &map)
{
...
uint8_t nes_vt09new_base_state::extrain_1_r()
{
	if (m_exin1)
		return m_exin1->read();
	else
...
uint8_t nes_vt09new_base_state::extrain_2_r()
{
	if (m_exin2)
		return m_exin2->read();
	else
...
uint8_t nes_vt09new_base_state::extrain_3_r()
{
	if (m_exin3)
		return m_exin3->read();
	else
...
uint8_t nes_vt09new_base_state::in0_r()
{
	//logerror("%s: in0_r\n", machine().describe_context());
	uint8_t ret = 0x40;
	ret |= m_latch0 & 1;
...
uint8_t nes_vt09new_base_state::in1_r()
{
	//logerror("%s: in1_r\n", machine().describe_context());
	uint8_t ret = 0x40;
	ret |= m_latch1 & 1;
...
void nes_vt09new_base_state::configure_soc(nes_vt_soc_device* soc)
{
	...
	soc->read_1_callback().set(FUNC(nes_vt09new_base_state::in1_r));
	soc->write_0_callback().set(FUNC(nes_vt09new_base_state::in0_w));

	soc->extra_read_0_callback().set(FUNC(nes_vt09new_base_state::extrain_0_r));
	soc->extra_read_1_callback().set(FUNC(nes_vt09new_base_state::extrain_1_r));
...
uint8_t nes_vt09new_base_state::upper_412d_r()
{
	logerror("%s: upper_412d_r\n", machine().describe_context());
	return 0x00;
}

void nes_vt09new_base_state::upper_412c_w(uint8_t data)
{
...
static INPUT_PORTS_START( nes_vt09new_msi )
	PORT_START("IO0")
	PORT_BIT( 0x01, IP_ACTIVE_HIGH, IPT_BUTTON2 ) PORT_PLAYER(1) PORT_NAME("A")
	PORT_BIT( 0x02, IP_ACTIVE_HIGH, IPT_BUTTON1 ) PORT_PLAYER(1) PORT_NAME("B")
	PORT_BIT( 0x04, IP_ACTIVE_HIGH, IPT_SELECT ) PORT_PLAYER(1)
...
static INPUT_PORTS_START( nes_vt09new_msi_mm2 )
	PORT_START("IO0")
	PORT_BIT( 0x01, IP_ACTIVE_HIGH, IPT_BUTTON2 ) PORT_PLAYER(1) PORT_NAME("A")
	PORT_BIT( 0x02, IP_ACTIVE_HIGH, IPT_BUTTON1 ) PORT_PLAYER(1) PORT_NAME("B")
	PORT_BIT( 0x04, IP_ACTIVE_HIGH, IPT_SELECT ) PORT_PLAYER(1)
...
static INPUT_PORTS_START( nes_vt09new_fa )
	PORT_INCLUDE(nes_vt09new)
...
CONS( 2017, bittboy,    0,        0,  nes_vt09new_bt_2x16mb, nes_vt09new, nes_vt09new_cy_state, empty_init, "BittBoy",   "BittBoy Mini FC 300 in 1", MACHINE_IMPERFECT_GRAPHICS ) // has external banking (2x 16mbyte banks)
// Broken GFX, investigate, is this really a system? research indicates it's a multicart for a regular NES?
CONS( 201?, mc_pg150,   0,        0,  nes_vt09new_bt_2x16mb, nes_vt09new, nes_vt09new_cy_state, empty_init, "<unknown>", "Pocket Games 150 in 1", MACHINE_NOT_WORKING ) // has external banking
...
CONS( 201?, mc_hh210,   0,        0,  nes_vt09new_4k_ram_16mb, nes_vt09new, nes_vt09new_state, empty_init, "<unknown>", "Handheld 210 in 1", MACHINE_NOT_WORKING )
// First half of games don't work, probably bad dump
CONS( 201?, dvnimbus,   0,        0,  nes_vt09new09_16mb, nes_vt09new, nes_vt09new09_state, empty_init, "<unknown>", "DVTech Nimbus 176 in 1", MACHINE_NOT_WORKING )
 // probably another Thumbs Up product? cursor doesn't work unless nes_vt09new_hh machine is used? possibly newer than VT02 as it runs from an SPI ROM, might just not use enhanced features.  Some minor game name changes to above (eg Smackdown just becomes Wrestling)
CONS( 201?, unkra200,   mc_tv200, 0,  nes_vt09new_hh_8mb, nes_vt09new, nes_vt09new09_state, empty_init, "<unknown>", "200 in 1 Retro Arcade", MACHINE_IMPERFECT_GRAPHICS )
...
CONS( 2017, fapocket,   0,        0,  nes_vt09new_fa_4x16mb, nes_vt09new_fa, nes_vt09new_dg_fapocket_state, empty_init, "<unknown>",   "Family Pocket 638 in 1", MACHINE_IMPERFECT_GRAPHICS ) // has external banking (4x 16mbyte banks)


// Runs well, minor GFX issues in intro
CONS( 2017, sy889,      0,        0,  nes_vt09new_hh_8mb, nes_vt09new, nes_vt09new09_state, empty_init, "SY Corp",   "SY-889 300 in 1 Handheld", MACHINE_IMPERFECT_GRAPHICS )
CONS( 2016, sy888b,     0,        0,  nes_vt09new_hh_4mb, nes_vt09new, nes_vt09new09_state, empty_init, "SY Corp",   "SY-888B 288 in 1 Handheld", MACHINE_IMPERFECT_GRAPHICS )

// Same hardware as SY-889
CONS( 201?, mc_cb280,   0,        0,  nes_vt09new_hh_4mb, nes_vt09new, nes_vt09new09_state, empty_init, "CoolBoy",   "Coolboy RS-18 (280 in 1)", MACHINE_IMPERFECT_GRAPHICS )

// Plays intro music but then crashes. same hardware as SY-88x but uses more features
CONS( 2016, mog_m320,   0,        0,  nes_vt09new_hh_8mb, nes_vt09new, nes_vt09new09_state, empty_init, "MOGIS",    "MOGIS M320 246 in 1 Handheld", MACHINE_NOT_WORKING )

// similar menus to above, but with opcode scrambling
CONS( 2015, dgun2573,  0,         0,  nes_vt09new_fp_32mb,     nes_vt09new, nes_vt09new09_state, empty_init, "dreamGEAR", "My Arcade Gamer V Portable Gaming System (DGUN-2573) (set 1, newer)",  MACHINE_WRONG_COLORS | MACHINE_IMPERFECT_GRAPHICS | MACHINE_IMPERFECT_SOUND )
CONS( 2015, dgun2573a, dgun2573,  0,  nes_vt09new_fp_32mb,     nes_vt09new, nes_vt09new09_state, empty_init, "dreamGEAR", "My Arcade Gamer V Portable Gaming System (DGUN-2573) (set 2, older)",  MACHINE_WRONG_COLORS | MACHINE_IMPERFECT_GRAPHICS | MACHINE_IMPERFECT_SOUND ) // some menu graphics haven't been updated to reflect 'Panda' theme to the sports games

CONS( 2015, rminitv,   0,  0,  nes_vt09new_fp_pal_32mb, nes_vt09new, nes_vt09new09_state, empty_init, "Orb Gaming", "Retro 'Mini TV' Console 300-in-1", MACHINE_IMPERFECT_GRAPHICS ) // single 32Mbyte bank!
...
CONS( 2016, fcpocket,  0,  0,  nes_vt09new_fp_4x16mb,   nes_vt09new_fp, nes_vt09new09_state, empty_init, "<unknown>",   "FC Pocket 600 in 1", MACHINE_IMPERFECT_GRAPHICS | MACHINE_IMPERFECT_SOUND )  // has external banking (2x 32mbyte banks)
...
CONS( 2012, dgun2561,  0,  0,  nes_vt09new_cy_bigger, nes_vt09new, nes_vt09new_cy_state, empty_init, "dreamGEAR", "My Arcade Portable Gaming System with 140 Games (DGUN-2561)", MACHINE_NOT_WORKING ) // 64Mbyte ROM, must be externally banked, or different addressing scheme
CONS( 2016, dgun2593,  0,  0,  nes_vt09new_cy_bigger, nes_vt09new, nes_vt09new_cy_state, empty_init, "dreamGEAR", "My Arcade Retro Arcade Machine - 300 Handheld Video Games (DGUN-2593)", MACHINE_NOT_WORKING ) // 128Mbyte ROM, must be externally banked or different addressing scheme

CONS( 200?, lxcmcy,    0,  0,  nes_vt09new_cy_bigger, nes_vt09new, nes_vt09new_cy_state, empty_init,    "Lexibook", "Lexibook Compact Cyber Arcade", MACHINE_NOT_WORKING ) // 64Mbyte ROM, must be externally banked, or different addressing scheme
CONS( 200?, lxcmc250,  0,  0,  nes_vt09new_cy_bigger, nes_vt09new, nes_vt09new_cy_state, empty_init,    "Lexibook", "Lexibook Compact Cyber Arcade - 250-in-1 (JL2375)", MACHINE_NOT_WORKING ) // 64Mbyte ROM, must be externally banked, or different addressing scheme
CONS( 200?, lxcmcysw,  0,  0,  nes_vt09new_cy_bigger, nes_vt09new, nes_vt09new_cy_state, empty_init,    "Lexibook", "Lexibook Compact Cyber Arcade - Star Wars Rebels", MACHINE_NOT_WORKING ) // 64Mbyte ROM, must be externally banked, or different addressing scheme
CONS( 200?, lxcmcyfz,  0,  0,  nes_vt09new_cy_bigger, nes_vt09new, nes_vt09new_cy_state, empty_init,    "Lexibook", "Lexibook Compact Cyber Arcade - Frozen", MACHINE_NOT_WORKING ) // 64Mbyte ROM, must be externally banked, or different addressing scheme
CONS( 200?, lxcmcydp,  0,  0,  nes_vt09new_cy_bigger, nes_vt09new, nes_vt09new_cy_state, empty_init,    "Lexibook", "Lexibook Compact Cyber Arcade - Disney Princess", MACHINE_NOT_WORKING ) // 64Mbyte ROM, must be externally banked, or different addressing scheme
CONS( 200?, lxcmcysp,  0,  0,  nes_vt09new_cy_bigger, nes_vt09new, nes_vt09new_cy_state, empty_init,    "Lexibook", "Lexibook Compact Cyber Arcade - Marvel Ultimate Spider-Man", MACHINE_NOT_WORKING ) // 64Mbyte ROM, must be externally banked, or different addressing scheme
CONS( 200?, lxcmcycr,  0,  0,  nes_vt09new_cy_bigger, nes_vt09new, nes_vt09new_cy_state, empty_init,    "Lexibook", "Lexibook Compact Cyber Arcade - Cars", MACHINE_NOT_WORKING ) // 64Mbyte ROM, must be externally banked, or different addressing scheme
// the data order is swapped for this one, maybe other internal differences?
CONS( 200?, lxcmcypp,  0,  0,  nes_vt09new_cy_bigger, nes_vt09new, nes_vt09new_cy_state, init_lxcmcypp, "Lexibook", "Lexibook Compact Cyber Arcade - Paw Patrol", MACHINE_NOT_WORKING ) // 64Mbyte ROM, must be externally banked, or different addressing scheme


CONS( 200?, lxccminn,  0,  0,  nes_vt09new_cy_bigger, nes_vt09new, nes_vt09new_cy_state, empty_init,    "Lexibook", "Lexibook Console Colour - Minnie Mouse", MACHINE_NOT_WORKING ) // 64Mbyte (used) ROM, must be externally banked, or different addressing scheme
CONS( 200?, lxccplan,  0,  0,  nes_vt09new_cy_bigger, nes_vt09new, nes_vt09new_cy_state, empty_init,    "Lexibook", "Lexibook Console Colour - Disney's Planes", MACHINE_NOT_WORKING ) // 64Mbyte (used) ROM, must be externally banked, or different addressing scheme


// GB-NO13-Main-VT389-2 on PCBs
CONS( 2016, rtvgc300,  0,  0,  nes_vt09new_cy_bigger, nes_vt09new, nes_vt09new_cy_state, empty_init,    "Lexibook", "Lexibook Retro TV Game Console - 300 Games", MACHINE_NOT_WORKING )  // 64Mbyte ROM, must be externally banked, or different addressing scheme
CONS( 2017, rtvgc300fz,0,  0,  nes_vt09new_cy_bigger, nes_vt09new, nes_vt09new_cy_state, empty_init,    "Lexibook", "Lexibook Retro TV Game Console - Frozen - 300 Games", MACHINE_NOT_WORKING )  // 64Mbyte ROM, must be externally banked, or different addressing scheme
...
CONS( 201?, red5mam,  0,  0,  nes_vt09new_cy_bigger, nes_vt09new, nes_vt09new_cy_state, empty_init, "Red5", "Mini Arcade Machine (Red5)", MACHINE_NOT_WORKING ) // 128Mbyte ROM, must be externally banked or different addressing scheme

CONS( 201?, denv150,   0,  0,  nes_vt09new_cy_bigger, nes_vt09new, nes_vt09new_cy_state, empty_init, "Denver", "Denver Game Console GMP-240C 150-in-1", MACHINE_NOT_WORKING | MACHINE_IMPERFECT_GRAPHICS )

...
void ajax_state::sound_bank_w(address_space &space, offs_t offset, uint8_t data, uint8_t mem_mask)
{
	int bank_A, bank_B;
...
void ajax_state::volume_callback0(address_space &space, offs_t offset, uint8_t data, uint8_t mem_mask)
{
	m_k007232_1->set_volume(0, (data >> 4) * 0x11, 0);
	m_k007232_1->set_volume(1, 0, (data & 0x0f) * 0x11);
}

void ajax_state::k007232_extvol_w(address_space &space, offs_t offset, uint8_t data, uint8_t mem_mask)
{
	/* channel A volume (mono) */
	m_k007232_2->set_volume(0, (data & 0x0f) * 0x11/2, (data & 0x0f) * 0x11/2);
}

void ajax_state::volume_callback1(address_space &space, offs_t offset, uint8_t data, uint8_t mem_mask)
{

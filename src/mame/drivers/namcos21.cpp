...
void namcos21_state::master_map(address_map &map)
{
	...
	map(0x200000, 0x20ffff).rw(m_namcos21_dsp_c67, FUNC(namcos21_dsp_c67_device::dspram16_r), FUNC(namcos21_dsp_c67_device::dspram16_hack_w)).share("namcos21dsp_c67:dspram16");
}
...
void namcos21_state::slave_map(address_map &map)
{
	...
	map(0x200000, 0x20ffff).rw(m_namcos21_dsp_c67, FUNC(namcos21_dsp_c67_device::dspram16_r), FUNC(namcos21_dsp_c67_device::dspram16_w)).share("namcos21dsp_c67:dspram16");
}
...
void namcos21_state::configure_c68_namcos21(machine_config &config)
{
	...
	m_c68->dp_in_callback().set(FUNC(namcos21_state::namcos2_dualportram_byte_r));
	m_c68->dp_out_callback().set(FUNC(namcos21_state::namcos2_dualportram_byte_w));
}
...
static INPUT_PORTS_START( driveyes )
	...
	PORT_MODIFY("DSW")
	PORT_DIPNAME( 0x0c, 0x0c, "Screen")
	PORT_DIPSETTING(    0x0c, "Center" )
	// these will show 'receive error' because they want comms from the main screen
	PORT_DIPSETTING(    0x08, "Left" )
	PORT_DIPSETTING(    0x04, "Right" )
	PORT_DIPSETTING(    0x00, "Right (duplicate)" )
INPUT_PORTS_END
...
GAME( 1992, driveyes,  0,        driveyes, driveyes,   namcos21_state, empty_init,    ROT0,    "Namco", "Driver's Eyes (Japan) (1992/01/10, Main Ver 2.1, Sub Ver 1.1)",                 MACHINE_NOT_WORKING | MACHINE_IMPERFECT_GRAPHICS | MACHINE_NODEVICE_LAN)

// uses 5x TMS320C25 (C67, has internal ROM - dumped) 
GAME( 1991, starblad,  0,        starblad, starblad,   namcos21_state, empty_init,    ROT0,    "Namco", "Starblade (World)",                     MACHINE_IMPERFECT_GRAPHICS )
GAME( 1991, starbladj, starblad, starblad, starblad,   namcos21_state, empty_init,    ROT0,    "Namco", "Starblade (Japan)",                     MACHINE_IMPERFECT_GRAPHICS )
GAME( 1991, solvalou,  0,        solvalou, s21default, namcos21_state, init_solvalou, ROT0,    "Namco", "Solvalou (Japan)",                      MACHINE_IMPERFECT_GRAPHICS | MACHINE_NOT_WORKING )
GAME( 1992, aircomb,   0,        aircomb,  aircomb,    namcos21_state, empty_init,    ROT0,    "Namco", "Air Combat (US)",                       MACHINE_NOT_WORKING | MACHINE_IMPERFECT_GRAPHICS ) // There's code for a SCI, is it even possible to play multiplayer?
GAME( 1992, aircombj,  aircomb,  aircomb,  aircomb,    namcos21_state, empty_init,    ROT0,    "Namco", "Air Combat (Japan)",                    MACHINE_NOT_WORKING | MACHINE_IMPERFECT_GRAPHICS )
GAME( 1993, cybsled,   0,        cybsled,  cybsled,    namcos21_state, empty_init,    ROT0,    "Namco", "Cyber Sled (World)",                    MACHINE_IMPERFECT_GRAPHICS | MACHINE_NODEVICE_LAN | MACHINE_NOT_WORKING )
GAME( 1993, cybsledj,  cybsled,  cybsled,  cybsled,    namcos21_state, empty_init,    ROT0,    "Namco", "Cyber Sled (Japan)",                    MACHINE_IMPERFECT_GRAPHICS | MACHINE_NODEVICE_LAN | MACHINE_NOT_WORKING )

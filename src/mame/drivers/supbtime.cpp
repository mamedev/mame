...
static ADDRESS_MAP_START( supbtime_map, AS_PROGRAM, 16, supbtime_state )
	...
	AM_RANGE(0x1a0000, 0x1a0001) AM_DEVWRITE8(DECOSND_TAG, deco_6280_base_device, soundlatch_w, 0x00ff)
...
static ADDRESS_MAP_START( chinatwn_map, AS_PROGRAM, 16, supbtime_state )
	AM_RANGE(0x000000, 0x03ffff) AM_ROM
	AM_RANGE(0x100000, 0x100001) AM_DEVWRITE8(DECOSND_TAG, deco_6280_base_device, soundlatch_w, 0x00ff)
	AM_RANGE(0x120000, 0x1207ff) AM_RAM AM_SHARE("spriteram")
	AM_RANGE(0x140000, 0x1407ff) AM_RAM_DEVWRITE("palette", palette_device, write16) AM_SHARE("palette")
	AM_RANGE(0x180000, 0x180001) AM_READ_PORT("INPUTS")
	AM_RANGE(0x180002, 0x180003) AM_READ_PORT("DSW")
...
MACHINE_CONFIG_START(supbtime_state::supbtime)
	...
	MCFG_DECO6280_ADD(DECOSND_TAG, XTAL(32'220'000) / 8)
	...
	MCFG_DEVICE_CLOCK(XTAL(21'477'272) / 20) // clock frequency & pin 7 not verified
MACHINE_CONFIG_END
...
ROM_START( supbtime )
	...
	ROM_REGION( 0x20000, DECOSND_OKI1_TAG, 0 )

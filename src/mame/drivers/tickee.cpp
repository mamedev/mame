...
void tickee_state::device_timer(timer_instance const &timer)
{
	int param = timer.param();
	switch (timer.id())
	{
	case TIMER_TRIGGER_GUN_INTERRUPT:
		trigger_gun_interrupt(timer.ptr(), param);
		break;
	case TIMER_CLEAR_GUN_INTERRUPT:
		clear_gun_interrupt(timer.ptr(), param);
		break;
	case TIMER_SETUP_GUN_INTERRUPTS:
		setup_gun_interrupts(timer.ptr(), param);
		break;
	default:
...
static INPUT_PORTS_START( tickee )
	...
	PORT_BIT( 0x02, IP_ACTIVE_HIGH, IPT_CUSTOM ) PORT_READ_LINE_DEVICE_MEMBER("ticket2", ticket_dispenser_device, line_r) /* right ticket status */

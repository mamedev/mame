...
static void crimfght_banking( int lines )
{
	...
	if (lines & 0x20)
	{
		memory_install_readwrite_handler(Machine, 0, ADDRESS_SPACE_PROGRAM, 0x0000, 0x03ff, 0, 0, SMH_BANK3, paletteram_xBBBBBGGGGGRRRRR_be_w);
		memory_set_bankptr(3, paletteram);
	}
	else
		memory_install_readwrite_handler(Machine, 0, ADDRESS_SPACE_PROGRAM, 0x0000, 0x03ff, 0, 0, SMH_BANK1, SMH_BANK1);								/* RAM */

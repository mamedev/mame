...
static WRITE16_HANDLER( bankselect_w )
{
	...
	base = &memory_region(machine, RGNCLASS_CPU, "main")[bankoffset[(newword >> 10) & 0x3f]];
...
ROM_START( paperboy )
	ROM_REGION( 0x90000, RGNCLASS_CPU, "main", 0 )	/* 9*64k for T11 code */
	...
	ROM_REGION( 0x10000, RGNCLASS_CPU, "sound", 0 )	/* 64k for 6502 code */
...
ROM_START( 720 )
	ROM_REGION( 0x90000, RGNCLASS_CPU, "main", 0 )     /* 9 * 64k T11 code */
...
static DRIVER_INIT( paperboy )
{
	...
	int i;
	UINT8 *cpu1 = memory_region(machine, RGNCLASS_CPU, "main");

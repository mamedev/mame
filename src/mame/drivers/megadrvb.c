...
static DRIVER_INIT( aladmdb )
{
	...
	memory_install_write16_handler(machine->device("maincpu")->memory().space(AS_PROGRAM), 0x220000, 0x220001, 0, 0, aladmdb_w);
	memory_install_read16_handler(machine->device("maincpu")->memory().space(AS_PROGRAM), 0x330000, 0x330001, 0, 0, aladmdb_r);
...
static DRIVER_INIT( mk3mdb )
{
	...
	memory_install_read16_handler(machine->device("maincpu")->memory().space(AS_PROGRAM), 0x770070, 0x770075, 0, 0, mk3mdb_dsw_r );
...
{
	memory_nop_write(machine->device("maincpu")->memory().space(AS_PROGRAM), 0xA130F0, 0xA130FF, 0, 0); // custom banking is disabled (!)
	memory_install_read_bank(machine->device("maincpu")->memory().space(AS_PROGRAM), 0x400000, 0x5fffff, 0, 0, "bank5");
	memory_unmap_write(machine->device("maincpu")->memory().space(AS_PROGRAM), 0x400000, 0x5fffff, 0, 0);

	memory_set_bankptr(machine,  "bank5", machine->region( "maincpu" )->base() + 0x400000 );

	memory_install_read16_handler(machine->device("maincpu")->memory().space(AS_PROGRAM), 0x770070, 0x770075, 0, 0, ssf2mdb_dsw_r );
...
static DRIVER_INIT( srmdb )
{
	...
	memory_install_read16_handler(machine->device("maincpu")->memory().space(AS_PROGRAM), 0x770070, 0x770075, 0, 0, srmdb_dsw_r );
...
{
	memory_install_read16_handler(machine->device("maincpu")->memory().space(AS_PROGRAM), 0x200050, 0x200051, 0, 0, topshoot_200051_r );
	memory_install_read_port(machine->device("maincpu")->memory().space(AS_PROGRAM), 0x200042, 0x200043, 0, 0, "IN0");
	memory_install_read_port(machine->device("maincpu")->memory().space(AS_PROGRAM), 0x200044, 0x200045, 0, 0, "IN1");
	memory_install_read_port(machine->device("maincpu")->memory().space(AS_PROGRAM), 0x200046, 0x200047, 0, 0, "IN2");
	memory_install_read_port(machine->device("maincpu")->memory().space(AS_PROGRAM), 0x200048, 0x200049, 0, 0, "IN3");

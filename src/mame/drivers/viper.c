...
static ADDRESS_MAP_START(viper_map, ADDRESS_SPACE_PROGRAM, 64)
	...
	AM_RANGE(0xfff00000, 0xfff3ffff) AM_ROM AM_REGION(RGNCLASS_USER, "user1", 0)		// Boot ROM
ADDRESS_MAP_END
...
static DRIVER_INIT(viper)
{
	...
	nvram = memory_region(machine, RGNCLASS_USER, "user2");
...
#define VIPER_BIOS \
	ROM_REGION64_BE(0x40000, RGNCLASS_USER, "user1", 0)	/* Boot ROM */ \
...
ROM_START(kviper)
	...
	ROM_REGION(0x2000, RGNCLASS_USER, "user2", ROMREGION_ERASE00)		/* M48T58 Timekeeper NVRAM */
ROM_END

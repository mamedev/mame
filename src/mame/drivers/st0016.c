...
{
	memory_set_bankptr( 1, memory_region(machine, RGNCLASS_CPU, "main") + (data* 0x4000) + 0x10000 );
...
ROM_START( renju )
	...
	ROM_COPY( RGNCLASS_CPU, "main",   0x210000, 0x000000, 0x08000 )
ROM_END
...
ROM_START(mayjinsn )
	...
	ROM_COPY( RGNCLASS_CPU, "main",   0x010000, 0x00000, 0x08000 )

	ROM_REGION32_LE( 0x20000*4, RGNCLASS_USER, "user1", 0 ) /* V810 code */
...
ROM_START(mayjisn2 )
	ROM_REGION( 0x110000, RGNCLASS_CPU, "main", 0 )
...
{
	st0016_game=4|0x80;
	memory_set_bankptr(2, memory_region(machine, RGNCLASS_USER, "user1"));
}

...
{
	memory_set_bankptr(4, memory_region(machine, RGNCLASS_SOUND, "adpcm") + ((data & 0xf) * 0x8000));
}
...
{
	memory_set_bankptr(4, memory_region(machine, RGNCLASS_SOUND, "adpcm"));
}
...
ROM_START( scessjoe )
	...
	ROM_REGION( 0x10000, RGNCLASS_CPU, "audio", 0 )     /* 32k for Z80 code */
	...
	ROM_REGION( 0x300000, RGNCLASS_GFX, "gfx5", ROMREGION_DISPOSE )
	...
	ROM_REGION( 0x80000, RGNCLASS_SOUND, "adpcm", 0 )   /* samples? */

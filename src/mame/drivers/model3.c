...
{
	model3_step = step;
	memory_set_bankptr( 1, memory_region( machine, RGNCLASS_USER, "user1" ) + 0x800000 ); /* banked CROM */

	memory_set_bankptr(4, memory_region(machine, RGNCLASS_SOUND, "samples") + 0x200000);
	memory_set_bankptr(5, memory_region(machine, RGNCLASS_SOUND, "samples") + 0x600000);

	// copy the 68k vector table into RAM
	memcpy(model3_soundram, memory_region(machine, RGNCLASS_CPU, "audio")+0x80000, 16);
	...
	switch (offset)
	{
		...
		case 0x08/8:
			if (ACCESSING_BITS_56_63)
			{
				...
				data >>= 56;
				data = (~data) & 0xf;
				memory_set_bankptr( 1, memory_region( machine, RGNCLASS_USER, "user1" ) + 0x800000 + (data * 0x800000)); /* banked CROM */
			}
			if (ACCESSING_BITS_24_31)
			{
				data >>= 24;
...
ROM_START( lemans24 )	/* step 1.5 */
	ROM_REGION64_BE( 0x4800000, RGNCLASS_USER, "user1", 0 ) /* program + data ROMs */
	// CROM
	...
	ROM_COPY(RGNCLASS_USER, "user1", 0x800000, 0x000000, 0x600000)
	...
	ROM_REGION( 0x1000000, RGNCLASS_USER, "user3", ROMREGION_DISPOSE )  /* Video ROMs Part 1 */
	...
	ROM_REGION( 0x1000000, RGNCLASS_USER, "user4", ROMREGION_DISPOSE )  /* Video ROMs Part 2 */
	...
	// Samples
	ROM_REGION( 0x800000, RGNCLASS_SOUND, "samples", 0 )	/* SCSP samples */
	...
	ROM_REGION( 0x20000, RGNCLASS_CPU, "cpu2", 0 )	/* Z80 code */
		ROM_FILL( 0x000000, 0x20000, 0 )

	ROM_REGION( 0x800000, RGNCLASS_SOUND, "dsb", 0 )	/* DSB samples */
		ROM_FILL( 0x000000, 0x800000, 0 )

	ROM_REGION( 0x80000, RGNCLASS_SOUND, "scsp1", 0 )		/* first SCSP's RAM */
		ROM_FILL( 0x000000, 0x80000, 0 )

	ROM_REGION( 0x80000, RGNCLASS_SOUND, "scsp2", 0 )		/* second SCSP's RAM */
		ROM_FILL( 0x000000, 0x80000, 0 )
ROM_END
...
ROM_START( scud )	/* step 1.5 */
	...
	ROM_REGION( 0x100000, RGNCLASS_CPU, "audio", 0 )	/* 68000 code */
...
ROM_START( scudp )	/* step 1.5 */
	...
	// Samples
	ROM_REGION( 0xc00000, RGNCLASS_SOUND, "samples", 0 )	/* SCSP samples */
	ROM_LOAD32_WORD( "mpr-19670.22", 0x000000, 0x400000, CRC(bd31cc06) SHA1(d1c85d0cf79b92de5bcbe20dfb8b626ad72de019) )
	ROM_LOAD32_WORD( "mpr-19671.24", 0x000002, 0x400000, CRC(8e8526ab) SHA1(3d2cbb09bd185660feea4dd80bee5af2e2a19aa6) )
...
ROM_START( vs2 )	/* Step 2.0 */
	...
	// mirror CROM0 to CROM
	ROM_COPY(RGNCLASS_USER, "user1", 0x800000, 0x000000, 0x400000)
...
ROM_START( von2 )	/* Step 2.0 */
	...
	ROM_REGION( 0x2000000, RGNCLASS_USER, "user3", ROMREGION_DISPOSE )  /* Video ROMs Part 1 */
	...
	ROM_REGION( 0x2000000, RGNCLASS_USER, "user4", ROMREGION_DISPOSE )  /* Video ROMs Part 2 */
	...
	// Samples
	ROM_REGION( 0x1000000, RGNCLASS_SOUND, "samples", 0 )	/* SCSP samples */
...
ROM_START( swtrilgy )	/* Step 2.1 */
	...
	ROM_REGION( 0x1000000, RGNCLASS_SOUND, "dsb", 0 )	/* DSB samples */
...
ROM_START( daytona2 )	/* Step 2.1 */
	ROM_REGION64_BE( 0x8800000, RGNCLASS_USER, "user1", 0 ) /* program + data ROMs */
	// CROM
...
static WRITE16_HANDLER( model3snd_ctrl )
{
	...
	if (memory_region_length(machine, RGNCLASS_SOUND, "scsp2") > 0x800000)
	{
		UINT8 *snd = memory_region(machine, RGNCLASS_SOUND, "scsp2");
		if (data & 0x20)
		{
...
static ADDRESS_MAP_START( model3_snd, ADDRESS_SPACE_PROGRAM, 16 )
	AM_RANGE(0x000000, 0x07ffff) AM_RAM AM_REGION(RGNCLASS_SOUND, "scsp1", 0) AM_BASE(&model3_soundram)
	AM_RANGE(0x100000, 0x100fff) AM_READWRITE(SCSP_0_r, SCSP_0_w)
	AM_RANGE(0x200000, 0x27ffff) AM_RAM AM_REGION(RGNCLASS_SOUND, "scsp2", 0)
	...
	AM_RANGE(0x800000, 0x9fffff) AM_ROM AM_REGION(RGNCLASS_SOUND, "samples", 0)
...
static void interleave_vroms(running_machine *machine)
{
	int start;
	int i,j,x;
	UINT16 *vrom1 = (UINT16*)memory_region(machine, RGNCLASS_USER, "user3");
	UINT16 *vrom2 = (UINT16*)memory_region(machine, RGNCLASS_USER, "user4");
	int vrom_length = memory_region_length(machine, RGNCLASS_USER, "user3");
	UINT16 *vrom;

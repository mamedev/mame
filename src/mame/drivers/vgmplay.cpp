...
private:
	...
	uint32_t m_segapcm_bank_shift;
	uint32_t m_segapcm_bank_mask;
...
{
	if (m_segapcm_bank_shift)
		offset = (((offset >> 13) & m_segapcm_bank_mask) << m_segapcm_bank_shift) + (offset & 0xffff);
...
{
	if (m_multipcma_banked > 0)
		offset &= 0x1fffff;

	if (offset & 0x100000) && (m_multipcma_banked > 0)
	{
		if (m_multipcma_bank_l == m_multipcma_bank_r)
		{
			offset = ((m_multipcma_bank_r << 16) & ~0xfffff) | (offset & 0xfffff);
		}
		else
		{
			if (offset & 0x80000)
			{
				offset = ((m_multipcma_bank_l << 16) & ~0x7ffff) | (offset & 0x7ffff);
			}
			else
			{
				offset = ((m_multipcma_bank_r << 16) & ~0x7ffff) | (offset & 0x7ffff);
			}
		}
	}
...
{
	if (m_multipcmb_banked > 0)
		offset &= 0x1fffff;

	if (offset & 0x100000) && (m_multipcmb_banked > 0)
	{
		if (m_multipcmb_bank_l == m_multipcmb_bank_r)
		{
			offset = ((m_multipcmb_bank_r << 16) & ~0xfffff) | (offset & 0xfffff);
		}
		else
		{
			if (offset & 0x80000)
			{
				offset = ((m_multipcmb_bank_l << 16) & ~0x7ffff) | (offset & 0x7ffff);
			}
			else
			{
				offset = ((m_multipcmb_bank_r << 16) & ~0x7ffff) | (offset & 0x7ffff);
			}
		}
	}
...
void vgmplay_state::machine_start()
{
	...
	if(m_file->exists() && m_file->length() > 0) {
		...
		if(version >= 0x151 && r32(0x3c))
		{
			...
			m_segapcm_bank_shift = segabank & 0xffff;
			m_segapcm_bank_mask = ((segabank >> 16) & 0xf8) | 0x70;
			m_segapcm->set_unscaled_clock(r32(0x3c));
		}
		...
		if (data_start > 0x80)
		{
			...
			if(version >= 0x161 && r8(0x96)) {
				...
				c140_device::set_c219(*m_c140, (bank == 2) ? true : false);
				m_c140_bank_type = bank;
			}

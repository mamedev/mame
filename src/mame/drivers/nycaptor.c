...
static WRITE8_HANDLER( nycaptor_generic_control_w )
{
	...
	memory_set_bankptr(1, memory_region(machine, RGNCLASS_CPU, "main") + 0x10000 + ((data&0x08)>>3)*0x4000 );
}
...
static WRITE8_HANDLER( cyclshtg_generic_control_w )
{
	int bank=(data>>2)&3;
	generic_control_reg = data;
	memory_set_bankptr(1, memory_region(machine, RGNCLASS_CPU, "main") + 0x10000 + bank*0x4000 );
}
...
static ADDRESS_MAP_START( bronx_io, ADDRESS_SPACE_IO, 8 )
 	AM_RANGE(0x0000, 0x7fff) AM_ROM AM_REGION(RGNCLASS_USER, "user1", 0)
ADDRESS_MAP_END
...
ROM_START( cyclshtg )
	...
	ROM_REGION( 0x0800, RGNCLASS_CPU, "cpu3", 0 )
...
ROM_START( colt )
	...
	ROM_COPY( RGNCLASS_CPU, "main",   0x10000, 0x014000, 0x04000 )
	ROM_COPY( RGNCLASS_CPU, "main",   0x18000, 0x01c000, 0x04000 )

	ROM_REGION( 0x10000, RGNCLASS_CPU, "sub", 0 )
	ROM_LOAD( "05.bin",   0x0000, 0x4000, CRC(2b6e017a) SHA1(60715e1c6fbcdd2c0e114035c342ba587dfc1b4b) )
	ROM_LOAD( "a50_06",   0x4000, 0x4000, CRC(450d6783) SHA1(e652bf83fb6bdd8152dbafb05bb695259c2619cc) )

	ROM_REGION( 0x08000, RGNCLASS_USER, "user1", ROMREGION_ERASE00 ) //empty

...
{
	memory_set_bankptr(1, memory_region(machine, RGNCLASS_CPU, "audio") + (banknum * 0x4000) + 0x10000);
}
...
ROM_START( syvalion )
	ROM_REGION( 0x80000, RGNCLASS_CPU, "main", 0 )		/* main cpu */
	...
	ROM_REGION( 0x1c000, RGNCLASS_CPU, "audio", 0 )		/* sound cpu */
	...
	ROM_REGION( 0x80000, RGNCLASS_SOUND, "ym.deltat", 0 )	/* samples */
	ROM_LOAD( "b51-18.bin", 0x00000, 0x80000, CRC(8b23ac83) SHA1(340b9e7f09c1809a332b41d3fb579f5f8cd6367f) )

	ROM_REGION( 0x80000, RGNCLASS_SOUND, "ym", 0 )	/* samples */
...
ROM_START( dleague )
	ROM_REGION( 0x60000, RGNCLASS_CPU, "main", 0 )
	...
	ROM_REGION( 0x1c000, RGNCLASS_CPU, "audio", 0 )

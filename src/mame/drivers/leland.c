...
static void init_master_ports(running_machine *machine, UINT8 mvram_base, UINT8 io_base)
{
	...
	memory_install_readwrite8_handler(machine->device("master")->memory().space(ADDRESS_SPACE_IO), mvram_base, mvram_base + 0x1f, 0, 0, leland_mvram_port_r, leland_mvram_port_w);

	/* set up the master CPU I/O ports */
	memory_install_read8_handler(machine->device("master")->memory().space(ADDRESS_SPACE_IO), io_base, io_base + 0x1f, 0, 0, leland_master_input_r);
	memory_install_write8_handler(machine->device("master")->memory().space(ADDRESS_SPACE_IO), io_base, io_base + 0x0f, 0, 0, leland_master_output_w);
}
...
static DRIVER_INIT( cerberus )
{
	...
	memory_install_read8_handler(machine->device("master")->memory().space(ADDRESS_SPACE_IO), 0x80, 0x80, 0, 0, cerberus_dial_1_r);
	memory_install_read8_handler(machine->device("master")->memory().space(ADDRESS_SPACE_IO), 0x90, 0x90, 0, 0, cerberus_dial_2_r);
}
...
static DRIVER_INIT( alleymas )
{
	...
	state->alleymas_kludge_mem = memory_install_write8_handler(machine->device("master")->memory().space(ADDRESS_SPACE_PROGRAM), 0xe0ca, 0xe0ca, 0, 0, alleymas_joystick_kludge);
}
...
static DRIVER_INIT( dangerz )
{
	...
	/* set up additional input ports */
	memory_install_read8_handler(machine->device("master")->memory().space(ADDRESS_SPACE_IO), 0xf4, 0xf4, 0, 0, dangerz_input_upper_r);
	memory_install_read8_handler(machine->device("master")->memory().space(ADDRESS_SPACE_IO), 0xf8, 0xf8, 0, 0, dangerz_input_y_r);
	memory_install_read8_handler(machine->device("master")->memory().space(ADDRESS_SPACE_IO), 0xfc, 0xfc, 0, 0, dangerz_input_x_r);
}
...
static DRIVER_INIT( redlin2p )
{
	...
	/* set up additional input ports */
	memory_install_read8_handler(machine->device("master")->memory().space(ADDRESS_SPACE_IO), 0xc0, 0xc0, 0, 0, redline_pedal_1_r);
	memory_install_read8_handler(machine->device("master")->memory().space(ADDRESS_SPACE_IO), 0xd0, 0xd0, 0, 0, redline_pedal_2_r);
	memory_install_read8_handler(machine->device("master")->memory().space(ADDRESS_SPACE_IO), 0xf8, 0xf8, 0, 0, redline_wheel_2_r);
	memory_install_read8_handler(machine->device("master")->memory().space(ADDRESS_SPACE_IO), 0xfb, 0xfb, 0, 0, redline_wheel_1_r);
}
...
static DRIVER_INIT( viper )
{
	...
	/* set up additional input ports */
	memory_install_read8_handler(machine->device("master")->memory().space(ADDRESS_SPACE_IO), 0xa4, 0xa4, 0, 0, dangerz_input_upper_r);
	memory_install_read8_handler(machine->device("master")->memory().space(ADDRESS_SPACE_IO), 0xb8, 0xb8, 0, 0, dangerz_input_y_r);
	memory_install_read8_handler(machine->device("master")->memory().space(ADDRESS_SPACE_IO), 0xbc, 0xbc, 0, 0, dangerz_input_x_r);
}
...
static DRIVER_INIT( teamqb )
{
	...
	/* set up additional input ports */
	memory_install_read_port(machine->device("master")->memory().space(ADDRESS_SPACE_IO), 0x7c, 0x7c, 0, 0, "IN4");
	memory_install_read_port(machine->device("master")->memory().space(ADDRESS_SPACE_IO), 0x7f, 0x7f, 0, 0, "IN5");
}
...
static DRIVER_INIT( offroad )
{
	...
	/* set up additional input ports */
	memory_install_read8_handler(machine->device("master")->memory().space(ADDRESS_SPACE_IO), 0xf8, 0xf8, 0, 0, offroad_wheel_3_r);
	memory_install_read8_handler(machine->device("master")->memory().space(ADDRESS_SPACE_IO), 0xf9, 0xf9, 0, 0, offroad_wheel_1_r);
	memory_install_read8_handler(machine->device("master")->memory().space(ADDRESS_SPACE_IO), 0xfb, 0xfb, 0, 0, offroad_wheel_2_r);
}
...
static DRIVER_INIT( pigout )
{
	...
	/* set up additional input ports */
	memory_install_read_port(machine->device("master")->memory().space(ADDRESS_SPACE_IO), 0x7f, 0x7f, 0, 0, "IN4");
}

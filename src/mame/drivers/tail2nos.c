...
static MACHINE_RESET( tail2nos )
{
	...
	memory_set_bankptr(3,memory_region(machine, RGNCLASS_CPU, "audio") + 0x10000);
}
...
{
	memory_set_bankptr(3,memory_region(machine, RGNCLASS_CPU, "audio") + 0x10000 + (data & 0x01) * 0x8000);
}
...
ROM_START( tail2nos )
	...
	ROM_REGION16_BE( 0x20000, RGNCLASS_USER, "user2", 0 )
	...
	ROM_REGION( 0x20000, RGNCLASS_CPU, "audio", 0 )	/* 64k for the audio CPU + banks */
	...
	ROM_REGION( 0x20000, RGNCLASS_GFX, "gfx3", ROMREGION_ERASE00 )	/* gfx data for the 051316 */
	/* RAM, not ROM - handled at run time */

	ROM_REGION( 0x20000, RGNCLASS_SOUND, "ym", 0 ) /* sound samples */

...
/***************************************************************************
    ...
    To do:
    - mouse
    - make 'mux' window system work, currently fails to start:
      "68ld: load protocol failed"
    - sound?
...
//	map(0x060028, 0x060029).w(FUNC(blit_state::init_write));
	...
	map(0x060030, 0x060031).rw(m_acia0, FUNC(acia6850_device::status_r), FUNC(acia6850_device::control_w));
	map(0x060032, 0x060033).rw(m_acia0, FUNC(acia6850_device::data_r), FUNC(acia6850_device::data_w));
	// octal 0070 - 16-bit
...
void blit_state::machine_start()
{
	...
	m_sysrom = (uint16_t *)(memregion(M68K_TAG)->base());
}

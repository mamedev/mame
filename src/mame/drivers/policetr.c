...
{
	return memory_region(machine, RGNCLASS_SOUND, "bsmt")[bsmt_data_bank * 0x10000 + bsmt_data_offset] << 8;
}
...
static ADDRESS_MAP_START( policetr_map, ADDRESS_SPACE_PROGRAM, 32 )
	...
	AM_RANGE(0x1fc00000, 0x1fc7ffff) AM_ROM AM_REGION(RGNCLASS_USER, "user1", 0)
ADDRESS_MAP_END
...
static ADDRESS_MAP_START( sshooter_map, ADDRESS_SPACE_PROGRAM, 32 )
	...
	AM_RANGE(0x00a40000, 0x00a40003) AM_READ(port2_r)
	AM_RANGE(0x00e00000, 0x00e00003) AM_WRITENOP		// watchdog???
	AM_RANGE(0x1fc00000, 0x1fcfffff) AM_ROM AM_REGION(RGNCLASS_USER, "user1", 0)
ADDRESS_MAP_END
...
ROM_START( policetr ) /* Rev 0.3 PCB , with all chips dated 04/01/97 */
	...
	ROM_REGION( 0x600000, RGNCLASS_SOUND, "bsmt", 0 )
...
ROM_START( polict11 ) /* Rev 0.3 PCB with all chips dated 01/06/97 */
	...
	ROM_REGION32_BE( 0x80000, RGNCLASS_USER, "user1", 0 )	/* 2MB for R3000 code */
...
ROM_START( sshooter ) /* Rev 0.5B PCB , unknown program rom date */
	ROM_REGION( 0x800000, RGNCLASS_GFX, "gfx1", ROMREGION_ERASE00 ) /* Graphics v1.0 */
	...
	ROM_REGION( 0x600000, RGNCLASS_SOUND, "bsmt", 0 ) /* Sound v1.2 */

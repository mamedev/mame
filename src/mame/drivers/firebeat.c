...
static ADDRESS_MAP_START( firebeat_map, ADDRESS_SPACE_PROGRAM, 32 )
	...
	AM_RANGE(0x7ff80000, 0x7fffffff) AM_ROM AM_REGION(RGNCLASS_USER, "user1", 0)		/* System BIOS */
ADDRESS_MAP_END
...
static MACHINE_RESET( firebeat )
{
	void *cd;
	int i;
	UINT8 *sound = memory_region(machine, RGNCLASS_SOUND, "ymz");
...
ROM_START( ppp )
	ROM_REGION32_BE(0x80000, RGNCLASS_USER, "user1", 0)
	ROM_LOAD16_WORD_SWAP("977jaa03.e21", 0x00000, 0x80000, CRC(7b83362a) SHA1(2857a93be58636c10a8d180dbccf2caeeaaff0e2))

	ROM_REGION(0x400000, RGNCLASS_SOUND, "ymz", ROMREGION_ERASE00)

	ROM_REGION(0xc0, RGNCLASS_USER, "user2", 0)	// Security dongle
...
ROM_START( kbm )
	...
	ROM_REGION(0xc0, RGNCLASS_USER, "user2", ROMREGION_ERASE00)	// Security dongle
...
ROM_START( popn7 )
	...
	ROM_REGION(0x80000, RGNCLASS_CPU, "audio", 0)			// SPU 68K program

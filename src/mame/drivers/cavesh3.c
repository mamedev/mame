...
static NVRAM_HANDLER( cavesh3 )
{
	...
	UINT8 *region_game = machine.region( "game" )->base();
	UINT8 *region_copy = machine.region( "copy" )->base();
	...
	if (read_or_write)
	{
		...
		while (page < size)
		{
			if (memcmp(region_game + page * FLASH_PAGE_SIZE, region_copy + page * FLASH_PAGE_SIZE, FLASH_PAGE_SIZE))
			{
				file->write(&page, 4);
				file->write(region_game + page * FLASH_PAGE_SIZE, FLASH_PAGE_SIZE);
			}
			page++;
		}
		file->write(&page, 4);
	...
	else
	{
		if (file)
		{
			...
			while (page < size)
			{
				file->read(region_game + page * FLASH_PAGE_SIZE, FLASH_PAGE_SIZE);
				file->read(&page, 4);
			}
		}
	}
...
ROM_START( mushisam )
	...
	ROM_REGION( 0x8400000, "copy", ROMREGION_ERASEFF)
	ROM_COPY( "game", 0,    0x000000, 0x8400000 )

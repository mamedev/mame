...
void cps3_state::video_start()
{
	...
	m_gfxdecode->set_gfx(0, auto_alloc(machine(), gfx_element(machine(), m_palette, cps3_tiles8x8_layout, (UINT8 *)m_ss_ram, m_palette->entries() / 16, 0)));

	//decode_ssram();

	/* create the char set (gfx will then be updated dynamically from RAM) */
	m_gfxdecode->set_gfx(1, auto_alloc(machine(), gfx_element(machine(), m_palette, cps3_tiles16x16_layout, (UINT8 *)m_char_ram, m_palette->entries() / 64, 0)));

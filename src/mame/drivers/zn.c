...
{
	memory_install_read_bank ( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1f000000, 0x1f3fffff, 0, 0, "bank1" );     /* fixed game rom */
	memory_install_read_bank ( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1f400000, 0x1f7fffff, 0, 0, "bank2" );     /* banked game rom */
	memory_install_write32_handler( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1fb00000, 0x1fb00003, 0, 0, bank_coh1000c_w ); /* bankswitch */
	memory_install_read32_handler ( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1fb40010, 0x1fb40013, 0, 0, capcom_kickharness_r );
	memory_install_read32_handler ( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1fb40020, 0x1fb40023, 0, 0, capcom_kickharness_r );
	memory_install_read_bank ( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1fb80000, 0x1fbfffff, 0, 0, "bank3" );     /* country rom */
	memory_install_write32_handler( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1fb60000, 0x1fb60003, 0, 0, zn_qsound_w );
...
static DRIVER_INIT( coh3002c )
{
	...
	memory_install_read32_handler ( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1fb40010, 0x1fb40013, 0, 0, capcom_kickharness_r );
	memory_install_read32_handler ( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1fb40020, 0x1fb40023, 0, 0, capcom_kickharness_r );
	memory_install_write32_handler( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1fb00000, 0x1fb00003, 0, 0, bank_coh3002c_w ); /* bankswitch */
	memory_install_read_bank ( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1fb80000, 0x1fbfffff, 0, 0, "bank3" );     /* country rom */
	memory_install_write32_handler( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1fb60000, 0x1fb60003, 0, 0, zn_qsound_w );
...
static DRIVER_INIT( coh1000ta )
{
	...
	memory_install_read_bank     ( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1f000000, 0x1f7fffff, 0, 0, "bank1" );     /* banked game rom */
	memory_install_write32_handler    ( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1fb40000, 0x1fb40003, 0, 0, bank_coh1000t_w ); /* bankswitch */
	memory_install_readwrite32_handler( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1fb80000, 0x1fb80003, 0, 0, taitofx1a_ymsound_r, taitofx1a_ymsound_w );
	memory_install_readwrite_bank( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1fbe0000, 0x1fbe0000 + ( state->taitofx1_eeprom_size1 - 1 ), 0, 0, "bank2" );
...
static DRIVER_INIT( coh1000tb )
{
	...
	memory_install_read_bank     ( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1f000000, 0x1f7fffff, 0, 0, "bank1" ); /* banked game rom */
	memory_install_readwrite_bank( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1fb00000, 0x1fb00000 + ( state->taitofx1_eeprom_size1 - 1 ), 0, 0, "bank2" );
	memory_install_write32_handler    ( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1fb40000, 0x1fb40003, 0, 0, bank_coh1000t_w ); /* bankswitch */
	memory_install_write32_handler    ( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1fb80000, 0x1fb8ffff, 0, 0, taitofx1b_volume_w );
	memory_install_write32_handler    ( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1fba0000, 0x1fbaffff, 0, 0, taitofx1b_sound_w );
	memory_install_read32_handler     ( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1fbc0000, 0x1fbc0003, 0, 0, taitofx1b_sound_r );
	memory_install_readwrite_bank( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1fbe0000, 0x1fbe0000 + ( state->taitofx1_eeprom_size2 - 1 ), 0, 0, "bank3" );
...
static DRIVER_INIT( coh1000w )
{
	...
	memory_install_read_bank         ( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1f000000, 0x1f1fffff, 0, 0, "bank1" );
	memory_nop_write                         ( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1f000000, 0x1f000003, 0, 0 );
	memory_install_readwrite32_device_handler( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), ide, 0x1f7e4000, 0x1f7e4fff, 0, 0, ide_controller32_r, ide_controller32_w );
	memory_nop_readwrite                     ( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1f7e8000, 0x1f7e8003, 0, 0 );
	memory_install_readwrite32_device_handler( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), ide, 0x1f7f4000, 0x1f7f4fff, 0, 0, ide_controller32_r, ide_controller32_w );
...
{
	memory_install_read_bank ( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1f000000, 0x1f7fffff, 0, 0, "bank1" );
	memory_install_write32_handler( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1fa10300, 0x1fa10303, 0, 0, coh1002e_bank_w );
	memory_install_write32_handler( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1fb00000, 0x1fb00007, 0, 0, coh1002e_latch_w );
...
{
	memory_install_read_bank ( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1f000000, 0x1f3fffff, 0, 0, "bank1" );
	memory_install_read_bank ( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1f400000, 0x1f7fffff, 0, 0, "bank2" );
	memory_install_read32_handler ( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1fb00000, 0x1fb00007, 0, 0, bam2_mcu_r );
	memory_install_read32_handler ( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1fa20000, 0x1fa20003, 0, 0, bam2_unk_r );
	memory_install_write32_handler( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1fa10300, 0x1fa10303, 0, 0, bam2_sec_w );
...
static DRIVER_INIT( coh1000a )
{
	...
	memory_install_read_bank ( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1f000000, 0x1f1fffff, 0, 0, "bank1" );
	memory_install_write32_handler( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1fbfff00, 0x1fbfff03, 0, 0, acpsx_00_w );
	memory_install_write32_handler( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1fbfff10, 0x1fbfff13, 0, 0, acpsx_10_w );
	...
	if( strcmp( machine->gamedrv->name, "nbajamex" ) == 0 )
	{
		...
		memory_install_readwrite_bank( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1f200000, 0x1f200000 + ( state->nbajamex_eeprom_size - 1 ), 0, 0, "bank2" );
		memory_install_read32_handler     ( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1fbfff08, 0x1fbfff0b, 0, 0, nbajamex_08_r );
		memory_install_readwrite32_handler( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1fbfff80, 0x1fbfff83, 0, 0, nbajamex_80_r, nbajamex_80_w );
		...
		( !strcmp( machine->gamedrv->name, "jdreddb" ) ) )
	{
		...
		memory_install_read32_device_handler( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), ide, 0x1fbfff8c, 0x1fbfff8f, 0, 0, jdredd_idestat_r );
		memory_nop_write                    ( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1fbfff8c, 0x1fbfff8f, 0, 0 );
		memory_install_readwrite32_device_handler( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), ide, 0x1fbfff90, 0x1fbfff9f, 0, 0, jdredd_ide_r, jdredd_ide_w );
	}
...
{
	memory_install_read_bank ( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1f000000, 0x1f7fffff, 0, 0, "bank1" ); /* banked rom */
	memory_install_write32_handler( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1fb00000, 0x1fb00003, 0, 0, coh1001l_bnk_w );
...
{
	memory_install_read_bank ( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1f000000, 0x1f27ffff, 0, 0, "bank1" );
	memory_install_read_bank ( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1fb00000, 0x1fbfffff, 0, 0, "bank2" );
	memory_install_write32_handler( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1fb00000, 0x1fb00003, 0, 0, coh1002v_bnk_w );
...
{
	memory_install_read_bank     ( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1f000000, 0x1f7fffff, 0, 0, "bank1" );
	memory_install_readwrite32_handler( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1fb00000, 0x1fb00003, 0, 0, cbaj_z80_r, cbaj_z80_w );
	memory_install_write32_handler    ( machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x1fb00004, 0x1fb00007, 0, 0, coh1002m_bank_w );

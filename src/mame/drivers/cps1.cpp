...
void cps_state::sf2m3_map(address_map &map)
{
	...
	map(0x800190, 0x800197).w(m_soundlatch[0], FUNC(generic_latch_8_device::write));    /* Sound command */
	map(0x800198, 0x80019f).w(m_soundlatch[1], FUNC(generic_latch_8_device::write)).umask(0x00ff);   /* Sound timer fade */
...
void cps_state::sf2m10_map(address_map &map)
{
	...
	map(0x800140, 0x80017f).rw(this, FUNC(cps_state::cps1_cps_b_r), FUNC(cps_state::cps1_cps_b_w)).share("cps_b_regs");
	map(0x800180, 0x800187).w(m_soundlatch[0], FUNC(generic_latch_8_device::write));
	map(0x800188, 0x80018f).w(m_soundlatch[1], FUNC(generic_latch_8_device::write)).umask(0x00ff);
...
{
	MACHINE_START_CALL_MEMBER(common);
	m_audiobank->configure_entries(0, 6, m_audioregion->base() + 0x8000, 0x4000);
}
...
ROM_START( chikij )
	...
	ROM_REGION( 0x10000, "audiocpu", 0 ) /* 64k for the audio CPU (+banks) */
	ROM_LOAD( "ch_23.13b",  0x00000, 0x10000, CRC(4d4255b7) SHA1(81a76b58043af7252a854b7efc4109957ef0e679) )    // == ch_09.12b

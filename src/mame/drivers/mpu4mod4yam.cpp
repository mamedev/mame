...
void mpu4_state::mod4yam_supst(machine_config &config)
{
	mod4yam(config);
	MPU4_CHARACTERISER_PAL(config, m_characteriser, 0);
}
...
#define GAME_CUSTOM(year, setname,parent,name,offset,length,hash,company,title) \
	ROM_START( setname ) \
		...
		M4SUPST_EXTRA_ROMS \
	ROM_END \
	GAME(year, setname, parent, mod4yam_supst, mpu4, mpu4_state, init_m4_showstring, ROT0, company, title, GAME_FLAGS )

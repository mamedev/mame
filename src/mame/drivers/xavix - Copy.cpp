...
{
	printf("%s: xavix_6ff1_w %02x\n", machine().describe_context().c_str(), data);
}

WRITE8_MEMBER(xavix_state::xavix_6ff2_w)
{
	printf("%s: xavix_6ff2_w %02x\n", machine().describe_context().c_str(), data);
}
...
{
	printf("%s: xavix_6ffa_w %02x\n", machine().describe_context().c_str(), data);
}

WRITE8_MEMBER(xavix_state::xavix_6ffb_w)
{
	printf("%s: xavix_6ffb_w %02x\n", machine().describe_context().c_str(), data);
}
...
void xavix_state::xavix_map(address_map &map)
{
	...
	map(0x006fe8, 0x006fe8).rw(this, FUNC(xavix_state::xavix_6fe8_r), FUNC(xavix_state::xavix_6fe8_w)); // r/w tested
	map(0x006fe9, 0x006fe9).rw(this, FUNC(xavix_state::xavix_6fe9_r), FUNC(xavix_state::xavix_6fe9_w)); // r/w tested
	map(0x006fea, 0x006fea).w(this, FUNC(xavix_state::xavix_6fea_w));

	map(0x006ff0, 0x006ff0).rw(this, FUNC(xavix_state::xavix_6ff0_r), FUNC(xavix_state::xavix_6ff0_w)); // r/w tested
	map(0x006ff1, 0x006ff1).w(this, FUNC(xavix_state::xavix_6ff1_w)); // startup - cleared in interrupt 0
	map(0x006ff2, 0x006ff2).w(this, FUNC(xavix_state::xavix_6ff2_w)); // set to 07 after clearing above things in interrupt 0
	...
	map(0x006ffa, 0x006ffa).w(this, FUNC(xavix_state::xavix_6ffa_w));
	map(0x006ffb, 0x006ffb).w(this, FUNC(xavix_state::xavix_6ffb_w)); // increases / decreases when you jump in snowboard, maybe raster irq pos or part of a clipping window?
	...
	map(0x007810, 0x007810).w(this, FUNC(xavix_state::xavix_7810_w)); // startup

	map(0x007900, 0x007900).w(this, FUNC(xavix_state::xavix_7900_w));
	map(0x007901, 0x007901).w(this, FUNC(xavix_state::xavix_7901_w));
	map(0x007902, 0x007902).w(this, FUNC(xavix_state::xavix_7902_w));
	...
	// GPIO stuff
	map(0x007a00, 0x007a00).rw(this, FUNC(xavix_state::xavix_io_0_r),FUNC(xavix_state::xavix_7a00_w));
	map(0x007a01, 0x007a01).rw(this, FUNC(xavix_state::xavix_io_1_r),FUNC(xavix_state::xavix_7a01_w)); // startup (taitons1)
	map(0x007a02, 0x007a02).rw(this, FUNC(xavix_state::xavix_7a02_r),FUNC(xavix_state::xavix_7a02_w)); // startup, gets set to 20, 7a00 is then also written with 20
	map(0x007a03, 0x007a03).rw(this, FUNC(xavix_state::xavix_7a03_r),FUNC(xavix_state::xavix_7a03_w)); // startup (gets set to 84 which is the same as the bits checked on 7a01, possible port direction register?)

	map(0x007a80, 0x007a80).w(this, FUNC(xavix_state::xavix_7a80_w)); // still IO? ADC related?

	map(0x007b00, 0x007b00).w(this, FUNC(xavix_state::xavix_7b00_w)); // rad_snow (not often)

	map(0x007b80, 0x007b80).rw(this, FUNC(xavix_state::xavix_7b80_r), FUNC(xavix_state::xavix_7b80_w)); // rad_snow (not often)
	map(0x007b81, 0x007b81).w(this, FUNC(xavix_state::xavix_7b81_w)); // written (often, m_trck, analog related?)

	map(0x007c00, 0x007c00).w(this, FUNC(xavix_state::xavix_7c00_w));
	map(0x007c01, 0x007c01).rw(this, FUNC(xavix_state::xavix_7c01_r), FUNC(xavix_state::xavix_7c01_w)); // r/w tested
	map(0x007c02, 0x007c02).w(this, FUNC(xavix_state::xavix_7c02_w));

	// this is a multiplication chip
	map(0x007ff2, 0x007ff4).w(this, FUNC(xavix_state::mult_param_w));
	map(0x007ff5, 0x007ff6).rw(this, FUNC(xavix_state::mult_r), FUNC(xavix_state::mult_w));

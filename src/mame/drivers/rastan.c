...
static WRITE8_HANDLER( rastan_bankswitch_w )
{
	...
	memory_set_bankptr( 1, memory_region(machine, RGNCLASS_CPU, "audio") + offs );
}
...
ROM_START( rastan )
	...
	ROM_REGION( 0x10000, RGNCLASS_SOUND, "adpcm", 0 )	/* 64k for the samples */
...
ROM_START( rastanu )
	...
	ROM_REGION( 0x10000, RGNCLASS_SOUND, "adpcm", 0 )	/* MSM5205 samples */

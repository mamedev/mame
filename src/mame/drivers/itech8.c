...
static MACHINE_RESET( itech8 )
{
	...
	if (machine->config->cpu[0].type == CPU_M6809)
		memory_set_bankptr(1, &memory_region(machine, RGNCLASS_CPU, "main")[0x4000]);
...
static WRITE8_HANDLER( blitter_w )
{
	...
	if (offset / 2 == 7)
		memory_set_bankptr(1, &memory_region(machine, RGNCLASS_CPU, "main")[0x4000 + 0xc000 * ((data >> 5) & 1)]);
...
static WRITE8_HANDLER( rimrockn_bank_w )
{
	...
	memory_set_bankptr(1, &memory_region(machine, RGNCLASS_CPU, "main")[0x4000 + 0xc000 * (data & 3)]);
}
...
static ADDRESS_MAP_START( ninclown_map, ADDRESS_SPACE_PROGRAM, 16 )
	AM_RANGE(0x000000, 0x00007f) AM_RAM AM_REGION(RGNCLASS_CPU, "main", 0)
...
ROM_START( wfortune )
	...
	ROM_COPY( RGNCLASS_CPU, "main", 0x14000, 0x8000, 0x8000 )
...
ROM_START( stratab )
	...
	ROM_COPY( RGNCLASS_CPU, "main",    0x8000, 0x14000, 0x8000 )
...
ROM_START( gtgt )
	...
	ROM_REGION( 0xb0000, RGNCLASS_GFX, "gfx1", 0 )
	ROM_LOAD( "grom0.bin", 0x00000, 0x20000, CRC(a29c688a) SHA1(32dbb996a5e4c23cfd44b79312ac4a767658f20a) )
	ROM_LOAD( "grom1.bin", 0x20000, 0x20000, CRC(b52a23f6) SHA1(092961acf47875179b44342e2dd8955670e67ea2) )
	ROM_LOAD( "grom2.bin", 0x40000, 0x20000, CRC(9b8e3a61) SHA1(1b5682b1328d6c97b604fb71512e8f72322a688f) )
	ROM_LOAD( "grom3.bin", 0x60000, 0x20000, CRC(b6e9fb15) SHA1(c1b28ea911696cb4ed56bfba212848693530b59f) )
...
ROM_START( slikshot )
	...
	ROM_REGION( 0x10000, RGNCLASS_SOUND, "oki", 0 )
...
ROM_START( rimrockn )
	ROM_REGION( 0x34000, RGNCLASS_CPU, "main", 0 )
	...
	ROM_COPY( RGNCLASS_CPU, "main", 0x2c000, 0x08000, 0x8000 )
	ROM_COPY( RGNCLASS_CPU, "main", 0x2c000, 0x14000, 0x8000 )
	ROM_COPY( RGNCLASS_CPU, "main", 0x2c000, 0x20000, 0x8000 )

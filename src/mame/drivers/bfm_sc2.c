...
static void decode_mainrom(running_machine *machine, const char *rom_region)
{
	...
	rom = memory_region(machine, RGNCLASS_CPU, rom_region);
...
static void adder2_common_init(running_machine *machine)
{
	...
	pal = memory_region(machine, RGNCLASS_PROMS, "proms");
	if ( pal )
	{
		memcpy(key, pal, 8);
	}
...
ROM_START( quintoon )
	...
	ROM_REGION( 0x20000, RGNCLASS_CPU, "adder2", 0 )
	ROM_LOAD("quinp132",		0x00000, 0x20000,  CRC(63896a7f) SHA1(81aa56874a15faa3aabdfc0fc524b2e25b751f22))

	ROM_REGION( 0x20000, RGNCLASS_SOUND, "upd", 0 ) // using Dutch samples, need to check a UK Quintoon PCB
	ROM_LOAD("95001016.snd",	0x00000, 0x20000, BAD_DUMP CRC(cf097d41) SHA1(6712f93896483360256d8baffc05977c8e532ef1))

	ROM_REGION( 0x40000, RGNCLASS_GFX, "gfx1", ROMREGION_DISPOSE | ROMREGION_ERASEFF )
...
ROM_START( gldncrwn )
	...
	ROM_REGION( 0x10, RGNCLASS_PROMS, "proms", ROMREGION_DISPOSE )

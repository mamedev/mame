...
public:
	...
	void hexaprez(machine_config &config);

private:
	void audio_mem_map(address_map &map);
	void audio_io_map(address_map &map);
	void hexaprez_audio_io_map(address_map &map);
...
void yuvomz80_state::hexaprez_audio_io_map(address_map &map)
{
	map.global_mask(0xff);
	map(0x00, 0x01).rw("ymz", FUNC(ymz280b_device::read), FUNC(ymz280b_device::write));
}

static INPUT_PORTS_START( hexapres )
INPUT_PORTS_END
...
void yuvomz80_state::hexaprez(machine_config &config)
{
	Z80(config, m_maincpu, XTAL(8'000'000));
	...
	z80_device &audiocpu(Z80(config, "audiocpu", XTAL(8'000'000)));
	audiocpu.set_addrmap(AS_PROGRAM, &yuvomz80_state::audio_mem_map);
	audiocpu.set_addrmap(AS_IO, &yuvomz80_state::hexaprez_audio_io_map);
...
GAME( 1995, hexapres, 0, hexapres, hexapres, yuvomz80_state, empty_init, ROT0, "Yubis", "Hexa President (YM2610 set)", MACHINE_IS_SKELETON_MECHANICAL )
GAME( 1995, hexaprez, 0, hexaprez, hexapres, yuvomz80_state, empty_init, ROT0, "Yubis", "Hexa President (YMZ280B set)", MACHINE_IS_SKELETON_MECHANICAL )

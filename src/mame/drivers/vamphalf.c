...
public:
	...
	DECLARE_READ32_MEMBER(yolijoli_1c_r);
	DECLARE_WRITE32_MEMBER(yolijoli_1c_w);
	DECLARE_READ32_MEMBER(yolijoli_10_r);
...
READ32_MEMBER(vamphalf_state::yolijoli_10_r)
{
	printf("yolijoli_10_r %08x\n", space.device().safe_pc());
	return 0xffffffff;
}
READ32_MEMBER(vamphalf_state::yolijoli_1c_r)
{
//  printf("yolijoli_1c_r %08x\n", space.device().safe_pc());
	return 0x00;// 0xaa;
}
WRITE32_MEMBER(vamphalf_state::yolijoli_1c_w)
{
//  printf("yolijoli_1c_w %08x %08x\n", space.device().safe_pc(), data);
}
...
static ADDRESS_MAP_START( yolijoli_io, AS_IO, 32, vamphalf_state )
	AM_RANGE(0x010, 0x013) AM_READ(yolijoli_10_r)
	AM_RANGE(0x01c, 0x01f) AM_READWRITE(yolijoli_1c_r,yolijoli_1c_w)
ADDRESS_MAP_END
...
static MACHINE_CONFIG_DERIVED( yolijoli, common )
	MCFG_CPU_REPLACE("maincpu", E132T, XTAL_50MHz)    /* 50 MHz */
	MCFG_CPU_PROGRAM_MAP(yolijoli_32bit_map)
	MCFG_CPU_IO_MAP(yolijoli_io)
	MCFG_CPU_VBLANK_INT_DRIVER("screen", vamphalf_state,  irq1_line_hold)
...
DRIVER_INIT_MEMBER(vamphalf_state,yolijoli)
{
...
GAME( 199?, yolijoli,  0,        yolijoli, common,   vamphalf_state, yolijoli, ROT0,   "Golden Bell Entertainment",         "Yoli Joli Kug Kug", GAME_NOT_WORKING | GAME_IMPERFECT_SOUND )

...
static WRITE16_HANDLER( control_w )
{
	...
	/* OKI banking here */
	if (offset == 0)
		OKIM6295_set_bank_base(0, (((data >> 4) & 1) * 0x40000) % memory_region_length(machine, RGNCLASS_SOUND, "oki"));
...
ROM_START( cheesech )
	ROM_REGION( 0x80000, RGNCLASS_CPU, "main", 0 )	/* 64k for 68000 code */
	...
	ROM_REGION16_LE( 0x100000, RGNCLASS_GFX, "gfx1", 0 )
...
ROM_START( ultennis )
	...
	ROM_REGION16_LE( 0x200000, RGNCLASS_GFX, "gfx1", 0 )
...
ROM_START( stonebal )
	...
	ROM_REGION16_LE( 0x400000, RGNCLASS_GFX, "gfx1", 0 )

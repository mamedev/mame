/*************************************************************************
    ...
    Games supported:
        ...
        * War Gods (1996)
        * Off Road Challenge (1997) [3 sets]
...
static MACHINE_RESET( midvunit )
{
	...
	memcpy(ram_base, memory_region(machine, RGNCLASS_USER, "user1"), 0x20000*4);
...
//  AM_RANGE(0x991050, 0x991050) AM_READ(SMH_RAM) // seems to be another port
	...
	AM_RANGE(0xc00000, 0xffffff) AM_ROM AM_REGION(RGNCLASS_USER, "user1", 0)
ADDRESS_MAP_END
...
ROM_START( crusnusa ) /* Version 4.1, Mon Feb 13 1995 - 16:53:40 */
	...
	ROM_REGION32_LE( 0x1000000, RGNCLASS_USER, "user1", 0 )
...
ROM_START( offroadc ) /* Version 1.40, Mon 10-06-97 */
	ROM_REGION16_LE( 0x1000000, RGNCLASS_SOUND, "dcs", ROMREGION_ERASEFF )	/* sound data */
...
ROM_START( wargods ) /* Boot EPROM Version 1.0, Game Type: 452 (12/11/1995) */
	ROM_REGION16_LE( 0x10000, RGNCLASS_SOUND, "dcs", 0 )	/* sound data */
...
GAME( 1997, offroadc, 0,        midvunit, offroadc, offroadc, ROT0, "Midway", "Off Road Challenge (v1.40)", GAME_NOT_WORKING | GAME_SUPPORTS_SAVE )

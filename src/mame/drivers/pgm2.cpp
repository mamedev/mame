...
/* PGM 2 hardware.
    ...
    PGM2 Motherboard Components:
     ...
     YMZ774-S(SOUND)
     R5F21256SN(Renesas R8C/25 MCU Family, extra MCU for ICcard communication)
...
void pgm2_state::postload()
{
	...
	memcpy(m_maindata->base(), &m_encrypted_copy[0], m_maindata->bytes());
	...
	if (m_has_decrypted)
	{
		igs036_decryptor decrypter(m_encryption_table.get());

		if (m_romboard_ram)
		{
			decrypter.decrypter_rom((uint16_t*)m_maindata->base(), m_maindata->bytes(), 0x0200000);
		}
		else
		{
			decrypter.decrypter_rom((uint16_t*)m_maindata->base(), m_maindata->bytes(), 0);
		}
	}
}
...
WRITE32_MEMBER(pgm2_state::encryption_do_w)
{
	...
	if (m_romboard_ram)
	{
		...
		decrypter.decrypter_rom((uint16_t*)m_maindata->base(), m_maindata->bytes(), 0x0200000);   // assume the rom at 0x0200000 also gets decrypted as if it was at 0x0200000 even if it isn't used (the game has already copied it to RAM where it properly decrypted)
	}
	else
	{
		decrypter.decrypter_rom((uint16_t*)m_maindata->base(), m_maindata->bytes(), 0);
...
{
	return m_shareram[offset | ((m_share_bank & 1) << 7)];
}
WRITE8_MEMBER(pgm2_state::shareram_w)
{
	m_shareram[offset | ((m_share_bank & 1) << 7)] = data;
}


TIMER_DEVICE_CALLBACK_MEMBER(pgm2_state::igs_interrupt)
{
	...
	if (is_command)
	{
		...
		case 0xe1: // shared ram access (unimplemented)
		{
			...
			if (mode == 2)
			{
				...
				std::fill_n(&m_shareram[(~m_share_bank & 1) << 7], 0x80, data);
			}
			m_mcu_result0 = cmd;
			m_mcu_result1 = 0;
		}
		...
		case 0xc2: // read data to shared ram
			...
			{
				if (m_memcard[arg1 & 3]->present() != -1)
					m_shareram[i + ((~m_share_bank & 1) << 7)] = m_memcard[arg1 & 3]->read(space, arg2 + i);
				else
					status = 0xf4;
			}
			m_mcu_result0 = cmd;
		...
		case 0xc3: // save data from shared ram
			...
			{
				if (m_memcard[arg1 & 3]->present() != -1)
					m_memcard[arg1 & 3]->write(space, arg2 + i, m_shareram[i + ((~m_share_bank & 1) << 7)]);
				else
					status = 0xf4;
			}
			m_mcu_result0 = cmd;
...
READ16_MEMBER(pgm2_state::module_rom_r)
{
	...
	return ((uint16_t *)m_maindata->base())[offset];
}
...
{
	map(0x00000000, 0x00003fff).rom(); //AM_REGION("maindata", 0x00000) // internal ROM
...
{
	pgm2_map(map);
	map(0x10000000, 0x10ffffff).rom().region("maindata", 0); // external ROM
}
...
void pgm2_state::pgm2_ram_rom_map(address_map &map)
{
	...
	map(0x10200000, 0x103fffff).rom().region("maindata", 0); // external ROM
}
...
{
	m_encryption_table = make_unique_clear<uint8_t[]>(0x100);
	m_shareram = make_unique_clear<uint8_t[]>(0x80*2);

	save_pointer(NAME(m_encryption_table.get()), 0x100);
	...
	save_pointer(NAME(m_shareram.get()), 0x80*2);
...
#define ORLEG2_VIDEO_SOUND_ROMS \
	...
	ROM_LOAD( "xyj2_nvram",              0x00000000, 0x0010000, CRC(ccccc71c) SHA1(585b5ccbf89dd28d8532da785d7c8af12f31c6d6) )
...
#define ORLEG2_PROGRAM_104(prefix, extension) \
	ROM_REGION( 0x1000000, "maindata", 0 ) \
...
#define KOV2NL_VIDEO_SOUND_ROMS \
	...
	\
	ROM_REGION(0x10000, "sram", 0) \
	ROM_LOAD("gsyx_nvram",                0x00000000, 0x0010000, CRC(22400c16) SHA1(f775a16299c30f2ce23d683161b910e06eff37c1) )
...
#define KOV2NL_PROGRAM_301(prefix, extension) \
	ROM_REGION( 0x1000000, "maindata", 0 ) \
	ROM_LOAD( #prefix "_v301" #extension ".u7", 0x00000000, 0x0800000, CRC(c4595c2c) SHA1(09e379556ef76f81a63664f46d3f1415b315f384) ) // V301 08-09-09 09:44:53

#define KOV2NL_PROGRAM_300(prefix, extension) \
	ROM_REGION( 0x1000000, "maindata", 0 ) \
	ROM_LOAD( #prefix "_v300" #extension ".u7", 0x00000000, 0x0800000, CRC(08da7552) SHA1(303b97d7694405474c8133a259303ccb49db48b1) ) // V300 08-08-06 18:21:23
...
#define DDPDOJT_VIDEO_SOUND_ROMS \
	...
	ROM_LOAD32_WORD( "ddpdoj_mapl0.u13",      0x00000000, 0x0800000, CRC(bcfbb0fc) SHA1(9ec478eba9905913cf997bd9b46c70c1ad383630) ) \
	ROM_LOAD32_WORD( "ddpdoj_maph0.u15",      0x00000002, 0x0800000, CRC(0cc75d4e) SHA1(6d1b5ef0fdebf1e84fa199b939ffa07b810b12c9) ) \
	\
	...
	\
	ROM_REGION( 0x10000, "sram", 0 ) \
	ROM_LOAD( "ddpdojt_sram",                 0x00000000, 0x0010000, CRC(af99e304) SHA1(e44fed22b902431298748eca84533f8685926afd) )

ROM_START( ddpdojt )
	ROM_REGION( 0x04000, "maincpu", 0 )
	ROM_LOAD( "ddpdoj_igs036_china.rom",      0x00000000, 0x0004000, CRC(5db91464) SHA1(723d8086285805bd815e62120dfa9a4269bcd932) )

	ROM_REGION( 0x0200000, "maindata", 0 )
...
#define KOV3_VIDEO_SOUND_ROMS \
	...
	\
	ROM_REGION( 0x10000, "sram", 0 ) \
	ROM_LOAD( "kov3_sram",                  0x00000000, 0x0010000, CRC(d9608102) SHA1(dec5631642393f4ec76912c81fd60249bb45aa13) )
...
#define KOF98UMH_VIDEO_SOUND_ROMS \
	...
	\
	ROM_REGION( 0x10000, "sram", 0 ) \
	ROM_LOAD( "kof98umh_sram",               0x00000000, 0x0010000, CRC(60460ed9) SHA1(55cd8de37cee04ff7ad940fb52f8fb8db042c26e) )
...
void pgm2_state::common_encryption_init()
{
	...
	m_encrypted_copy.resize(m_maindata->bytes());
	memcpy(&m_encrypted_copy[0], m_maindata->base(), m_maindata->bytes());
...
{
	uint16_t *src = (uint16_t *)m_maindata->base();
	uint32_t size = m_maindata->bytes();

...
void dt7_state::video_start()
{
	...
	// both screens have to come from the same tilemap, with different scroll positions
...
void dt7_state::draw_tx_tilemap(screen_device& screen, bitmap_ind16& bitmap, const rectangle& cliprect, int table)
{
	...
	for (int y = cliprect.min_y; y <= cliprect.max_y; y++)
	{
		...
		m_tx_tilemap[0]->set_scrolly(0, scroll1 - y);
		m_tx_tilemap[0]->draw(screen, bitmap, clip, 0);
	}
}

...
Supported games:
    ...
    demonwld1   TP-O16      Demon's World/Horror Story [1989] (Taito license)
    demonwld2   TP-O16      Demon's World/Horror Story [1989] (early edition)
    demonwld3   TP-O16      Demon's World/Horror Story [1989] (first edition)
    ...
    outzonecv   TP-O15      Out Zone (Zero Wing conversion)
...
4) 'demonwld' and "clones"

    TO DO !


5) 'outzone' and "clones"
...
{
	map(0x0, 0x3).w(m_bcu, FUNC(toaplan_bcu_device::tile_offsets_w));
	map(0x6, 0x6).w(m_fcu, FUNC(toaplan_fcu_device::flipscreen_w));
}
...
void toaplan1_state::truxton_main_map(address_map &map)
{
	...
	map(0x0c0000, 0x0c0007).m(m_fcu, FUNC(toaplan_fcu_device::host_map));
...
void toaplan1_state::hellfire_main_map(address_map &map)
{
	...
	map(0x080000, 0x087fff).m(m_vctrl, FUNC(toaplan_video_controller_device::host_map));
	map(0x0c0000, 0x0c0fff).rw(FUNC(toaplan1_state::shared_r), FUNC(toaplan1_state::shared_w)).umask16(0x00ff);
	map(0x100000, 0x10001f).m(m_bcu, FUNC(toaplan_bcu_device::host_map));
	map(0x140000, 0x140007).m(m_fcu, FUNC(toaplan_fcu_device::host_map));
...
void toaplan1_state::zerowing_main_map(address_map &map)
{
	...
	map(0x400000, 0x407fff).m(m_vctrl, FUNC(toaplan_video_controller_device::host_map));
	map(0x440000, 0x440fff).rw(FUNC(toaplan1_state::shared_r), FUNC(toaplan1_state::shared_w)).umask16(0x00ff);
	map(0x480000, 0x48001f).m(m_bcu, FUNC(toaplan_bcu_device::host_map));
	map(0x4c0000, 0x4c0007).m(m_fcu, FUNC(toaplan_fcu_device::host_map));
}
...
void toaplan1_demonwld_state::main_map(address_map &map)
{
	...
	map(0x800000, 0x80001f).m(m_bcu, FUNC(toaplan_bcu_device::host_map));
	map(0xa00000, 0xa00007).m(m_fcu, FUNC(toaplan_fcu_device::host_map));
...
{
	map(0x000000, 0x03ffff).rom();
	map(0x100000, 0x100007).m(m_fcu, FUNC(toaplan_fcu_device::host_map));
	map(0x140000, 0x140fff).rw(FUNC(toaplan1_state::shared_r), FUNC(toaplan1_state::shared_w)).umask16(0x00ff);
	map(0x200000, 0x20001f).m(m_bcu, FUNC(toaplan_bcu_device::host_map));
	map(0x240000, 0x243fff).ram();
	map(0x300000, 0x307fff).m(m_vctrl, FUNC(toaplan_video_controller_device::host_map));
...
void toaplan1_state::vimana_main_map(address_map &map)
{
	...
	map(0x4c0000, 0x4c001f).m(m_bcu, FUNC(toaplan_bcu_device::host_map));
}
...
// HTOTAL and VTOTAL taken from CRTC registers (m_vctrl->vtiming_w)
...
void toaplan1_state::common_video_config(machine_config &config)
{
	...
	m_fcu->set_pri_callback(FUNC(toaplan1_state::pri_cb));
...
void toaplan1_demonwld_state::demonwld(machine_config &config)
{
	...
	TOAPLAN_DSP(config, m_dsp, XTAL(28'000'000) / 2);
	m_dsp->set_host_addr_callback(FUNC(toaplan1_demonwld_state::dsp_host_addr_cb));
	m_dsp->set_host_read_callback(FUNC(toaplan1_demonwld_state::dsp_host_read_cb));
	m_dsp->set_host_write_callback(FUNC(toaplan1_demonwld_state::dsp_host_write_cb));
	...
	m_screen->screen_vblank().set(m_fcu, FUNC(toaplan_fcu_device::vblank));
	m_screen->screen_vblank().append(FUNC(toaplan1_demonwld_state::screen_vblank));

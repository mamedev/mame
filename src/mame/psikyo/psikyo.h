...
public:
	...
	void sngkace(machine_config &config) ATTR_COLD;
	void s1945n(machine_config &config) ATTR_COLD;
	void gunbird(machine_config &config) ATTR_COLD;

	void init_sngkace() ATTR_COLD;
	void init_gunbird() ATTR_COLD;
...
// bootleg hardwares
class psikyo_bootleg_state : public psikyo_state
{
public:
	psikyo_bootleg_state(const machine_config &mconfig, device_type type, const char *tag)
		: psikyo_state(mconfig, type, tag)
		, m_bootleg_spritebuffer(*this, "boot_spritebuf")
		, m_okibank(*this, "okibank%u", 0U)
	{
	}

	void s1945bl(machine_config &config) ATTR_COLD;
	void tengaibl(machine_config &config) ATTR_COLD;

	void init_s1945bl() ATTR_COLD;
	void init_tengaibl() ATTR_COLD;

private:
	/* memory pointers */
	required_shared_ptr<u32> m_bootleg_spritebuffer;

	optional_memory_bank_array<2> m_okibank;

	template <u8 Which> void s1945bl_okibank_w(u8 data);
	...
	template <u8 Which> void s1945bl_oki_map(address_map &map) ATTR_COLD;
	void tengaibl_bootleg_map(address_map &map) ATTR_COLD;
};

// with PIC MCU
class s1945_state : public psikyo_state
{
public:
	s1945_state(const machine_config &mconfig, device_type type, const char *tag)
		: psikyo_state(mconfig, type, tag)
	{
	}

	void s1945(machine_config &config) ATTR_COLD;

	void init_s1945a() ATTR_COLD;
	void init_s1945j() ATTR_COLD;
	void init_s1945() ATTR_COLD;
	void init_tengai() ATTR_COLD;
...
private:
	...
	void s1945_mcu_data_w(u8 data);
	void s1945_mcu_control_w(u8 data);
	void s1945_mcu_direction_w(u8 data);
	void s1945_mcu_bctrl_w(u8 data);
	void s1945_mcu_command_w(u8 data);
	u32 s1945_mcu_data_r();
	u8 s1945_mcu_control_r();

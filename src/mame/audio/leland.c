	...
	switch (offset)
	{
		...
		case 0xa8/2:
			...
			if (i80186.mem.middle_size & 0x0040)
				memory_install_readwrite_handler(machine, 2, ADDRESS_SPACE_PROGRAM, temp, temp + 0x2ff, 0, 0, peripheral_r, peripheral_w);
			else
			{
				temp &= 0xffff;
				memory_install_readwrite_handler(machine, 2, ADDRESS_SPACE_IO, temp, temp + 0x2ff, 0, 0, peripheral_r, peripheral_w);
			}
		...
		case 0xfe/2:
			...
			temp = (data & 0x0fff) << 8;
			if (data & 0x1000)
				memory_install_readwrite_handler(machine, 2, ADDRESS_SPACE_PROGRAM, temp, temp + 0xff, 0, 0, i80186_internal_port_r, i80186_internal_port_w);
			else
			{
				temp &= 0xffff;
				memory_install_readwrite_handler(machine, 2, ADDRESS_SPACE_IO, temp, temp + 0xff, 0, 0, i80186_internal_port_r, i80186_internal_port_w);
			}

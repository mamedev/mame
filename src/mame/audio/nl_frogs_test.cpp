...
NETLIST_START(frogs_jump)
	...
	TTL_INPUT(I_INPUT, 0)
	NET_C(GND, I_INPUT.GND)
	NET_C(I_V5, I_INPUT.VCC)
...
NETLIST_START(frogs_mixer)

	SOLVER(Solver, 1000)
	PARAM(Solver.DYNAMIC_TS, 1)
	PARAM(Solver.DYNAMIC_MIN_TIMESTEP, 4e-6)

	ANALOG_INPUT(BOING, 0)
	ANALOG_INPUT(ZIP, 0)
	ANALOG_INPUT(HOP, 0)
	ANALOG_INPUT(CROAK, 0)
	ANALOG_INPUT(SPLASH, 0)
	ANALOG_INPUT(BUZZZ, 0)

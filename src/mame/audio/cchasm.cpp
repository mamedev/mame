...
void cchasm_state::reset_coin_flag_w(address_space &space, offs_t offset, uint8_t data, uint8_t mem_mask)
{
	if (m_coin_flag)
	{
		m_coin_flag = 0;
...
uint8_t cchasm_state::coin_sound_r(address_space &space, offs_t offset, uint8_t mem_mask)
{
	uint8_t coin = (ioport("IN3")->read() >> 4) & 0x7;
	return m_sound_flags | (m_coin_flag << 3) | coin;
}

uint8_t cchasm_state::soundlatch2_r(address_space &space, offs_t offset, uint8_t mem_mask)
{
	m_sound_flags &= ~0x80;
	m_ctc->trg2(0);
...
void cchasm_state::soundlatch4_w(address_space &space, offs_t offset, uint8_t data, uint8_t mem_mask)
{
	m_sound_flags |= 0x40;
...
void cchasm_state::io_w(address_space &space, offs_t offset, uint16_t data, uint16_t mem_mask)
{
	//static int led;
...
uint16_t cchasm_state::io_r(address_space &space, offs_t offset, uint16_t mem_mask)
{
	switch (offset & 0xf)
	{
	case 0x0:

...
void hyprolyb_adpcm_device::write(address_space &space, offs_t offset, uint8_t data, uint8_t mem_mask)
{
	m_soundlatch2->write(*m_space, offset, data);
	m_adpcm_ready = 0x80;
}

uint8_t hyprolyb_adpcm_device::busy_r(address_space &space, offs_t offset, uint8_t mem_mask)
{
	return m_adpcm_busy ? 0x10 : 0x00;
}

void hyprolyb_adpcm_device::msm_data_w(address_space &space, offs_t offset, uint8_t data, uint8_t mem_mask)
{
	m_msm->data_w(data);
	m_adpcm_busy = ~data & 0x80;
}

uint8_t hyprolyb_adpcm_device::msm_vck_r(address_space &space, offs_t offset, uint8_t mem_mask)
{
...
uint8_t hyprolyb_adpcm_device::ready_r(address_space &space, offs_t offset, uint8_t mem_mask)
{
	return m_adpcm_ready;
}

uint8_t hyprolyb_adpcm_device::data_r(address_space &space, offs_t offset, uint8_t mem_mask)
{
	m_adpcm_ready = 0x00;

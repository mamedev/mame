...
MACHINE_CONFIG_FRAGMENT( qix_audio )
	...
	MCFG_PIA6821_WRITEPA_HANDLER(WRITE8(qix_state, sync_sndpia1_porta_w))
	MCFG_PIA6821_WRITEPB_HANDLER(WRITE8(qix_state, qix_vol_w))
	MCFG_PIA6821_CA2_HANDLER(DEVWRITELINE("sndpia1", pia6821_device, ca1_w))
	MCFG_PIA6821_CB2_HANDLER(WRITELINE(qix_state, qix_flip_screen_w))
	MCFG_PIA6821_IRQA_HANDLER(WRITELINE(qix_state, qix_pia_dint))
	MCFG_PIA6821_IRQB_HANDLER(WRITELINE(qix_state, qix_pia_dint))

	MCFG_DEVICE_ADD("sndpia1", PIA6821, 0)
	MCFG_PIA6821_WRITEPA_HANDLER(DEVWRITE8("sndpia0", pia6821_device, porta_w))
	MCFG_PIA6821_WRITEPB_HANDLER(WRITE8(qix_state,qix_dac_w))
	MCFG_PIA6821_CA2_HANDLER(DEVWRITELINE("sndpia0", pia6821_device, ca1_w))
	MCFG_PIA6821_IRQA_HANDLER(WRITELINE(qix_state, qix_pia_sint))
	MCFG_PIA6821_IRQB_HANDLER(WRITELINE(qix_state, qix_pia_sint))

	MCFG_DEVICE_ADD("sndpia2", PIA6821, 0)
	MCFG_PIA6821_WRITEPA_HANDLER(WRITE8(qix_state, sndpia_2_warning_w))
	MCFG_PIA6821_WRITEPB_HANDLER(WRITE8(qix_state, sndpia_2_warning_w))
	MCFG_PIA6821_CA2_HANDLER(WRITE8(qix_state, sndpia_2_warning_w))
	MCFG_PIA6821_CB2_HANDLER(WRITE8(qix_state, sndpia_2_warning_w))
...
MACHINE_CONFIG_FRAGMENT( slither_audio )
	MCFG_DEVICE_ADD("sndpia0", PIA6821, 0)
	MCFG_PIA6821_READPA_HANDLER(IOPORT("P2"))
	MCFG_PIA6821_WRITEPB_HANDLER(WRITE8(qix_state, slither_coinctl_w))
	MCFG_PIA6821_CB2_HANDLER(WRITELINE(qix_state, qix_flip_screen_w))
	MCFG_PIA6821_IRQA_HANDLER(WRITELINE(qix_state, qix_pia_dint))
	MCFG_PIA6821_IRQB_HANDLER(WRITELINE(qix_state, qix_pia_dint))

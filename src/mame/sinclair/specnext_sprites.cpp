...
specnext_sprites_device::specnext_sprites_device(const machine_config &mconfig, const char *tag, device_t *owner, u32 clock)
    ...
    , device_gfx_interface(mconfig, *this, gfx_sprites, ":palette")
...
void specnext_sprites_device::draw(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
    ...
    for (auto i = 0; i < m_sprites_cache.size(); i++ ) {
        ...
        gfx((spr->rotate << 1) | spr->h)->zoom_transpen(bitmap, clipped
            ...
            , (spr->x & 0x1ff) + m_offset.first, (spr->y & 0x1ff) + m_offset.second
...
void specnext_sprites_device::update_config()
{
    ...
    m_clip_window.set_origin(m_offset.first, m_offset.second);
}

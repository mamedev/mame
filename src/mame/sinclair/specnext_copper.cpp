...
void specnext_copper_device::copper_en_w(u8 data)
{
	if (m_copper_en != data)
	{
		...
		case 0b01:
			LOGCTRL("RESET\n");
			m_timer->adjust(attotime::from_ticks(m_in_until_pos_cb(0x0000), clock()));
			break;
		case 0b10:
			LOGCTRL("START\n");
		...
		case 0b11:
			LOGCTRL("FRAME\n");
			m_frame_timer->adjust(attotime::from_ticks(m_in_until_pos_cb(0x0000), clock()));
			break;
		}
	}
}
...
TIMER_CALLBACK_MEMBER(specnext_copper_device::timer_callback)
{
	...
	{
		m_timer->adjust(attotime::from_ticks(m_in_until_pos_cb(0x8000 | m_copper_list_data), clock()));
		++m_copper_list_addr;
		m_copper_dout = 0;
		return;
	}
...
DEFINE_DEVICE_TYPE(SPECNEXT_COPPER, specnext_copper_device, "copper", "Spectrum Next Copper")

...
void specnext_tiles_device::draw(screen_device &screen, bitmap_rgb32 &bitmap, const rectangle &cliprect, bool over_ula)
{
	...
	if (!clip.empty())
		m_tilemap[BIT(m_control, 6)]->draw(screen, bitmap, clip, TILEMAP_DRAW_CATEGORY(over_ula ? 2 : 1));
}

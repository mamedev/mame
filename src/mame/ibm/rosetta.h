...
public:
	...
	virtual bool mem_load(offs_t address, u8 &data, u8 mask, rsc_mode const mode, bool sp) override { return load<u8>(address, data, mask, mode, sp); }
	virtual bool mem_load(offs_t address, u16 &data, u16 mask, rsc_mode const mode, bool sp) override { return load<u16>(address, data, mask, mode, sp); }
	virtual bool mem_load(offs_t address, u32 &data, u32 mask, rsc_mode const mode, bool sp) override { return load<u32>(address, data, mask, mode, sp); }
	virtual bool mem_store(offs_t address, u8 data, u8 mask, rsc_mode const mode, bool sp) override { return store<u8>(address, data, mask, mode, sp); }
	virtual bool mem_store(offs_t address, u16 data, u16 mask, rsc_mode const mode, bool sp) override { return store<u16>(address, data, mask, mode, sp); }
	virtual bool mem_store(offs_t address, u32 data, u32 mask, rsc_mode const mode, bool sp) override { return store<u32>(address, data, mask, mode, sp); }
	virtual bool mem_modify(offs_t address, std::function<u8(u8)> f, u8 mask, rsc_mode const mode) override { return modify<u8>(address, f, mask, mode); }
	virtual bool mem_modify(offs_t address, std::function<u16(u16)> f, u16 mask, rsc_mode const mode) override { return modify<u16>(address, f, mask, mode); }
	virtual bool mem_modify(offs_t address, std::function<u32(u32)> f, u32 mask, rsc_mode const mode) override { return modify<u32>(address, f, mask, mode); }
...
protected:
	...
	template <typename T> bool load(offs_t address, T &data, T mask, rsc_mode const mode, bool sp);
	template <typename T> bool store(offs_t address, T data, T mask, rsc_mode const mode, bool sp);
	template <typename T> bool modify(offs_t address, std::function<T(T)> f, T mask, rsc_mode const mode);

...
void hp9825_state::hp9825_base(machine_config &config)
{
	...
	TIMER(config , m_cursor_timer).configure_generic(FUNC(hp9825_state::cursor_blink));

	// Keyboard scan timer. A scan of the whole keyboard should take 2^14 KDP clocks.
	TIMER(config , "kb_timer").configure_periodic(FUNC(hp9825_state::kb_scan), attotime::from_ticks(16384 , KDP_CLOCK));
	...
	TIMER(config , m_prt_timer).configure_generic(FUNC(hp9825_state::prt_timer));
	...
	TIMER(config , m_beep_timer).configure_generic(FUNC(hp9825_state::beep_timer));

...
public:
	...
	void pkscramble(machine_config &config) ATTR_COLD;
...
private:
	...
	static constexpr uint8_t m_interrupt_scanline = 192;
	...
	void fgtilemap_w(offs_t offset, uint16_t data, uint16_t mem_mask = ~0);
	void mdtilemap_w(offs_t offset, uint16_t data, uint16_t mem_mask = ~0);
	void bgtilemap_w(offs_t offset, uint16_t data, uint16_t mem_mask = ~0);
...
void pkscram_state::fgtilemap_w(offs_t offset, uint16_t data, uint16_t mem_mask)
{
	COMBINE_DATA(&m_fgtilemap_ram[offset]);
	m_fg_tilemap->mark_tile_dirty(offset >> 1);
}

void pkscram_state::mdtilemap_w(offs_t offset, uint16_t data, uint16_t mem_mask)
{
	COMBINE_DATA(&m_mdtilemap_ram[offset]);
	m_md_tilemap->mark_tile_dirty(offset >> 1);
}

void pkscram_state::bgtilemap_w(offs_t offset, uint16_t data, uint16_t mem_mask)
{
...
void pkscram_state::output_w(uint16_t data)
{
	// OUTPUT
	// BIT
	// 0x0001 -> STL
...
void pkscram_state::program_map(address_map &map)
{
	map.global_mask(0x7ffff);
	map(0x000000, 0x01ffff).rom();
	map(0x040000, 0x0400ff).ram().share("nvram");
	map(0x041000, 0x043fff).ram(); // main RAM
	map(0x044000, 0x044fff).ram().w(FUNC(pkscram_state::fgtilemap_w)).share(m_fgtilemap_ram);
	map(0x045000, 0x045fff).ram().w(FUNC(pkscram_state::mdtilemap_w)).share(m_mdtilemap_ram); // just a copy of fg?
	map(0x046000, 0x046fff).ram().w(FUNC(pkscram_state::bgtilemap_w)).share(m_bgtilemap_ram);
	...
	map(0x049008, 0x049009).w(FUNC(pkscram_state::output_w));
...
static INPUT_PORTS_START( pkscramble )
	PORT_START("DSW")
	PORT_DIPNAME( 0x0007, 0x0003, "Level" ) PORT_DIPLOCATION("DSW1:1,2,3")
	...
	PORT_DIPSETTING(      0x0000, DEF_STR( Off ) ) // only the whistle at boot plays
	PORT_DIPSETTING(      0x0080, DEF_STR( On ) ) // during attract
	PORT_DIPNAME( 0x0700, 0x0100, "Coin to Start" ) PORT_DIPLOCATION("DSW2:1,2,3")
	...
	PORT_DIPNAME( 0x3800, 0x0800, DEF_STR( Coinage ) ) PORT_DIPLOCATION("DSW2:4,5,6")
	...
	PORT_SERVICE( 0x8000, IP_ACTIVE_HIGH ) PORT_DIPLOCATION("DSW2:8")
...
TIMER_DEVICE_CALLBACK_MEMBER(pkscram_state::scanline_callback)
{
	...
	else
	{
		if (m_interrupt_line_active)
			m_maincpu->set_input_line(1, CLEAR_LINE);
		m_scan_timer->adjust(m_screen->time_until_pos(m_interrupt_scanline), m_interrupt_scanline);
		m_interrupt_line_active = 0;
	}
}
...
uint32_t pkscram_state::screen_update(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
...
void pkscram_state::pkscramble(machine_config &config)
{
	...
	m_maincpu->set_addrmap(AS_PROGRAM, &pkscram_state::program_map);
	...
	m_screen->set_screen_update(FUNC(pkscram_state::screen_update));
	...
	ym2203_device &ymsnd(YM2203(config, "ymsnd", 12_MHz_XTAL / 4));

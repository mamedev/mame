...
victor_9000_fdc_device::victor_9000_fdc_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock) :
	...
	m_floppy(*this, M6522_6_TAG":%u", 0U),
	m_gcr_rom(*this, "gcr"),
...
void victor_9000_fdc_device::device_start()
{
	...
	t_scp = timer_alloc(FUNC(victor_9000_fdc_device::scp_tick), this);
...
// This emulates the SCP coming out of reset.  The timer is needed because the
// SCP is not expected to come out of reset and start driving RDY0/RDY1 right
// away.  The SCP is also expected to output zeros on L0MS/L1MS until it comes
// out of reset and reads the values from VIA4.
TIMER_CALLBACK_MEMBER(victor_9000_fdc_device::scp_tick)
{
	m_scp_rdy0 = (m_via_l0ms == 15) ? 0 : 1;
	m_scp_rdy1 = (m_via_l1ms == 15) ? 0 : 1;

    m_scp_l0ms = m_via_l0ms;
    m_scp_l1ms = m_via_l1ms;
}
...
void victor_9000_fdc_device::via4_pa_w(uint8_t data)
{
	...
	*/
	...
	int motor_on = (m_via_l0ms == 0xf) ? 0 : 1;

	if (m_floppy[0]->get_device())
	{
		m_floppy[0]->get_device()->mon_w(!motor_on);
...
void victor_9000_fdc_device::via4_pb_w(uint8_t data)
{
	...
	*/
	...
	int motor_on = (m_via_l1ms == 0xf) ? 0 : 1;

	if (m_floppy[1]->get_device())
	{
		m_floppy[1]->get_device()->mon_w(!motor_on);
...
void victor_9000_fdc_device::via6_pb_w(uint8_t data)
{
	...
	*/
	...
	if (!BIT(data, 2))
	{
		...
		// The SCP may drive L0MS and L1MS as outputs, and is expected to
		// output zeros after reset.
		m_scp_l0ms = 0;
		m_scp_l1ms = 0;

		t_scp->adjust(attotime::from_usec(50));
	}

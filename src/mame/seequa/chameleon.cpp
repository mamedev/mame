...
{
	if(!(offset & 8))
		return bitswap<8>(m_charram[(offset & 7) | ((offset & ~7) >> 1)], 0, 1, 2, 3, 4, 5, 6, 7);
	return 0;
}
...
{
	if(!(offset & 8))
		m_charram[(offset & 7) | ((offset & ~7) >> 1)] = bitswap<8>(data, 0, 1, 2, 3, 4, 5, 6, 7);
}
...
COMP(1983, chameleon, 0, 0, chameleon, 0, chameleon_state, empty_init, "Seequa", "Chameleon", MACHINE_NOT_WORKING)

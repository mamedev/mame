...
#define VERBOSE (LOG_GENERAL | LOG_IDP | LOG_SGP)
...
{
	COMBINE_DATA(&m_sgp.vdp_address[offset]);
}

/*
 * ---- ---x (1) busy flag
 */
u8 pc88va_state::sgp_status_r()
{
	return 0;
}

void pc88va_state::sgp_trigger_w(u8 data)
{
	// TODO: under a timer
	if (BIT(data, 0))
		sgp_exec();
...
void pc88va_state::sgp_exec()
{
	...
	address_space &sgp_space = m_maincpu->space(AS_PROGRAM);
	bool end_issued = false;

	while( vdp_pointer < end_pointer )
	{
		const u16 cur_opcode = sgp_space.read_word(vdp_pointer);
		u16 next_pc = 2;
		...
		switch(cur_opcode)
		{
			...
			case 0x0003:
			{
				...
				// SGP internals ...
				const u16 lower_offset = sgp_space.read_word(vdp_pointer + 2);
				const u16 upper_offset = sgp_space.read_word(vdp_pointer + 4);

				m_sgp.work_address = lower_offset | upper_offset << 16;
			...
			case 0x0005:
			{
				 ...
				 * ---- ---- ---- --11 RGB565
				 */
				const u16 param1 = sgp_space.read_word(vdp_pointer + 2);
				//const u8 start_dot = (param1 & 0xf0) >> 4;
				//const u8 pixel_mode = (param1 & 0x03);
				const u16 src_hsize = sgp_space.read_word(vdp_pointer + 4) & 0x0fff;
				const u16 src_vsize = sgp_space.read_word(vdp_pointer + 6) & 0x0fff;
				const u16 fb_pitch = sgp_space.read_word(vdp_pointer + 8) & 0xfffc;
				const u32 src_address = (sgp_space.read_word(vdp_pointer + 10) & 0xfffe)
					| (sgp_space.read_word(vdp_pointer + 12) << 16);
			...
			case 0x0006:
			{
				const u16 color_code = sgp_space.read_word(vdp_pointer + 2);
			...
			case 0x0008:
			{
				...
				const u16 draw_mode = sgp_space.read_word(vdp_pointer + 2);
			...
			case 0x0009:
			{
				...
				const u16 unk_param = sgp_space.read_word(vdp_pointer + 4);
				// in pixels
				const u16 h_size = sgp_space.read_word(vdp_pointer + 6);
				const u16 v_size = sgp_space.read_word(vdp_pointer + 8);
				const u16 fb_pitch = sgp_space.read_word(vdp_pointer + 10) & 0xfffc;
				const u32 src_address = (sgp_space.read_word(vdp_pointer + 12) & 0xfffe)
					| (sgp_space.read_word(vdp_pointer + 14) << 16);
			...
			case 0x000a:
			{
				const u32 src_address = (sgp_space.read_word(vdp_pointer + 2) & 0xfffe)
					| (sgp_space.read_word(vdp_pointer + 4) << 16);
				const u32 word_size = (sgp_space.read_word(vdp_pointer + 6))
					| (sgp_space.read_word(vdp_pointer + 8) << 16);

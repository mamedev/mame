...
public:
	hvyunit_state(const machine_config &mconfig, device_type type, const char *tag)
		...
		, m_mainbank(*this, "main_bank")
		, m_subbank(*this, "sub_bank")
...
private:
	void trigger_nmi_on_sub_cpu(u8 data);
	...
	void sub_ack_w(u8 data);
	void sound_bankswitch_w(u8 data);
	void mermaid_p0_w(u8 data);
	u8 mermaid_p1_r();
	void mermaid_p1_w(u8 data);
	u8 mermaid_p2_r();
	void mermaid_p2_w(u8 data);
	u8 mermaid_p3_r();
	void mermaid_p3_w(u8 data);
	...
	void sub_memory(address_map &map) ATTR_COLD;
...
void hvyunit_state::machine_start()
{
	...
	m_subbank->configure_entries(0, 4, memregion("subcpu")->base(), 0x4000);
...
{
	u32 const attr = m_colorram[tile_index];
	u32 const code = m_videoram[tile_index] + ((attr & 0x0f) << 8);
	u32 const color = (attr >> 4);
...
u32 hvyunit_state::screen_update(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
...
void hvyunit_state::trigger_nmi_on_sub_cpu(u8 data)
{
	m_subcpu->pulse_input_line(INPUT_LINE_NMI, attotime::zero);
}

void hvyunit_state::main_bankswitch_w(u8 data)
{
	m_mainbank->set_entry(data & 7);
}

u8 hvyunit_state::mermaid_status_r()
{
	return (!m_sublatch->pending_r() << 2) | (m_mermaidlatch->pending_r() << 3);
}
...
void hvyunit_state::videoram_w(offs_t offset, u8 data)
{
	m_videoram[offset] = data;
	m_bg_tilemap->mark_tile_dirty(offset);
}

void hvyunit_state::colorram_w(offs_t offset, u8 data)
{
	m_colorram[offset] = data;
	m_bg_tilemap->mark_tile_dirty(offset);
}

void hvyunit_state::sub_bankswitch_w(u8 data)
{
	m_port0_data = data;
	m_subbank->set_entry(data & 3);
}

void hvyunit_state::scrollx_w(u8 data)
{
	m_scrollx = data;
}

void hvyunit_state::scrolly_w(u8 data)
{
	m_scrolly = data;
}

void hvyunit_state::sub_ack_w(u8 data)
{
...
void hvyunit_state::sound_bankswitch_w(u8 data)
{
...
void hvyunit_state::mermaid_p0_w(u8 data)
{
	if (!BIT(m_mermaid_p[0], 1) && BIT(data, 1))
		m_sublatch->write(m_mermaid_p[1]);
...
u8 hvyunit_state::mermaid_p1_r()
{
	return m_mermaid_p[1];
}

void hvyunit_state::mermaid_p1_w(u8 data)
{
	m_mermaid_p[1] = data;
}

u8 hvyunit_state::mermaid_p2_r()
{
...
void hvyunit_state::mermaid_p2_w(u8 data)
{
	m_mermaid_p[2] = data;
}

u8 hvyunit_state::mermaid_p3_r()
{
	u8 dsw = 0;
	u8 const dsw1 = m_port_dsw[0]->read();
	u8 const dsw2 = m_port_dsw[1]->read();
	...
	return (dsw << 4) | (m_sublatch->pending_r() << 3) | (!m_mermaidlatch->pending_r() << 2);
}

void hvyunit_state::mermaid_p3_w(u8 data)
{
	m_mermaid_p[3] = data;
...
void hvyunit_state::main_memory(address_map &map)
{
...
void hvyunit_state::main_io(address_map &map)
{
	map.global_mask(0xff);
	map(0x00, 0x00).w(FUNC(hvyunit_state::main_bankswitch_w));
	map(0x01, 0x01).w(FUNC(hvyunit_state::main_bankswitch_w)); // correct?
	map(0x02, 0x02).w(FUNC(hvyunit_state::trigger_nmi_on_sub_cpu));
}


void hvyunit_state::sub_memory(address_map &map)
{
	map(0x0000, 0x7fff).rom();
...
void hvyunit_state::sub_io(address_map &map)
{
	map.global_mask(0xff);
	map(0x00, 0x00).w(FUNC(hvyunit_state::sub_bankswitch_w));
	map(0x02, 0x02).w(m_soundlatch, FUNC(generic_latch_8_device::write));
	map(0x04, 0x04).r(m_sublatch, FUNC(generic_latch_8_device::read)).w(m_mermaidlatch, FUNC(generic_latch_8_device::write));
	...
	map(0x0e, 0x0e).w(FUNC(hvyunit_state::sub_ack_w));

//  map(0x22, 0x22).r(FUNC(hvyunit_state::scrolly_hi_reset)); //22/a2 taken from ram $f065
//  map(0xa2, 0xa2).r(FUNC(hvyunit_state::scrolly_hi_set));
}
...
{
	Z80(config, m_maincpu, XTAL(12'000'000)/2); // 6MHz verified on PCB
	m_maincpu->set_addrmap(AS_PROGRAM, &hvyunit_state::main_memory);
	m_maincpu->set_addrmap(AS_IO, &hvyunit_state::main_io);
	TIMER(config, "scantimer").configure_scanline(FUNC(hvyunit_state::scanline), "screen", 0, 1);

	Z80(config, m_subcpu, XTAL(12'000'000)/2); // 6MHz verified on PCB
	m_subcpu->set_addrmap(AS_PROGRAM, &hvyunit_state::sub_memory);
	m_subcpu->set_addrmap(AS_IO, &hvyunit_state::sub_io);
	m_subcpu->set_vblank_int("screen", FUNC(hvyunit_state::irq0_line_assert));
...
ROM_START( hvyunit )
	...
	ROM_LOAD( "b73_12.7e",  0x00000, 0x10000, CRC(d1d24fab) SHA1(ed0312535d0b136d79aa885b9e6eea19ebde6409) )
...
ROM_START( hvyunitja )
	...
	ROM_REGION( 0x200000, "sprites", ROMREGION_ERASE00 ) // The data in first half of b73_15.0b actually differs slightly to the other sets, a 0x22 fill is replaced by 0xff on empty tiles

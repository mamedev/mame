...
void kaneko_calc3_device::mcu_com_w(offs_t offset, uint16_t data, uint16_t mem_mask, int _n_)
{
...
/* this should probably be derived from the final block at the end of the MCU rom somehow ...
    (see 'finalblock' debug output when DUMP_TABLES = 1)
...
static const int16_t f_calc3_keydata[0x100][0x40] = {
...
int kaneko_calc3_device::decompress_table(int tabnum, uint8_t* dstram, int dstoffset)
{
	...
	uint8_t numregions = datarom[offset + 0];
	...
	// we're at the start of the block, get the info about it
	{
		...
		//if (dstram)
		{
			if (length == 0x00)
			{
				...
				else if (m_mode == 0x08 && !dstram)
				{
...

					{
						...
						{
							m_eeprom->internal_write(i, space.read_word(m_eeprom_addr + 0x200000 + 2*i));
						}
			...
			{
				const int16_t (&key)[0x40] = f_calc3_keydata[m_decryption_key_byte];
...
void kaneko_calc3_device::mcu_run()
{
	...
	uint16_t mcu_command = m_mcuram[m_mcu_command_offset / 2 + 0];
	...
	if (mcu_command > 0)
	{
		...
		if (mcu_command == 0xff)
		{
			...
			m_writeaddress =      (m_mcuram[(0>>1) + 6] << 16) | (m_mcuram[(0>>1) + 7]);

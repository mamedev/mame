...
public:
	djboy_state(const machine_config &mconfig, device_type type, const char *tag)
		...
		, m_mainbank(*this, "main_bank%u", 0U)
...
private:
	...
	u8 beast_status_r();
	void trigger_nmi_on_maincpu(u8 data);
	void maincpu_bankswitch_w(u8 data);
	void subcpu_bankswitch_w(u8 data);
	void coin_count_w(u8 data);
	void soundcpu_bankswitch_w(u8 data);
	u8 beast_p0_r();
	void beast_p0_w(u8 data);
	u8 beast_p1_r();
	void beast_p1_w(u8 data);
	u8 beast_p2_r();
	void beast_p2_w(u8 data);
	u8 beast_p3_r();
	void beast_p3_w(u8 data);
	...
	void maincpu_port_am(address_map &map) ATTR_COLD;
	void subcpu_am(address_map &map) ATTR_COLD;
	void subcpu_port_am(address_map &map) ATTR_COLD;
...
void djboy_state::scrollx_w(u8 data)
{
	m_scrollx = data;
}

void djboy_state::scrolly_w(u8 data)
{
	m_scrolly = data;
}
...
{
	u8 const attr = m_videoram[tile_index + 0x800];
	u32 code = m_videoram[tile_index] + ((attr & 0xf) << 8);
...
void djboy_state::videoram_w(offs_t offset, u8 data)
{
...
u32 djboy_state::screen_update(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
...
u8 djboy_state::beast_status_r()
{
	return (m_sublatch->pending_r() ? 0x0 : 0x4) | (m_beastlatch->pending_r() ? 0x8 : 0x0);
}

/******************************************************************************/

void djboy_state::trigger_nmi_on_maincpu(u8 data)
{
	m_maincpu->pulse_input_line(INPUT_LINE_NMI, attotime::zero);
}

void djboy_state::maincpu_bankswitch_w(u8 data)
{
	data ^= m_bankxor;
	m_mainbank[0]->set_entry(data);
	m_mainbank[1]->set_entry(0); // unsure if / how this area is banked
}
 ...
 * ----xxxx bank
 */
void djboy_state::subcpu_bankswitch_w(u8 data)
{
	m_videoreg = data;

	if ((data & 0xc) != 4)
		m_subbank->set_entry((data & 0xf));
}

void djboy_state::coin_count_w(u8 data)
{
...
void djboy_state::soundcpu_bankswitch_w(u8 data)
{
...
void djboy_state::maincpu_am(address_map &map)
{
	map(0x0000, 0x7fff).rom();
	map(0x8000, 0xafff).bankr(m_mainbank[1]);
	map(0xb000, 0xbfff).rw(m_pandora, FUNC(kaneko_pandora_device::spriteram_r), FUNC(kaneko_pandora_device::spriteram_w));
	map(0xc000, 0xdfff).bankr(m_mainbank[0]);
	map(0xe000, 0xffff).ram().share("main_sub");
}

void djboy_state::maincpu_port_am(address_map &map)
{
	map.global_mask(0xff);
	map(0x00, 0x00).w(FUNC(djboy_state::maincpu_bankswitch_w));
}

/******************************************************************************/

void djboy_state::subcpu_am(address_map &map)
{
	map(0x0000, 0x7fff).rom();
...
void djboy_state::subcpu_port_am(address_map &map)
{
	map.global_mask(0xff);
	map(0x00, 0x00).w(FUNC(djboy_state::subcpu_bankswitch_w));
	map(0x02, 0x02).w(m_soundlatch, FUNC(generic_latch_8_device::write));
	map(0x04, 0x04).r(m_sublatch, FUNC(generic_latch_8_device::read)).w(m_beastlatch, FUNC(generic_latch_8_device::write));
	map(0x06, 0x06).w(FUNC(djboy_state::scrolly_w));
	map(0x08, 0x08).w(FUNC(djboy_state::scrollx_w));
	map(0x0a, 0x0a).w(FUNC(djboy_state::trigger_nmi_on_maincpu));
...
u8 djboy_state::beast_p0_r()
{
	// ?
	return 0;
}

void djboy_state::beast_p0_w(u8 data)
{
	if (!BIT(m_beast_p[0], 1) && BIT(data, 1))
	{
		m_sublatch->write(m_beast_p[1]);
	}
...
void djboy_state::beast_p1_w(u8 data)
{
	m_beast_p[1] = data;
}

u8 djboy_state::beast_p2_r()
{
...
void djboy_state::beast_p2_w(u8 data)
{
	m_beast_p[2] = data;
}

u8 djboy_state::beast_p3_r()
{
	u8 dsw = 0;
	...
	return (dsw << 4) | (m_beastlatch->pending_r() ? 0x0 : 0x4) | (m_sublatch->pending_r() ? 0x8 : 0x0);
}

void djboy_state::beast_p3_w(u8 data)
{
	m_beast_p[3] = data;
...
void djboy_state::machine_start()
{
	...
	u8 *sound = memregion("soundcpu")->base();

	m_mainbank[0]->configure_entries(0, 32, &main[0x00000], 0x2000);
	m_subbank->configure_entries(0, 4, &sub[0x00000], 0x4000); // TODO: Mirrored?
	m_subbank->configure_entries(8, 8, &sub[0x10000], 0x4000);
	m_soundbank->configure_entries(0, 8, &sound[0x00000], 0x4000);
	m_mainbank[1]->configure_entry(0, &main[0x08000]); // unsure if / how this area is banked
...
{
	Z80(config, m_maincpu, 12_MHz_XTAL / 2); // 6.000MHz, verified
	m_maincpu->set_addrmap(AS_PROGRAM, &djboy_state::maincpu_am);
	m_maincpu->set_addrmap(AS_IO, &djboy_state::maincpu_port_am);
	TIMER(config, "scantimer").configure_scanline(FUNC(djboy_state::scanline), "screen", 0, 1);

	Z80(config, m_subcpu, 12_MHz_XTAL / 2); // 6.000MHz, verified
	m_subcpu->set_addrmap(AS_PROGRAM, &djboy_state::subcpu_am);
	m_subcpu->set_addrmap(AS_IO, &djboy_state::subcpu_port_am);
	m_subcpu->set_vblank_int("screen", FUNC(djboy_state::irq0_line_hold));

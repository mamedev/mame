...
static INPUT_PORTS_START( coco3_keyboard )
	...
	PORT_BIT(0x08, IP_ACTIVE_LOW, IPT_KEYBOARD) PORT_NAME("ALT") PORT_CODE(KEYCODE_LALT) PORT_CHAR(UCHAR_MAMEKEY(LALT))
	PORT_BIT(0x10, IP_ACTIVE_LOW, IPT_KEYBOARD) PORT_NAME("CTRL") PORT_CODE(KEYCODE_LCONTROL) PORT_CHAR(UCHAR_MAMEKEY(LCONTROL), UCHAR_SHIFT_2)
...
INPUT_PORTS_START( coco3_analog_control )
...
INPUT_PORTS_START( coco3_joystick )
	PORT_START(JOYSTICK_RX_TAG)
		...
		PORT_CHANGED_MEMBER(DEVICE_SELF, FUNC(coco_state::joystick_changed), 0)
		PORT_SENSITIVITY(JOYSTICK_SENSITIVITY)
		PORT_KEYDELTA(JOYSTICK_DELTA)
		PORT_MINMAX(0,1023)
		PORT_CODE_DEC(KEYCODE_4_PAD)
		PORT_CODE_INC(KEYCODE_6_PAD)
		PORT_CODE_DEC(JOYCODE_X_LEFT_SWITCH)
		PORT_CODE_INC(JOYCODE_X_RIGHT_SWITCH)
		PORT_PLAYER(1)
	...
	PORT_START(JOYSTICK_RY_TAG)
		...
		PORT_CHANGED_MEMBER(DEVICE_SELF, FUNC(coco_state::joystick_changed), 1)
		PORT_SENSITIVITY(JOYSTICK_SENSITIVITY)
		PORT_KEYDELTA(JOYSTICK_DELTA)
		PORT_MINMAX(0,1023)
		PORT_CODE_DEC(KEYCODE_8_PAD)
		PORT_CODE_INC(KEYCODE_2_PAD)
		PORT_CODE_DEC(JOYCODE_Y_UP_SWITCH)
		PORT_CODE_INC(JOYCODE_Y_DOWN_SWITCH)
		PORT_PLAYER(1)
 		PORT_CONDITION(CTRL_SEL_RIGHT, 0x0f, LESSTHAN, coco_state::JOY_DEVICE_RAT_MOUSE)
	PORT_START(JOYSTICK_LX_TAG)
		PORT_BIT(0x3ff, 0x200,  IPT_AD_STICK_X)
		PORT_NAME("Left Joystick X")
		PORT_CHANGED_MEMBER(DEVICE_SELF, FUNC(coco_state::joystick_changed), 2)
		PORT_SENSITIVITY(JOYSTICK_SENSITIVITY)
		PORT_KEYDELTA(JOYSTICK_DELTA)
		PORT_MINMAX(0,1023)
		PORT_CODE_DEC(KEYCODE_4_PAD)
	...
	PORT_START(JOYSTICK_LY_TAG)
		...
		PORT_CHANGED_MEMBER(DEVICE_SELF, FUNC(coco_state::joystick_changed), 3)
		PORT_SENSITIVITY(JOYSTICK_SENSITIVITY)
		PORT_KEYDELTA(JOYSTICK_DELTA)
...
static INPUT_PORTS_START( coco_lightgun )
	PORT_START(DIECOM_LIGHTGUN_RX_TAG)
		PORT_BIT(0xfff, 266, IPT_LIGHTGUN_X)
			PORT_NAME("Lightgun X (Right Port)")
	...
	PORT_START(DIECOM_LIGHTGUN_RY_TAG)
		PORT_BIT(0xff, 121, IPT_LIGHTGUN_Y)
			PORT_NAME("Lightgun Y (Right Port)")
	...
	PORT_START(DIECOM_LIGHTGUN_BUTTONS_TAG)
		PORT_BIT(0x01, IP_ACTIVE_HIGH, IPT_BUTTON1)
			PORT_NAME("Lightgun Trigger (Right Port)")
			PORT_CHANGED_MEMBER(DEVICE_SELF, FUNC(coco_state::joystick_changed), 0)
			PORT_CODE(KEYCODE_0_PAD)
			PORT_CODE(JOYCODE_BUTTON1)
			PORT_CODE(MOUSECODE_BUTTON1)
...
void coco3_state::coco3(machine_config &config)
{
	...
	m_mux->z_analog_callback().set(FUNC(coco_state::pia0_pa7_w));
	m_mux->set_analog_width(mc14529_device::SEL_X, 6 /* bits */);
	m_mux->set_mode(mc14529_device::SEL_Y, mc14529_device::MODE_SOUND);
	m_mux->add_route(mc14529_device::w_sound_output(), "speaker", 1.0);

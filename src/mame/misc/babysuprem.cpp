...
/*
    ...
    Note there are a nuumber of errors in the manual/schematic.
...
private:
	...
	void ay8910_u2_porta_w(uint8_t data);
	void ay8910_u2_portb_w(uint8_t data);
...
static INPUT_PORTS_START( bsuprem )
	...
	PORT_BIT(0x01, IP_ACTIVE_LOW, IPT_BUTTON3) PORT_NAME("Right stop")
	PORT_BIT(0x02, IP_ACTIVE_LOW, IPT_BUTTON2) PORT_NAME("Middle stop")
	PORT_BIT(0x04, IP_ACTIVE_LOW, IPT_BUTTON1) PORT_NAME("Left stop")
	PORT_BIT(0x08, IP_ACTIVE_LOW, IPT_BUTTON4)
	PORT_DIPNAME(0x80, 0x80, "SW1-3 (Test 4)")
	PORT_DIPSETTING(   0x00, DEF_STR( On ))
	PORT_DIPSETTING(   0x80, DEF_STR( Off ))
	PORT_DIPNAME(0x40, 0x40, "SW1-1 (Test 2)")
	PORT_DIPSETTING(   0x00, DEF_STR( On ))
	PORT_DIPSETTING(   0x40, DEF_STR( Off ))
	PORT_DIPNAME(0x20, 0x20, "SW1-2 (Test 3)")
	PORT_DIPSETTING(   0x00, DEF_STR( On ))
	PORT_DIPSETTING(   0x20, DEF_STR( Off ))
	PORT_DIPNAME(0x10, 0x10, "SW1-4 (Test 5)")
	PORT_DIPSETTING(   0x00, DEF_STR( On ))
	PORT_DIPSETTING(   0x10, DEF_STR( Off ))

	PORT_START("DSW2")
	PORT_DIPNAME(0x80, 0x80, "SW2-2 (Door)")
	PORT_DIPSETTING(   0x00, DEF_STR( On ))
	PORT_DIPSETTING(   0x80, DEF_STR( Off ))
	PORT_DIPNAME(0x40, 0x40, "SW2-4")
	PORT_DIPSETTING(   0x00, DEF_STR( On ))
	PORT_DIPSETTING(   0x40, DEF_STR( Off ))
	PORT_DIPNAME(0x20, 0x20, "SW2-6 Auto start")
	PORT_DIPSETTING(   0x00, DEF_STR( Yes ))
	PORT_DIPSETTING(   0x20, DEF_STR( No ))
	PORT_DIPNAME(0x10, 0x10, "SW2-8 Percent")
	PORT_DIPSETTING(   0x00, "80%")
	PORT_DIPSETTING(   0x10, "85%")
	PORT_DIPNAME(0x08, 0x08, "SW2-7")
	PORT_DIPSETTING(   0x00, DEF_STR( On ))
	PORT_DIPSETTING(   0x08, DEF_STR( Off ))
	PORT_DIPNAME(0x04, 0x04, "SW2-5 Test")
	PORT_DIPSETTING(   0x00, DEF_STR( On ))
	PORT_DIPSETTING(   0x04, DEF_STR( Off ))
	PORT_DIPNAME(0x02, 0x02, "SW2-3 Instant start")
	PORT_DIPSETTING(   0x00, DEF_STR( Yes ))
	PORT_DIPSETTING(   0x02, DEF_STR( No ))
	PORT_DIPNAME(0x01, 0x01, "SW2-1 (Bet/Collect)")
	PORT_DIPSETTING(   0x00, DEF_STR( On ))
	PORT_DIPSETTING(   0x01, DEF_STR( Off ))

	PORT_START("U13_SWITCHES")
	PORT_BIT(0x01, IP_ACTIVE_LOW, IPT_BUTTON5)  PORT_NAME("Bet/Collect")
	PORT_BIT(0x02, IP_ACTIVE_LOW, IPT_BUTTON6)  PORT_NAME("Start")
	PORT_BIT(0x80, IP_ACTIVE_LOW, IPT_SERVICE)  PORT_NAME("Door") PORT_CODE(KEYCODE_D) PORT_TOGGLE
...
void bsuprem_state::bsuprem(machine_config &config)
{
	...
	m_psg[1]->port_a_write_callback().set(FUNC(bsuprem_state::ay8910_u2_porta_w));
	m_psg[1]->port_b_write_callback().set(FUNC(bsuprem_state::ay8910_u2_portb_w));
}
...
void bsuprem_state::bsuprem_map(address_map &map)
{
	...
	map(0x7000, 0x77ff).mirror(0x0800).rw(FUNC(bsuprem_state::nvram_r), FUNC(bsuprem_state::nvram_w));
...
void bsuprem_state::machine_start()
{
	...
	m_nvram8 = std::make_unique<uint8_t[]>(2048);
	m_nvram->set_base(m_nvram8.get(),2048);
...
void bsuprem_state::nvram_w(offs_t offset, uint8_t data)
{
	// according to the schematic the ram c/s is disabled unless
	// bit 6 of u10 is 1. However, implementing this makes it impossible
	// to enter test/setup. As there are a number of other errors on
	// the schematic it's likely this is wrong.
	m_nvram8[offset] = data;
}

uint8_t bsuprem_state::nvram_r(offs_t offset)
{
...
{
	return m_dsw2->read() & (0x7c | m_u13_switches->read());
}
...
void bsuprem_state::ay8910_u1_portb_w(uint8_t data)
{
	...
	switch(data)
	{
		...
		case 0x0a:
			m_digits[4]=patterns[(m_u1_porta_data>>4)&0x0f];
			m_digits[5]=patterns[m_u1_porta_data&0x0f];
			break;

		case 0x0b:
			m_digits[2]=patterns[(m_u1_porta_data>>4)&0x0f];
			m_digits[3]=patterns[m_u1_porta_data&0x0f];
			break;

		case 0x0c:
			m_digits[0]=patterns[(m_u1_porta_data>>4)&0x0f];
			m_digits[1]=patterns[m_u1_porta_data&0x0f];
			break;
	}
}
...
void bsuprem_state::ay8910_u2_porta_w(uint8_t data)
{
}

void bsuprem_state::ay8910_u2_portb_w(uint8_t data)
{
}

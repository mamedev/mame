...
#define LOG_LAMPS     (1U << 1)

// #define VERBOSE (LOG_GENERAL | LOG_LAMPS)
...
public:
	...
	void chance32(machine_config &config) ATTR_COLD;
...
private:
	...
	void key_matrix_w(u8 data);
	u8 key_matrix_r();
	void lamps_ff_w(u8 data);
...
u32 chance32_state::screen_update(screen_device &screen, bitmap_rgb32 &bitmap, const rectangle &cliprect)
{
...
void chance32_state::key_matrix_w(u8 data)
{
	m_port_select = data;
}

u8 chance32_state::key_matrix_r()
{
	u8 res = 0;
...
*/
void chance32_state::lamps_ff_w(u8 data)
{
	...
	else
	{
		...
		LOGLAMPS("Lamps B: %02x\n", data);
	}
}

void chance32_state::hopper_w(u8 data)
{
	machine().bookkeeping().coin_counter_w(0, BIT(data, 0)); // coin in (only coin 1, though)
	machine().bookkeeping().coin_counter_w(0, BIT(data, 1)); // key in

	// bit3 constantly written... watchdog?
...
void chance32_state::program_map(address_map &map)
{
...
void chance32_state::io_map(address_map &map)
{
	map.global_mask(0xff);
	map(0x10, 0x10).w(FUNC(chance32_state::hopper_w));
	...
	map(0x26, 0x26).portr("IN4"); // vblank (other bits are checked for different reasons)
...
// TODO: DIP locations are unconfirmed
static INPUT_PORTS_START( chance32 )
	PORT_START("DSW0")
	PORT_DIPNAME( 0x01, 0x00, DEF_STR( Unknown ) ) PORT_DIPLOCATION("SW0:1")
	...
	PORT_BIT( 0x10, IP_ACTIVE_HIGH, IPT_COIN2 ) PORT_IMPULSE(3) PORT_NAME("Coin B") // or service? doesn't have a counter
	...
	PORT_START("IN4")
	PORT_DIPNAME( 0x01, 0x00, "Freeze?" ) // S6? - checked at end of irq routine
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x01, DEF_STR( On ) )
	...
	PORT_BIT( 0x10, IP_ACTIVE_HIGH, IPT_UNKNOWN )
	PORT_BIT( 0x20, IP_ACTIVE_HIGH, IPT_UNKNOWN )
	PORT_BIT( 0x40, IP_ACTIVE_HIGH, IPT_UNKNOWN ) // TODO: this is checked at 0x8b9e. For what?
...
static GFXDECODE_START( gfx_chance32 )
	GFXDECODE_ENTRY( "bgtiles", 0, tiles8x8_layout, 0, 8 )
	GFXDECODE_ENTRY( "fgtiles", 0, tiles8x8_layout, 0, 8 )
GFXDECODE_END
...
void chance32_state::chance32(machine_config &config)
{
	...
	m_maincpu->set_addrmap(AS_PROGRAM, &chance32_state::program_map);
	m_maincpu->set_addrmap(AS_IO, &chance32_state::io_map);
	m_maincpu->set_vblank_int("screen", FUNC(chance32_state::irq0_line_hold));

	NVRAM(config, "nvram", nvram_device::DEFAULT_ALL_0);

	MSM58321(config, "rtc", 32.768_kHz_XTAL);
...
//  screen.set_refresh_hz(60);
	...
	hd6845s_device &crtc(HD6845S(config, "crtc", 12_MHz_XTAL / 16)); // 52.786 Hz (similar to Major Poker)
...
ROM_START( chance32 )
	...
	ROM_REGION( 0x80000, "bgtiles", ROMREGION_INVERT )
	...
	ROM_REGION( 0x40000, "fgtiles", ROMREGION_INVERT )
	...
	ROM_REGION( 0x1400, "gals", ROMREGION_ERASE00 ) // no idea if these are any good
	ROM_LOAD( "gal20v8a.u53.jed.bin", 0x0000, 0x157, CRC(9f680800) SHA1(2fa41ead85136e851d465432a7b9d3ec848c7a22) )
	ROM_LOAD( "gal20v8a.u54.jed.bin", 0x0200, 0x157, CRC(9f680800) SHA1(2fa41ead85136e851d465432a7b9d3ec848c7a22) )
	ROM_LOAD( "gal20v8a.u55.jed.bin", 0x0400, 0x157, CRC(9f680800) SHA1(2fa41ead85136e851d465432a7b9d3ec848c7a22) )
	ROM_LOAD( "gal20v8a.u56.jed.bin", 0x0600, 0x157, CRC(6bab01ad) SHA1(c69e4be41a989a52788af8062f48bbe26bc3dab8) )
	ROM_LOAD( "gal20v8a.u57.jed.bin", 0x0800, 0x157, CRC(787c4159) SHA1(f4a869b317c6be1024f1ca21bcc4af478c8227c8) )
	ROM_LOAD( "gal20v8a.u58.jed.bin", 0x0a00, 0x157, CRC(7b16053b) SHA1(cdb289d4f27c7a1a918393943bb8db9712e2f52e) )

	ROM_LOAD( "gal16v8a.u47.jed.bin", 0x0c00, 0x117, NO_DUMP )
	ROM_LOAD( "gal16v8a.u48.jed.bin", 0x0e00, 0x117, CRC(5f1360ef) SHA1(56e4ee0dbae5602d810b2f7c744a71eb1a1e08a8) )

	ROM_LOAD( "gal16v8a.u32.jed.bin", 0x1000, 0x117, CRC(c0784cd1) SHA1(0ae2ce482d379e29c2a9f130fc0d9ed928faef98) )

	ROM_LOAD( "gal16v8a.u24.jed.bin", 0x1200, 0x117, NO_DUMP )
ROM_END
...
GAMEL( 19??, chance32, 0,      chance32, chance32, chance32_state, empty_init, ROT0, "PAL System Co, Ltd.", "Chance Thirty Two", MACHINE_SUPPORTS_SAVE, layout_chance32 )

...
/*
  ...
  - hrthouse shows incorrect sprite / tilemaps priorities
  - hrthouse shows incorrect sprite scrolling
...
class limenko_state : public base_state
{
public:
	...
	void limenko(machine_config &config) ATTR_COLD;
	void hrthouse(machine_config &config) ATTR_COLD;

	void init_common() ATTR_COLD;
	void init_dynabomb() ATTR_COLD;
	void init_hrthouse() ATTR_COLD;
	void init_legendoh() ATTR_COLD;
	void init_sb2003() ATTR_COLD;
...
private:
	...
	u32 hrthouse_speedup_r();
	u32 legendoh_speedup_r();
	u32 sb2003_speedup_r();

	void io_map(address_map &map) ATTR_COLD;
	void program_map(address_map &map) ATTR_COLD;
	void hrthouse_program_map(address_map &map) ATTR_COLD;
};

class spotty_state : public base_state
{
public:
	spotty_state(const machine_config &mconfig, device_type type, const char *tag)
		: base_state(mconfig, type, tag)
		, m_oki(*this, "oki")
	{
	}

	void spotty(machine_config &config) ATTR_COLD;

	void init_spotty() ATTR_COLD;
...
private:
	...
	u8 audiocpu_p1_r();
	void audiocpu_p1_w(u8 data);
	u8 audiocpu_p3_r();
	void audiocpu_p3_w(u8 data);
...
{
	machine().bookkeeping().coin_counter_w(0, BIT(data, 16));
}


void base_state::bg_videoram_w(offs_t offset, u32 data, u32 mem_mask)
{
	COMBINE_DATA(&m_bg_videoram[offset]);
	m_bg_tilemap->mark_tile_dirty(offset);
}

void base_state::md_videoram_w(offs_t offset, u32 data, u32 mem_mask)
{
	COMBINE_DATA(&m_md_videoram[offset]);
	m_md_tilemap->mark_tile_dirty(offset);
}

void base_state::fg_videoram_w(offs_t offset, u32 data, u32 mem_mask)
{
	COMBINE_DATA(&m_fg_videoram[offset]);
	m_fg_tilemap->mark_tile_dirty(offset);
}

int base_state::spriteram_bit_r()
{
	return m_spriteram_bit;
}

void base_state::spriteram_buffer_w(u32 data)
{
	// toggle spriteram's location in the memory map
	m_spriteram_bit ^= 1;
	...
	m_prev_sprites_count = (m_videoreg[0] & 0x3ff0000) >> 16; // was & 0x1ff0000 but hrthouse writes 0x2000000
}
...
u8 spotty_state::audiocpu_p1_r()
{
	return m_audiocpu_p1;
}

void spotty_state::audiocpu_p1_w(u8 data)
{
	m_audiocpu_p1 = data;
}
...
u8 spotty_state::audiocpu_p3_r()
{
	return (m_soundlatch->pending_r() ? 0 : 1) << 2;
}

void spotty_state::audiocpu_p3_w(u8 data)
{
...
void limenko_state::program_map(address_map &map)
{
	map(0x00000000, 0x001fffff).ram().share(m_mainram);
	map(0x40000000, 0x403fffff).rom().region("maindata", 0);
	map(0x80000000, 0x80007fff).ram().w(FUNC(limenko_state::fg_videoram_w)).share(m_fg_videoram);
	map(0x80008000, 0x8000ffff).ram().w(FUNC(limenko_state::md_videoram_w)).share(m_md_videoram);
	map(0x80010000, 0x80017fff).ram().w(FUNC(limenko_state::bg_videoram_w)).share(m_bg_videoram);
	map(0x80018000, 0x80019fff).ram().share(m_spriteram);
	...
	map(0x8001ffec, 0x8001ffff).ram().share(m_videoreg);
	map(0x8003e000, 0x8003e003).w(FUNC(limenko_state::spriteram_buffer_w));
	map(0xffe00000, 0xffffffff).rom().region("maincpu", 0);
}

void limenko_state::hrthouse_program_map(address_map &map)
{
	map(0x00000000, 0x001fffff).ram().share(m_mainram);
	map(0x40000000, 0x40000fff).ram().share("nvram");
	map(0x80000000, 0x80007fff).ram().w(FUNC(limenko_state::fg_videoram_w)).share(m_fg_videoram);
	map(0x80008000, 0x8000ffff).ram().w(FUNC(limenko_state::md_videoram_w)).share(m_md_videoram);
	map(0x80010000, 0x80017fff).ram().w(FUNC(limenko_state::bg_videoram_w)).share(m_bg_videoram);
	map(0x80018000, 0x80019fff).ram().share(m_spriteram);
...
void limenko_state::io_map(address_map &map)
{
...
void spotty_state::program_map(address_map &map)
{
	map(0x00000000, 0x001fffff).ram().share(m_mainram);
	map(0x40002000, 0x400024d3).ram(); //?
	map(0x80000000, 0x80007fff).ram().w(FUNC(spotty_state::fg_videoram_w)).share(m_fg_videoram);
	map(0x80008000, 0x8000ffff).ram().w(FUNC(spotty_state::md_videoram_w)).share(m_md_videoram);
	map(0x80010000, 0x80017fff).ram().w(FUNC(spotty_state::bg_videoram_w)).share(m_bg_videoram);
	map(0x80018000, 0x80019fff).ram().share(m_spriteram);
	map(0x8001c000, 0x8001dfff).ram().w(m_palette, FUNC(palette_device::write32)).share("palette");
	map(0x8001e000, 0x8001ebff).ram(); // ? not used
	map(0x8001ffec, 0x8001ffff).ram().share(m_videoreg);
	map(0x8003e000, 0x8003e003).w(FUNC(spotty_state::spriteram_buffer_w));
	map(0xfff00000, 0xffffffff).rom().region("maincpu", 0);
}

void spotty_state::io_map(address_map &map)
{
	map(0x0000, 0x0000).portr("IN0");
	map(0x0200, 0x0200).portr("IN1");
...
TILE_GET_INFO_MEMBER(base_state::get_md_tile_info)
{
...
void base_state::copy_sprites(bitmap_ind16 &bitmap, bitmap_ind16 &sprites_bitmap, bitmap_ind8 &priority_bitmap, const rectangle &cliprect)
{
...
u32 base_state::screen_update(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
...
static INPUT_PORTS_START(hrthouse)
	PORT_START("IN0")
	PORT_BIT(0x00010000, IP_ACTIVE_LOW, IPT_POKER_HOLD2) // ok
	PORT_BIT(0x00020000, IP_ACTIVE_LOW, IPT_POKER_HOLD4) // ok
	PORT_BIT(0x00040000, IP_ACTIVE_LOW, IPT_POKER_HOLD1) // ok
	PORT_BIT(0x00080000, IP_ACTIVE_LOW, IPT_POKER_HOLD3) // ok
	PORT_BIT(0x00100000, IP_ACTIVE_LOW, IPT_START1)  PORT_NAME("Start / Even") // ok
	PORT_BIT(0x00200000, IP_ACTIVE_LOW, IPT_GAMBLE_TAKE) PORT_NAME("Take / Odd") // ok
	PORT_BIT(0x00400000, IP_ACTIVE_LOW, IPT_GAMBLE_BET) PORT_NAME("Bet / Double Up") // ok
	PORT_BIT(0xff80ffff, IP_ACTIVE_LOW, IPT_UNKNOWN) // no effects spotted

	PORT_START("IN1")
	PORT_BIT(0xffffffff, IP_ACTIVE_LOW, IPT_UNKNOWN) // no effects spotted

	PORT_START("IN2")
	PORT_BIT(0x00040000, IP_ACTIVE_LOW, IPT_COIN1) // ok
	PORT_BIT(0x00080000, IP_ACTIVE_LOW, IPT_COIN2) // ok
	PORT_BIT(0x00100000, IP_ACTIVE_LOW, IPT_UNKNOWN)
	PORT_BIT(0x00200000, IP_ACTIVE_LOW, IPT_MEMORY_RESET) // strange one. first time will show '50 per divr', then 60, then 70. 4th time will start a count up and then reset
	PORT_BIT(0x00400000, IP_ACTIVE_LOW, IPT_CUSTOM) //security bit
	PORT_BIT(0x00800000, IP_ACTIVE_HIGH, IPT_CUSTOM) PORT_READ_LINE_DEVICE_MEMBER("eeprom", FUNC(eeprom_serial_93cxx_device::do_read))
	PORT_BIT(0x04000000, IP_ACTIVE_LOW, IPT_COIN3) // ok
	PORT_BIT(0x08000000, IP_ACTIVE_LOW, IPT_COIN4) // ok
	PORT_DIPNAME(0x20000000, 0x00000000, "Sound Enable")
	PORT_DIPSETTING(         0x20000000, DEF_STR(Off))
	PORT_DIPSETTING(         0x00000000, DEF_STR(On))
	PORT_BIT(0x80000000, IP_ACTIVE_HIGH, IPT_CUSTOM) PORT_READ_LINE_MEMBER(FUNC(limenko_state::spriteram_bit_r)) //changes spriteram location
	PORT_BIT(0x5303ffff, IP_ACTIVE_LOW, IPT_UNKNOWN) // no effects spotted
...
void limenko_state::limenko(machine_config &config)
{
	...
	m_maincpu->set_addrmap(AS_PROGRAM, &limenko_state::program_map);
	m_maincpu->set_addrmap(AS_IO, &limenko_state::io_map);
	...
	QS1000(config, m_qs1000, 24_MHz_XTAL);
...
void limenko_state::hrthouse(machine_config &config)
{
	limenko(config);

	m_maincpu->set_addrmap(AS_PROGRAM, &limenko_state::hrthouse_program_map);

	NVRAM(config, "nvram");
}

void spotty_state::spotty(machine_config &config)
{
	GMS30C2232(config, m_maincpu, 20_MHz_XTAL * 4);   // 20 MHz, 4x internal multiplier
	m_maincpu->set_addrmap(AS_PROGRAM, &spotty_state::program_map);
	m_maincpu->set_addrmap(AS_IO, &spotty_state::io_map);
	m_maincpu->set_vblank_int("screen", FUNC(spotty_state::irq0_line_hold));

	at89c4051_device &audiocpu(AT89C4051(config, "audiocpu", 4_MHz_XTAL));
	audiocpu.port_in_cb<1>().set(FUNC(spotty_state::audiocpu_p1_r));
	audiocpu.port_out_cb<1>().set(FUNC(spotty_state::audiocpu_p1_w));
	audiocpu.port_in_cb<3>().set(FUNC(spotty_state::audiocpu_p3_r));
	audiocpu.port_out_cb<3>().set(FUNC(spotty_state::audiocpu_p3_w));
	...
	screen.set_screen_update(FUNC(spotty_state::screen_update));
	screen.set_palette(m_palette);
	...
	OKIM6295(config, m_oki, 4_MHz_XTAL / 4 , okim6295_device::PIN7_HIGH); // divider not verified
...
ROM_START(dynabomb)
	...
	ROM_REGION(0x80000, "qs1000:cpu", 0)
...
// LMSYS_F2 ROM board. Adds battery backed RAM, plus an empty space probably for a protection device
ROM_START(hrthouse)
	ROM_REGION32_BE(0x200000, "maincpu", ROMREGION_ERASEFF) // Hyperstone CPU Code
	ROM_LOAD16_WORD_SWAP("01.sys_rom2", 0x180000, 0x080000, CRC(29884065) SHA1(adb1c507c3421ccbd38705ebea5ea9ae29667a21))

	ROM_REGION(0x80000, "qs1000:cpu", 0)
	ROM_LOAD("07.sou_prg", 0x00000, 0x40000, CRC(42872e1f) SHA1(6d90b53eff488aaedf41dd6ffe7b768fb833b4ed)) // 1ST AND 2ND HALF IDENTICAL
	ROM_FILL(              0x40000, 0x40000, 0xff)

	ROM_REGION(0x200000, "gfx", 0)
	ROM_LOAD32_BYTE("05.cg_rom1", 0x000000, 0x080000, CRC(a114b9da) SHA1(5205bd76ab8a76b58a0ff3fcde310ee4b1c57bdd)) // 1xxxxxxxxxxxxxxxxxx = 0xFF
	ROM_LOAD32_BYTE("04.cg_rom2", 0x000001, 0x080000, CRC(e3b2a85f) SHA1(aba59fe651db613fa8ced0d47bcdd38b1db777fa)) // 1xxxxxxxxxxxxxxxxxx = 0xFF
	ROM_LOAD32_BYTE("03.cg_rom3", 0x000002, 0x080000, CRC(ca648641) SHA1(7c66482a80c013eec7e9d09ea0214f7d7bd49ae0)) // 1xxxxxxxxxxxxxxxxxx = 0xFF
	ROM_LOAD32_BYTE("02.cg_rom4", 0x000003, 0x080000, CRC(4c5ea0d3) SHA1(85a1a7795e3a64350338472576f57370bb8196ae)) // 1xxxxxxxxxxxxxxxxxx = 0xFF

	ROM_REGION(0x1000000, "qs1000", 0) // QDSP wavetable ROMs
	ROM_LOAD("06.sou_rom1", 0x000000, 0x080000, CRC(95afe511) SHA1(1e1650b86c4393e6fdd0391837fb0beb98ca6738))
	ROM_LOAD("qs1003.u4",   0x200000, 0x200000, CRC(19e4b469) SHA1(9460e5b6a0fbf3fdd6a9fa0dcbf5062a2e07fe02))
ROM_END
...
u32 limenko_state::hrthouse_speedup_r()
{
	if (m_maincpu->pc() == 0xf538)
	{
		m_maincpu->eat_cycles(50);
	}

	return m_mainram[0x2639c / 4];
}

u32 spotty_state::speedup_r()
{
...
void limenko_state::init_hrthouse()
{
	m_maincpu->space(AS_PROGRAM).install_read_handler(0x2639c, 0x2639f, read32smo_delegate(*this, FUNC(limenko_state::hrthouse_speedup_r)));

	init_common();
}

void spotty_state::init_spotty()
{
	...
	m_maincpu->space(AS_PROGRAM).install_read_handler(0x6626c, 0x6626f, read32smo_delegate(*this, FUNC(spotty_state::speedup_r)));
...
GAME(200?, hrthouse, 0,      hrthouse, hrthouse, limenko_state, init_hrthouse, ROT0, "Limenko",    "To Heart House",                     MACHINE_IMPERFECT_GRAPHICS | MACHINE_SUPPORTS_SAVE)

// this game only uses the same graphics chip used in Limenko's system
GAME(2001, spotty,   0,      spotty,   spotty,   spotty_state,  init_spotty,   ROT0, "Prince Co.", "Spotty (Ver. 2.0.2)",                MACHINE_SUPPORTS_SAVE)

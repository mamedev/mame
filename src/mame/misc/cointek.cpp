...
TODO:
- Audio CPU ROM decryption may be incomplete, though code flow looks sane.
...
{
	m_tilemap = &machine().tilemap().create(*m_gfxdecode, tilemap_get_info_delegate(*this, FUNC(cointek_state::get_tile_info)), TILEMAP_SCAN_ROWS, 8, 8, 64, 32);
	m_reel_tilemap[0] = &machine().tilemap().create(*m_gfxdecode, tilemap_get_info_delegate(*this, FUNC(cointek_state::get_reel_tile_info<0>)), TILEMAP_SCAN_ROWS, 8, 8, 64, 32);
	m_reel_tilemap[1] = &machine().tilemap().create(*m_gfxdecode, tilemap_get_info_delegate(*this, FUNC(cointek_state::get_reel_tile_info<1>)), TILEMAP_SCAN_ROWS, 8, 8, 64, 32);
	m_reel_tilemap[2] = &machine().tilemap().create(*m_gfxdecode, tilemap_get_info_delegate(*this, FUNC(cointek_state::get_reel_tile_info<2>)), TILEMAP_SCAN_ROWS, 8, 8, 64, 32);
...
{
	int const tile = m_tileram[tile_index] | ((m_attrram[tile_index] & 0x0f) << 8);

	// int const color = (m_attrram[tile_index] & 0xf0) >> 8;

	tileinfo.set(0, tile, 0, 0);
}

template <uint8_t Which>
TILE_GET_INFO_MEMBER(cointek_state::get_reel_tile_info)
{
	int const tile = m_reel_tileram[Which][tile_index] | ((m_reel_attrram[Which][tile_index] & 0x0f) << 8);

	// int const color = (m_reel_attrram[Which][tile_index] & 0xf0) >> 8;
...
template <uint8_t Which>
void cointek_state::reel_tileram_w(offs_t offset, uint8_t data)
{
...
template <uint8_t Which>
void cointek_state::reel_attrram_w(offs_t offset, uint8_t data)
{
...
void cointek_state::machine_start()
{
	m_mainrombank->configure_entries(0, 2, memregion("maincpu")->base() + 0x8000, 0x4000);

	save_item(NAME(m_control_reg));
}

void cointek_state::control_w(uint8_t data)
{
	m_control_reg = data;
	logerror("%s bank_w: %02x\n", machine().describe_context(), m_control_reg);

	// bit 0 is ROM banking
	m_mainrombank->set_entry(BIT(m_control_reg, 0));
	...
	// bit 4 is reel RAM / reel attribute RAM banking
	m_gfxview.select(BIT(m_control_reg, 4));
...
void cointek_state::program_map(address_map &map)
{
	...
	map(0xc800, 0xcfff).ram().w(FUNC(cointek_state::tileram_w)).share(m_tileram);
	...
	map(0xf804, 0xf804).lr8(NAME([] () -> uint8_t { return 0x00; })); // if bit 0 is set, it stops updating. VBLANK flag?
	map(0xf808, 0xf808).lr8(NAME([] () -> uint8_t { return 0x07; })); // some kind of flag from the audio CPU?
	map(0xf810, 0xf815).lr8(NAME([this] (offs_t offset) -> uint8_t { if (m_maincpu->pc() == 0x75) { return m_key[offset]; } else return 0x00; })); // protection involving the audio CPU uploading a string;
	map(0xfa00, 0xfa00).w(FUNC(cointek_state::control_w));
...
static INPUT_PORTS_START( goldline )
	PORT_START("IN0")
...
static GFXDECODE_START( gfx_goldline )
	GFXDECODE_ENTRY( "tiles1", 0, gfx_8x8x8, 0x100, 0x10 ) // TODO: color offset may be wrong
	GFXDECODE_ENTRY( "tiles2", 0, gfx_8x8x8, 0x000, 0x10 ) // "
GFXDECODE_END
...
void cointek_state::cointek(machine_config &config)
{
	...
	screen.set_visarea(0, 64*8-1, 2*8, 30*8-1);
	screen.set_screen_update(FUNC(cointek_state::screen_update));
	screen.set_palette("palette");

	GFXDECODE(config, "gfxdecode", "palette", gfx_goldline);
	PALETTE(config, "palette", palette_device::RGB_444_PROMS, "proms", 0x200);
...
ROM_START( goldline ) // all labels handwritten but with Cointek logo
	...
	ROM_REGION( 0x600, "proms", 0 ) // 2 almost identical banks?
	ROM_LOAD( "sn82s129n.b9",  0x000, 0x100, CRC(13dfde1e) SHA1(09d9261c9a0b4f947d9cca54d0137dd15df3ee47) )
	ROM_LOAD( "sn82s129n.b10", 0x100, 0x100, CRC(bf3847e6) SHA1(18a02d38c54199aa40d65b236012ef181371e7c4) )
	ROM_LOAD( "sn82s129n.a9",  0x200, 0x100, CRC(1fb14640) SHA1(a0eb126f813fc39710e044f253e37a70cfa9afb5) )
	ROM_LOAD( "sn82s129n.a10", 0x300, 0x100, CRC(458ca6e2) SHA1(e195444db4541c7a99cb68c51483f67837e36ae7) )
...
void cointek_state::init_goldline()
{
...
GAME( 1989, goldline, 0, cointek, goldline, cointek_state, init_goldline, ROT0, "Cointek Enterprise Corp", "Gold Line", MACHINE_NO_SOUND | MACHINE_NOT_WORKING ) // string in ROM at 0x7839: Ver 4.00 1989-08-01

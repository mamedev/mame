...
PORT_START("KEYS1")
   ...
   PORT_START("DSW")
	...
	PORT_BIT(0x01, IP_ACTIVE_HIGH, IPT_BUTTON1) PORT_NAME("K0") // K0
	PORT_BIT(0x02, IP_ACTIVE_HIGH, IPT_BUTTON2) PORT_NAME("K1") // K1
	PORT_BIT(0x04, IP_ACTIVE_HIGH, IPT_BUTTON3) PORT_NAME("K2") // K2
	PORT_BIT(0x08, IP_ACTIVE_HIGH, IPT_BUTTON4) PORT_NAME("K3") // K3
...
static INPUT_PORTS_START( mary1s )
	PORT_BIT(0x80, IP_ACTIVE_LOW, IPT_KEYPAD) PORT_NAME("Apple") PORT_CODE(KEYCODE_Q) // dsp1
	PORT_BIT(0x40, IP_ACTIVE_LOW, IPT_KEYPAD) PORT_NAME("Orange") PORT_CODE(KEYCODE_W) // dsp12
	PORT_BIT(0x20, IP_ACTIVE_LOW, IPT_KEYPAD) PORT_NAME("Mango") PORT_CODE(KEYCODE_E) // dsp3
	PORT_BIT(0x10, IP_ACTIVE_LOW, IPT_KEYPAD) PORT_NAME("Bell") PORT_CODE(KEYCODE_R) // dsp4
	PORT_BIT(0x08, IP_ACTIVE_LOW, IPT_KEYPAD) PORT_NAME("Watermelon") PORT_CODE(KEYCODE_T) // dsp5
	PORT_BIT(0x04, IP_ACTIVE_LOW, IPT_KEYPAD) PORT_NAME("Stars") PORT_CODE(KEYCODE_Y) // dsp6
	PORT_BIT(0x02, IP_ACTIVE_LOW, IPT_KEYPAD) PORT_NAME("77") PORT_CODE(KEYCODE_U) // dsp7
	PORT_BIT(0x01, IP_ACTIVE_LOW, IPT_KEYPAD) PORT_NAME("BAR") PORT_CODE(KEYCODE_I) // dsp8

	PORT_START("KEYS2")
	PORT_BIT(0x80, IP_ACTIVE_HIGH, IPT_COIN1)  PORT_CODE(KEYCODE_5) // will cause error 30.
	PORT_BIT(0x40, IP_ACTIVE_LOW,  IPT_KEYPAD) PORT_CODE(KEYCODE_O) // ???
	PORT_BIT(0x20, IP_ACTIVE_LOW,  IPT_KEYPAD) PORT_CODE(KEYCODE_P) // dsp11. increase won number.
	PORT_BIT(0x10, IP_ACTIVE_LOW,  IPT_KEYPAD) PORT_CODE(KEYCODE_A) // dsp12. decrease won number.
	PORT_BIT(0x08, IP_ACTIVE_LOW,  IPT_KEYPAD) PORT_CODE(KEYCODE_S) // dsp13. // ???
	PORT_BIT(0x04, IP_ACTIVE_LOW, IPT_GAMBLE_D_UP) PORT_CODE(KEYCODE_D)
	PORT_BIT(0x02, IP_ACTIVE_LOW, IPT_KEYPAD) PORT_CODE(KEYCODE_F) // ???
	PORT_BIT(0x01, IP_ACTIVE_LOW, IPT_GAMBLE_KEYOUT) PORT_CODE(KEYCODE_G) // key out

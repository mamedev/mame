...
void chinhero_state::nmiq_1_w(uint8_t data)
{
	if (m_nmi_enable[0])
		m_maincpu->set_input_line(INPUT_LINE_NMI, ASSERT_LINE);
}

void chinhero_state::nmiq_2_w(uint8_t data)
{
	if (m_nmi_enable[1])
...
void chinhero_state::ay8910_porta_w(uint8_t data)
{
	if (BIT(data, 0))
...
void shangkid_state::ay8910_porta_w(uint8_t data)
{
	if (BIT(data, 0))
		// 0->1 transition triggers interrupt on Sound CPU
		m_audiocpu->set_input_line(0, HOLD_LINE);

	m_soundbank->set_entry((data & 0xfe) ? 0 : 1);
}


/***************************************************************************************/

void dynamski_state::machine_start()
{
	save_item(NAME(m_int_enable[0]));
}

void chinhero_state::machine_start()
{
	m_gfx_type = 0;
...
void shangkid_state::machine_start()
{
	chinhero_state::machine_start();

	m_gfx_type = 1;

	// set up banking
	membank("mainbank")->configure_entries(0, 2, memregion("maincpu")->base() + 0x8000, 0x8000);
	m_soundbank->configure_entries(0, 2, memregion("audiocpu")->base() + 0x0000, 0x10000);
}

/***************************************************************************************/

void shangkid_state::machine_reset()
{
...
void shangkid_state::main_map(address_map &map)
{
	...
	map(0x8000, 0x9fff).bankr("mainbank");
	...
	map(0xb803, 0xb803).portr("P1");
	map(0xc000, 0xc002).writeonly().share(m_videoreg);
	map(0xd000, 0xdfff).ram().w(FUNC(shangkid_state::videoram_w)).share(m_videoram);
	map(0xe000, 0xfdff).ram().share("share2");
	map(0xfe00, 0xffff).ram().share(m_spriteram);
}
...
void chinhero_state::bbx_map(address_map &map)
{
	map(0x0000, 0x9fff).rom();
	map(0xa000, 0xa000).w(FUNC(chinhero_state::nmiq_1_w));
	map(0xa800, 0xa800).w(FUNC(chinhero_state::nmiq_2_w));
...
void shangkid_state::bbx_map(address_map &map)
{
	map(0x0000, 0x9fff).rom();
	map(0xa000, 0xa000).w(FUNC(shangkid_state::nmiq_1_w));
	map(0xa800, 0xa800).w(FUNC(shangkid_state::nmiq_2_w));
...
void chinhero_state::bbx_portmap(address_map &map)
{
	map.global_mask(0xff);
	map(0x00, 0x01).w(m_aysnd, FUNC(ay8910_device::address_data_w));
}

void shangkid_state::bbx_portmap(address_map &map)
{
	map.global_mask(0xff);
	map(0x00, 0x01).w(m_aysnd, FUNC(ay8910_device::address_data_w));
}
...
void chinhero_state::sound_map(address_map &map)
{
	map(0x0000, 0xdfff).rom();
	map(0xe000, 0xe7ff).ram().mirror(0x0800);
}

void shangkid_state::sound_map(address_map &map)
{
	map(0x0000, 0xdfff).bankr(m_soundbank); // sample player writes to ROM area
	map(0xe000, 0xe7ff).ram().mirror(0x0800);
}

void chinhero_state::sound_portmap(address_map &map)
{
	map.global_mask(0xff);
	map(0x00, 0x00).r("soundlatch", FUNC(generic_latch_8_device::read)).w("dac", FUNC(dac_byte_interface::data_w));
}

/***************************************************************************************/

void chinhero_state::chinhero(machine_config &config)
{
...
static INPUT_PORTS_START( shangkid )
	...
	PORT_START("P1")
	PORT_BIT( 0x01, IP_ACTIVE_HIGH, IPT_BUTTON2 ) PORT_PLAYER(1) PORT_NAME("P1 Kick")
	PORT_BIT( 0x02, IP_ACTIVE_HIGH, IPT_BUTTON1 ) PORT_PLAYER(1) PORT_NAME("P1 Punch")
	...
	PORT_START("P2")
	PORT_BIT( 0x01, IP_ACTIVE_HIGH, IPT_BUTTON2 ) PORT_PLAYER(2) PORT_NAME("P2 Kick")
	PORT_BIT( 0x02, IP_ACTIVE_HIGH, IPT_BUTTON1 ) PORT_PLAYER(2) PORT_NAME("P2 Punch")
	PORT_BIT( 0x04, IP_ACTIVE_HIGH, IPT_UNUSED )
	PORT_BIT( 0x08, IP_ACTIVE_HIGH, IPT_UNUSED )
	...
	*/
	...
	PORT_DIPSETTING(    0x00, DEF_STR( Cocktail ) )
	PORT_DIPNAME( 0x1c, 0x04, DEF_STR( Coinage ) ) PORT_DIPLOCATION("SW1:3,4,5")
	PORT_DIPSETTING(    0x10, DEF_STR( 4C_1C ) )
	PORT_DIPSETTING(    0x0c, DEF_STR( 3C_1C ) )
	PORT_DIPSETTING(    0x08, DEF_STR( 2C_1C ) )
	PORT_DIPSETTING(    0x00, "1 Coin/1 Credit without coin counter" )

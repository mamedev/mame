...
uint32_t gaiden_state::screen_update_gaiden(screen_device &screen, bitmap_rgb32 &bitmap, const rectangle &cliprect)
{
	...
	// non buffered?
	m_sprgen->gaiden_draw_sprites(screen, cliprect, m_spriteram->live(), m_sprite_sizey, flip_screen() ? -m_spr_offset_y : m_spr_offset_y, flip_screen(), m_sprite_bitmap);
...
void gaiden_state::screen_vblank_raiga(int state)
{
	if (state)
	{
		...
		m_sprgen->gaiden_draw_sprites(*m_screen, visarea, m_spriteram->buffer(), m_sprite_sizey, flip_screen() ? -m_spr_offset_y : m_spr_offset_y, flip_screen(), m_sprite_bitmap);

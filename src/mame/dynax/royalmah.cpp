...
void royalmah_prgbank_state::ichiban_iomap(address_map &map)
{
	...
	map(0x11, 0x11).portr("SYSTEM").w(FUNC(royalmah_prgbank_state::input_port_select_w));
	map(0x12, 0x12).portr("DSW-B").lw8(NAME([this] (uint8_t data) { m_mainbank->set_entry(data); })); // only seems to write 0x00, 0x01, 0x02 and 0x04
...
ROM_START( ichiban )
	...
	ROM_IGNORE(0x18000) // TODO: where is this mapped? doesn't seem to ever be needed
...
GAME( 1993,  ichiban,  0,        ichiban,  ichiban,  royalmah_prgbank_state, init_ichiban,  ROT0,   "Excel",                      "Ichi Ban Jyan",                         MACHINE_NOT_WORKING | MACHINE_IMPERFECT_GRAPHICS ) // ROM banking may be incomplete / wrong, several GFX problems

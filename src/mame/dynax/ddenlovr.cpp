...
{
	/*if (!BIT(m_dsw_sel, 0)) logerror("DSW4 read = %x\n", ioport("DSW4")->read());
	if (!BIT(m_dsw_sel, 1)) logerror("DSW3 read = %x\n", ioport("DSW3")->read());
	if (!BIT(m_dsw_sel, 2)) logerror("DSW2 read = %x\n", ioport("DSW2")->read());
	if (!BIT(m_dsw_sel, 3)) logerror("DSW1 read = %x\n", ioport("DSW1")->read());
	if (!BIT(m_dsw_sel, 4)) {
		logerror("DSWs high bits read = %x\n", ((ioport("DSW1")->read() & 0x300) >> 2) | ((ioport("DSW2")->read() & 0x300) >> 4) | ((ioport("DSW3")->read() & 0x300) >> 6) | ((ioport("DSW4")->read() & 0x300) >> 8));
		logerror("DSW1 high bits read = %x\n", (ioport("DSW1")->read() & 0x300));
		logerror("DSW2 high bits read = %x\n", (ioport("DSW2")->read() & 0x300));
		logerror("DSW3 high bits read = %x\n", (ioport("DSW3")->read() & 0x300));
		logerror("DSW4 high bits read = %x\n", (ioport("DSW4")->read() & 0x300));
	}*/
...
static INPUT_PORTS_START( mjmyornt )
	...
	PORT_DIPSETTING(    0x03, DEF_STR(1C_1C) )       PORT_CONDITION("DSW5", 0x01, EQUALS, 0x01)   // １コイン　　　１プレイ
	PORT_DIPSETTING(    0x02, DEF_STR(1C_2C) )       PORT_CONDITION("DSW5", 0x01, EQUALS, 0x01)   // １コイン　　　２プレイ
	PORT_DIPSETTING(    0x01, DEF_STR(1C_5C) )       PORT_CONDITION("DSW5", 0x01, EQUALS, 0x01)   // １コイン　　　５プレイ
	...
	MAHJONG_NOTE_CREDITS(6, "SW 1:9", "DSW2", 0)                                                  // ＮＯＴＥ　ＲＡＴＥ  (TODO: this is affected by the alternate coinage setting)

...
{
	if (!BIT(m_dsw_sel, 0))   return (ioport("DSW1")->read() & 0xff);
	if (!BIT(m_dsw_sel, 1))   return (ioport("DSW2")->read() & 0xff);
	if (!BIT(m_dsw_sel, 2))   return (ioport("DSW3")->read() & 0xff);
	if (!BIT(m_dsw_sel, 3))   return (ioport("DSW4")->read() & 0xff);

	if (!BIT(m_dsw_sel, 4))  return ((ioport("DSW4")->read() & 0x300) >> 2) |   // DSWs high 2 bits
									((ioport("DSW3")->read() & 0x300) >> 4) |
									((ioport("DSW2")->read() & 0x300) >> 6) |
									((ioport("DSW1")->read() & 0x300) >> 8) ;
...
uint8_t hanakanz_state::mjchuuka_dsw_r()
{
	return  ((ioport("DSW4")->read() & 0x300) >> 2) |   // DSWs high 2 bits
			((ioport("DSW3")->read() & 0x300) >> 4) |
			((ioport("DSW2")->read() & 0x300) >> 6) |
			((ioport("DSW1")->read() & 0x300) >> 8) ;
}
...
{     // 16 bit I/O
	...
	map(0x64, 0x64).mirror(0xff00).r(FUNC(hanakanz_state::mjchuuka_dsw_r));     // DSW 1-4 high bits
...
uint8_t ddenlovr_state::mjschuka_dsw_r()
{
	return  ((ioport("DSW3")->read() & 0x300) >> 2) |   // DSWs high 2 bits
			((ioport("DSW4")->read() & 0x300) >> 4) |
			((ioport("DSW2")->read() & 0x300) >> 6) |
			((ioport("DSW1")->read() & 0x300) >> 8) ;
}
...
void ddenlovr_state::mjschuka_portmap(address_map &map)
{
	...
	map(0x6a, 0x6a).portr("DSW4");
	map(0x6b, 0x6b).portr("DSW3");
	map(0x6c, 0x6c).r(FUNC(ddenlovr_state::mjschuka_dsw_r));     // DSW 1-4 high bits
...
void ddenlovr_state::mjmyorntr_portmap(address_map &map)
{
	...
	map(0x5a, 0x5a).portr("DSW4");
	map(0x5b, 0x5b).portr("DSW3");
	map(0x5c, 0x5c).r(FUNC(ddenlovr_state::mjschuka_dsw_r));     // DSW 1-4 high bits
...
{
	if (!BIT(m_dsw_sel, 0))   return (ioport("DSW4")->read() & 0xff);
	if (!BIT(m_dsw_sel, 1))   return (ioport("DSW3")->read() & 0xff);
	if (!BIT(m_dsw_sel, 2))   return (ioport("DSW2")->read() & 0xff);
	if (!BIT(m_dsw_sel, 3))   return (ioport("DSW1")->read() & 0xff);

	if (!BIT(m_dsw_sel, 4))  return ((ioport("DSW1")->read() & 0x300) >> 2) |   // DSWs high 2 bits
									((ioport("DSW2")->read() & 0x300) >> 4) |
									((ioport("DSW3")->read() & 0x300) >> 6) |
									((ioport("DSW4")->read() & 0x300) >> 8) ;
...
static INPUT_PORTS_START( htengoku )
	PORT_START("COINS")
	PORT_BIT( 0x01, IP_ACTIVE_LOW, IPT_GAMBLE_PAYOUT ) PORT_CODE(KEYCODE_4) PORT_CONDITION("DSW4", 0x100, EQUALS, 0x00) // pay
	PORT_BIT( 0x01, IP_ACTIVE_LOW, IPT_GAMBLE_KEYOUT )                      PORT_CONDITION("DSW4", 0x100, EQUALS, 0x10)
	...
	PORT_DIPNAME( 0x01, 0x01, "Show Girls" )               PORT_DIPLOCATION("SW1:1")
	PORT_DIPSETTING(    0x01, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )
	PORT_DIPNAME( 0x02, 0x02, "Select Stage" )             PORT_DIPLOCATION("SW1:2")
	PORT_DIPSETTING(    0x02, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )
	PORT_DIPNAME( 0x04, 0x04, "Secret Skill" )
	PORT_DIPSETTING(    0x04, DEF_STR( On ) )              PORT_DIPLOCATION("SW1:3")
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )
	PORT_DIPNAME( 0x08, 0x08, "Secret Character" )         PORT_DIPLOCATION("SW1:4")
	PORT_DIPSETTING(    0x08, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )
	PORT_DIPNAME( 0x10, 0x10, "In-Game Sounds" )           PORT_DIPLOCATION("SW1:5")
	PORT_DIPSETTING(    0x10, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )
	PORT_DIPNAME( 0x20, 0x20, DEF_STR( Demo_Sounds ) )     PORT_DIPLOCATION("SW1:6")
	PORT_DIPSETTING(    0x20, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )
	PORT_DIPNAME( 0x40, 0x40, "In-Game Hints" )            PORT_DIPLOCATION("SW1:7")
	PORT_DIPSETTING(    0x40, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )
	PORT_DIPNAME( 0x80, 0x80, DEF_STR( Flip_Screen ) )     PORT_DIPLOCATION("SW1:8")
	PORT_DIPSETTING(    0x80, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_SERVICE( 0x100, IP_ACTIVE_LOW )                   PORT_DIPLOCATION("SW1:9")
	PORT_DIPNAME( 0x200, 0x200, "Set Clock" )              PORT_DIPLOCATION("SW1:10")
	PORT_DIPSETTING(    0x200, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )

	PORT_START("DSW2")  // IN12 - DSW2
	PORT_DIPNAME( 0x07, 0x07, "Payout Rate" )              PORT_DIPLOCATION("SW2:1,2,3")
	...
	PORT_DIPNAME( 0x08, 0x08, "Payout Wave" )              PORT_DIPLOCATION("SW2:4")
	PORT_DIPSETTING(    0x08, "Big" )
	PORT_DIPSETTING(    0x00, "Small" )
	PORT_DIPNAME( 0x10, 0x10, "Goko Yaku" )                PORT_DIPLOCATION("SW2:5")
	PORT_DIPSETTING(    0x10, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )
	PORT_DIPNAME( 0x60, 0x60, "Double-Up Win Rate" )       PORT_DIPLOCATION("SW2:6,7")
	...
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )
	PORT_DIPNAME( 0x100, 0x00, DEF_STR( Unused ) )         PORT_DIPLOCATION("SW2:9")
	PORT_DIPSETTING(    0x100, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )
	PORT_DIPNAME( 0x200, 0x00, DEF_STR( Unused ) )         PORT_DIPLOCATION("SW2:10")
	PORT_DIPSETTING(    0x200, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )

	PORT_START("DSW3")  // IN13 - DSW3
	MAHJONG_COINAGE(0, "SW3:1,2")
	MAHJONG_NOTE_CREDITS(2, "SW 3:3", "DSW3", 0)
	PORT_DIPNAME( 0x08, 0x08, "Maximum Bet" )              PORT_DIPLOCATION("SW3:4")
	...
	PORT_DIPSETTING(    0x00, "5" )
	PORT_DIPNAME( 0xc0, 0xc0, "Credits Limit" )            PORT_DIPLOCATION("SW3:7,8")
	PORT_DIPSETTING(    0xc0, "1000" )
	PORT_DIPSETTING(    0x80, "2000" )
	PORT_DIPSETTING(    0x40, "3000" )
	PORT_DIPSETTING(    0x00, "5000" )
	PORT_DIPNAME( 0x100, 0x00, "Payout Mode" )             PORT_DIPLOCATION("SW3:9")
	PORT_DIPSETTING(    0x100, "Key-out" )
	PORT_DIPSETTING(    0x00, "Hopper" )
	PORT_DIPNAME( 0x200, 0x00, "Hopper Switch" )           PORT_DIPLOCATION("SW3:10")
	PORT_DIPSETTING(    0x200, "Active Low" )
	PORT_DIPSETTING(    0x00, "Active High" )

	PORT_START("DSW4")  // IN14 - DSW4
	PORT_DIPNAME( 0x03, 0x03, "Odds For Goko" )            PORT_DIPLOCATION("SW4:1,2")
	PORT_DIPSETTING(    0x03, "100" )
	PORT_DIPSETTING(    0x02, "200" )
	PORT_DIPSETTING(    0x01, "250" )
	PORT_DIPSETTING(    0x00, "300" )
	PORT_DIPNAME( 0x0c, 0x0c, "Odds For Shiko" )           PORT_DIPLOCATION("SW4:3,4")
	PORT_DIPSETTING(    0x0c, "50" )
	PORT_DIPSETTING(    0x08, "60" )
	PORT_DIPSETTING(    0x04, "70" )
	PORT_DIPSETTING(    0x00, "80" )
	PORT_DIPNAME( 0x30, 0x30, "Odds For Ameshiko" )        PORT_DIPLOCATION("SW4:5,6")
	PORT_DIPSETTING(    0x30, "20" )
	PORT_DIPSETTING(    0x20, "30" )
	PORT_DIPSETTING(    0x10, "40" )
	PORT_DIPSETTING(    0x00, "50" )
	PORT_DIPNAME( 0x40, 0x40, DEF_STR( Unused ) )          PORT_DIPLOCATION("SW4:7")
	PORT_DIPSETTING(    0x40, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )
	PORT_DIPNAME( 0x80, 0x80, DEF_STR( Unused ) )          PORT_DIPLOCATION("SW4:8")
	PORT_DIPSETTING(    0x80, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )
	PORT_DIPNAME( 0x100, 0x00, DEF_STR( Unused ) )         PORT_DIPLOCATION("SW4:9")
	PORT_DIPSETTING(    0x100, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )
	PORT_DIPNAME( 0x200, 0x00, DEF_STR( Unused ) )         PORT_DIPLOCATION("SW4:10")
	PORT_DIPSETTING(    0x200, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )
INPUT_PORTS_END
...
static INPUT_PORTS_START( hanakanz )
	...
	//No manual, try to use same DSW settings of hkagerou.
	//Checked in-game, and seems to be almost identical in DSW1 and DSW2. Only a change in Coinage (SW2-1,2)
	//DSW3 and DSW4 improved by comparison with other games, and testing in game
	PORT_START("DSW1")
	...
	MAHJONG_NOTE_CREDITS(7, "SW 1:8", "DSW2", 0)
	PORT_DIPNAME( 0x100, 0x00, "Maximum Bet" )             PORT_DIPLOCATION("SW1:9")
	PORT_DIPSETTING(    0x100, "10" )
	PORT_DIPSETTING(    0x00, "5" )
	PORT_DIPNAME( 0x200, 0x00, DEF_STR( Flip_Screen ) )    PORT_DIPLOCATION("SW1:10")
	PORT_DIPSETTING(    0x200, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )

	PORT_START("DSW2")
	PORT_DIPNAME( 0x03, 0x03, DEF_STR( Coinage ) )         PORT_DIPLOCATION("SW2:1,2")
	PORT_DIPSETTING(    0x03, DEF_STR( 1C_1C ) )     PORT_CONDITION("DSW3", 0x03, EQUALS, 0x03)
	PORT_DIPSETTING(    0x02, DEF_STR( 1C_2C ) )     PORT_CONDITION("DSW3", 0x03, EQUALS, 0x03)
	PORT_DIPSETTING(    0x01, DEF_STR( 2C_1C ) )     PORT_CONDITION("DSW3", 0x03, EQUALS, 0x03)
	PORT_DIPSETTING(    0x00, DEF_STR( 3C_1C ) )     PORT_CONDITION("DSW3", 0x03, EQUALS, 0x03)
	PORT_DIPSETTING(    0x03, DEF_STR( 1C_1C ) )     PORT_CONDITION("DSW3", 0x03, NOTEQUALS, 0x03)
	PORT_DIPSETTING(    0x02, DEF_STR( 1C_2C ) )     PORT_CONDITION("DSW3", 0x03, NOTEQUALS, 0x03)
	PORT_DIPSETTING(    0x01, DEF_STR( 1C_5C ) )     PORT_CONDITION("DSW3", 0x03, NOTEQUALS, 0x03)
	PORT_DIPSETTING(    0x00, "1 Coin / 10 Credit" ) PORT_CONDITION("DSW3", 0x03, NOTEQUALS, 0x03)
	...
	PORT_DIPSETTING(    0x00, "80" )
	PORT_DIPNAME( 0x300, 0x300, "Ameshikou Odds" )         PORT_DIPLOCATION("SW2:9,10")
	PORT_DIPSETTING(    0x300, "20" )
	PORT_DIPSETTING(    0x200, "30" )	
	PORT_DIPSETTING(    0x100, "40" )
	PORT_DIPSETTING(    0x00, "50" )
	...
	PORT_DIPNAME( 0x04, 0x04, "(C) Nihon (Censored)" )     PORT_DIPLOCATION("SW3:3")
	PORT_DIPSETTING(    0x04, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x18, 0x08, "Credit Limit" )             PORT_DIPLOCATION("SW3:4,5")
	PORT_DIPSETTING(    0x18, "1000" )
	PORT_DIPSETTING(    0x10, "2000" )
	PORT_DIPSETTING(    0x08, "3000" )
	PORT_DIPSETTING(    0x00, "5000" )
	PORT_DIPNAME( 0x60, 0x60, "Shikou Limit" )             PORT_DIPLOCATION("SW3:6,7")
	PORT_DIPSETTING(    0x60, "Set 1" )
	PORT_DIPSETTING(    0x40, "Set 2" )
	PORT_DIPSETTING(    0x20, "Set 3" )
	PORT_DIPSETTING(    0x00, "Set 4" )
	PORT_DIPNAME( 0x80, 0x80, "Girl" )                     PORT_DIPLOCATION("SW3:8")
	PORT_DIPSETTING(    0x80, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )
	PORT_DIPNAME( 0x100, 0x100, "Unknown-SW3:9" )          PORT_DIPLOCATION("SW3:9")
	PORT_DIPSETTING(    0x100, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )  
	PORT_DIPNAME( 0x200, 0x200, "In-Game Voice" )          PORT_DIPLOCATION("SW3:10")
	PORT_DIPSETTING(    0x200, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )
	...
	PORT_DIPSETTING(    0x02, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )
	PORT_DIPNAME( 0x04, 0x00, "Unknown-SW4:3" )            PORT_DIPLOCATION("SW4:3")
	PORT_DIPSETTING(    0x04, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )
	PORT_DIPNAME( 0x08, 0x00, "Unknown-SW4:4" )            PORT_DIPLOCATION("SW4:4")
	PORT_DIPSETTING(    0x08, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )
	PORT_DIPNAME( 0x10, 0x00, "In-game Hints" )            PORT_DIPLOCATION("SW4:5")
	PORT_DIPSETTING(    0x10, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )
	PORT_DIPNAME( 0x20, 0x00, "Unknown-SW4:6" )            PORT_DIPLOCATION("SW4:6")
	PORT_DIPSETTING(    0x20, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )                                         //								   8   9
	PORT_DIPNAME( 0xc0, 0xc0, "Panel" )                    PORT_DIPLOCATION("SW4:7,8")  // SW4-8,9      Hanafuda Panel	: OFF OFF
	PORT_DIPSETTING(    0xc0, "Hanafuda" ) //                                                           Mahjong Panel 	: ON  OFF
	PORT_DIPSETTING(    0x80, "Mahjong" ) // Manual says 'Mahjong Panel'                                Amusement1 Panel: OFF ON
	PORT_DIPSETTING(    0x40, "Amusement1 ?" ) // Translation not clear                                 Amusement2 Panel: ON  ON
	PORT_DIPSETTING(    0x00, "Amusement2 ?" ) // Translation not clear
	PORT_DIPNAME( 0x100, 0x00, "Unknown-SW4:9" )           PORT_DIPLOCATION("SW4:9")
	PORT_DIPSETTING(    0x100, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) ) 
	PORT_SERVICE_DIPLOC(  0x0200, IP_ACTIVE_LOW, "SW4:10" )
	...
	PORT_START("HOPPER")
	PORT_DIPNAME( 0x40, 0x40, "Hopper Microswitch" )
	PORT_DIPSETTING(    0x40, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
INPUT_PORTS_END
...
static INPUT_PORTS_START( hkagerou )
	...
	PORT_START("DSW3")
	PORT_DIPNAME( 0x01, 0x01, "Continue Mode" )            PORT_DIPLOCATION("SW3:1")
	PORT_DIPSETTING(    0x01, DEF_STR( Off ) ) 
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x02, 0x02, DEF_STR( Unused ) )          PORT_DIPLOCATION("SW3:2") // always OFF on Manual
	PORT_DIPSETTING(    0x02, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )  
	PORT_DIPNAME( 0x04, 0x04, DEF_STR( Unused ) )          PORT_DIPLOCATION("SW3:3") // always OFF on Manual
	PORT_DIPSETTING(    0x04, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )  
	PORT_DIPNAME( 0x18, 0x08, "Credit Limit" )             PORT_DIPLOCATION("SW3:4,5")
	PORT_DIPSETTING(    0x18, "1000" )
	...
	PORT_DIPSETTING(    0x20, "Set 3" )
	PORT_DIPSETTING(    0x00, "Set 4" )
	PORT_DIPNAME( 0x80, 0x80, "Show Girl at Game Start" )  PORT_DIPLOCATION("SW3:8")
	PORT_DIPSETTING(    0x80, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )
	PORT_DIPNAME( 0x100, 0x100, DEF_STR( Unused ) )        PORT_DIPLOCATION("SW3:9") // always OFF on Manual
	PORT_DIPSETTING(    0x100, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )  
	PORT_DIPNAME( 0x200, 0x200, "In-Game Voice" )          PORT_DIPLOCATION("SW3:10")
	PORT_DIPSETTING(    0x200, DEF_STR( On ) )
	...
	PORT_DIPSETTING(    0x04, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )
	PORT_DIPNAME( 0x08, 0x08, "Secret Skills" )            PORT_DIPLOCATION("SW4:4")
	PORT_DIPSETTING(    0x08, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )
	...
	PORT_DIPSETTING(    0x40, "Amusement1 ?" ) // Translation not clear                                 Amusement2 Panel: ON  ON
	PORT_DIPSETTING(    0x00, "Amusement2 ?" ) // Translation not clear
	PORT_DIPNAME( 0x100, 0x00, DEF_STR( Unused ) )         PORT_DIPLOCATION("SW4:9") // Manual says it is always OFF
	PORT_DIPSETTING(    0x100, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) ) 
	PORT_SERVICE_DIPLOC(  0x0200, IP_ACTIVE_LOW, "SW4:10" )
...
static INPUT_PORTS_START( kotbinyo )
	...
	PORT_BIT( 0x40, IP_ACTIVE_LOW, IPT_SERVICE1   ) //TODO: Service Mode Access does not work
	...
	PORT_DIPSETTING(    0x80, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x100, 0x100, DEF_STR( Demo_Sounds ) )  PORT_DIPLOCATION("SW1:9")
	PORT_DIPSETTING(    0x100, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x200, 0x200, DEF_STR( Flip_Screen ) )  PORT_DIPLOCATION("SW1:10")
	PORT_DIPSETTING(    0x200, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )

	PORT_START("DSW2")
	PORT_DIPNAME( 0x03, 0x03, "Unknown SW2:1,2" )         PORT_DIPLOCATION("SW2:1,2")
	PORT_DIPSETTING(    0x03, "0" )
	PORT_DIPSETTING(    0x02, "1" )
	PORT_DIPSETTING(    0x01, "2" )
	PORT_DIPSETTING(    0x00, "3" )
	PORT_DIPNAME( 0x04, 0x04, "Unknown SW2:3" )           PORT_DIPLOCATION("SW2:3")
	PORT_DIPSETTING(    0x04, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x18, 0x18, "Unknown SW2:4,5" )         PORT_DIPLOCATION("SW2:4,5")
	PORT_DIPSETTING(    0x18, "0" )
	PORT_DIPSETTING(    0x10, "1" )
	PORT_DIPSETTING(    0x08, "2" )
	PORT_DIPSETTING(    0x00, "3" )
	PORT_DIPNAME( 0x20, 0x20, "Unknown SW2:6" )           PORT_DIPLOCATION("SW2:6")
	PORT_DIPSETTING(    0x20, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x40, 0x40, "Unknown SW2:7" )           PORT_DIPLOCATION("SW2:7")
	PORT_DIPSETTING(    0x40, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x80, 0x80, "Unknown SW2:8" )           PORT_DIPLOCATION("SW2:8")
	PORT_DIPSETTING(    0x80, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x100, 0x100, "Unknown SW2:9" )         PORT_DIPLOCATION("SW2:9")
	PORT_DIPSETTING(    0x100, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x200, 0x200, "Unknown SW2:10" )        PORT_DIPLOCATION("SW2:10")
	PORT_DIPSETTING(    0x200, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
...
static INPUT_PORTS_START( kotbinsp )
	...
	// Swapped DSW1 & 2
	PORT_START("DSW1")
	PORT_DIPNAME( 0x03, 0x03, "Unknown  SW1:1,2" )        PORT_DIPLOCATION("SW1:1,2")
	PORT_DIPSETTING(    0x03, "0" )
	PORT_DIPSETTING(    0x02, "1" )
	PORT_DIPSETTING(    0x01, "2" )
	PORT_DIPSETTING(    0x00, "3" )
	PORT_DIPNAME( 0x04, 0x04, "Unknown  SW1:3" )          PORT_DIPLOCATION("SW1:3")
	PORT_DIPSETTING(    0x04, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x08, 0x08, "Unknown  SW1:4" )          PORT_DIPLOCATION("SW1:4")
	PORT_DIPSETTING(    0x08, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x10, 0x10, "Unknown  SW1:5" )          PORT_DIPLOCATION("SW1:5")
	PORT_DIPSETTING(    0x10, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	...
	PORT_DIPSETTING(    0x80, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x100, 0x100, "Unknown SW1:9" )         PORT_DIPLOCATION("SW1:9")
	PORT_DIPSETTING(    0x100, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x200, 0x200, "Unknown SW1:10" )        PORT_DIPLOCATION("SW1:10")
	PORT_DIPSETTING(    0x0200, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )

	PORT_START("DSW2")
	PORT_DIPNAME( 0x07, 0x07, "Difficulty?" )             PORT_DIPLOCATION("SW2:1,2,3")
	PORT_DIPSETTING(    0x07, "0" )
	PORT_DIPSETTING(    0x06, "1" )
	PORT_DIPSETTING(    0x05, "2" )
	PORT_DIPSETTING(    0x04, "3" )
	...
	PORT_DIPSETTING(    0x01, "6" )
	PORT_DIPSETTING(    0x00, "7" )
	PORT_DIPNAME( 0x38, 0x38, DEF_STR( Coinage ) )        PORT_DIPLOCATION("SW2:4,5,6")
	PORT_DIPSETTING(    0x00, DEF_STR( 5C_1C ) )
	PORT_DIPSETTING(    0x08, DEF_STR( 4C_1C ) )
	PORT_DIPSETTING(    0x10, DEF_STR( 3C_1C ) )
	PORT_DIPSETTING(    0x18, DEF_STR( 2C_1C ) )
	...
	PORT_DIPSETTING(    0x80, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x100, 0x100, DEF_STR( Demo_Sounds ) )  PORT_DIPLOCATION("SW2:9")
	PORT_DIPSETTING(    0x100, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x200, 0x200, DEF_STR( Flip_Screen ) )  PORT_DIPLOCATION("SW2:10")
	PORT_DIPSETTING(    0x200, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
...
static INPUT_PORTS_START( mjreach1 )
	...
	PORT_DIPSETTING(    0x00, "1 2 3 4 6 8 10 15" )
	PORT_DIPNAME( 0xc0, 0xc0, "Max Rate" )                PORT_DIPLOCATION("SW1:7,8")
	PORT_DIPSETTING(    0xc0, "1" )
	PORT_DIPSETTING(    0x80, "5" )
	PORT_DIPSETTING(    0x40, "10" )
	PORT_DIPSETTING(    0x00, "20" )
	PORT_DIPNAME( 0x100, 0x100, "Credits Per Note" )      PORT_DIPLOCATION("SW1:9")
	PORT_DIPSETTING(    0x100, "5" )
	PORT_DIPSETTING(    0x00, "10" )
	PORT_DIPNAME( 0x200, 0x200, DEF_STR( Flip_Screen ) )  PORT_DIPLOCATION("SW1:10")
	PORT_DIPSETTING(    0x200, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	...
	PORT_DIPSETTING(    0x04, "3" )
	PORT_DIPSETTING(    0x00, "5" )
	PORT_DIPNAME( 0x30, 0x30, "Payout" )                  PORT_DIPLOCATION("SW2:5,6")
	PORT_DIPSETTING(    0x30, "300" )
	PORT_DIPSETTING(    0x20, "500" )
	PORT_DIPSETTING(    0x10, "700" )
	PORT_DIPSETTING(    0x00, "1000" )
	PORT_DIPNAME( 0x40, 0x40, "W-BET" )                   PORT_DIPLOCATION("SW2:7")
	PORT_DIPSETTING(    0x40, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x80, 0x80, "Last Chance" )             PORT_DIPLOCATION("SW2:8")
	PORT_DIPSETTING(    0x80, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x300, 0x300, "Computer Strength" )     PORT_DIPLOCATION("SW2:9,10")
	PORT_DIPSETTING(    0x300, "Weak" )
	PORT_DIPSETTING(    0x200, DEF_STR( Normal ))
	PORT_DIPSETTING(    0x100, "Strong" )
	PORT_DIPSETTING(    0x00, "Very Strong" )

	PORT_START("DSW3")
	PORT_DIPNAME( 0x07, 0x07, "YAKUMAN Bonus" )           PORT_DIPLOCATION("SW3:1,2,3")
	...
	PORT_DIPNAME( 0x18, 0x18, "YAKUMAN Times" )           PORT_DIPLOCATION("SW3:4,5")
//  PORT_DIPSETTING(    0x00, "1" )
	PORT_DIPSETTING(    0x18, "1" )
	PORT_DIPSETTING(    0x10, "2" )
	PORT_DIPSETTING(    0x08, "3" )
	PORT_DIPNAME( 0x20, 0x20, "3 BAI In YAKUMAN Bonus Chance" ) PORT_DIPLOCATION("SW3:6")
	PORT_DIPSETTING(    0x20, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x40, 0x40, "Auto Tsumo" )                    PORT_DIPLOCATION("SW3:7")
	PORT_DIPSETTING(    0x40, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x80, 0x80, "Credit Timing" )                 PORT_DIPLOCATION("SW3:8")
	PORT_DIPSETTING(    0x80, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x100, 0x100, "Game Style" )                  PORT_DIPLOCATION("SW3:9")
	PORT_DIPSETTING(    0x100, "Credit" )
	PORT_DIPSETTING(    0x00, "Credit Time" )
	PORT_DIPNAME( 0x200, 0x200, "Start Method (Credit Time)" )  PORT_DIPLOCATION("SW3:10")
	PORT_DIPSETTING(    0x200, "?" )
	...
	PORT_DIPSETTING(    0x02, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x04, 0x00, "Girls (Demo)" )                  PORT_DIPLOCATION("SW4:3")
	PORT_DIPSETTING(    0x04, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x08, 0x08, "Girls Show After 3 Renso" )      PORT_DIPLOCATION("SW4:4")
	PORT_DIPSETTING(    0x08, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x10, 0x00, "Girls (Play)" )                  PORT_DIPLOCATION("SW4:5")  // Shown as always OFF in dips sheet
	PORT_DIPSETTING(    0x10, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x20, 0x20, "Boys In Game" )                  PORT_DIPLOCATION("SW4:6")
	PORT_DIPSETTING(    0x20, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x40, 0x40, "Boys" )                          PORT_DIPLOCATION("SW4:7")
	PORT_DIPSETTING(    0x40, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x80, 0x80, "Don Den Key" )                   PORT_DIPLOCATION("SW4:8")
	PORT_DIPSETTING(    0x80, "Start" )
	PORT_DIPSETTING(    0x00, "Flip/Flop" )
	PORT_DIPNAME( 0x100, 0x100, "Unknown SW4:9" )               PORT_DIPLOCATION("SW4:9")
	PORT_DIPSETTING(    0x100, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x200, 0x200, "Unknown SW4:10" )              PORT_DIPLOCATION("SW4:10")
	PORT_DIPSETTING(    0x200, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
...
static INPUT_PORTS_START( jongtei )
	...
	PORT_DIPSETTING(    0x10, "1 2 3 5 10 25 50 100" )
	PORT_DIPSETTING(    0x00, "2 3 6 8 12 15 30 50" )
	PORT_DIPNAME( 0xc0, 0xc0, "Max Rate" )                      PORT_DIPLOCATION("SW1:7,8")
	PORT_DIPSETTING(    0xc0, "1" )
	PORT_DIPSETTING(    0x80, "5" )
	PORT_DIPSETTING(    0x40, "10" )
	PORT_DIPSETTING(    0x00, "20" )
	MAHJONG_NOTE_CREDITS(8, "SW1:9", "DSW2", 0)
	PORT_DIPSETTING(    0x100, "5" )
	PORT_DIPSETTING(    0x00, "10" )
	PORT_DIPNAME( 0x200, 0x200, DEF_STR( Flip_Screen ) )        PORT_DIPLOCATION("SW1:10")
	PORT_DIPSETTING(    0x200, DEF_STR( Off ) )
...
//  PORT_DIPSETTING(    0x00, "1" )
	...
	PORT_DIPSETTING(    0x02, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x04, 0x00, "Girls" )                          PORT_DIPLOCATION("SW4:3")
	PORT_DIPSETTING(    0x04, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x08, 0x08, "Girls Show After 3 Renso" )       PORT_DIPLOCATION("SW4:4")
	PORT_DIPSETTING(    0x08, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x10, 0x00, "Unknown SW4:5" )               PORT_DIPLOCATION("SW4:5")
	PORT_DIPSETTING(    0x10, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x20, 0x20, "Unknown SW4:6" )               PORT_DIPLOCATION("SW4:6")
	PORT_DIPSETTING(    0x20, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x40, 0x40, "Unknown SW4:7" )               PORT_DIPLOCATION("SW4:7")
	PORT_DIPSETTING(    0x40, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x80, 0x80, "Unknown SW4:8" )               PORT_DIPLOCATION("SW4:8")
	PORT_DIPSETTING(    0x80, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x100, 0x100, "Unknown SW4:9" )                PORT_DIPLOCATION("SW4:9")
	PORT_DIPSETTING(    0x100, DEF_STR( Off ) )
...
static INPUT_PORTS_START( mjchuuka )
	...
	PORT_DIPSETTING(    0x0e, "93" )
	PORT_DIPSETTING(    0x0f, "96" )
	PORT_DIPNAME( 0x30, 0x30, "Odds Rate" )                     PORT_DIPLOCATION("SW1:5,6") //TOCHECK: Seems to have no effect
	PORT_DIPSETTING(    0x30, "1 2 4 8 12 16 24 32" )
	PORT_DIPSETTING(    0x20, "2 3 6 8 12 15 30 50" )
	PORT_DIPSETTING(    0x10, "1 2 3 5 10 25 50 100" )
	PORT_DIPSETTING(    0x00, "2 3 6 8 12 15 30 50" )
	...
	PORT_DIPSETTING(    0x100, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x200, 0x200, "Credits Per Note" )            PORT_DIPLOCATION("SW1:10")
	PORT_DIPSETTING(    0x200, "5" )
	PORT_DIPSETTING(    0x00, "10" )
...
//  PORT_DIPSETTING(    0x00, "1" )
	...
	PORT_DIPSETTING(    0x20, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x40, 0x40, "DonDen Key" )                    PORT_DIPLOCATION("SW3:7")
	...
	PORT_DIPSETTING(    0x80, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x100, 0x100, DEF_STR( Unknown ) )            PORT_DIPLOCATION("SW4:9")
	PORT_DIPSETTING(    0x100, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x200, 0x200, DEF_STR( Unknown ) )            PORT_DIPLOCATION("SW4:10")
	PORT_DIPSETTING(    0x200, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
...
static INPUT_PORTS_START( mjschuka )
	...
	PORT_START("DSW1")
	MAHJONG_PAYOUT_RATE(0, "SW 1:1,2,3,4")                      PORT_DIPLOCATION("SW1:1,2,3,4")
	...
	PORT_START("DSW3")
	PORT_DIPNAME( 0x01, 0x00, DEF_STR( Demo_Sounds ) )          PORT_DIPLOCATION("SW3:1")
	PORT_DIPSETTING(    0x01, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x02, 0x00, "In Game Music" )                 PORT_DIPLOCATION("SW3:2")
	PORT_DIPSETTING(    0x02, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	...
	PORT_DIPSETTING(    0x08, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x10, 0x00, "Girls" )                         PORT_DIPLOCATION("SW3:5")
	PORT_DIPSETTING(    0x10, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x20, 0x00, "Girls Speech" )                  PORT_DIPLOCATION("SW3:6")
	PORT_DIPSETTING(    0x20, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	...
	PORT_START("DSW4")
	PORT_DIPNAME( 0x07, 0x07, "YAKUMAN Bonus" )                 PORT_DIPLOCATION("SW4:1,2,3")
	PORT_DIPSETTING(    0x07, "Cut" )
	PORT_DIPSETTING(    0x06, "1 T" )
	PORT_DIPSETTING(    0x05, "300" )
	PORT_DIPSETTING(    0x04, "500" )
	...
	PORT_DIPSETTING(    0x01, "1000?" )
	PORT_DIPSETTING(    0x00, "1000?" )
	PORT_DIPNAME( 0x08, 0x08, "YAKUMAN Times?" )                PORT_DIPLOCATION("SW4:4")
	PORT_DIPSETTING(    0x00, "1" )
	PORT_DIPSETTING(    0x08, "2" )
	...
	PORT_DIPSETTING(    0x20, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x40, 0x40, "DonDen Key" )                    PORT_DIPLOCATION("SW4:7")
	PORT_DIPSETTING(    0x40, "Start" )
	PORT_DIPSETTING(    0x00, "Flip Flop" )
	PORT_DIPNAME( 0x80, 0x80, DEF_STR( Unknown ) )              PORT_DIPLOCATION("SW4:8")
	PORT_DIPSETTING(    0x80, DEF_STR( Off ) )
...
static INPUT_PORTS_START( mjmyster )
	...
	PORT_DIPSETTING(    0x40, "10" )
	PORT_DIPSETTING(    0x00, "20" )
	MAHJONG_NOTE_CREDITS(8, "SW 1:9", "DSW2", 0)
	PORT_DIPNAME( 0x200, 0x00, DEF_STR(Flip_Screen) )            PORT_DIPLOCATION("SW 1:10")
	PORT_DIPSETTING(    0x200, DEF_STR(On) )
	PORT_DIPSETTING(    0x00, DEF_STR(Off) )
	...
	PORT_DIPNAME( 0x300, 0x00, "Yakuman/Fever Chance Ratio" )    PORT_DIPLOCATION("SW 2:9,10")    // ボーナスチャンス周期設定時の役満チャンス＆フィバーチャンスの比率
	PORT_DIPSETTING(    0x300, "Yakuman Chance Only" )                                            // 役満チャンスのみ
	PORT_DIPSETTING(    0x200, "Equal Yakuman/Fever Chance" )                                     // 役満チャンスとフィバーチャンスが同等
	PORT_DIPSETTING(    0x100, "Frequent Fever Chance" )                                          // フィバーチャンスが多い
	...
	PORT_DIPNAME( 0x18, 0x00, "Credit Limit" )                  PORT_DIPLOCATION("SW 3:4,5")
	PORT_DIPSETTING(    0x18, "300" )
	PORT_DIPSETTING(    0x10, "500" )
	PORT_DIPSETTING(    0x08, "700" )
	PORT_DIPSETTING(    0x00, "1000" )
	PORT_DIPNAME( 0x20, 0x00, "Computer Strength" )             PORT_DIPLOCATION("SW 3:6")
	...
	PORT_DIPSETTING(    0x80, DEF_STR(Off) )
	PORT_DIPSETTING(    0x00, DEF_STR(On) )
	PORT_DIPNAME( 0x100, 0x00, "Auto Reach" )                   PORT_DIPLOCATION("SW 3:9")
	PORT_DIPSETTING(    0x100, DEF_STR(Off) )
	PORT_DIPSETTING(    0x00, DEF_STR(On) )
	PORT_DIPNAME( 0x200, 0x200, "Don Den Button" )              PORT_DIPLOCATION("SW 3:10")
	PORT_DIPSETTING(    0x200, "Start" )
	PORT_DIPSETTING(    0x00, "Flip Flop" )
	...
	PORT_DIPSETTING(    0x04, DEF_STR(Off) )
	PORT_DIPSETTING(    0x00, DEF_STR(On) )
	PORT_DIPNAME( 0x10, 0x00, "In-Game Hints" )                 PORT_DIPLOCATION("SW 4:5")
	PORT_DIPSETTING(    0x10, DEF_STR(Off) )
	PORT_DIPSETTING(    0x00, DEF_STR(On) )
	PORT_DIPNAME( 0x20, 0x00, DEF_STR(Unused) )                 PORT_DIPLOCATION("SW 4:6")
	PORT_DIPSETTING(    0x00, DEF_STR(Off) )
	PORT_DIPSETTING(    0x20, DEF_STR(On) )  
	PORT_DIPNAME( 0x40, 0x40, DEF_STR( Unused ) )               PORT_DIPLOCATION("SW 4:7")
	PORT_DIPSETTING(    0x40, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x80, 0x80, DEF_STR(Region) )                 PORT_DIPLOCATION("SW 4:8") //Manual says always OFF with no name indication
	PORT_DIPSETTING(    0x80, DEF_STR(Japan) )
	PORT_DIPSETTING(    0x00, DEF_STR(Hong_Kong) )
	PORT_DIPNAME( 0x100, 0x100, DEF_STR( Unused ) )             PORT_DIPLOCATION("SW 4:9")
	PORT_DIPSETTING(    0x100, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x200, 0x200, DEF_STR( Unused ) )             PORT_DIPLOCATION("SW 4:10")
	PORT_DIPSETTING(    0x200, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
INPUT_PORTS_END
...
//  PORT_BIT( 0x80, IP_ACTIVE_LOW, IPT_UNKNOWN )
	...
	PORT_DIPSETTING(    0x02, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )
	PORT_DIPNAME( 0x04, 0x04, "Show Girls" )               PORT_DIPLOCATION("SW4:3")
	PORT_DIPSETTING(    0x04, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )
	PORT_DIPNAME( 0x08, 0x08, "Secret Skills" )            PORT_DIPLOCATION("SW4:4")
	PORT_DIPSETTING(    0x08, DEF_STR( On ) )
	...
	PORT_DIPSETTING(    0x20, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )
	PORT_DIPNAME( 0x40, 0x00, DEF_STR( Unused ) )          PORT_DIPLOCATION("SW4:7") // Manual says it is always OFF and not used
	PORT_DIPSETTING(    0x40, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )                                          //								 8	 9
	PORT_DIPNAME( 0x180, 0x180, "Panel" )                  PORT_DIPLOCATION("SW4:8,9")  // SW4-8,9      Hanafuda Panel: OFF OFF
	PORT_DIPSETTING(    0x180, "Hanafuda" ) //                                                          Mahjong Panel : ON  OFF
	PORT_DIPSETTING(    0x100, "Mahjong" ) // Manual says 'Mahjong Panel'                               Hanaroku Panel: OFF ON
	PORT_DIPSETTING(    0x80, "Hanaroku" ) // Manual says 'Hanaroku Panel'                              Not used	  : ON 	ON
	PORT_DIPSETTING(    0x00, DEF_STR( Unused ) )
	PORT_SERVICE_DIPLOC(  0x0200, IP_ACTIVE_LOW, "SW4:10" )
	
	PORT_START("DSW3")
	PORT_DIPNAME( 0x03, 0x01, "Machine" )                  PORT_DIPLOCATION("SW3:1,2")
	PORT_DIPSETTING(    0x03, "Credit Type" )
	PORT_DIPSETTING(    0x02, "Hopper Type" )
	PORT_DIPSETTING(    0x01, "Timer Type" )
	PORT_DIPSETTING(    0x00, DEF_STR( Unused ) )
	...
	PORT_DIPSETTING(    0x00, "Set 4" )
	PORT_DIPNAME( 0x100, 0x100, "Show Girl at Game Start" )  PORT_DIPLOCATION("SW3:9")
	PORT_DIPSETTING(    0x100, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )
	PORT_DIPNAME( 0x200, 0x200, "In-Game Voice" )            PORT_DIPLOCATION("SW3:10")
	PORT_DIPSETTING(    0x200, DEF_STR( On ) )
	...
	PORT_DIPSETTING(    0x08, "Often" )
	PORT_DIPSETTING(    0x00, "Rarely" )
	PORT_DIPNAME( 0x10, 0x00, "Biggest Bonus Enabled" )    PORT_DIPLOCATION("SW1:5")
	PORT_DIPSETTING(    0x10, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )
...
static INPUT_PORTS_START( hgokou )
	...
	PORT_DIPSETTING(    0x01, "7" )
	PORT_DIPSETTING(    0x00, "8 (Hard)" )
	PORT_DIPNAME( 0x08, 0x00, "Level Increase" )           PORT_DIPLOCATION("SW1:4")
	PORT_DIPSETTING(    0x08, "Often" )
	PORT_DIPSETTING(    0x00, "Rarely" )
	PORT_DIPNAME( 0x10, 0x10, "Biggest Bonus Enabled" )    PORT_DIPLOCATION("SW1:5")
	PORT_DIPSETTING(    0x10, DEF_STR( On ) )
	...
	PORT_DIPSETTING(    0x20, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )
	PORT_DIPNAME( 0x40, 0x00, DEF_STR( Unused ) )          PORT_DIPLOCATION("SW4:7") // Manual says it is always OFF
	PORT_DIPSETTING(    0x40, DEF_STR( On ) )
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )                                          //								 8	 9
	PORT_DIPNAME( 0x180, 0x180, "Panel" )                  PORT_DIPLOCATION("SW4:8,9")  // SW4-8,9      Hanafuda Panel: OFF OFF
	PORT_DIPSETTING(    0x180, "Hanafuda" ) //                                                          Mahjong Panel : ON  OFF
...
static INPUT_PORTS_START( mjmyornt )
	...
	PORT_START("DSW1")
	MAHJONG_PAYOUT_RATE(0, "SW1:1,2,3,4")                                                        // ＰＡＹ－ＯＵＴ　管理
	PORT_DIPNAME( 0x30, 0x00, "Odds Rate" )                     PORT_DIPLOCATION("SW1:5,6")      // ＯＤＤＳ　設定
	PORT_DIPSETTING(    0x30, "1 2 4 8 12 16 24 32" )
	PORT_DIPSETTING(    0x00, "1 2 3 5 8 15 30 50" )
	PORT_DIPSETTING(    0x20, "2 3 6 8 12 15 30 50" )
	PORT_DIPSETTING(    0x10, "1 2 3 5 10 25 50 100" )
	PORT_DIPNAME( 0xc0, 0x40, "Maximum Bet" )                   PORT_DIPLOCATION("SW1:7,8")      // ＢＥＴ－ＭＡＸ
	PORT_DIPSETTING(    0xc0, "1" )
	PORT_DIPSETTING(    0x80, "5" )
	PORT_DIPSETTING(    0x40, "10" )
	PORT_DIPSETTING(    0x00, "20" )
	MAHJONG_NOTE_CREDITS(8, "SW1:9", "DSW2", 0) //TODO: Manage Alternate Coinage
	PORT_DIPNAME( 0x200, 0x200, DEF_STR( Flip_Screen ) )        PORT_DIPLOCATION("SW1:10")       // ー画面反転
	PORT_DIPSETTING(    0x200, DEF_STR(Off) )                                                     // 通常
	PORT_DIPSETTING(    0x00, DEF_STR(On) )                                                       // 反転

	PORT_START("DSW2")
	PORT_DIPNAME( 0x03, 0x03, DEF_STR(Coinage) )                PORT_DIPLOCATION("SW2:1,2")      // ＣＯＩＮ　ＲＡＴＥ
	PORT_DIPSETTING(    0x03, DEF_STR(1C_1C) )       PORT_CONDITION("DSW4", 0x100, EQUALS, 0x100) // １コイン　　　１プレイ
	PORT_DIPSETTING(    0x02, DEF_STR(1C_2C) )       PORT_CONDITION("DSW4", 0x100, EQUALS, 0x100) // １コイン　　　２プレイ
	PORT_DIPSETTING(    0x01, DEF_STR(1C_5C) )       PORT_CONDITION("DSW4", 0x100, EQUALS, 0x100) // １コイン　　　５プレイ
	PORT_DIPSETTING(    0x00, "1 Coin/10 Credits" )  PORT_CONDITION("DSW4", 0x100, EQUALS, 0x100) // １コイン　　１０プレイ
	PORT_DIPSETTING(    0x03, DEF_STR( 1C_1C ) )     PORT_CONDITION("DSW4", 0x100, EQUALS, 0x00)
	PORT_DIPSETTING(    0x02, DEF_STR( 1C_2C ) )     PORT_CONDITION("DSW4", 0x100, EQUALS, 0x00)
	PORT_DIPSETTING(    0x01, DEF_STR( 2C_2C ) )     PORT_CONDITION("DSW4", 0x100, EQUALS, 0x00)
	PORT_DIPSETTING(    0x00, DEF_STR( 2C_3C ) )     PORT_CONDITION("DSW4", 0x100, EQUALS, 0x00)
	PORT_DIPNAME( 0x0c, 0x0c, "Minimum Bet" )                   PORT_DIPLOCATION("SW2:3,4")      // 最低　ＢＥＴ　ＲＡＴＥ
	...
	PORT_DIPNAME( 0x300, 0x00, "Yakuman/Fever Chance Ratio" )    PORT_DIPLOCATION("SW2:9,10")    // ボーナスチャンス周期設定時の役満チャンス＆フィバーチャンスの比率
	PORT_DIPSETTING(    0x300, "Yakuman Chance Only" )                                            // 役満チャンスのみ
	PORT_DIPSETTING(    0x200, "Equal Yakuman/Fever Chance" )                                     // 役満チャンスとフィバーチャンスが同等
	PORT_DIPSETTING(    0x100, "Frequent Fever Chance" )                                          // フィバーチャンスが多い
	PORT_DIPSETTING(    0x00, "Fever Chance Only" )                                               // フィバーチャンスのみ
	...
	PORT_DIPNAME( 0x04, 0x04, "Hopper Polarity" )               PORT_DIPLOCATION("SW3:3")        // ホッパー・アクティブ
	PORT_DIPSETTING(    0x04, DEF_STR(Normal) )                                                   // 通常
	PORT_DIPSETTING(    0x00, "Inverted" )                                                        // 反転
	PORT_DIPNAME( 0x18, 0x00, "Credit Limit" )                  PORT_DIPLOCATION("SW3:4,5")      // クレジット・コイン リミット
	PORT_DIPSETTING(    0x18, "300" )
	PORT_DIPSETTING(    0x10, "500" )
	PORT_DIPSETTING(    0x08, "700" )
	PORT_DIPSETTING(    0x00, "1000" )
	...
	PORT_DIPNAME( 0x40, 0x00, "Double Bet" )                    PORT_DIPLOCATION("SW3:7")        // Ｗ－ＢＥＴ機能 有無
	PORT_DIPSETTING(    0x40, DEF_STR(Off) )                                                      // 無
	PORT_DIPSETTING(    0x00, DEF_STR(On) )                                                       // 有
	PORT_DIPNAME( 0x80, 0x00, "Last Chance" )                   PORT_DIPLOCATION("SW3:8")        // ラスト・チャンス 有無
	PORT_DIPSETTING(    0x80, DEF_STR(Off) )                                                      // 無
	PORT_DIPSETTING(    0x00, DEF_STR(On) )                                                       // 有
	PORT_DIPNAME( 0x100, 0x00, "Auto Reach" )                    PORT_DIPLOCATION("SW3:9")       // オート・リーチ機能の有無
	PORT_DIPSETTING(    0x100, DEF_STR(Off) )                                                     // 無
	PORT_DIPSETTING(    0x00, DEF_STR(On) )                                                       // 有
	PORT_DIPNAME( 0x200, 0x00, "Don Den Button" )                PORT_DIPLOCATION("SW3:10")      // Ｄｏｎ・Ｄｅｎ　機能ボタン変更
	PORT_DIPSETTING(    0x200, "Start" )                                                          // スタート・ボタ
	PORT_DIPSETTING(    0x00, "Flip Flop" )                                                       // Ｆ／Ｆ・ボタン

	PORT_START("DSW4")
	PORT_DIPNAME( 0x01, 0x00, DEF_STR(Demo_Sounds) )            PORT_DIPLOCATION("SW4:1")        // デモ・サウンド 有無
	PORT_DIPSETTING(    0x01, DEF_STR(Off) )                                                      // 無
	PORT_DIPSETTING(    0x00, DEF_STR(On) )                                                       // 有
	PORT_DIPNAME( 0x02, 0x00, "In-Game Music" )                 PORT_DIPLOCATION("SW4:2")        // ゲーム・サウンド 有無
	PORT_DIPSETTING(    0x02, DEF_STR(Off) )                                                      // 無
	PORT_DIPSETTING(    0x00, DEF_STR(On) )                                                       // 有
	PORT_DIPNAME( 0x04, 0x00, "Show Gals" )                     PORT_DIPLOCATION("SW4:3")        // ギャル表示 有無
	PORT_DIPSETTING(    0x04, DEF_STR(Off) )                                                      // 無
	PORT_DIPSETTING(    0x00, DEF_STR(On) )                                                       // 有
	PORT_DIPNAME( 0x08, 0x00, "Gal Select" )                    PORT_DIPLOCATION("SW4:4")        // ギャル選択機能 有無
	PORT_DIPSETTING(    0x08, DEF_STR(Off) )                                                      // 無
	PORT_DIPSETTING(    0x00, DEF_STR(On) )                                                       // 有
	PORT_DIPNAME( 0x10, 0x00, "Show Game Action" )              PORT_DIPLOCATION("SW4:5")        // ゲームアクション表示 有無
	PORT_DIPSETTING(    0x10, DEF_STR(Off) )                                                      // 無
	PORT_DIPSETTING(    0x00, DEF_STR(On) )                                                       // 有
	PORT_DIPNAME( 0x20, 0x00, "In-Game Mat" )                   PORT_DIPLOCATION("SW4:6")        // ゲーム中のマット変更
	PORT_DIPSETTING(    0x00, "Plain" )                                                           // 無地
	PORT_DIPSETTING(    0x20, "With Character" )                                                  // キャラクター有
	PORT_DIPNAME( 0x40, 0x40, "Time Settings Mode" )            PORT_DIPLOCATION("SW4:7")        // 時間設定モード
	PORT_DIPSETTING(    0x40, DEF_STR(Off) )                                                      // 設定時
	PORT_DIPSETTING(    0x00, DEF_STR(On) )                                                       // 通常
	PORT_DIPUNKNOWN_DIPLOC( 0x80, 0x80, "SW4:8" )                                                // ＯＦＦ固定
	PORT_DIPNAME( 0x100, 0x100, "Alternate Coinage" )             PORT_DIPLOCATION("SW4:9")      // ＯＦＦ固定
	PORT_DIPSETTING(    0x100, DEF_STR(Off) )
	PORT_DIPSETTING(    0x00, DEF_STR(On) )
	PORT_DIPUNKNOWN_DIPLOC( 0x200, 0x200, "SW4:10" )                                             // ＯＦＦ固定
INPUT_PORTS_END
...
static INPUT_PORTS_START( mjmyorn2 )
	...
	PORT_MODIFY("DSW2")
	PORT_DIPNAME( 0x03, 0x03, DEF_STR( Coin_A ) ) PORT_DIPLOCATION("SW2:1,2")
	PORT_DIPSETTING(    0x03, DEF_STR( 1C_1C ) )     PORT_CONDITION("DSW4", 0x100, EQUALS, 0x100)
	PORT_DIPSETTING(    0x02, DEF_STR( 1C_2C ) )     PORT_CONDITION("DSW4", 0x100, EQUALS, 0x100)
	PORT_DIPSETTING(    0x01, DEF_STR( 1C_5C ) )     PORT_CONDITION("DSW4", 0x100, EQUALS, 0x100)
	PORT_DIPSETTING(    0x00, "1 Coin/10 Credits" )  PORT_CONDITION("DSW4", 0x100, EQUALS, 0x100)
	PORT_DIPSETTING(    0x03, DEF_STR( 1C_1C ) )     PORT_CONDITION("DSW4", 0x100, EQUALS, 0x00) // different alternate coinage
	PORT_DIPSETTING(    0x02, DEF_STR( 2C_1C ) )     PORT_CONDITION("DSW4", 0x100, EQUALS, 0x00) 
	PORT_DIPSETTING(    0x01, DEF_STR( 2C_2C ) )     PORT_CONDITION("DSW4", 0x100, EQUALS, 0x00)
	PORT_DIPSETTING(    0x00, DEF_STR( 2C_3C ) )     PORT_CONDITION("DSW4", 0x100, EQUALS, 0x00)
INPUT_PORTS_END
...
//  PORT_DIPSETTING(    0x30, "2 3 6 8 12 15 30 50" )
	...
	PORT_DIPSETTING(    0x04, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0x38, 0x38, "Multiplier" )                    PORT_DIPLOCATION("SW2:4,5,6")
	PORT_DIPSETTING(    0x38, "1" )
	PORT_DIPSETTING(    0x30, "2" )
	PORT_DIPSETTING(    0x28, "3" )
	PORT_DIPSETTING(    0x20, "4" )
	...
	PORT_DIPSETTING(    0x08, "7" )
	PORT_DIPSETTING(    0x00, "8" )
	PORT_DIPNAME( 0xc0, 0xc0, "Payout" )                        PORT_DIPLOCATION("SW2:7,8")
	PORT_DIPSETTING(    0xc0, "300" )
	PORT_DIPSETTING(    0x80, "500" )
	PORT_DIPSETTING(    0x40, "700" )
	PORT_DIPSETTING(    0x00, "1000" )
...
//  PORT_DIPSETTING(    0x00, "1000" )
	...
	PORT_START("DSW4")
	PORT_SERVICE( 0x01, IP_ACTIVE_LOW )                         PORT_DIPLOCATION("SW4:1")
	PORT_DIPNAME( 0x02, 0x02, "DonDen Key" )                    PORT_DIPLOCATION("SW4:2")
...
void ddenlovr_state::mjmyornt(machine_config &config)
{
	...
	ay8910_device &aysnd(AY8910(config, "aysnd", 1789772));
	//aysnd.port_a_read_callback().set(FUNC(ddenlovr_state::mjmyster_dsw_r));
	aysnd.port_b_write_callback().set(FUNC(ddenlovr_state::ddenlovr_select_w));
	aysnd.add_route(ALL_OUTPUTS, "mono", 0.30);
}

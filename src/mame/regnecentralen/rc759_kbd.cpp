...
static INPUT_PORTS_START( keyboard )
	...
	PORT_BIT(0x8000, IP_ACTIVE_HIGH, IPT_KEYBOARD) /* 15 */ PORT_CODE(KEYCODE_ESC)        PORT_CHAR(UCHAR_MAMEKEY(ESC)) PORT_NAME("Esc") // firmware keymap[15]=0x1b in all tables; this is the real Esc key (was mislabeled "Alt")
	...
	PORT_BIT(0x0080, IP_ACTIVE_HIGH, IPT_UNUSED)   /* 55 */ // firmware keymap[55]=0xff (dead position); host Esc now drives position 15 above

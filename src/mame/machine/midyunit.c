...
static void init_generic(running_machine *machine, int bpp, int sound, int prot_start, int prot_end)
{
	...
	switch (sound)
	{
		case SOUND_CVSD_SMALL:
			...
			memory_install_write8_handler(machine->device("cvsdcpu")->memory().space(ADDRESS_SPACE_PROGRAM), prot_start, prot_end, 0, 0, cvsd_protection_w);
			state->cvsd_protection_base = machine->region("cvsdcpu")->base() + 0x10000 + (prot_start - 0x8000);
			break;

		case SOUND_CVSD:
			williams_cvsd_init(machine);
			memory_install_ram(machine->device("cvsdcpu")->memory().space(ADDRESS_SPACE_PROGRAM), prot_start, prot_end, 0, 0, NULL);
			break;

		case SOUND_ADPCM:
			williams_adpcm_init(machine);
			memory_install_ram(machine->device("adpcm")->memory().space(ADDRESS_SPACE_PROGRAM), prot_start, prot_end, 0, 0, NULL);
			break;

		case SOUND_NARC:
			williams_narc_init(machine);
			memory_install_ram(machine->device("narc1cpu")->memory().space(ADDRESS_SPACE_PROGRAM), prot_start, prot_end, 0, 0, NULL);
			break;
...
{
	/* protection */
	memory_install_read16_handler(machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0xfffff400, 0xfffff40f, 0, 0, mkturbo_prot_r);
...
static void term2_init_common(running_machine *machine, write16_space_func hack_w)
{
	...
	/* special inputs */
	memory_install_read16_handler(machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x01c00000, 0x01c0005f, 0, 0, term2_input_r);
	memory_install_write16_handler(machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x01e00000, 0x01e0001f, 0, 0, term2_sound_w);

	/* HACK: this prevents the freeze on the movies */
	/* until we figure whats causing it, this is better than nothing */
	state->t2_hack_mem = memory_install_write16_handler(machine->device("maincpu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x010aa0e0, 0x010aa0ff, 0, 0, hack_w);
}

...
void install_kof98_protection( running_machine *machine )
{
	...
	memory_install_write16_handler(machine->device("maincpu")->memory().space(AS_PROGRAM), 0x20aaaa, 0x20aaab, 0, 0, kof98_prot_w);
}
...
static void sma_install_random_read_handler( running_machine *machine, int addr1, int addr2 )
{
	...
	memory_install_read16_handler(machine->device("maincpu")->memory().space(AS_PROGRAM), addr1, addr1 + 1, 0, 0, sma_random_r);
	memory_install_read16_handler(machine->device("maincpu")->memory().space(AS_PROGRAM), addr2, addr2 + 1, 0, 0, sma_random_r);
}


void kof99_install_protection( running_machine *machine )
{
	memory_install_write16_handler(machine->device("maincpu")->memory().space(AS_PROGRAM), 0x2ffff0, 0x2ffff1, 0, 0, kof99_bankswitch_w);
	memory_install_read16_handler(machine->device("maincpu")->memory().space(AS_PROGRAM), 0x2fe446, 0x2fe447, 0, 0, prot_9a37_r);
...
{
	memory_install_write16_handler(machine->device("maincpu")->memory().space(AS_PROGRAM), 0x2fffc0, 0x2fffc1, 0, 0, garou_bankswitch_w);
	memory_install_read16_handler(machine->device("maincpu")->memory().space(AS_PROGRAM), 0x2fe446, 0x2fe447, 0, 0, prot_9a37_r);
...
{
	memory_install_write16_handler(machine->device("maincpu")->memory().space(AS_PROGRAM), 0x2fffc0, 0x2fffc1, 0, 0, garouo_bankswitch_w);
	memory_install_read16_handler(machine->device("maincpu")->memory().space(AS_PROGRAM), 0x2fe446, 0x2fe447, 0, 0, prot_9a37_r);
...
{
	memory_install_write16_handler(machine->device("maincpu")->memory().space(AS_PROGRAM), 0x2fffe4, 0x2fffe5, 0, 0, mslug3_bankswitch_w);
	memory_install_read16_handler(machine->device("maincpu")->memory().space(AS_PROGRAM), 0x2fe446, 0x2fe447, 0, 0, prot_9a37_r);
...
{
	memory_install_write16_handler(machine->device("maincpu")->memory().space(AS_PROGRAM), 0x2fffec, 0x2fffed, 0, 0, kof2000_bankswitch_w);
	memory_install_read16_handler(machine->device("maincpu")->memory().space(AS_PROGRAM), 0x2fe446, 0x2fe447, 0, 0, prot_9a37_r);
...
void install_pvc_protection( running_machine *machine )
{
	...
	memory_install_readwrite16_handler(machine->device("maincpu")->memory().space(AS_PROGRAM), 0x2fe000, 0x2fffff, 0, 0, pvc_prot_r, pvc_prot_w);
}

...
void mcpx_ohci_device::device_timer(timer_instance const &timer)
{
	if (ohci_usb)
		ohci_usb->timer(timer.id(), timer.param(), timer.ptr());
}
...
void mcpx_apu_device::device_timer(timer_instance const &timer)
{
	int cmd;
	int bb, b, v;
	uint64_t bv;

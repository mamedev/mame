...
// $c3ff00-$c3ffff
uint16_t namcond1_state::cuskey_r(address_space &space, offs_t offset, uint16_t mem_mask)
{
	switch( offset )
	{
...
void namcond1_state::cuskey_w(address_space &space, offs_t offset, uint16_t data, uint16_t mem_mask)
{
	switch( offset )
	{
		case (0x0a>>1):

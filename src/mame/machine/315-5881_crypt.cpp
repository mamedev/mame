...
void sega_315_5881_crypt_device::enc_start()
{
	...
	if ((buffer_bit2 != 15) && (buffer_bit2 < 14)) // if we have remaining bits in the decompression buffer we shouldn't read the next word yet but should instead use the bits we have?? (twcup98) (might just be because we should be pulling bytes not words?)
	{

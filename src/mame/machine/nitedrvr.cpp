...
uint8_t nitedrvr_state::in0_r(offs_t offset)
{
	...
	switch (offset & 0x03)
	{
		...
		case 0x03:                      // Remap our steering
			return (ioport("DSW2")->read() | steering());
		default:
			return 0xff;
	}
}
...
{
	m_speed1->write_line(BIT(data, 0));
	m_speed2->write_line(BIT(data, 1));
	m_speed3->write_line(BIT(data, 2));
	m_speed4->write_line(BIT(data, 3));
	m_skid1->write_line(BIT(data, 4));
	m_skid2->write_line(BIT(data, 5));
}
...
void nitedrvr_state::out1_w(uint8_t data)
{
	...
	m_crash_en = BIT(data, 0);

	m_crash->write_line(m_crash_en); // Crash enable
	m_attract->write_line(BIT(data, 1));      // Attract enable (sound disable)

...
void rmnimbus_state::decode_dssi_generic(uint16_t ds, uint16_t si)
{
	...
	uint16_t *params = (uint16_t *)get_regpair_ptr(space, ds, si);
...
void rmnimbus_state::decode_dssi_f_fill_area(uint16_t ds, uint16_t si)
{
	...
	t_area_params *area_params = (t_area_params *)get_regpair_ptr(space, ds, si);

	if (!DEBUG_SET(DECODE_BIOS_RAW))
		OUTPUT_SEGOFS("SegBrush:OfsBrush", area_params->seg_brush, area_params->ofs_brush);

	t_nimbus_brush *brush = (t_nimbus_brush *)space.get_read_ptr(LINEAR_ADDR(area_params->seg_brush, area_params->ofs_brush));
	...
	uint16_t *addr_ptr = (uint16_t *)space.get_read_ptr(LINEAR_ADDR(area_params->seg_data, area_params->ofs_data));
	for (int cocount = 0; cocount < area_params->count; cocount++)
	{
		if (DEBUG_SET(DECODE_BIOS_RAW))
		{
...
void rmnimbus_state::decode_dssi_f_plot_character_string(uint16_t ds, uint16_t si)
{
	...
	t_plot_string_params *plot_string_params = (t_plot_string_params *)get_regpair_ptr(space, ds, si);
	...
	uint8_t *char_ptr = (uint8_t*)space.get_read_ptr(LINEAR_ADDR(plot_string_params->seg_data, plot_string_params->ofs_data));

	if (plot_string_params->length == 0xffff)
		logerror("%s",char_ptr);
	else
		for (int charno = 0; charno < plot_string_params->length; charno++)
...
{
	address_space &space = m_maincpu->space(AS_PROGRAM);
	uint16_t *new_colours = (uint16_t *)get_regpair_ptr(space, ds, si);
...
void rmnimbus_state::decode_dssi_f_rw_sectors(uint16_t ds, uint16_t si)
{
	...
	for (int param_no = 0; param_no < 16; param_no++)
...
void rmnimbus_state::decode_dos21(device_t *device,offs_t pc)
{
	...
	{
		char *path = (char *)get_regpair_ptr(space, ds, dx);
...
void rmnimbus_state::nimbus_bank_memory()
{
	...
	for (int blockno = 0; blockno < 8; blockno++)
	{
		char bank[10];
		...
		int block_ofs = (ramsel == 0x07) ? 0 : ((blockno % 4) * 128);
		...
		{
			uint8_t *map_base = (ramsel == 0x07) ? map_blocks[map_blockno] : &map_blocks[map_blockno][block_ofs * 1024];
...
uint8_t rmnimbus_state::nimbus_pc8031_iou_r(offs_t offset)
{
	...
	{
		case 0x00:
			result = m_ipc_interface.ipc_in;
			m_ipc_interface.status_out &= ~IPC_OUT_READ_PEND;
			m_ipc_interface.status_in &= ~IPC_IN_BYTE_AVAIL;
			break;

		case 0x01:
			result = m_ipc_interface.status_in;
			break;
	}
...
void rmnimbus_state::nimbus_pc8031_port1_w(uint8_t data)
{
	...
	if (!BIT(data, 3))
		m_eeprom_state = 0;
	else if (!BIT(data, 1) || m_eeprom_state == 2)
		m_eeprom_state = 2;
	else if (BIT(data, 3) && !(m_eeprom_bits & 8))
		m_eeprom_state = 1;
	else if (!BIT(data, 0) && (m_eeprom_bits & 1) && (m_eeprom_state == 1))
		m_eeprom_state = 2; //wait until 1 clk after cs rises to set di else it's seen as a start bit

	m_eeprom->di_write((BIT(data, 1) && (m_eeprom_state == 2)) ? 1 : 0);
	m_eeprom->clk_write(data & 1);
	m_eeprom_bits = data;
...
/*
    ...
    this is currently un-implemented (and may never be as no rompacks have been dumped).
...
TIMER_CALLBACK_MEMBER(rmnimbus_state::do_mouse)
{
	...
	// Read mouse positions and calculate difference from previous value
	uint8_t mouse_x = m_io_mousex->read();
	uint8_t mouse_y = m_io_mousey->read();

	// Difference from previous X and Y
	int8_t xdiff = m_nimbus_mouse.m_mouse_x - mouse_x;
	int8_t ydiff = m_nimbus_mouse.m_mouse_y - mouse_y;
	...
	m_nimbus_mouse.m_mouse_pcx &= 3;
	m_nimbus_mouse.m_mouse_pcy &= 3;

	// get value of mouse quadrature encoders for this wheel position
	uint8_t mxa = MOUSE_XYA[m_nimbus_mouse.m_mouse_pcx]; // XA
	uint8_t mxb = MOUSE_XYB[m_nimbus_mouse.m_mouse_pcx]; // XB
	uint8_t mya = MOUSE_XYA[m_nimbus_mouse.m_mouse_pcy]; // YA
	uint8_t myb = MOUSE_XYB[m_nimbus_mouse.m_mouse_pcy]; // YB

	// calculate interrupt state
	uint8_t intstate_x = (mxb ^ mxa) ^ ((m_ay8910_a & 0x40) >> 6);
	uint8_t intstate_y = (myb ^ mya) ^ ((m_ay8910_a & 0x80) >> 7);

	// Generate interrupts if enabled, otherwise return values in mouse register
	if (m_iou_reg092 & MOUSE_INT_ENABLE)
	{
		if (intstate_x == 1 && m_nimbus_mouse.m_intstate_x == 0)
		{
			uint8_t xint = mxa ? EXTERNAL_INT_MOUSE_XL : EXTERNAL_INT_MOUSE_XR;
			external_int(xint, true);
		}

		if (intstate_y == 1 && m_nimbus_mouse.m_intstate_y == 0)
		{
			uint8_t yint = myb ? EXTERNAL_INT_MOUSE_YD : EXTERNAL_INT_MOUSE_YU;

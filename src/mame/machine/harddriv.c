...
WRITE16_HANDLER( hd68k_nwr_w )
{
	...
	switch (offset)
	{
		...
		case 6:	/* /GSPRES */
			...
			if (hdcpu_gsp != -1)
				cpu_set_input_line(machine->cpu[hdcpu_gsp], INPUT_LINE_RESET, data ? CLEAR_LINE : ASSERT_LINE);
			break;
		case 7:	/* /MSPRES */
			logerror("Write to /MSPRES(%d)\n", data);
			if (hdcpu_msp != -1)
				cpu_set_input_line(machine->cpu[hdcpu_msp], INPUT_LINE_RESET, data ? CLEAR_LINE : ASSERT_LINE);
			break;
	}
}
...
WRITE16_HANDLER( hdgsp_io_w )
{
	...
	if ((offset == REG_HEBLNK || offset == REG_HSBLNK) && data != tms34010_io_register_r(machine, offset, 0xffff))
...
WRITE16_HANDLER( hd68k_adsp_data_w )
{
	...
	if (offset == 0x1fff)
	{
		...
		cpu_triggerint(machine->cpu[hdcpu_adsp]);
	}
	else
	...
	switch (offset)
	{
		...
		case 5:
			...
			if (adsp_br || adsp_halt)
				cpu_set_input_line(machine->cpu[hdcpu_adsp], INPUT_LINE_HALT, ASSERT_LINE);
			else
			{
		...
		case 7:
			...
			cpu_set_input_line(machine->cpu[hdcpu_adsp], INPUT_LINE_RESET, val ? CLEAR_LINE : ASSERT_LINE);
			cpu_yield(machine->activecpu);
			break;
...
READ16_HANDLER( hd68k_ds3_gdata_r )
{
	...
	logerror("%06X:hd68k_ds3_gdata_r(%04X)\n", cpu_get_previouspc(machine->activecpu), ds3_gdata);
	...
	{
		UINT32 destaddr = cpu_get_reg(machine->activecpu, M68K_A1);
		UINT16 count68k = cpu_get_reg(machine->activecpu, M68K_D1);
		UINT16 mstat = cpu_get_reg(machine->cpu[hdcpu_adsp], ADSP2100_MSTAT);
		UINT16 i6 = cpu_get_reg(machine->cpu[hdcpu_adsp], (mstat & 1) ? ADSP2100_MR0 : ADSP2100_MR0_SEC);
		UINT16 l6 = cpu_get_reg(machine->cpu[hdcpu_adsp], ADSP2100_L6) - 1;
		UINT16 m7 = cpu_get_reg(machine->cpu[hdcpu_adsp], ADSP2100_M7);

		logerror("%06X:optimizing 68k transfer, %d words\n", cpu_get_previouspc(machine->activecpu), count68k);
		...
		cpu_set_reg(machine->activecpu, M68K_D1, count68k);
		cpu_set_reg(machine->cpu[hdcpu_adsp], (mstat & 1) ? ADSP2100_MR0 : ADSP2100_MR0_SEC, i6);
	...
	cpu_spinuntil_trigger(machine->activecpu, DS3_TRIGGER);
	cpuexec_triggertime(machine, DS3_TRIGGER, ATTOTIME_IN_USEC(5));
...
WRITE16_HANDLER( hdds3_special_w )
{
	...
	switch (offset & 7)
	{
		case 0:
			...
			cpuexec_trigger(machine, DS3_TRIGGER);
			break;

		case 1:
			logerror("%04X:ADSP sets interrupt = %d\n", cpu_get_previouspc(machine->activecpu), (data >> 1) & 1);
...
WRITE16_HANDLER( hd68k_dsk_control_w )
{
	...
	{
		case 0:	/* DSPRESTN */
			cpu_set_input_line(machine->cpu[hdcpu_dsp32], INPUT_LINE_RESET, val ? CLEAR_LINE : ASSERT_LINE);
			break;

		case 1:	/* DSPZN */
			cpu_set_input_line(machine->cpu[hdcpu_dsp32], INPUT_LINE_HALT, val ? CLEAR_LINE : ASSERT_LINE);
			break;
...
{
	slapstic_tweak(machine, offset & 0x3fff);
}


READ16_HANDLER( rd68k_slapstic_r )
{
	int bank = slapstic_tweak(machine, offset & 0x3fff) * 0x4000;
...
READ16_HANDLER( hdgsp_speedup_r )
{
	...
	if (result != 0xffff && hdgsp_speedup_addr[1][0] != 0xffff &&
		cpunum_get_active() == hdcpu_gsp && cpu_get_pc(machine->activecpu) == hdgsp_speedup_pc)
	{
		gsp_speedup_count[0]++;
...
WRITE16_HANDLER( hdgsp_speedup1_w )
{
	...
	if (hdgsp_speedup_addr[0][offset] == 0xffff)
		cpu_triggerint(machine->cpu[hdcpu_gsp]);
}
...
READ16_HANDLER( rdgsp_speedup1_r )
{
	...
	if (cpunum_get_active() == hdcpu_gsp && cpu_get_pc(machine->activecpu) == hdgsp_speedup_pc &&
		(result & 0xff) < cpu_get_reg(machine->activecpu, TMS34010_A1))
	{
		gsp_speedup_count[0]++;
		cpu_spinuntil_int(machine->activecpu);
	}
...
READ16_HANDLER( hdmsp_speedup_r )
{
	...
	if (data == 0 && cpu_get_pc(machine->activecpu) == hdmsp_speedup_pc && cpunum_get_active() == hdcpu_msp)
	{
		msp_speedup_count[0]++;
		cpu_spinuntil_int(machine->activecpu);
	}
...
READ16_HANDLER( stmsp_speedup_r )
{
	...
	if (stmsp_sync[0][0] == 0 &&		/* 80010 */
		...
		cpu_get_pc(machine->activecpu) == 0x3c0)
	{
		msp_speedup_count[0]++;
		cpu_spinuntil_int(machine->activecpu);
	}
...
READ16_HANDLER( hdadsp_speedup_r )
{
	...
	if (data == 0xffff && cpu_get_pc(machine->activecpu) <= 0x3b && cpunum_get_active() == hdcpu_adsp)
	{
...
READ16_HANDLER( hdds3_speedup_r )
{
	...
	if (data != 0 && cpu_get_pc(machine->activecpu) == hdds3_speedup_pc && cpunum_get_active() == hdcpu_adsp)
	{

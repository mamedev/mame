...
int apollo_kbd_device::push_scancode(uint8_t code, uint8_t repeat)
{
	...
	uint8_t caps = BIT(modifiers,0);
	uint8_t shift = BIT(modifiers,1) | BIT(modifiers,5);
	uint8_t ctrl = BIT(modifiers,2);
	uint8_t numlock = BIT(modifiers,6);
	uint16_t index;
	...
	if (m_mode == KBD_MODE_0_COMPATIBILITY)
	{
		if (code & 0x80)
		{
			// skip up code in ASCII mode
				...
				&& s_code_table[index + CODE_TABLE_AUTO_REPEAT_CODE] != Yes)
		{
			// skip repeat in ASCII mode
		}
		else if (ctrl)
		{
			key_code = s_code_table[index + CODE_TABLE_CONTROL_CODE];
		}
		else if (shift)
		{
			key_code = s_code_table[index + CODE_TABLE_SHIFTED_CODE];
		}
		else if (caps)
		{
			key_code = s_code_table[index + CODE_TABLE_CAPS_LOCK_CODE];
		}
		else
		{
			key_code = s_code_table[index + CODE_TABLE_UNSHIFTED_CODE];
		}
	}
	else
	{
		...
		{
			key_code = s_code_table[index + CODE_TABLE_UP_CODE];
		}
		else
		{
			key_code = s_code_table[index + CODE_TABLE_DOWN_CODE];
		}
	}
...
uint16_t const apollo_kbd_device::s_code_table[] = {

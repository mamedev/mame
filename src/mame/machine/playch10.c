...
MACHINE_START( pc10 )
{
	...
	memory_install_readwrite8_handler(machine->device("ppu")->memory().space(ADDRESS_SPACE_PROGRAM), 0, 0x1fff, 0, 0, pc10_chr_r, pc10_chr_w);
	memory_install_readwrite8_handler(machine->device("ppu")->memory().space(ADDRESS_SPACE_PROGRAM), 0x2000, 0x3eff, 0, 0, pc10_nt_r, pc10_nt_w);
...
DRIVER_INIT( pcaboard )
{
	...
	memory_install_write8_handler(machine->device("cart")->memory().space(ADDRESS_SPACE_PROGRAM), 0x8000, 0x8fff, 0, 0, aboard_vrom_switch_w );
...
DRIVER_INIT( pcbboard )
{
	...
	memory_install_write8_handler(machine->device("cart")->memory().space(ADDRESS_SPACE_PROGRAM), 0x8000, 0xffff, 0, 0, bboard_rom_switch_w );
...
DRIVER_INIT( pccboard )
{
	...
	memory_install_write8_handler(machine->device("cart")->memory().space(ADDRESS_SPACE_PROGRAM), 0x6000, 0x6000, 0, 0, cboard_vrom_switch_w );
...
DRIVER_INIT( pcdboard )
{
	...
	memory_install_write8_handler(machine->device("cart")->memory().space(ADDRESS_SPACE_PROGRAM), 0x8000, 0xffff, 0, 0, mmc1_rom_switch_w );
...
DRIVER_INIT( pcdboard_2 )
{
	...
	memory_install_ram(machine->device("cart")->memory().space(ADDRESS_SPACE_PROGRAM), 0x6000, 0x7fff, 0, 0, NULL );
...
DRIVER_INIT( pceboard )
{
	...
	memory_install_write8_handler(machine->device("cart")->memory().space(ADDRESS_SPACE_PROGRAM), 0x8000, 0xffff, 0, 0, eboard_rom_switch_w );

	/* ppu_latch callback */
	ppu2c0x_set_latch(machine->device("ppu"), mapper9_latch);

	/* nvram at $6000-$6fff */
	memory_install_ram(machine->device("cart")->memory().space(ADDRESS_SPACE_PROGRAM), 0x6000, 0x6fff, 0, 0, NULL );
...
DRIVER_INIT( pcgboard )
{
	...
	memory_install_write8_handler(machine->device("cart")->memory().space(ADDRESS_SPACE_PROGRAM), 0x8000, 0xffff, 0, 0, gboard_rom_switch_w );
...
DRIVER_INIT( pciboard )
{
	...
	/* Roms are banked at $8000 to $bfff */
	memory_install_write8_handler(machine->device("cart")->memory().space(ADDRESS_SPACE_PROGRAM), 0x8000, 0xffff, 0, 0, iboard_rom_switch_w );
...
DRIVER_INIT( pchboard )
{
	...
	/* Roms are banked at $8000 to $bfff */
	memory_install_write8_handler(machine->device("cart")->memory().space(ADDRESS_SPACE_PROGRAM), 0x8000, 0xffff, 0, 0, hboard_rom_switch_w );

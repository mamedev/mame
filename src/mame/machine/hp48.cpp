...
void hp48_state::base_machine_start(hp48_models model)
{
	...
	/* serial receive done */
	m_recv_done_timer = machine().scheduler().timer_alloc(timer_expired_delegate(FUNC(hp48_state::rs232_byte_recv_cb), this));
	m_recv_done_timer->adjust(attotime::never);

	/* serial send done */
	m_send_done_timer = machine().scheduler().timer_alloc(timer_expired_delegate(FUNC(hp48_state::rs232_byte_sent_cb), this));

...
protected:
	...
	s32 m_ttl_gfx_index = 0;
	tilemap_t *m_ttl_tilemap;
	u8 m_control = 0;
	u16 m_scrollx = 0;
	u32 m_ttlrom_offset = 0;
...
static INPUT_PORTS_START( quickpick5 )
	...
	PORT_BIT(0x80, IP_ACTIVE_HIGH, IPT_CUSTOM) PORT_READ_LINE_DEVICE_MEMBER("screen", FUNC(screen_device::vblank)) // guess
...
static INPUT_PORTS_START( waijockey )
	...
	PORT_BIT(0x60, IP_ACTIVE_LOW, IPT_CUSTOM) // battery sensor
	PORT_BIT(0x80, IP_ACTIVE_HIGH, IPT_CUSTOM) PORT_READ_LINE_DEVICE_MEMBER("screen", FUNC(screen_device::vblank)) // guess
INPUT_PORTS_END

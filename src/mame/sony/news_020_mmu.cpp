...
void news_020_mmu_device::mmu_entry_w(offs_t offset, uint32_t data, uint32_t mem_mask)
{
	...
	news_020_pte modified_entry = unpack_mmu_entry(m_mmu_system_ram[offset]);
	modified_entry.valid = (data & 0x80000000) != 0;
	modified_entry.access_settings = (data & 0x78000000) >> 27;
	modified_entry.modified = (data & 0x04000000) != 0;
	modified_entry.fill_on_demand = (data & 0x02000000) != 0;
	modified_entry.pfnum = data & 0x000fffff;
	LOGMASKED(LOG_ENTRY, "(%s) mmu %s entry write (0x%x, 0x%x, 0x%x) -> entry 0x%x: tag = 0x%x, pfnum = 0x%x (valid = %d, modified = %d, fill_on_demand = %d, access = 0x%x)\n", machine().describe_context(),
			...
			modified_entry.access_settings);
	#endif
}
...
uint32_t news_020_mmu_device::hyperbus_r(offs_t offset, uint32_t mem_mask, bool is_supervisor)
{
	...
	else
	{
		...
		const uint32_t pte = system ? m_mmu_system_ram[vpgnum % MMU_ENTRY_COUNT] : m_mmu_user_ram[vpgnum % MMU_ENTRY_COUNT];
...
news_020_mmu_device::news_020_pte news_020_mmu_device::unpack_mmu_entry(const uint32_t entry)
{
	...
	new_entry.access_settings = (entry & 0x78000000) >> 27;

...
TODO:
- correct GFX decode
- inputs / outputs (remaining CPU ports)
...
static INPUT_PORTS_START( carniv37 ) // TODO: do inputs once GFX is correctly decoded
	PORT_START("IN0")
	...
	PORT_START("IN2")
	PORT_BIT( 0x01, IP_ACTIVE_LOW, IPT_BUTTON1 ) PORT_PLAYER(4) // coin 1
	PORT_BIT( 0x02, IP_ACTIVE_LOW, IPT_BUTTON2 ) PORT_PLAYER(4) // coin 2
	PORT_BIT( 0x04, IP_ACTIVE_LOW, IPT_BUTTON3 ) PORT_PLAYER(4) // coin 3 / keyin?
	PORT_BIT( 0x08, IP_ACTIVE_LOW, IPT_BUTTON4 ) PORT_PLAYER(4) // coin 4 / keyin?
	PORT_BIT( 0x10, IP_ACTIVE_LOW, IPT_BUTTON5 ) PORT_PLAYER(4)
	PORT_BIT( 0x20, IP_ACTIVE_LOW, IPT_BUTTON6 ) PORT_PLAYER(4)
	PORT_BIT( 0x40, IP_ACTIVE_LOW, IPT_BUTTON7 ) PORT_PLAYER(4) // payout (shows hopper empty message)
	PORT_BIT( 0x80, IP_ACTIVE_LOW, IPT_BUTTON8 ) PORT_PLAYER(4) // keyout?
...
static GFXDECODE_START( gfx ) // TODO: wrong
...
void carnival37_state::carniv37(machine_config &config)
{
	...
	m_maincpu->out_pd_callback().set([this] (uint8_t data) { logerror("%s CPU port D write: %02x\n", machine().describe_context(), data); }); // seems coin / payout counters, hopper mottor
	...
	HD6845S(config, "crtc", 12_MHz_XTAL / 3); // actually HD6445P4, a superset according to datasheet

	GFXDECODE(config, m_gfxdecode, "palette", gfx);

	PALETTE(config, "palette").set_entries(0x100);

	RAMDAC(config, "ramdac", "palette").set_addrmap(0, &carnival37_state::ramdac_map);
...
ROM_START( carniv37 )
	...
	ROM_LOAD( "512.ic40", 0x00000, 0x10000, CRC(789a2336) SHA1(3f441f063c113b787aff919259ae974b3b4e4a62) ) // handwritten label
	// ic41 not populated
ROM_END
...
GAME( 1992, carniv37, 0, carniv37, carniv37, carnival37_state, empty_init, ROT0, "Able", "Carnival 37", MACHINE_IMPERFECT_GRAPHICS | MACHINE_NOT_WORKING )

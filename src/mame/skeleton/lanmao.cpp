...
private:
	...
	void data2_map(address_map &map) ATTR_COLD;
	void program2_map(address_map &map) ATTR_COLD;
...
void panda3_state::program2_map(address_map &map)
{
	map(0x0000, 0xffff).rom();
}

void panda3_state::data2_map(address_map &map)
{	
    map(0x8000, 0x87ff).ram().share("nvram");
	map(0x9000, 0x9001).w("ay1", FUNC(ay8910_device::address_data_w));
	map(0x9002, 0x9003).w("ay2", FUNC(ay8910_device::address_data_w));
...
void panda3_state::panda3(machine_config &config)
{
	...
	m_maincpu->set_addrmap(AS_PROGRAM, &panda3_state::program2_map);
	m_maincpu->set_addrmap(AS_DATA, &panda3_state::data2_map);
...
ROM_START( baowang )
	ROM_REGION( 0x10000, "maincpu", 0 )
	ROM_LOAD( "w78e065-12.bin", 0x00000, 0x10000, CRC(F67B8DB6) SHA1(96cc23afadc98728538bfafd3783c7cd2997c608) )
	
	ROM_REGION( 0x80000, "oki", 0 ) // adpcm rom 
	ROM_LOAD( "adpcm.bin", 0x00000, 0x80000, CRC(232A3840) SHA1(ced2f04566b1d2cf821e20334630a43476cca0ae) )

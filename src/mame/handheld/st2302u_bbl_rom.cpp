...
void st22xx_bbl338_state::st22xx_dphh8213_map(address_map &map)
{
	...
	map(0x0600000, 0x0600000).w(m_lcdc, FUNC(st7735s_lcdc_device::lcdc_command_w));
	map(0x0604000, 0x0604000).rw(m_lcdc, FUNC(st7735s_lcdc_device::lcdc_data_r), FUNC(st7735s_lcdc_device::lcdc_data_w));
}
...
void st22xx_bbl338_state::st22xx_cutepet_map(address_map &map)
{
	...
	map(0x0600000, 0x0600000).w(m_lcdc, FUNC(st7735s_lcdc_device::lcdc_command_w));
	map(0x0604100, 0x0604100).rw(m_lcdc, FUNC(st7735s_lcdc_device::lcdc_data_r), FUNC(st7735s_lcdc_device::lcdc_data_w));
}

...
static INPUT_PORTS_START( koinomp )
	...
	PORT_START("DSWA")
	PORT_DIPNAME(0x03, 0x03, "Difficulty")   //ゲーム難易度(4が最强)
	PORT_DIPSETTING(   0x03, "1")   //OFF OFF
	PORT_DIPSETTING(   0x02, "2")  //ON OFF
	PORT_DIPSETTING(   0x01, "3")  //OFF ON
	PORT_DIPSETTING(   0x00, "4")  //ON ON
	PORT_DIPUNKNOWN_DIPLOC(0x04, 0x04, "Coinage")  //プレイ料金（1 コイン）//ON 2 OFF 1
	PORT_DIPUNKNOWN_DIPLOC(0x08, 0x08, "Demo Sounds")  //テモ· サウンド
	PORT_DIPUNKNOWN_DIPLOC(0x10, 0x10, "Game Sounds")  //ゲーム· サウンド
	PORT_DIPNAME(0x20, 0x00, "Flip_Screen ")   //画面反転
	PORT_DIPSETTING(   0x00, DEF_STR( Off ))  //逆
	PORT_DIPSETTING(   0x20, DEF_STR( On ))  //正
	PORT_DIPNAME(0x40, 0x40, "Test Mode")  //テストモード
	PORT_DIPSETTING(   0x40, DEF_STR( Off ))  //通常ゲーム
	PORT_DIPSETTING(   0x00, DEF_STR( On ))  //テストモード
	PORT_DIPNAME(0x80, 0x80, "Graphic ROM Test")  //常时OFF
	PORT_DIPSETTING(   0x80, DEF_STR( Off ))
	PORT_DIPSETTING(   0x00, DEF_STR( On ))
...
static INPUT_PORTS_START( patimono )
	...
	PORT_DIPNAME(0x04, 0x04, DEF_STR(  )) PORT_DIPLOCATION("DSWA:3")

...
{
	m_reset_write(reset_line_read());
}

u8 tk10_keyboard_device::reset_line_read()
{
	u8 key = m_kbspecial->read();
	if (!BIT(key, 3) && !BIT(key, 4)) return 0;
	else return 1;
}
...
static INPUT_PORTS_START(tk10_keyboard)
	...
	PORT_BIT(0x010, IP_ACTIVE_LOW, IPT_KEYBOARD) PORT_NAME("Call -151")  PORT_CODE(KEYCODE_0_PAD)   // CALL -151
	PORT_BIT(0x020, IP_ACTIVE_LOW, IPT_KEYBOARD) PORT_NAME("Pause Key") PORT_CODE(KEYCODE_UP)  // CTRL+S 0x13 PAUSE OUTPUT
	PORT_BIT(0x040, IP_ACTIVE_LOW, IPT_KEYBOARD) PORT_NAME("DELETE") PORT_CODE(KEYCODE_DEL)  // 7F
	PORT_BIT(0x080, IP_ACTIVE_LOW, IPT_KEYBOARD) PORT_NAME("TAB") PORT_CODE(KEYCODE_TAB)   // CTRL+i 0x09  tab??

	PORT_START("P21")
	PORT_BIT(0x001, IP_ACTIVE_LOW, IPT_KEYBOARD) PORT_CODE(KEYCODE_QUOTE)      PORT_CHAR('_') PORT_CHAR('^') //  _ SHIFT=^
	PORT_BIT(0x002, IP_ACTIVE_LOW, IPT_KEYBOARD) PORT_CODE(KEYCODE_CLOSEBRACE) PORT_CHAR(']') PORT_CHAR('}')
	PORT_BIT(0x004, IP_ACTIVE_LOW, IPT_KEYBOARD) PORT_CODE(KEYCODE_OPENBRACE)  PORT_CHAR('[') PORT_CHAR('{')
	PORT_BIT(0x008, IP_ACTIVE_LOW, IPT_KEYBOARD) PORT_CODE(KEYCODE_TILDE)      PORT_CHAR('`') PORT_CHAR('~') //  backtick and tilde
	PORT_BIT(0x010, IP_ACTIVE_LOW, IPT_KEYBOARD) PORT_NAME("Slash Pad")    PORT_CODE(KEYCODE_SLASH_PAD)
	PORT_BIT(0x020, IP_ACTIVE_LOW, IPT_KEYBOARD) PORT_NAME("Minus Pad")    PORT_CODE(KEYCODE_MINUS_PAD)
	PORT_BIT(0x040, IP_ACTIVE_LOW, IPT_KEYBOARD) PORT_NAME("Plus Pad")     PORT_CODE(KEYCODE_PLUS_PAD)
	PORT_BIT(0x080, IP_ACTIVE_LOW, IPT_KEYBOARD) PORT_NAME("Asterisk Pad") PORT_CODE(KEYCODE_ASTERISK)

	PORT_START("P22")
	PORT_BIT(0x001, IP_ACTIVE_LOW, IPT_KEYBOARD) PORT_CODE(KEYCODE_BACKSLASH) PORT_CHAR('\\') PORT_CHAR('|')// BACKSLASH
	...
	PORT_BIT(0x020, IP_ACTIVE_LOW, IPT_KEYBOARD) PORT_CODE(KEYCODE_PGUP)  PORT_CHAR('^') PORT_CHAR('[')

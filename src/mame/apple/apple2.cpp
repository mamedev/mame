...
II Plus: RAM options reduced to 16/32/48 KB.
    ...
    in place of another game I/O input.  (These word processors generally use
    CONTROL and SHIFT pressed together as a shift lock and other CONTROL
    combinations as function keys.)
    ...
    the base configuration required to run Apple Pascal, ProDOS and some larger
    games.

    Originally, the Apple II used a RO-3-2513 character generator supporting a
    64-character uppercase-only subset of ASCII.  The Revision 7 motherboard
    replaced this with a standard 2316 ROM allowing 128 different codepoints
    to be displayed in normal video, though Apple did not actually add the
    lowercase ASCII characters until the IIe.  (A conventional workaround
    for the lack of a true mixed case display was using inverse video as
    ersatz capitalization.)  The monitor input routines in both versions of
    the F8 ROM also mask off any lowercase character codes generated by the
    keyboard, which Apple's own keyboards did not normally do.  The a2lca2,
    a2plca2 and apple2pe sets feature third-party lowercase character generator
    ROMs, and their BIOS options include miscellaneous homebrew modifications
    to the F8 ROMs that facilitate the input of lowercase characters (and
    sometimes add other features as well).

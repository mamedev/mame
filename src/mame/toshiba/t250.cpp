...
private:
	...
	u8 mem_low_r(offs_t offset);
	void mem_low_w(offs_t offset, u8 data);
	u8 mem_mid_r(offs_t offset);
	void mem_mid_w(offs_t offset, u8 data);
	u8 mem_high_r(offs_t offset);
	void mem_high_w(offs_t offset, u8 data);
	...
	void usart_rxrdy_w(int state); // 8251 RxRDY (rdint bit 5)
	void usart_txrdy_w(int state); // 8251 TxRDY (rdint bit 6)
	u8 dsw_r();                    // 0xD1
	...
	void ccm_w(u8 data);           // 0xA6 (CCM mode register)
	u8 cicts_r();                  // 0xA7 (CCM CI/CTS status)
	void update_rst65();
	...
	void sasi_ack_off(s32 param);
	void sasi_sel_off(s32 param);
	void sasi_rst_off(s32 param);
	...
	required_device<nscsi_callback_device> m_sasi_cb;
	...
	bool m_rom_at_low = true;
	bool m_vram_at_high = true;
	...
	bool m_usart_rxrdy = false;  // 8251 RxRDY  -> rdint bit 5 (active low)
	bool m_usart_txrdy = false;  // 8251 TxRDY  -> rdint bit 6 (active low)
	bool m_usart_inited = false; // false until BIOS writes the mode byte
	...
	// RS232 modem-control inputs tracked for cicts_r (0xA7 bit 5) and
	// ios_r (0xD3 bits 7,6).  MAME-convention: 1 = signal asserted (line
	// at positive voltage).  CTS defaults to asserted so the BIOS comm
	// output routine can send when the connected RS232 device doesn't
	// drive modem-control lines (e.g. null_modem).
	bool m_rs232_cts = true;
	bool m_rs232_ri  = false;
	...
	// requirement).  Values used: SASI_IDLE..SASI_SWALLOW_STATUS, defined as
	// static constexpr u8 below.
	...
	static constexpr u8 SASI_SWALLOW_GARBAGE = 5;  // post-cmd pickup-garbage read
	static constexpr u8 SASI_SWALLOW_STATUS  = 6;
	int  m_sasi_refid = 7;          // actual bus refid of our callback (resolved at machine_start)
	u8 m_sasi_phase = SASI_IDLE;
	u8   m_sasi_cmd_first  = 0;   // first byte of current command (for swallow decision)
	u8   m_sasi_swallow_remaining = 0;  // bytes left in swallowed cmd block
	bool m_sasi_synth_bsy  = false; // true while we synthesize BSY to BIOS (swallow path)
	bool m_sasi_synth_req  = false; // synthetic REQ for swallow path
...
{
	save_item(NAME(m_rom_at_low));
	save_item(NAME(m_vram_at_high));
	...
	save_item(NAME(m_rs232_cts));
	save_item(NAME(m_rs232_ri));
	...
	m_sasi_ack_timer = timer_alloc(FUNC(t250_state::sasi_ack_off), this);
	m_sasi_sel_timer = timer_alloc(FUNC(t250_state::sasi_sel_off), this);
	m_sasi_rst_timer = timer_alloc(FUNC(t250_state::sasi_rst_off), this);

	// Cache the actual bus refid of our callback device.  MAME's nscsi_bus
	// only counts non-empty slots in m_devcnt, so our SCSI ID 7 callback may
	// land at refid 1 (when only one target is also attached).  Writes to a
	// refid >= m_devcnt are silently dropped by regen_ctrl/regen_data.
	// Access the protected member m_scsi_refid via a using-public derived
	// class trick.
	struct refid_accessor : public nscsi_callback_device {
		using nscsi_callback_device::m_scsi_refid;
	};
	m_sasi_refid = static_cast<refid_accessor *>(m_sasi_cb.target())->m_scsi_refid;
	save_item(NAME(m_sasi_refid));
}
...
void t250_state::machine_reset()
{
	...
	m_sasi->ctrl_w(m_sasi_refid, 0, nscsi_device_interface::S_ALL);
	m_sasi->data_w(m_sasi_refid, 0);
}


u8 t250_state::mem_low_r(offs_t offset)
{
	return m_rom_at_low ? m_rom[offset] : m_ram[offset];
}

void t250_state::mem_low_w(offs_t offset, u8 data)
{
	// ROM bank is read-only; writes always go to underlying DRAM.
	m_ram[offset] = data;
}

u8 t250_state::mem_mid_r(offs_t offset)
{
	return m_ram[0x1000 + offset];
}

void t250_state::mem_mid_w(offs_t offset, u8 data)
{
	m_ram[0x1000 + offset] = data;
}

u8 t250_state::mem_high_r(offs_t offset)
{
	return m_vram_at_high ? m_vram[offset] : m_vpg[offset];
}

void t250_state::mem_high_w(offs_t offset, u8 data)
{
	if (m_vram_at_high)
		m_vram[offset] = data;
	else
...
void t250_state::bank_w(u8 data)
{
	...
	m_rom_at_low   = !(data & 0x08);
	m_vram_at_high = !(data & 0x04);
...
u8 t250_state::rdint_r()
{
	...
	if (m_usart_txrdy) d &= ~0x40; // bit 6 = comm out (TxRDY)
	if (m_usart_rxrdy) d &= ~0x20; // bit 5 = comm in  (RxRDY)
	if (m_prn_irq)    d &= ~0x10;  // bit 4 = printer
	return d;
}

void t250_state::update_rst65()
{
	bool any = m_kbd_irq || m_prn_irq || m_usart_rxrdy || m_usart_txrdy;
	m_maincpu->set_input_line(I8085_RST65_LINE, any ? ASSERT_LINE : CLEAR_LINE);
}
...
void t250_state::usart_rxrdy_w(int state)
{
	m_usart_rxrdy = state;
	update_rst65();
}

void t250_state::usart_txrdy_w(int state)
{
	m_usart_txrdy = state;
	update_rst65();
}
...
u8 t250_state::ios_r()
{
	...
	if (m_rs232_cts)          d &= ~0x40; // /CTS asserted -> bit 6 low
	if (m_rs232_ri)           d &= ~0x80; // /CI asserted -> bit 7 low
	return d;
}

// CCM CI/CTS status register (0xA7).  BIOS reads bit 5 to test /CTS
// (active low: 0 = ready to send).
u8 t250_state::cicts_r()
{
	u8 d = 0xff;
	if (m_rs232_cts) d &= ~0x20;
	return d;
}
...
// m_rs232_cts == true means "asserted" (matches cicts_r / ios_r consumers).
// Same for RI.
void t250_state::rs232_cts_w(int state) { m_rs232_cts = !state; }
...
void t250_state::usart_w(offs_t offset, u8 data)
{
...
void t250_state::rs232_ri_w(int state)  { m_rs232_ri  = !state; }
...
TIMER_CALLBACK_MEMBER(t250_state::sasi_ack_off)
{
	m_sasi->ctrl_w(m_sasi_refid, 0, nscsi_device_interface::S_ACK);
...
TIMER_CALLBACK_MEMBER(t250_state::sasi_sel_off)
{
	m_sasi->ctrl_w(m_sasi_refid, 0, nscsi_device_interface::S_SEL);
}

TIMER_CALLBACK_MEMBER(t250_state::sasi_rst_off)
{
	m_sasi->ctrl_w(m_sasi_refid, 0, nscsi_device_interface::S_RST);
}
...
void t250_state::sasi_req_w(int state)
{
...
void t250_state::sasi_bsy_w(int state)
{
...
void t250_state::hconfg_w(u8 data)
{
	(void)data;
}
...
void t250_state::hrset_w(u8 data)
{
	(void)data;
	m_sasi->ctrl_w(m_sasi_refid, nscsi_device_interface::S_RST, nscsi_device_interface::S_RST);
...
void t250_state::hdata_w(u8 data)
{
	switch (m_sasi_phase)
	{
	case SASI_IDLE:
		...
		if (data == 0xff)
		{
			m_sasi->data_w(m_sasi_refid, 0x01);                                          // target id 0
			m_sasi->ctrl_w(m_sasi_refid, nscsi_device_interface::S_SEL, nscsi_device_interface::S_SEL);
			...
			if (m_sasi->ctrl_r() & nscsi_device_interface::S_BSY)
			{
	...
	case SASI_CMD_FIRST:
		...
		else
		{
			...
			m_sasi->data_w(m_sasi_refid, data);
			m_sasi->ctrl_w(m_sasi_refid, nscsi_device_interface::S_ACK, nscsi_device_interface::S_ACK);
...
void t250_state::sasi_finish_swallow()
{
...
u8 t250_state::hdata1_r()
{
	...
	case SASI_PASSTHROUGH:
		data = m_sasi->data_r();
		if (m_sasi->ctrl_r() & nscsi_device_interface::S_REQ)
		{
			m_sasi->ctrl_w(m_sasi_refid, nscsi_device_interface::S_ACK, nscsi_device_interface::S_ACK);
			m_sasi_ack_timer->adjust(SASI_PULSE);
		}
...
{
	map(0x0000, 0x0fff).rw(FUNC(t250_state::mem_low_r), FUNC(t250_state::mem_low_w));
	map(0x1000, 0xf7ff).rw(FUNC(t250_state::mem_mid_r), FUNC(t250_state::mem_mid_w));
	map(0xf800, 0xffff).rw(FUNC(t250_state::mem_high_r), FUNC(t250_state::mem_high_w));
}
...
void t250_state::io_map(address_map &map)
{
	...
	// via -sasi:0 harddisk.  See t250_state::h*_r/h*_w for the protocol.
	map(0x00, 0x00).rw(FUNC(t250_state::hconfg_r), FUNC(t250_state::hconfg_w));
	map(0x01, 0x01).w(FUNC(t250_state::hdata_w));
	map(0x02, 0x02).r(FUNC(t250_state::hdata1_r));
	map(0x03, 0x03).r(FUNC(t250_state::hstat_r));
	map(0x08, 0x08).rw(FUNC(t250_state::hsts_r), FUNC(t250_state::hrset_w));
	...
	map(0xa0, 0xa3).rw(m_pit, FUNC(pit8253_device::read), FUNC(pit8253_device::write));
	map(0xa4, 0xa5).r(m_usart, FUNC(i8251_device::read)).w(FUNC(t250_state::usart_w));
	map(0xa6, 0xa6).w(FUNC(t250_state::ccm_w));
	map(0xa7, 0xa7).r(FUNC(t250_state::cicts_r));
...
// Keyboard matrix.  The 8279 scans positions 0-7 (SL output); for each
// position it samples RL0-RL7.  The BIOS kbint handler builds a 6-bit key
// code from (return * 8) + scan and indexes into keytrans[].
//
...
void t250_state::common(machine_config &config)
{
	...
	// CCM serial board: 8253 PIT counter 2 generates the baud-rate clock
	// for the 8251 USART (TxC and RxC tied together).  Per BIOS initiu()
	// the divisors are programmed via port 0xA2 with control 0xB6
	// (counter 2, mode 3, binary).  The standard baud crystal is 1.8432
	// MHz, giving exact 19200 baud at divisor 6 with the 8251 in 16x mode.
	PIT8253(config, m_pit, 0);
	// CCM-board baud clock is 1.9968 MHz (15.9744 MHz crystal / 8), not the
	// standard 1.8432 MHz async baud crystal.  Confirmed by Dave from the
	// set utility's divisor table: 19200 -> 6 gives 1996800/6/16 = 20800 (an
	// 8% approximation that matches real hardware; lower rates are exact).
	...
	// 8251 RxRDY and TxRDY both wire into the RST 6.5 source register
	// (rdint at 0xD0), bits 5 and 6 respectively (both active low).
	// BIOS rst65 dispatches RxRDY -> comint (read char into rbuf) and
	// TxRDY -> comont (write 0x36 to 8251 cmd reg, disabling TxEN).
	m_usart->rxrdy_handler().set(FUNC(t250_state::usart_rxrdy_w));
	m_usart->txrdy_handler().set(FUNC(t250_state::usart_txrdy_w));
	...
	// Real T-250 ties the 8251's CTS pin to ground (always asserted), so
	// transmission doesn't depend on the external /CTS signal.  BIOS does
	// its own /CTS handshaking via cicts (port 0xA7).  RS232 CTS goes only
	// to our tracker, not the 8251.
	m_rs232->cts_handler().set(FUNC(t250_state::rs232_cts_w));
	m_rs232->ri_handler().set(FUNC(t250_state::rs232_ri_w));
	...
	NSCSI_CONNECTOR(config, "sasi:7", default_scsi_devices, "sasicb", true)
		...
		.machine_config([this] (device_t *device) {
			downcast<nscsi_callback_device &>(*device).req_callback().set(*this, FUNC(t250_state::sasi_req_w));
			downcast<nscsi_callback_device &>(*device).bsy_callback().set(*this, FUNC(t250_state::sasi_bsy_w));
		});

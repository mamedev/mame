...
void roland_d70_state::midi_w(u16 data) {
...
u8 roland_d70_state::dsp_io_r(offs_t offset) { return dsp_io_buffer[offset]; }
...
void roland_d70_state::d70_map(address_map &map) {
  ...
  map(0x1000, 0x7FFF).rom().region("progrom", 0x1000);
...
void roland_d70_state::d70(machine_config &config) {
  ...
  TIMER(config, "samples_timer")
      .configure_periodic(FUNC(roland_d70_state::samples_timer_cb),
                          attotime::from_hz(32000 * 2));

  midi_port_device &mdin(MIDI_PORT(config, "mdin", midiin_slot, "midiin"));
  mdin.rxd_handler().set([this](int state) {
    // logerror("midi bit %x pos %x\n", state, m_midi_pos);
...
void roland_d70_state::descramble_rom_external(u8 *dst, const u8 *src) {
  for (offs_t srcpos = 0x00; srcpos < 0x80000; srcpos++) {

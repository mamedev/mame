...
public:
	...
	template <typename T>
	namco_c148_device(const machine_config &mconfig, const char *tag, device_t *owner, const XTAL &clock, T &&hostcpu, bool is_master)
		: namco_c148_device(mconfig, tag, owner, clock)
	{
		set_hostcpu(std::forward<T>(hostcpu), is_master);
	}
	namco_c148_device(const machine_config &mconfig, const char *tag, device_t *owner, const XTAL &clock);

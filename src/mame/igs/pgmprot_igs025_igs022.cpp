...
u8 pgm_022_025_state::killbld_source_data_r(u32 region, u8 addr)
{
	// these were all xored by a table at $178B2A
	static constexpr u8 killbld_source_data[0x0c][0xec] =  // offsets to these tables stored at $155ed0
	{
		{ // region 16, $178772
	...
	return (region < 0x0c) ? killbld_source_data[region][addr] : 0;
}

u8 pgm_022_025_state::drgw3_source_data_r(u32 region, u8 addr)
{
	// all tables xored with data from $149c4c
	// tables are the same as olds and drgw2
	static constexpr u8 dw3_source_data[0x08][0xec] =
	{
		{ 0, },
		{ // region 1, $14c21a
	...
	return (region < 8) ? dw3_source_data[region][addr] : 0;
}
...
MACHINE_RESET_MEMBER(pgm_022_025_state,killbld)
{
	...
	m_igs025->set_region(region - 0x16);
	m_igs025->set_game_id(0x89911400 | region);
...
MACHINE_RESET_MEMBER(pgm_022_025_state, dw3)
{
	...
	m_igs025->set_game_id(0x00060000 | region);

	pgm_state::machine_reset();
}


void pgm_022_025_state::igs025_to_igs022_callback()
{
...
void pgm_022_025_state::init_killbld()
{
	...
	m_maincpu->space(AS_PROGRAM).install_readwrite_handler(0xd40000, 0xd40003, read16sm_delegate(*m_igs025, FUNC(igs025_device::prot_r)), write16sm_delegate(*m_igs025, FUNC(igs025_device::killbld_igs025_prot_w)));
}
...
void pgm_022_025_state::init_drgw3()
{
	...
	// install and configure protection device(s)
	m_maincpu->space(AS_PROGRAM).install_readwrite_handler(0xda5610, 0xda5613, read16sm_delegate(*m_igs025, FUNC(igs025_device::prot_r)), write16sm_delegate(*m_igs025, FUNC(igs025_device::killbld_igs025_prot_w)));
}
...
{
	pgm_022_025(config);
	m_igs025->set_source_cb(FUNC(pgm_022_025_state::drgw3_source_data_r));
...
{
	pgm_022_025(config);
	m_igs025->set_source_cb(FUNC(pgm_022_025_state::killbld_source_data_r));

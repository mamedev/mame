...
public:
	...
	void cbcomm(machine_config &config) ATTR_COLD;
	void glcx(machine_config &config) ATTR_COLD;
...
void glcx_state::glcx(machine_config &config)
{
	...
	CR16B(config, m_maincpu, 10'000'000); // FIXME: determine exact type and clock
...
void glcx_state::cbcomm(machine_config &config)
{
	glcx(config);

	SOFTWARE_LIST(config.replace(), "cart_list").set_original("cbcomm");
}
...
ROM_START( cbcomm ) // 703484-H PCB. CPU under a blob, so unrecognized
	ROM_REGION( 0x200000, "maincpu", 0 )
	ROM_LOAD( "27-6436-11.u1", 0x000000, 0x200000, CRC(2054f3fd) SHA1(5e6b3d810e19abc36beb79e5a0ab7a90d1b97b73) )
ROM_END
...
COMP( 2000, cbcomm,   0, 0, cbcomm, glcx, glcx_state, empty_init, "VTech / EG", "Computer Bit Communicator (Italy)",        MACHINE_NO_SOUND | MACHINE_NOT_WORKING )

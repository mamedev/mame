...
uint16_t fromanc2_base_state::keymatrix_r()
{
	...
	for (unsigned i = 0; 4U > i; ++i)
	{
		if (BIT(m_portselect, i))
			ret &= 0xc0c0 | m_in_key[i]->read() | (m_in_key[i + 5]->read() << 8);
	}
...
static INPUT_PORTS_START( fromanc2 )
	...
	PORT_BIT( 0x0010, IP_ACTIVE_HIGH, IPT_CUSTOM ) PORT_READ_LINE_MEMBER(FUNC(fromanc2_state::subcpu_int_r)) // SUBCPU INT FLAG
	PORT_BIT( 0x0020, IP_ACTIVE_HIGH, IPT_CUSTOM ) PORT_READ_LINE_MEMBER(FUNC(fromanc2_state::sndcpu_nmi_r)) // SNDCPU NMI FLAG
	PORT_BIT( 0x0040, IP_ACTIVE_HIGH, IPT_CUSTOM ) PORT_READ_LINE_MEMBER(FUNC(fromanc2_state::subcpu_nmi_r)) // SUBCPU NMI FLAG
	...
	PORT_BIT( 0x0100, IP_ACTIVE_HIGH, IPT_OUTPUT ) PORT_WRITE_LINE_DEVICE_MEMBER("eeprom", FUNC(eeprom_serial_93cxx_device::di_write))
	PORT_BIT( 0x0200, IP_ACTIVE_HIGH, IPT_OUTPUT ) PORT_WRITE_LINE_DEVICE_MEMBER("eeprom", FUNC(eeprom_serial_93cxx_device::clk_write))
	PORT_BIT( 0x0400, IP_ACTIVE_HIGH, IPT_OUTPUT ) PORT_WRITE_LINE_DEVICE_MEMBER("eeprom", FUNC(eeprom_serial_93cxx_device::cs_write))
...
static INPUT_PORTS_START( fromanc4 )
	...
	PORT_BIT( 0x0020, IP_ACTIVE_HIGH, IPT_CUSTOM ) PORT_READ_LINE_MEMBER(FUNC(fromanc4_state::sndcpu_nmi_r)) // SNDCPU NMI FLAG

...
// base state; no blending
class xsedae_state : public driver_device
{
public:
	xsedae_state(const machine_config &mconfig, device_type type, const char *tag)
...
protected:
	...
	void base_cop(machine_config &config) ATTR_COLD;
	void base_sound(machine_config &config) ATTR_COLD;
	void base_video(machine_config &config) ATTR_COLD;
	void base_ym2151(machine_config &config) ATTR_COLD;
	void base_ym3812(machine_config &config) ATTR_COLD;
	void zeroteam_base(machine_config &config) ATTR_COLD;

	void xsedae_cop_mem(address_map &map) ATTR_COLD;
	void xsedae_mem(address_map &map) ATTR_COLD;
	void zeroteam_sound_map(address_map &map) ATTR_COLD;

private:
	u32 pri_callback(u8 pri);
};

// above with blending and (optional)bankswitching
class raiden2_state : public xsedae_state
{
public:
	raiden2_state(const machine_config &mconfig, device_type type, const char *tag)
		: xsedae_state(mconfig, type, tag)
	...
	void init_raidendx() ATTR_COLD;
	void init_zeroteam() ATTR_COLD;
	void init_raiden2() ATTR_COLD;

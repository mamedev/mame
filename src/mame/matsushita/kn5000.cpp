...
void kn5000_state::maincpu_mem(address_map &map)
{
	...
	map(0x200000, 0x2fffff).view(m_extension_view); // OK
	m_extension_view[0](0x200000, 0x2fffff).noprw(); // OK
	m_extension_view[1](0x200000, 0x27ffff).ram(); // OK // optional hsram: 2 * 256k bytes Static RAM @ IC5, IC6 (CS5)
	m_extension_view[1](0x280000, 0x2fffff).rom(); // OK // 512k bytes FLASH ROM @ IC4 (CS5)
#endif // EXTENSION_VIEW

	map(0x300000, 0x3fffff).rom().region("custom_data", 0); // 8MBit FLASH ROM @ IC19 (CS5)
	/* OK-ish */	map(0x400000, 0x7fffff).rom().region("rhythm_data", 0); // 32MBit ROM @ IC14 (A22=1 and CS5)
	/* guessed */	map(0x800000, 0x82ffff).rom().region("subprogram", 0); // not sure yet in which chip this is stored, but I suspect it should be IC19
//	/* wrong */	map(0xc00000, 0xdfffff).mirror(0x200000).rom().region("table_data", 0);//2 * 8MBit ROMs @ IC1, IC3 (CS2)
	/* OK */		map(0xe00000, 0xffffff).mask(0x1fffff).rom().region("program", 0); //2 * 8MBit FLASH ROMs @ IC4, IC6
	// map(0xffffe8, 0xffffe8).lr8(NAME([this](offs_t offset) { return 0xff; })); // HACK: status of control panel button presses
}

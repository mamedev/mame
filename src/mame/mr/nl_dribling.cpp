...
// 2. The output goes through a TDA2003 amplifier which act as a low-pass filter as well cutting the frequency
//    generated by the TOS when it's idle. By defining FILTER_TOS the $FF gets translated into no sound since emulated
//    logic chips are free.
#define FILTER_TOS
...
NETLIST_START(dribling)
{
...
#ifdef FILTER_TOS
	// ADDITIONAL CIRCUITRY TO FILTER 40Khz output from TOS;
	// This is usually not required as the output C++ component does normally a good job filtering.
	// But in this case the high frequency square 40Khz square wave leaves too many harmonics unfiltered,
	// making the sound too loud and annoying.
	// Between adding a pass-band filter which would distort all other sounds and this fake circuit,
	// this seems to be the lesser evil. The circuit is equivalento to performing:
	// AND (flip-flop, NAND(all input pins))
	TTL_7430_DIP(NAND)
	NET_C(I_PB0, NAND.1)
	NET_C(I_PB1, NAND.2)
	NET_C(I_PB2, NAND.3)
	...
	NET_C(NAND.8, AND.1)
	NET_C(IC_C7.5, AND.2)
	NET_C(GND.Q, AND.4, AND.5, AND.9, AND.10, AND.12, AND.13)
	NET_C(GND, AND.7)
	NET_C(I_V5, AND.14)
	ALIAS(TOS, AND.3)
#else
	// Use original flip-flop output as from schematics.
	ALIAS(TOS, IC_C7.5)
#endif

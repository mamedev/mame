...
/***********************************************************************************************************************************
    ...
    All Emax boot past 'loading software' to a blank screen (manual stating it is scanner related?)
...
#define LOG_FLOPPY     (1U << 5)
#define LOG_HARD_DRIVE (1U << 6)
...
private:
    ...
    void irq_w(int state)
    {
        ...
        // 0x3C checks active-high sources (Bits 2, 3, 4, 5) when set HIGH
        // 0x03 checks active-low sources (Bits 0, 1) when pulled LOW
        bool const irq_state = (m_irq_latch & 0x3C) || (~m_irq_latch & 0x03);
    
        LOGMASKED(LOG_WARN, "irq_w: IRQ=%u state=%d active=%d latch_before=%02x pc=%s\n",
            unsigned(IRQ), state, irq_state, m_irq_latch,
        ...
        if (irq_state != m_irq_state)
        {
            ...
            LOGMASKED(LOG_WARN, "irq_w: new state: %d (latch=%02x)\n",
    ...
    void emaxii_ic35_latch_led_b_w(u8 data);

    HD44780_PIXEL_UPDATE(pixel_update);

	// temporary functions to print floppy commands
    ...
    // CTC Timer 2 output write-line callback
    ...
    optional_device<rs232_port_device> m_acia_port;
...
{
    LOGMASKED(LOG_WARN, "[emax] emaxi_ic21_latch_led_a_w (%s): 0x%02X (DynAllo=%s Trans=%s PreDef=%s AnaProc=%s Mast=%s Samp=%s DigProc=%s PreMan=%s)\n",
...
{
    LOGMASKED(LOG_WARN, "[emax] emaxi_ic22_latch_led_b_w (%s): 0x%02X (ScnDta=%s ScnClk=%s SyncTime=%s Unlabeled=%s Side=%s Mtr=%s Seq=%s Enter=%s)\n",
        machine().describe_context(), data,
...
{
    LOGMASKED(LOG_WARN, "emaxii_ic34_latch_led_a_w (%s): 0x%02X (DrvSelect=%s Trans=%s PreDef=%s DynProc=%s Mast=%s Samp=%s DigProc=%s PreMan=%s)\n",
        machine().describe_context(), data,
...
void emax_state::emaxii_ic35_latch_led_b_w(u8 data)
{
    LOGMASKED(LOG_WARN, "emaxii_ic35_latch_led_b_w (%s): 0x%02X (ScnDta=%s ScnClk=%s SyncTime=%s Unlabeled=%s Side=%s Mtr=%s Seq=%s Enter=%s)\n",
        machine().describe_context(), data,
...
{
    LOGMASKED(LOG_FLOPPY, "[emax] %s: FDC cmd_w = %02x\n", machine().describe_context(), data);
    m_fdc->cmd_w(data);
}
...
u8 emax_state::fdc_status_r()
{
    ...
    LOGMASKED(LOG_FLOPPY, "[emax] %s: FDC status_r = %02x or %u \n", machine().describe_context(), result, result);
    return result;
}

void emax_state::fdc_track_w(u8 data)
{
    LOGMASKED(LOG_FLOPPY, "[emax] %s: FDC track_w = %02x\n", machine().describe_context(), data);
    m_fdc->track_w(data);
}
...
u8 emax_state::fdc_track_r()
{
    ...
    LOGMASKED(LOG_FLOPPY, "[emax] %s: FDC track_r = %02x\n", machine().describe_context(), result);
    return result;
}

void emax_state::fdc_sector_w(u8 data)
{
    LOGMASKED(LOG_FLOPPY, "[emax] %s: FDC sector_w = %02x\n", machine().describe_context(), data);
    m_fdc->sector_w(data);
}
...
u8 emax_state::fdc_sector_r()
{
    ...
    LOGMASKED(LOG_FLOPPY, "[emax] %s: FDC sector_r = %02x\n", machine().describe_context(), result);
    return result;
}

void emax_state::fdc_data_w(u8 data)
{
    LOGMASKED(LOG_FLOPPY, "[emax] %s: FDC data_w = %02x\n", machine().describe_context(), data);
    m_fdc->data_w(data);
}
...
u8 emax_state::fdc_data_r()
{
    ...
    LOGMASKED(LOG_FLOPPY, "[emax] %s: FDC data_r = %02x\n", machine().describe_context(), result);
    return result;
}
...
void emax_state::machine_start()
{
    ...
    m_maincpu->space(AS_PROGRAM).install_write_tap(
        ...
        [this](offs_t offset, u8 &data, u8 mem_mask) {    
            LOGMASKED(LOG_WARN, "[emax] Write to RAM 0x%08X! Value: 0x%02X (PC: 0x%08X)\n", 
        ...
        0x008000, 0x017fff, "ram_debug_tap",
        [this](offs_t offset, u8 &data, u8 mem_mask) {    
            LOGMASKED(LOG_WARN, "[emax] Reading from RAM 0x%08X! Value: 0x%02X (PC: 0x%08X)\n", 
                            offset, data, m_maincpu->pc());
        });
        */
}
...
u8 emax_state::hdc_r(offs_t offset)
{
    ...
    LOGMASKED(LOG_HARD_DRIVE, "[emax] hdc_r: offset=%x reg=%x result=%02x pc=%s\n", offset, offset >> 1, res, machine().describe_context());
    return res;
}

void emax_state::hdc_w(offs_t offset, u8 data)
{
    LOGMASKED(LOG_HARD_DRIVE, "[emax] hdc_w: offset=%x reg=%x result=%02x pc=%s\n", offset, offset >> 1, data, machine().describe_context());
...
u8 emax_state::timer_r(offs_t offset)
{
    ...
    LOGMASKED(LOG_TIMER, "[emax] timer_r: offset=%x reg=%x result=%02x pc=%s\n", offset, offset >> 1, result, machine().describe_context());
    return result;
}

void emax_state::timer_w(offs_t offset, u8 data)
{
    LOGMASKED(LOG_TIMER, "[emax] timer_w: offset=%x reg=%x data=%02x pc=%s\n", offset, offset >> 1, data, machine().describe_context());
    m_ctc->write(offset >> 1, data);
}

void emax_state::mux_w(u8 data)
{
    LOGMASKED(LOG_DACMUX, "[emax] mux_w: data=%02x pc=%s\n", data, machine().describe_context());
}

void emax_state::dac_w(u8 data)
{
    LOGMASKED(LOG_DACMUX, "[emax] dac_w: data=%02x pc=%s\n", data, machine().describe_context());
}

void emax_state::echip_cmd_w(u8 data)
{
    LOGMASKED(LOG_ECHIP, "[emax] (%s): E-CHIP CMD <- %02X\n", machine().describe_context(), data);
    ...
    case 0x7c:
        ...
        LOGMASKED(LOG_ECHIP, "[emax] (%s): E-CHIP RAM TEST WRITE <- %02X\n", machine().describe_context(), m_echip_ram_test_byte);
        break;
...
{
    LOGMASKED(LOG_ECHIP, "[emax] (%s): E-CHIP DATA[%d] <- %02X\n", machine().describe_context(), offset, data);
...
u8 emax_state::echip_data_r()
{
    ...
    LOGMASKED(LOG_ECHIP, "[emax] (%s): E-CHIP READ -> %02X\n", machine().describe_context(), result);
    return result;
}

void emax_state::swtime_w(int state)
{
    // todo: reset STINT (all untested, this function never hits)
...
void emax_state::emax_periphs(address_map &map)
{
    ...
    map(0xfffe00, 0xfffe00).lr8([this]() {
        LOGMASKED(LOG_WARN, "[emax] %s: interrupt vector port read = 0x%02X\n", machine().describe_context(), m_irq_latch);
        return m_irq_latch; 
        }, "irq_latch_r");
    
    //map(0xAE4000, 0xAE4000).lw8([this](u8 data) { LOGMASKED(LOG_WARN, "tgint \n"); irq_w<TGINT>(0); }, "tgint_w");
...
void emax_state::emax_map(address_map &map)
{
    ...
    map(0x818048, 0x818048).w(m_acia, FUNC(acia6850_device::control_w));
    map(0x81804a, 0x81804a).r(m_acia, FUNC(acia6850_device::status_r));
    map(0x81804c, 0x81804c).w(m_acia, FUNC(acia6850_device::data_w));
    map(0x81804e, 0x81804e).r(m_acia, FUNC(acia6850_device::data_r));
    emax_periphs(map);
}
...
void emax_state::emax2_map(address_map &map)
{
    ...
    // fffe00 = interrupt vector
    map(0xfffe00, 0xfffe01).lr16([this]() {
        LOGMASKED(LOG_WARN, "[emax] %s: interrupt vector port read = 0x%02X\n", machine().describe_context(), m_irq_latch);
        return m_irq_latch; 
        }, "irq_latch_r");
    
    map(0xCF0000, 0xCF0000).w(FUNC(emax_state::emaxii_ic34_latch_led_a_w));
    map(0xDF0000, 0xDF0000).w(FUNC(emax_state::emaxii_ic35_latch_led_b_w));
...
void emax_state::emax(machine_config &config)
{
    ...
    m_acia->irq_handler().set(*this, FUNC(emax_state::irq_w<MIDINT>));
    
    RS232_PORT(config, m_acia_port, emax_rs232, nullptr);
    m_acia_port->rxd_handler().set(m_acia, FUNC(acia6850_device::write_rxd)).invert();
    m_acia->rts_handler().set(m_acia_port, FUNC(rs232_port_device::write_rts)).invert();
    m_acia->txd_handler().set(m_acia_port, FUNC(rs232_port_device::write_txd)).invert();

...
static const XTAL MASTER_CLOCK_PONG  = XTAL::u(static_cast<int>((double) netlist::config::INTERNAL_RES::value / (double) NS_PER_CLOCK_PONG + 0.5));
...
//#define MASTER_CLOCK_BREAKOUT    XTAL::u(14318000)
static const int NS_PER_CLOCK_BREAKOUT  = static_cast<int>((double) netlist::config::INTERNAL_RES::value / (double) 14318000 + 0.5);
static const XTAL MASTER_CLOCK_BREAKOUT  = XTAL::u(static_cast<int>((double) netlist::config::INTERNAL_RES::value / (double) NS_PER_CLOCK_BREAKOUT + 0.5));
...
void pong_state::pong(machine_config &config)
{
	...
	NETLIST_CPU(config, "maincpu").set_source(NETLIST_NAME(pong));
	...
	DAC_16BIT_R2R_TWOS_COMPLEMENT(config, m_dac).add_route(ALL_OUTPUTS, "speaker", 0.5); // unknown DAC
}

void breakout_state::breakout(machine_config &config)
{
	/* basic machine hardware */
	NETLIST_CPU(config, "maincpu").set_source(NETLIST_NAME(breakout));
	...
	NETLIST_ANALOG_OUTPUT(config, "maincpu:lamp_credit1").set_params("CON_CREDIT1", FUNC(breakout_state::credit1_cb));
	NETLIST_ANALOG_OUTPUT(config, "maincpu:lamp_credit2").set_params("CON_CREDIT2", FUNC(breakout_state::credit2_cb));
...
{
	/* basic machine hardware */
	NETLIST_CPU(config, "maincpu").set_source(NETLIST_NAME(pongdoubles));
...
{
	/* basic machine hardware */
	NETLIST_CPU(config, "maincpu").set_source(NETLIST_NAME(rebound));

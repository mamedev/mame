...
/***********************************************************************
    ...
    Atari Lunar Lander / Asteroids hardware

	NOTE:
        * The self-test screen for Asteroids shows:
            - 0000 (left to right: Coin Bonus Adder, Left
              Mech Mutiplier, Right Mech Multiplier & Game
              Price)
            - 01000 (left to right: Game Language, Ships
              at Game Start, Minimum Plays, Difficulty,
              Bonus Ship)
            - ^^ (Graphic display of the number of ships
              per game [up to 7])
            - 10000 (Point score at which a bonus ship is
              granted, blank is no bonus ship)

        * Previous program versions, for the second line, would
          only show 4 digits. The 6th switch has a currently
          unknown effect in the game. However, on the Minimum
          Number of Plays display (in the self-test screen)
          changes the values shown from 0 for a 1-Play Minimum
          to show a 2 and from 1 for a 2-Play Minimum to show a
          3. Known documentation for ealier game versions state
          the 6th switch is "Unused."
    ...
    BUG:
        * The ERROR message in the Asteroids Deluxe self test
          is related to a POKEY problem.
...
class llander_state : public driver_device
{
public:
	llander_state(machine_config const &mconfig, device_type type, char const *tag)
	...
	// input handlers
	template <bool Invert> void selftest_w(int state);
	int clock_r();

protected:
	// constants
	static inline constexpr XTAL MASTER_CLOCK = 12.096_MHz_XTAL;
	static inline constexpr XTAL CLOCK_3KHZ = MASTER_CLOCK / 4096;

	// initialization
	void virtual machine_reset() override;
	...
	// input handlers
	template <int Input> u8 in_r(offs_t offset);
	u8 dsw_r(offs_t offset);

	// memory maps
	void llander_base_map(address_map &map, bool outlatch);
	void llander_map(address_map &map);

	// devices, etc.
	required_device<m6502_device>          m_maincpu;
	required_device<watchdog_timer_device> m_watchdog;
	optional_device<output_latch_device>   m_outlatch;

	required_ioport_array<2>               m_in;
	required_ioport                        m_dsw;
	required_device<ttl153_device>         m_dswsel;
...
class asteroid_state : public llander_state
{
public:
	asteroid_state(machine_config const &mconfig, device_type type, char const *tag)
		: llander_state(mconfig, type, tag)
		, m_ram(*this, "ram%u", 0U)
		, m_sram(*this, "sram%u", 0U, 0x100U, ENDIANNESS_LITTLE)
		, m_cocktail(*this, "COCKTAIL")
	{ }

	void asteroid(machine_config &config);
	void asteroidb(machine_config &config);
	void asterock(machine_config &config);

protected:
	// initialization
	void virtual machine_start() override;
	void virtual machine_reset() override;

	// line handlers
	void invert_w(int state);
	template <int Position> void coin_w(int state);

	// memory maps
	void asteroid_base_map(address_map &map, bool outlatch);
	template <bool Latch> void asteroid_map(address_map &map);
	void asteroidb_map(address_map &map);
	void asterock_map(address_map &map);

	// devices, etc.
	required_memory_bank_array<2>     m_ram;
	memory_share_array_creator<u8, 2> m_sram;

	required_ioport                   m_cocktail;
};

class astdelux_state : public asteroid_state
{
public:
	astdelux_state(machine_config const &mconfig, device_type type, char const *tag)
...
protected:
	...
	// high score EAROM
	u8 earom_r();
	void earom_w(offs_t offset, u8 data);
	void earom_control_w(u8 data);

	// memory maps
	void astdelux_map(address_map &map);

	// devices, etc.
	required_device<ls259_device>  m_ls259;
...
{
	m_ram[0]->configure_entry(0, m_sram[0]);
	m_ram[0]->configure_entry(1, m_sram[1]);
	m_ram[1]->configure_entry(0, m_sram[1]);
	m_ram[1]->configure_entry(1, m_sram[0]);
}

void llander_state::machine_reset()
{
	m_dvg->reset_w();
}
...
{
	llander_state::machine_reset();

	m_ram[0]->set_entry(0);
	m_ram[1]->set_entry(0);
}

void astdelux_state::machine_reset()
{
	asteroid_state::machine_reset();
 ...
 *
 *************************************/
INTERRUPT_GEN_MEMBER(llander_state::interrupt)
{
 ...
 *************************************/
template <int Input>
u8 llander_state::in_r(offs_t offset)
{
	u8 const val = m_in[Input]->read();
	u8 ret = 1 << 7;
...
u8 llander_state::dsw_r(offs_t offset)
{
	// 765432--  not used
	// ------1-  LS253 DSW selector 2Y
	// -------0  LS253 DSW selector 1Y
	u8 const val = m_dsw->read();

	m_dswsel->i3a_w(BIT(val, 0));
	m_dswsel->i3b_w(BIT(val, 1));
	m_dswsel->i2a_w(BIT(val, 2));
	m_dswsel->i2b_w(BIT(val, 3));
	m_dswsel->i1a_w(BIT(val, 4));
	m_dswsel->i1b_w(BIT(val, 5));
	m_dswsel->i0a_w(BIT(val, 6));
	m_dswsel->i0b_w(BIT(val, 7));

	m_dswsel->s_w(offset);

	return 0xfc | (m_dswsel->zb_r() << 1) | m_dswsel->za_r();
}

int llander_state::clock_r()
{
	return BIT(m_maincpu->total_cycles(), 8);
}

template <bool Invert>
void llander_state::selftest_w(int state)
{
	bool active = state;
	// NOTE: needed for Lunar Lander and Asterock
	if (Invert)
		active = !active;
	m_selftest = active;
}
 ...
 *
 *************************************/
void asteroid_state::invert_w(int state)
{
	// NOTE: is only hooked up for cocktail machines
	bool const flip = state && m_cocktail->read();
	m_dvg->set_flip_x(flip);
	m_dvg->set_flip_y(flip);
}

template <int Position>
void asteroid_state::coin_w(int state)
{
	// 0 = LEFT, 1 = CENTER, 2 = RIGHT
	machine().bookkeeping().coin_counter_w(Position, state);
}
 ...
 *
 *************************************/
u8 astdelux_state::earom_r()
{
	return m_earom->data();
}

void astdelux_state::earom_w(offs_t offset, u8 data)
{
	m_earom->set_address(offset & 0x3f);
	m_earom->set_data(data);
}

void astdelux_state::earom_control_w(u8 data)
{
 ...
 *
 *************************************/
void llander_state::llander_base_map(address_map &map, bool outlatch)
{
	// All games in this driver follow this memory scheme:
	map.global_mask(0x7fff);

	// (0x0000, 0x1fff)
	// (0x2000, 0x23ff)
	map(0x2400, 0x2407).r(FUNC(llander_state::in_r<1>));
	map(0x2800, 0x2803).r(FUNC(llander_state::dsw_r));
	// (0x2c00, 0x2fff)
	map(0x3000, 0x3000).w(m_dvg, FUNC(dvg_device::go_w));
	if (outlatch)
		map(0x3200, 0x3200).w(m_outlatch, FUNC(output_latch_device::write));
	map(0x3400, 0x3400).w(m_watchdog, FUNC(watchdog_timer_device::reset_w));
	// (0x3600, 0x3600)
	// (0x3800, 0x3800)
	// (0x3a00, 0x3a00)
	// (0x3c00, 0x3c00)
...
void llander_state::llander_map(address_map &map)
{
	llander_base_map(map, true);

	// 256 bytes of RAM
	map(0x0000, 0x00ff).ram().mirror(0x1f00);

	// simple ports used for decoding IN0 and THRUST
	map(0x2000, 0x2000).portr("IN0");
	map(0x2c00, 0x2c00).portr("THRUST");

	// TTL audio for making ship noises and explosion
	map(0x3c00, 0x3c00).w("audiogen", FUNC(llander_audio_device::sound_w));
	map(0x3e00, 0x3e00).w("audiogen", FUNC(llander_audio_device::noise_reset_w));

	// INC access?
	map(0x5800, 0x5800).nopw();
}

void asteroid_state::asteroid_base_map(address_map &map, bool outlatch)
{
	llander_base_map(map, outlatch);

	// 1024 bytes of RAM space, part of which can be swapped
	map(0x0000, 0x01ff).ram();
	map(0x0200, 0x02ff).bankrw(m_ram[0]);
	map(0x0300, 0x03ff).bankrw(m_ram[1]);

	// Has a different, more complicated audio circuit
	map(0x3600, 0x3600).w("audiogen", FUNC(asteroid_audio_device::explode_w));
	map(0x3a00, 0x3a00).w("audiogen", FUNC(asteroid_audio_device::thump_w));
	map(0x3c00, 0x3c07).w("audiogen", FUNC(asteroid_audio_device::sound_w));
	map(0x3e00, 0x3e00).w("audiogen", FUNC(asteroid_audio_device::noise_reset_w));
}

template <bool Latch>
void asteroid_state::asteroid_map(address_map &map)
{
	asteroid_base_map(map, Latch);

	// IN0 is mapped the same way Lunar Lander maps IN1
	map(0x2000, 0x2007).r(FUNC(asteroid_state::in_r<0>)).nopw();
}

void asteroid_state::asteroidb_map(address_map &map)
{
	asteroid_base_map(map, true);

	// uses simple port mechanism for IN0 like Lunar Lander
	// seperate port is needed for hyperspace functionality
	map(0x2000, 0x2000).portr("IN0");
	map(0x2003, 0x2003).portr("HS");
}

void asteroid_state::asterock_map(address_map &map)
{
	asteroid_base_map(map, true);

	// same as Asteroids, with inverted inputs ONLY for IN0
	map(0x2000, 0x2007).lr8(NAME([this](offs_t offset) { return ~in_r<0>(offset); })).nopw();
}

void astdelux_state::astdelux_map(address_map &map)
{
	asteroid_map<false>(map);

	// sound is now mainly done through the POKEY generator
	map(0x2c00, 0x2c0f).rw("audiogen", FUNC(astdelux_audio_device::pokey_r), FUNC(astdelux_audio_device::pokey_w));

	// high score EAROM mapped to individual ports, LS259 now also
	// handles the output latch and the THRUST line for the audio
	map(0x2c40, 0x2c7f).r(FUNC(astdelux_state::earom_r));
	map(0x3200, 0x323f).w(FUNC(astdelux_state::earom_w)).nopr();
	map(0x3a00, 0x3dff).noprw();
	map(0x3a00, 0x3a00).w(FUNC(astdelux_state::earom_control_w));
	map(0x3c00, 0x3c07).w(m_ls259, FUNC(ls259_device::write_d7));
}
...
static INPUT_PORTS_START( llander )
	PORT_START("IN0")
	PORT_BIT( 0x01, IP_ACTIVE_HIGH, IPT_CUSTOM )   PORT_READ_LINE_DEVICE_MEMBER("dvg", dvg_device, done_r) // VG_HALT
	PORT_SERVICE( 0x02, IP_ACTIVE_LOW )            PORT_WRITE_LINE_MEMBER(llander_state, selftest_w<true>)
	PORT_BIT( 0x04, IP_ACTIVE_LOW,  IPT_TILT )
	PORT_BIT( 0x38, IP_ACTIVE_LOW,  IPT_UNKNOWN )
	PORT_BIT( 0x40, IP_ACTIVE_HIGH, IPT_CUSTOM )   PORT_READ_LINE_MEMBER(llander_state, clock_r) // 3KHz source
	...
	// The next one is a potentiometer. The way the DAC/counter
	// circuit always tries to self center at the voltage derived
	// from the thrust control. I don't think it ever expected to
	// get to 0xff. We can not emulate the external DAC circuit
	// exactly, so changing the range to 0xfe seems to solve the
	// problem. The thrust control is basically a hand operated
	// pedal, so IPT_PEDAL is used because it more accurately
	// emulates the control then using IPT_PADDLE.
	PORT_START("THRUST")
...
static INPUT_PORTS_START( llandert )
	...
	PORT_START("DSW1")
	PORT_DIPUNKNOWN_DIPLOC( 0x01, 0x00, "P8:1" ) // locks up
	PORT_DIPUNKNOWN_DIPLOC( 0x02, 0x00, "P8:2" ) // locks up
	PORT_DIPUNKNOWN_DIPLOC( 0x08, 0x00, "P8:4" ) // locks up
...
static INPUT_PORTS_START( asteroid )
	...
	PORT_BIT( 0x02, IP_ACTIVE_HIGH, IPT_CUSTOM )   PORT_READ_LINE_MEMBER(asteroid_state, clock_r) // 3KHz source
	PORT_BIT( 0x04, IP_ACTIVE_LOW,  IPT_CUSTOM )   PORT_READ_LINE_DEVICE_MEMBER("dvg", dvg_device, done_r) // VG_HALT
	...
	PORT_SERVICE( 0x80, IP_ACTIVE_HIGH )           PORT_WRITE_LINE_MEMBER(asteroid_state, selftest_w<false>)
...
static INPUT_PORTS_START( asteroidb )
	...
	PORT_BIT( 0x80, IP_ACTIVE_LOW, IPT_CUSTOM )  PORT_READ_LINE_DEVICE_MEMBER("dvg", dvg_device, done_r) // VG_HALT
...
static INPUT_PORTS_START( asterock )
	...
	PORT_SERVICE( 0x80, IP_ACTIVE_LOW )            PORT_WRITE_LINE_MEMBER(asteroid_state, selftest_w<true>)
...
static INPUT_PORTS_START( astdelux )
	...
	PORT_DIPSETTING (   0x02, DEF_STR( French ) )
	PORT_DIPSETTING (   0x03, DEF_STR( Spanish ) )
	// Default lives is 2,3,4,5. Values incremented by 1 if Bonus
	// Life set to None or Coinage set to 2C_1C. Incremented by 2
	// if both are set at the same time. PORT_CONDITION() can only
	// test for 1 switch at a time.
	...
	// The manual includes a 3rd DIP controlling the number &
	// configuration of coin counters, defined as:
#if 0
 ...
 *
 *************************************/
void llander_state::base(machine_config &config)
{
	// CPU
	M6502(config, m_maincpu, MASTER_CLOCK / 8);
	m_maincpu->set_periodic_int(FUNC(llander_state::interrupt), attotime::from_hz(CLOCK_3KHZ / 12));

	WATCHDOG_TIMER(config, m_watchdog);

	TTL153(config, m_dswsel);

	OUTPUT_LATCH(config, m_outlatch); // LS174 at N11
	...
	m_screen->set_screen_update(m_vector, FUNC(vector_device::screen_update));

	DVG(config, m_dvg, 0);
	m_dvg->set_vector(m_vector);
	m_dvg->set_memory(m_maincpu, AS_PROGRAM, 0x4000);
}

void llander_state::llander(machine_config &config)
{
	base(config);

	// CPU
	m_maincpu->set_addrmap(AS_PROGRAM, &llander_state::llander_map);

	m_outlatch->bit_handler<0>().set_output("lamp5"); // LAMP5 (COMMAND MISSION)
	m_outlatch->bit_handler<1>().set_output("lamp4"); // LAMP4 (PRIME MISSION)
	m_outlatch->bit_handler<2>().set_output("lamp3"); // LAMP3 (CADET MISSION)
	m_outlatch->bit_handler<3>().set_output("lamp2"); // LAMP2 (TRAINING MISSION)
	m_outlatch->bit_handler<4>().set_output("lamp1"); // START/SELECT LEDs
	m_outlatch->bit_handler<5>().set_nop();

	// audio
	llander_audio_device &audio(LLANDER_AUDIO(config, "audiogen"));
	audio.add_route(ALL_OUTPUTS, "mono", 1.0);

	// video
	m_screen->set_refresh_hz(CLOCK_3KHZ / 12 / 6);
	m_screen->set_visarea(522, 1566, 270, 1070);
}
...
void asteroid_state::asteroid(machine_config &config)
{
	...
	// CPU
	m_maincpu->set_addrmap(AS_PROGRAM, &asteroid_state::asteroid_map<true>);

	m_outlatch->bit_handler<0>().set_output("lamp2").invert(); // 2 PLYR START LAMP
	m_outlatch->bit_handler<1>().set_output("lamp1").invert(); // 1 PLYR START LAMP
	m_outlatch->bit_handler<2>().set_membank(m_ram[0]); // RAMSEL
	m_outlatch->bit_handler<2>().append_membank(m_ram[1]);
	m_outlatch->bit_handler<2>().append(FUNC(asteroid_state::invert_w));
	m_outlatch->bit_handler<3>().set(FUNC(asteroid_state::coin_w<0>)); // COIN CNTRL
	m_outlatch->bit_handler<4>().set(FUNC(asteroid_state::coin_w<1>)); // COIN CNTRC
	m_outlatch->bit_handler<5>().set(FUNC(asteroid_state::coin_w<2>)); // COIN CNTRR

	// audio
	asteroid_audio_device &audio(ASTEROID_AUDIO(config, "audiogen"));
	audio.add_route(ALL_OUTPUTS, "mono", 1.0);

	// video
	m_screen->set_refresh_hz(CLOCK_3KHZ / 12 / 4);
	m_screen->set_visarea(522, 1566, 394, 1182);
}

void asteroid_state::asteroidb(machine_config &config)
{
	asteroid(config);

	// CPU
	m_maincpu->set_addrmap(AS_PROGRAM, &asteroid_state::asteroidb_map);
}
...
void asteroid_state::asterock(machine_config &config)
{
	...
	// CPU
	m_maincpu->set_addrmap(AS_PROGRAM, &asteroid_state::asterock_map);
}

void astdelux_state::astdelux(machine_config &config)
{
	asteroid(config);

	// CPU
	m_maincpu->set_addrmap(AS_PROGRAM, &astdelux_state::astdelux_map);

	config.device_remove("outlatch");
	LS259(config, m_ls259); // M10
	m_ls259->q_out_cb<0>().set_output("lamp1").invert(); // START1
	m_ls259->q_out_cb<1>().set_output("lamp2").invert(); // START2
	m_ls259->q_out_cb<3>().set("audiogen", FUNC(astdelux_audio_device::thrust_w));
	m_ls259->q_out_cb<4>().set_membank(m_ram[0]); // RAMSEL
	m_ls259->q_out_cb<4>().append_membank(m_ram[1]);
	m_ls259->q_out_cb<4>().append(FUNC(astdelux_state::invert_w));
	m_ls259->q_out_cb<5>().set(FUNC(astdelux_state::coin_w<0>)); // LEFT COIN
	m_ls259->q_out_cb<6>().set(FUNC(astdelux_state::coin_w<1>)); // CENTER COIN
	m_ls259->q_out_cb<7>().set(FUNC(astdelux_state::coin_w<2>)); // RIGHT COIN

	// EAROM
	ER2055(config, m_earom);

	// audio
	astdelux_audio_device &audio(ASTDELUX_AUDIO(config.replace(), "audiogen", MASTER_CLOCK));
	audio.add_route(ALL_OUTPUTS, "mono", 1.0);
	audio.dsw().set_ioport("DSW2");
}
...
As a result of inquires regarding the achievement of extremely high
scores on Asteroids, we have developed a modification that will
accomplish the following: When the small flying saucer enters the
screen, it fires immediately in the direction of the player. The
missiles also have a wraparound capability. Originally, the program
was entered so that the saucer would go one-sixth of the way across
the screen before firing, without the wraparound capability.

There are two revisions currently in the field, ROM revision 02 has
the flip-flop capability, version 01 does not. Kits are available
immediately from your Atari Distributor.  To determine which kit you
require, check ROMs on your Asteroids PCB.  If ROM code is "-01"
order kit no. 08-0303009. If ROM code is "-02", order no. 08-0303008.
...
ROM_START( asteroidb2 ) // Based on 'asteroid1', the only difference is that the Atari copyright is removed by zeroing the ROM area
	ROM_REGION( 0x8000, "maincpu", 0 )
...
ROM_START( aerolitol ) // Pasatiempos Laguna bootleg on Rodmar PCB
	ROM_REGION( 0x8000, "maincpu", 0 )
...
ROM_START( meteorbl ) // The PCB was found inside a "Kasteroides" cab (a Spanish Asteroids bootleg from "Sede 3")
	ROM_REGION( 0x8000, "maincpu", 0 )
...
The read-only memories (ROMs) in this kit contain a new program
that changes the Asteroids Deluxe game play. To attract new players,
the game play is now operator-adjustable to be either easy for
approximately the first 30,000 points or hard through-out the game.

The technical manual describes the game play correctly if the game
PCB option switch at R5 is set to "hard". If you set the switch to
"easy", then the following game-play changes happen:

* FOUR large asteroids begin the game. The second wave of asteroids
  begins with FIVE, and the subsequent waves start with SIX through
  NINE large asteroids. In addition, the asteroids move much more
  slowly across the screen. (If the option switch is set to hard,
  the waves begin with SIX to NINE large asteroids.)
* The large ships ("death stars") when shot will break up into
  three slowly-moving diamonds. (If the option switch is set to
  hard, diamonds would immediatly begin chasing the player's
  spaceship at high speed.)

After installing these five ROMs, we recommend you set your game
to easy game play.  To do so, refer to the figure that follows.
You should note also that the self-test now deisplays and addition
0 or 1 to represent your game difficulty selection.
...
I have had a number of Asteroids deluxe pcbs pass through my hands,
every one i have come across has had a revision -01 marked romset
apart from the rom at J1, this is always marked -02.

This revision 2 rom in that location works fine, and i believe was
a factory upgrade, and production fit on 99% of pcbs.

This pcb was from a cabinet with a serial number of 000967, so its
an early one, and it has a completely -01 romset on the pcb.

Coincidentally i went through a pile of my manuals and came across
some service bulletins which might explain why there is this mismatch
in revision numbers, it seems Atari released the game, and found they
needed to change default settings becasue of earnings potential in
their default first set (revision 1), not once, but twice! then they
changed the romset altogether. The documents in question are CO-174-02
(2 pages, 3 sides). this document shows what i think was the final
revision of roms to be produced, they consist of a wholly -02 romset,
with the exception being a -03 rom at location J1.

So, anyhow, find in this file, the FIRST revision of rom -01 in J1.
This PCB contained the following roms in the following locations :

all roms are TMS 2516, marked with silver atari labels 'ATARI 8105 '
then the info below:
...
GAME( 1979, llander,  0,       llander, llander,  llander_state, empty_init, ROT0, "Atari", "Lunar Lander (rev 2)",       MACHINE_SUPPORTS_SAVE )
GAME( 1979, llander1, llander, llander, llander1, llander_state, empty_init, ROT0, "Atari", "Lunar Lander (rev 1)",       MACHINE_SUPPORTS_SAVE )
GAME( 1979, llandert, llander, llander, llandert, llander_state, empty_init, ROT0, "Atari", "Lunar Lander (screen test)", MACHINE_SUPPORTS_SAVE ) // No copyright shown, assume it's an in-house diagnostics romset (PCB came from a seller that has had Atari prototypes in his possession before)
...
GAME( 1981, spcrocks,   asteroid, asteroid,  aerolitos, asteroid_state, empty_init, ROT0, "Atari (J.Estevez license)",    "Space Rocks (Spanish clone of Asteroids)",            MACHINE_SUPPORTS_SAVE ) // Seems to be a legit set. Cabinet registered to 'J.Estevez' (Barcelona).
GAME( 1979, asteroidb1, asteroid, asteroidb, asteroidb, asteroid_state, empty_init, ROT0, "bootleg",                      "Asteroids (bootleg on Lunar Lander hardware, set 1)", MACHINE_SUPPORTS_SAVE )
...
GAME( 1979, asterock,   asteroid, asterock,  asterock,  asteroid_state, empty_init, ROT0, "bootleg (Sidam)",              "Asterock (Sidam bootleg of Asteroids)",               MACHINE_SUPPORTS_SAVE )
GAME( 1979, asterockv,  asteroid, asterock,  asterock,  asteroid_state, empty_init, ROT0, "bootleg (Videotron)",          "Asterock (Videotron bootleg of Asteroids)",           MACHINE_SUPPORTS_SAVE )
GAME( 1979, meteorite,  asteroid, asterock,  asterock,  asteroid_state, empty_init, ROT0, "bootleg (Proel)",              "Meteorite (Proel bootleg of Asteroids)",              MACHINE_SUPPORTS_SAVE )
...
GAME( 1979, meteorbl,   asteroid, asterock,  asterock,  asteroid_state, empty_init, ROT0, "bootleg",                      "Meteor (bootleg of Asteroids)",                       MACHINE_SUPPORTS_SAVE )
...
GAMEL(1980, astdelux,  0,        astdelux, astdelux, astdelux_state, empty_init, ROT0, "Atari", "Asteroids Deluxe (rev 3)", MACHINE_SUPPORTS_SAVE, layout_astdelux )
GAMEL(1980, astdelux2, astdelux, astdelux, astdelux, astdelux_state, empty_init, ROT0, "Atari", "Asteroids Deluxe (rev 2)", MACHINE_SUPPORTS_SAVE, layout_astdelux )
GAMEL(1980, astdelux1, astdelux, astdelux, astdelux, astdelux_state, empty_init, ROT0, "Atari", "Asteroids Deluxe (rev 1)", MACHINE_SUPPORTS_SAVE, layout_astdelux )

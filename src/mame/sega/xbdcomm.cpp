...
public:
	...
	void start()
	{
		...
		m_thread = std::thread(
				[this] ()
				{
					LOG("XBDCOMM: network thread started\n");
					try {
						m_ioctx.run();
					} catch (const std::exception& e) {
						LOG("XBDCOMM: Exception in network thread: %s\n", e.what());
					} catch (...) { // Catch any other unknown exceptions
						LOG("XBDCOMM: Unknown exception in network thread\n");
					}
					LOG("XBDCOMM: network thread completed\n");
				});
	}

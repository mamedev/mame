...
static INPUT_PORTS_START(ai_kbd)
	...
	PORT_BIT(0xff, 0x80, IPT_LIGHTGUN_X) PORT_CROSSHAIR(X, 1.0, 0.0, 0) PORT_SENSITIVITY(30) PORT_KEYDELTA(10) PORT_MINMAX(0, 255) PORT_PLAYER(2) PORT_NAME("Touchpad X")

	PORT_START("TOUCHPADY")
	PORT_BIT(0xff, 0x80, IPT_LIGHTGUN_Y) PORT_CROSSHAIR(Y, 1.0, 0.0, 0) PORT_SENSITIVITY(30) PORT_KEYDELTA(10) PORT_MINMAX(0, 255) PORT_PLAYER(2) PORT_NAME("Touchpad Y")
INPUT_PORTS_END
...
void segaai_state::machine_start()
{
	...
	machine().crosshair().get_crosshair(1).set_screen(CROSSHAIR_SCREEN_NONE);
}

...
public:
	...
	void start()
	{
		...
		m_thread = std::thread(
				[this] ()
				{
					LOG("M2COMM: network thread started\n");
					try {
						m_ioctx.run();
					} catch (const std::exception& e) {
						LOG("M2COMM: Exception in network thread: %s\n", e.what());
					} catch (...) { // Catch any other unknown exceptions
						LOG("M2COMM: Unknown exception in network thread\n");
					}
					LOG("M2COMM: network thread completed\n");
				});
	}

...
static void liberatr_init_planet(running_machine *machine, planet *liberatr_planet, UINT8 *planet_rom)
{
	...
	const UINT8 *latitude_scale = memory_region(machine, REGION_USER1);
	const UINT8 *longitude_scale = memory_region(machine, REGION_USER2);
...
VIDEO_START( liberatr )
{
	...
	liberatr_init_planet(machine, liberatr_planets[0], &memory_region(machine, REGION_GFX1)[0x2000]);
	liberatr_init_planet(machine, liberatr_planets[1], &memory_region(machine, REGION_GFX1)[0x0000]);
}

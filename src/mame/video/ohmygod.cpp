...
void ohmygod_state::ohmygod_videoram_w(address_space &space, offs_t offset, uint16_t data, uint16_t mem_mask)
{
	COMBINE_DATA(&m_videoram[offset]);
	m_bg_tilemap->mark_tile_dirty(offset / 2);
}

void ohmygod_state::ohmygod_spritebank_w(address_space &space, offs_t offset, uint16_t data, uint16_t mem_mask)
{
	if (ACCESSING_BITS_8_15)
		m_spritebank = data & 0x8000;
}

void ohmygod_state::ohmygod_scrollx_w(address_space &space, offs_t offset, uint16_t data, uint16_t mem_mask)
{
	COMBINE_DATA(&m_scrollx);
	m_bg_tilemap->set_scrollx(0, m_scrollx - 0x81ec);
}

void ohmygod_state::ohmygod_scrolly_w(address_space &space, offs_t offset, uint16_t data, uint16_t mem_mask)
{

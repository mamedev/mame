...
READ8_HANDLER( psychic5_paged_ram_r )
{
	...
	return state->ps5_pagedram[state->ps5_vram_page][offset];
}
...
WRITE8_HANDLER( psychic5_paged_ram_w )
{
	...
	else
	{
		...
		{
			UINT16 bg_scrollx = state->ps5_io_ram[BG_SCROLLX_LSB] | (state->ps5_io_ram[BG_SCROLLX_MSB] << 8);
			tilemap_set_scrollx(state->bg_tilemap, 0, bg_scrollx);
		}
		else if (offset == BG_SCROLLY_LSB || offset == BG_SCROLLY_MSB)
		{
			UINT16 bg_scrolly = state->ps5_io_ram[BG_SCROLLY_LSB] | (state->ps5_io_ram[BG_SCROLLY_MSB] << 8);
			tilemap_set_scrolly(state->bg_tilemap, 0, bg_scrolly);
		}
...
VIDEO_START( psychic5 )
{
	...
	state->ps5_pagedram[0] = auto_alloc_array(machine, UINT8, 0x2000);
	state->ps5_pagedram[1] = auto_alloc_array(machine, UINT8, 0x2000);

	state->bg_videoram  = &state->ps5_pagedram[0][0x0000];
	state->ps5_dummy_bg_ram      = &state->ps5_pagedram[0][0x1000];
	state->ps5_io_ram            = &state->ps5_pagedram[1][0x0000];
...
static void draw_background(running_machine *machine, bitmap_t *bitmap, const rectangle *cliprect)
{
	...
	{
		state->bg_clip_mode = 0;
		state->sx1 = state->sy1 = state->sy2 = 0;
	}
	else
	{
		int sy1_old = state->sy1;
		int sx1_old = state->sx1;
		int sy2_old = state->sy2;

		state->sy1 = spriteram[11];		/* sprite 0 */
		state->sx1 = spriteram[12];
		state->sy2 = spriteram[11+128];	/* sprite 8 */

		switch (state->bg_clip_mode)
		{
		case  0: case  4: if (sy1_old != state->sy1) state->bg_clip_mode++; break;
		case  2: case  6: if (sy2_old != state->sy2) state->bg_clip_mode++; break;
		case  8: case 10:
		case 12: case 14: if (sx1_old != state->sx1) state->bg_clip_mode++; break;
		case  1: case  5: if (state->sy1 == 0xf0) state->bg_clip_mode++; break;
		case  3: case  7: if (state->sy2 == 0xf0) state->bg_clip_mode++; break;
		case  9: case 11: if (state->sx1 == 0xf0) state->bg_clip_mode++; break;
		case 13: case 15: if (sx1_old == 0xf0) state->bg_clip_mode++;
		case 16: if (state->sy1 != 0x00) state->bg_clip_mode = 0; break;
		}
		...
		case  0: case  4: case  8: case 12: case 16:
			...
			break;
		case  1: clip.min_y = state->sy1; break;
		case  3: clip.max_y = state->sy2; break;
		case  5: clip.max_y = state->sy1; break;
		case  7: clip.min_y = state->sy2; break;
		case  9: case 15: clip.min_x = state->sx1; break;
		case 11: case 13: clip.max_x = state->sx1; break;
		}

...
{
	m_irq_on_timer = machine().scheduler().timer_alloc(timer_expired_delegate(FUNC(ts2068_state::irq_on), this));
	m_irq_off_timer = machine().scheduler().timer_alloc(timer_expired_delegate(FUNC(ts2068_state::irq_off), this));

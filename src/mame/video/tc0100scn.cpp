...
void tc0100scn_device::device_start()
{
	...
	m_tilemap[0][0] = &machine().tilemap().create(*m_gfxdecode, tilemap_get_info_delegate(&tc0100scn_device::get_bg_tile_info<0x0000, 0>, "bg0_64x64", this), TILEMAP_SCAN_ROWS, 8, 8, 64, 64);
	m_tilemap[1][0] = &machine().tilemap().create(*m_gfxdecode, tilemap_get_info_delegate(&tc0100scn_device::get_bg_tile_info<0x4000, 1>, "bg1_64x64", this), TILEMAP_SCAN_ROWS, 8, 8, 64, 64);
	m_tilemap[2][0] = &machine().tilemap().create(*this,        tilemap_get_info_delegate(&tc0100scn_device::get_tx_tile_info<0x2000, 0>, "fg0_64x64", this), TILEMAP_SCAN_ROWS, 8, 8, 64, 64);
	...
	xd = (!m_multiscrn_hack) ?  (-m_x_offset) : (-m_x_offset - 2);
	yd = (!m_multiscrn_hack) ?  (8 - m_y_offset) : (1 - m_y_offset);
	...
	set_gfx(0, std::make_unique<gfx_element>(&palette(), tc0100scn_charlayout, (uint8_t *)(&m_ram[0x6000 / 2]), NATIVE_ENDIAN_VALUE_LE_BE(8,0), 256, 0));
	set_gfx(1, std::make_unique<gfx_element>(&palette(), tc0100scn_charlayout, (uint8_t *)(&m_ram[0x11000 / 2]), NATIVE_ENDIAN_VALUE_LE_BE(8,0), 256, 0));
	...
	gfx(1)->set_granularity(tgfx->granularity());
...
TILE_GET_INFO_MEMBER(tc0100scn_device::get_bg_tile_info)
{
	...
	SET_TILE_INFO_MEMBER(m_gfxnum,
			code,
			(color + m_bg_colbank[Layer]) & 0xff,
			TILE_FLIPYX((attr & 0xc000) >> 14));
}

template<int Offset, int Layout>
TILE_GET_INFO_MEMBER(tc0100scn_device::get_tx_tile_info)
{
	int attr = m_ram[Offset + tile_index];

	SET_TILE_INFO_MEMBER(Layout,
			attr & 0x00ff,
			((attr & 0x3f00) >> 8) + m_tx_colbank,
			TILE_FLIPYX((attr & 0xc000) >> 14));
}

void tc0100scn_device::set_colbanks(u8 bg0, u8 bg1, u8 tx)
{
	m_bg_colbank[0] = bg0;
	m_bg_colbank[1] = bg1;
	m_tx_colbank = tx;
...
{
	data = COMBINE_DATA(&m_ctrl[offset]);
...
void tc0100scn_device::tilemap_draw_fg(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect, tilemap_t* tmap, int flags, u32 priority, u32 pri_mask)
{
		...
		for (x = 0; x < cliprect.width(); x++)
		{
			...
			if ((p & 0xf)!= 0 || (flags & TILEMAP_DRAW_OPAQUE))
			{
				...
				if (screen.priority().valid())
				{
					...
					pri[x + cliprect.min_x] = (pri[x + cliprect.min_x] & pri_mask) | priority;
				}
			}
...
int tc0100scn_device::tilemap_draw(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect, int layer, int flags, u32 priority, u32 pri_mask)
{
...
if (disable != 0 && disable != 3 && disable != 7)
	...
	switch (layer)
	{
		case 0:
			if (disable & 0x01)
				return 1;
			m_tilemap[0][m_dblwidth]->draw(screen, bitmap, clip, flags, priority, pri_mask);
			break;
		case 1:
			if (disable & 0x02)
				return 1;
			tilemap_draw_fg(screen, bitmap, clip, m_tilemap[1][m_dblwidth], flags, priority, pri_mask);
			break;
		case 2:
			if (disable & 0x04)
				return 1;
			m_tilemap[2][m_dblwidth]->draw(screen, bitmap, clip, flags, priority, pri_mask);
			break;
	}
	return 0;
}

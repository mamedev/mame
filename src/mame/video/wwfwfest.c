...
WRITE16_HANDLER( wwfwfest_fg0_videoram_w )
{
	...
	if (ACCESSING_BITS_8_15 && ACCESSING_BITS_0_7) {
		...
		state->fg0_videoram[offset]=(data>>8)&0xff;
	} else {
		state->fg0_videoram[offset]=data&0xff;
	}
...
static TILE_GET_INFO( get_bg1_tile_info )
{
    ...
    **- End of Comments -*/
	...
	int tileno;
	int colbank;
	tilebase =  &state->bg1_videoram[tile_index];
...
VIDEO_START( wwfwfest )
{
    ...
    state_save_register_global(machine, state->bg1_scrollx);
    state_save_register_global(machine, state->bg1_scrolly);
	...
	state->sprite_xoff = state->bg0_dx = state->bg1_dx[0] = state->bg1_dx[1] = 0;
}
...
SCREEN_UPDATE( wwfwfest )
{
	...
	if (state->pri == 0x0078) {
		...
		tilemap_set_scrolly( state->bg1_tilemap, 0, state->bg1_scrolly  );
		tilemap_set_scrollx( state->bg1_tilemap, 0, state->bg1_scrollx  + state->bg1_dx[0]);
	} else {
		tilemap_set_scrolly( state->bg1_tilemap, 0, state->bg0_scrolly  );
		tilemap_set_scrollx( state->bg1_tilemap, 0, state->bg0_scrollx  + state->bg1_dx[1]);
		tilemap_set_scrolly( state->bg0_tilemap, 0, state->bg1_scrolly  );
		tilemap_set_scrollx( state->bg0_tilemap, 0, state->bg1_scrollx  + state->bg0_dx);
	}

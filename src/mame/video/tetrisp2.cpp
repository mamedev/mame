...
{
	uint16_t code_hi = m_vram_fg[ 2 * tile_index ] >> 8;
	uint16_t code_lo = m_vram_fg[ 2 * tile_index ] & 0xf; ///printf("tile_idx[$%2x]=$%3x, palette=$%2x\n", tile_index, code_hi, code_lo);////
if(m_vram_fg[ 2 * tile_index + 1 ] != 0) printf("VRAM ASCII Haut-Mot Non-Zero!!!\n");/////////
	SET_TILE_INFO_MEMBER(3,
			code_hi,
			code_lo,
			0);

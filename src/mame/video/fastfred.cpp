...
{
	if (m_gfxdecode->flip_screen_x() != (data & 0x01))
	{
		m_gfxdecode->flip_screen_x_set(data & 0x01);

		m_bg_tilemap->set_flip((m_gfxdecode->flip_screen_x() ? TILEMAP_FLIPX : 0) | (m_gfxdecode->flip_screen_y() ? TILEMAP_FLIPY : 0));
	}
}

WRITE8_MEMBER(fastfred_state::fastfred_flip_screen_y_w )
{
	if (m_gfxdecode->flip_screen_y() != (data & 0x01))
	{
...
void fastfred_state::draw_sprites(bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	...
	for (offs = m_spriteram.bytes() - 4; offs >= 0; offs -= 4)
	{
		...
		m_gfxdecode->gfx(1)->transpen(bitmap,m_gfxdecode->flip_screen_x() ? spritevisibleareaflipx : spritevisiblearea,
				code,

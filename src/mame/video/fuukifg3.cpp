...
UINT32 fuuki32_state::screen_update(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	...
	*/
	...
	m_gfxdecode->flip_screen_set((m_vregs[0x1e / 4] & 0x0000ffff) & 1);

	/* Layers scrolling */

	scrolly_offs = ((m_vregs[0xc / 4] & 0xffff0000) >> 16) - (m_gfxdecode->flip_screen() ? 0x103 : 0x1f3);
	scrollx_offs =  (m_vregs[0xc / 4] & 0x0000ffff) - (m_gfxdecode->flip_screen() ? 0x2c7 : 0x3f6);
	...
	m_fuukivid->draw_sprites(screen, bitmap, cliprect, m_gfxdecode->flip_screen(), m_spr_buffered_tilebank);
	return 0;
}

...
static void mrokumei_handleblit( running_machine *machine, int rom_base )
{
	...
	UINT8 *pBlitData = memory_region(machine, RGNCLASS_USER, "user1") + rom_base;
...
{
	pteacher_handleblit(machine, (blitter_bank >> 5) * 0x10000 & (memory_region_length(machine, RGNCLASS_USER, "user1") - 1));
}

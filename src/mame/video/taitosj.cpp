...
uint8_t taitosj_state::gfxrom_r()
{
	uint8_t ret;
...
void taitosj_state::characterram_w(offs_t offset, uint8_t data)
{
...
{
	characterram_w(offset, data ^ 0xfc);
}


void taitosj_state::collision_reg_clear_w(uint8_t data)
{
	m_collision_reg[0] = 0;
	m_collision_reg[1] = 0;
	m_collision_reg[2] = 0;
...
void taitosj_state::copy_layer(bitmap_ind16 &bitmap, const rectangle &cliprect, int which, int *sprites_on, rectangle *sprite_areas)
{
...
uint32_t taitosj_state::screen_update(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	return video_update_common(bitmap, cliprect, &taitosj_state::copy_layer);
}

...
static void draw_sprite(running_machine *machine, bitmap_t *bitmap,int spr_number)
{
	...
	bank &= (memory_region_length(machine, RGNCLASS_GFX, "gfx2")-1);	/* limit to the range of available ROMs */
	...
	gfx = &memory_region(machine, RGNCLASS_GFX, "gfx2")[bank];
...
VIDEO_UPDATE( choplifter )
{
	...
	{
		memory_region(screen->machine, RGNCLASS_CPU, "main")[0xC085]=33;
	}
#endif
	return 0;
}

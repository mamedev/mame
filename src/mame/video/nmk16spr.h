...
public:
	...
	// configuration
	template <typename... T> void set_colpri_callback(T &&... args) { m_colpri_cb = nmk16_spr_colpri_cb_delegate(std::forward<T>(args)...); }
	template <typename... T> void set_ext_callback(T &&... args) { m_ext_cb = nmk16_spr_ext_cb_delegate(std::forward<T>(args)...); }

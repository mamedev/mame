...
VIDEO_START( lordgun )
{
	...
	tilemap_set_transparent_pen(state->tilemap_0,0x3f);

	// Has line scroll
	tilemap_set_scroll_rows(state->tilemap_1,0x200);
	tilemap_set_scroll_cols(state->tilemap_1,1);
	tilemap_set_transparent_pen(state->tilemap_1,0x3f);

	tilemap_set_scroll_rows(state->tilemap_2,1);
	tilemap_set_scroll_cols(state->tilemap_2,1);
	tilemap_set_transparent_pen(state->tilemap_2,0x3f);

	tilemap_set_scroll_rows(state->tilemap_3,1);
	tilemap_set_scroll_cols(state->tilemap_3,1);
	tilemap_set_transparent_pen(state->tilemap_3,0x3f);

	// Buffer state->bitmaps for 4 tilemaps (0-3) + sprites (4)
	for (i = 0; i < 5; i++)
		state->bitmaps[i] = auto_bitmap_alloc(machine, w, h, BITMAP_FORMAT_INDEXED16);
}
...
//  popmessage("%03x, %02x", input_port_read(machine, "LIGHT0_X"), input_port_read(machine, "LIGHT0_Y"));
	...
	state->gun[i].scr_x = lordgun_gun_x_table[x];
...
void lordgun_update_gun(running_machine *machine, int i)
{
	...
	if (	(state->gun[i].scr_x < visarea.min_x)	||
			(state->gun[i].scr_x > visarea.max_x)	||
			(state->gun[i].scr_y < visarea.min_y)	||
			(state->gun[i].scr_y > visarea.max_y)	)
		state->gun[i].hw_x = state->gun[i].hw_y = 0;
}
...
SCREEN_UPDATE( lordgun )
{
	...
	tilemap_set_scrollx( state->tilemap_0, 0, *state->scroll_x_0 );
	tilemap_set_scrolly( state->tilemap_0, 0, *state->scroll_y_0 );

	for (y = 0; y < 0x200; y++)
		tilemap_set_scrollx( state->tilemap_1, y, (*state->scroll_x_1) + state->scrollram[y * 4/2 + 2/2]);
	tilemap_set_scrolly( state->tilemap_1, 0, *state->scroll_y_1 );

	tilemap_set_scrollx( state->tilemap_2, 0, *state->scroll_x_2 );
	tilemap_set_scrolly( state->tilemap_2, 0, *state->scroll_y_2 );

	tilemap_set_scrollx( state->tilemap_3, 0, *state->scroll_x_3 );
	tilemap_set_scrolly( state->tilemap_3, 0, *state->scroll_y_3 );
	...
	int l;
	for (l = 0; l < 5; l++)
		bitmap_fill(state->bitmaps[l], cliprect, trans_pen);

	if (layers_ctrl & 1)	tilemap_draw(state->bitmaps[0], cliprect, state->tilemap_0, 0, 0);
	...
	for (y = cliprect->min_y; y <= cliprect->max_y; y++)
	{
		for (x = cliprect->min_x; x <= cliprect->max_x; x++)
		{
			...
			for (l = 0; l < 5; l++)
			{
				pens[l] = *BITMAP_ADDR16(state->bitmaps[l], y, x);
				if (pens[l] == trans_pen)
			...
			l	=	pri2layer[state->priority_ram[pri_addr] & 7];

...
PALETTE_INIT_MEMBER(cclimber_state,swimmer)
{
	...
	for (i = 0; i < 0x20; i++)
	{
		...
		palette.set_pen_color(i + 0x100, MAKE_RGB(r, g, b));
	}

	/* side panel backgrond pen */
#if 0
	// values calculated from the resistors don't seem to match the real board
	palette.set_pen_color(SWIMMER_SIDE_BG_PEN, MAKE_RGB(0x24, 0x5d, 0x4e));
#endif
	palette.set_pen_color(SWIMMER_SIDE_BG_PEN, MAKE_RGB(0x20, 0x98, 0x79));
}
...
PALETTE_INIT_MEMBER(cclimber_state,yamato)
{
	...
	for (i = 0; i < 0x20; i++)
	{
		...
		palette.set_pen_color(i + 0x40, MAKE_RGB(r, g, b));
	}

	/* fake colors for bg gradient */
	for (i = 0; i < 0x100; i++)
		palette.set_pen_color(YAMATO_SKY_PEN_BASE + i, MAKE_RGB(0, 0, i));
}
...
void cclimber_state::swimmer_set_background_pen()
{
	...
	palette.set_pen_color(CCLIMBER_BG_PEN, MAKE_RGB(r, g, b));
}

...
PALETTE_INIT_MEMBER(snk6502_state,snk6502)
{
	...
	for (i = 0; i < TOTAL_COLORS(0); i++)
		palette.set_pen_color(0, i), m_palette[i]);

	for (i = 0; i < TOTAL_COLORS(1); i++)
	{
		if (i % 4 == 0)
			palette.set_pen_color(1, i), m_palette[4 * m_backcolor + 0x20]);
		else
			palette.set_pen_color(1, i), m_palette[i + 0x20]);
	}
}
...
WRITE8_MEMBER(snk6502_state::snk6502_flipscreen_w)
{
	...
	if (m_backcolor != (data & 7))
	{
		...
		for (i = 0;i < 32;i += 4)
			m_palette->set_pen_color(1, i), m_palette[4 * m_backcolor + 0x20]);
	}
...
PALETTE_INIT_MEMBER(snk6502_state,satansat)
{
	...
	for (i = 0; i < TOTAL_COLORS(machine(),0); i++)
		palette.set_pen_color(0, i), m_palette[4 * (i % 4) + (i / 4)]);

	for (i = 0; i < TOTAL_COLORS(1); i++)
	{
		if (i % 4 == 0)
			palette.set_pen_color(1, i), m_palette[m_backcolor + 0x10]);
		else
			palette.set_pen_color(1, i), m_palette[4 * (i % 4) + (i / 4) + 0x10]);
	}
}
...
WRITE8_MEMBER(snk6502_state::satansat_backcolor_w)
{
	...
	if (m_backcolor != (data & 0x03))
	{
		...
		for (i = 0; i < 16; i += 4)
			m_palette->set_pen_color(1, i), m_palette[m_backcolor + 0x10]);
	}
}

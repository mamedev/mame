...
uint32_t k001006_device::read(address_space &space, offs_t offset, uint32_t mem_mask)
{
	if (offset == 1)
	{
		switch (m_device_sel)
...
void k001006_device::write(address_space &space, offs_t offset, uint32_t data, uint32_t mem_mask)
{
	if (offset == 0)
	{
		COMBINE_DATA(&m_addr);

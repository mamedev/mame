...
- verify midi interface operation.
- FS-A1GT does not boot into VShell, keyboard is unresponsive at prompt.
- Implement DAC/PCM related hardware and hook it up to the S1990.
- Microphone input.
...
void msxtr_state::s1990_io_map(address_map &map)
{
	...
	map(0xe4, 0xe4).w(m_s1990, FUNC(msx_s1990_device::reg_write));
...
void msxtr_state::turbor(ay8910_type ay8910_type, machine_config &config, const internal_layout &layout)
{
	...
	// The R800 and Z80 see exactly the same memory layouts, only one of the two CPUs is active at any one time.
	// The S1990 controls which CPU is active and can inject wait states.

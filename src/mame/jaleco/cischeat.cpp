...
Palette RAM         RW      0b8000-0bffff       <
    ...
    Palette Scroll 2        0bec00-0befff0      0bee00-0befff
----------------------------------------------------------------------------------
...
public:
	cischeat_state(const machine_config &mconfig, device_type type, const char *tag)
		...
		, m_road1cpu(*this, "road1cpu")
		, m_road2cpu(*this, "road2cpu")
		, m_iocpu(*this, "iocpu")
		...
		, m_road(*this, "road%u", 0U)
		, m_tmap(*this, "scroll%u", 0U)
		, m_ram(*this, "ram")
		, m_roadram(*this, "road%u", 0U)
		, m_f1gpstr2_ioready(*this, "ioready")
		, m_io_in(*this, "IN%u", 0U)
		, m_io_pedal(*this, "PEDAL")
...
protected:
	...
	required_device<cpu_device> m_road1cpu;
	required_device<cpu_device> m_road2cpu;
	...
	required_device_array<jaleco_road_device, 2> m_road;
	required_device_array<megasys1_tilemap_device, 3> m_tmap;

	required_shared_ptr<u16> m_ram;
	required_shared_ptr_array<u16, 2> m_roadram;
	optional_shared_ptr<u16> m_f1gpstr2_ioready;

	optional_ioport_array<7> m_io_in;
	optional_ioport m_io_pedal;

	output_finder<3> m_leds;

	u16 *m_spriteram = nullptr;
	u16 m_active_layers = 0U;
	...
	void bigrun_soundbank_w(offs_t offset, u16 data, u16 mem_mask = ~0);
	u16 bigrun_ip_select_r();
	void leds_out_w(offs_t offset, u16 data, u16 mem_mask = ~0);
	void unknown_out_w(u16 data);
	void motor_out_w(offs_t offset, u16 data, u16 mem_mask = ~0);
	...
	void cischeat_comms_w(u16 data);
	u16 f1gpstar_wheel_r();
	u16 f1gpstr2_ioready_r();
	void f1gpstar_motor_w(offs_t offset, u16 data, u16 mem_mask = ~0);
	void f1gpstar_soundint_w(u16 data);
	void f1gpstar_comms_w(u16 data);
	void f1gpstr2_io_w(offs_t offset, u16 data, u16 mem_mask = ~0);
	template <unsigned Which> void cischeat_soundbank_w(offs_t offset, u16 data, u16 mem_mask = ~0);
	void sound_irq(int state);
	u32 screen_update_bigrun(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect);
	u32 screen_update_cischeat(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect);
	u32 screen_update_f1gpstar(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect);
...
private:
	required_ioport_array<2> m_io_x;
	required_ioport_array<2> m_io_y;
	required_ioport_array<2> m_io_in1_sub;
	required_ioport m_io_in1_common;

	u16 *m_buffer_spriteram = nullptr;
	std::unique_ptr<u16[]> m_allocated_spriteram;

	u16 m_sprite_dma_reg = 0U;

	u16 m_wildplt_output = 0U;

	u16 wildplt_xy_r();
	u16 wildplt_mux_r();
	void wildplt_mux_w(u16 data);

	void sprite_dma_w(offs_t offset, u16 data, u16 mem_mask = ~0);
...
void wildplt_state::video_start()
{
	...
	save_item(NAME(m_sprite_dma_reg));
	save_pointer(NAME(m_allocated_spriteram), 0x1000 / 2);
}
...
u32 cischeat_state::screen_update_bigrun(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	bitmap.fill(0x1000, cliprect);

	for (int i = 7; i >= 4; i--)
	{
		m_road[0]->draw(bitmap, cliprect, i, i, (i != 7));
		m_road[1]->draw(bitmap, cliprect, i, i, true);
		m_spritegen->draw_sprites(bitmap, cliprect, i + 1, i, m_spriteram, 0x1000 / 2);
	}
	...
	for (int i = 3; i >= 0; i--)
	{
		m_road[0]->draw(bitmap, cliprect, i, i, true);
		m_road[1]->draw(bitmap, cliprect, i, i, true);
		m_spritegen->draw_sprites(bitmap, cliprect, i + 1, i, m_spriteram, 0x1000 / 2);
	}
...
u32 cischeat_state::screen_update_cischeat(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	bitmap.fill(0, cliprect);

	m_road[0]->draw(bitmap, cliprect, 7, 5, false);
	m_road[1]->draw(bitmap, cliprect, 7, 5, true);

	m_tmap[0]->draw(screen, bitmap, cliprect, 0, 0);
	m_tmap[1]->draw(screen, bitmap, cliprect, 0, 0);

	m_spritegen->draw_sprites(bitmap, cliprect, 15, 3, m_spriteram, 0x1000 / 2);
	m_road[0]->draw(bitmap, cliprect, 4, 1, true);
	m_road[1]->draw(bitmap, cliprect, 4, 1, true);
	m_spritegen->draw_sprites(bitmap, cliprect, 2, 2, m_spriteram, 0x1000 / 2);
	m_road[0]->draw(bitmap, cliprect, 0, 0, true);
	m_road[1]->draw(bitmap, cliprect, 0, 0, true);
	m_spritegen->draw_sprites(bitmap, cliprect, 1, 0, m_spriteram, 0x1000 / 2);
	m_tmap[2]->draw(screen, bitmap, cliprect, 0, 0);

	/* for the map screen */
	m_spritegen->draw_sprites(bitmap, cliprect, 16, 16, m_spriteram, 0x1000 / 2);
...
u32 cischeat_state::screen_update_f1gpstar(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	bitmap.fill(0, cliprect);
...
/*  1: clouds 5, grad 7, road 0     2: clouds 5, grad 7, road 0, tunnel roof 0 */
	...
	/* road 1!! 0!! */
	m_road[1]->draw(bitmap, cliprect, 6, 7, false);
	m_road[0]->draw(bitmap, cliprect, 6, 7, true);

	m_tmap[0]->draw(screen, bitmap, cliprect, 0, 0);
	m_tmap[1]->draw(screen, bitmap, cliprect, 0, 0);

	/* road 1!! 0!! */
	m_road[1]->draw(bitmap, cliprect, 1, 5, true);
	m_road[0]->draw(bitmap, cliprect, 1, 5, true);

	m_spritegen->draw_sprites(bitmap, cliprect, 15, 2, m_spriteram, 0x1000 / 2);

	/* road 1!! 0!! */
	m_road[1]->draw(bitmap, cliprect, 0, 0, true);
	m_road[0]->draw(bitmap, cliprect, 0, 0, true);

	m_spritegen->draw_sprites(bitmap, cliprect, 1, 1, m_spriteram, 0x1000 / 2);
	m_tmap[2]->draw(screen, bitmap, cliprect, 0, 0);
	m_spritegen->draw_sprites(bitmap, cliprect, 0, 0, m_spriteram, 0x1000 / 2);
...
u16 cischeat_state::bigrun_ip_select_r()
{
	switch (m_ip_select & 0x3)
	{
		case 0 : return m_io_in[6]->read();      // Driving Wheel
		case 1 : return 0xffff;                 // Cockpit: Up / Down Position
		case 2 : return 0xffff;                 // Cockpit: Left / Right Position?
		case 3 : return m_io_pedal->read();    // Accelerator (Pedal)
		default: return 0xffff;
	}
}


void cischeat_state::unknown_out_w(u16 data)
{
	// ?? 91/1/91/1 ...
	LOGUNKNOWN("%s: Unknown unknown_out_w write %04x", machine().describe_context(), data);
}


void cischeat_state::motor_out_w(offs_t offset, u16 data, u16 mem_mask)
{
	// motor (seat?)
...
void cischeat_state::wheel_out_w(u16 data)
{
	// motor (wheel?)
	LOGUNKNOWN("%s: Unknown wheel_out_w write %04x", machine().describe_context(), data);
}


void cischeat_state::ip_select_w(u16 data)
{
	m_ip_select = data;
}


void cischeat_state::ip_select_plus1_w(u16 data)
{
	// value above + 1
	m_ip_select = data + 1;
}
...
u16 cischeat_state::cischeat_ip_select_r()
{
	switch (m_ip_select & 0x3)
	{
		case 0 : return m_io_in[6]->read();  // Driving Wheel
...
void cischeat_state::cischeat_soundlatch_w(u16 data)
{
...
void cischeat_state::cischeat_comms_w(u16 data)
{
...
u16 cischeat_state::f1gpstr2_ioready_r()
{
	return BIT(m_f1gpstr2_ioready[0], 0) ? 0xff : 0xf0;
}


void cischeat_state::f1gpstar_motor_w(offs_t offset, u16 data, u16 mem_mask)
{
	// "shudder" motors, leds
	if (ACCESSING_BITS_0_7)
	{
...
void cischeat_state::f1gpstar_soundint_w(u16 data)
{
...
void cischeat_state::f1gpstar_comms_w(u16 data)
{
	/* Not sure about this one. Values: $10 then 0, $7 then 0 */
	m_road1cpu->set_input_line(INPUT_LINE_RESET, BIT(data, 0) ? ASSERT_LINE : CLEAR_LINE);
	m_road2cpu->set_input_line(INPUT_LINE_RESET, BIT(data, 1) ? ASSERT_LINE : CLEAR_LINE);
	m_soundcpu->set_input_line(INPUT_LINE_RESET, BIT(data, 2) ? ASSERT_LINE : CLEAR_LINE);
}


void cischeat_state::f1gpstr2_io_w(offs_t offset, u16 data, u16 mem_mask)
{
	if (ACCESSING_BITS_0_7)
	{
...
void cischeat_state::machine_start()
{
	...
	save_item(NAME(m_ip_select));
	save_item(NAME(m_motor_value));
	save_item(NAME(m_io_value));
}
...
void cischeat_state::leds_out_w(offs_t offset, u16 data, u16 mem_mask)
{
	// leds
	if (ACCESSING_BITS_0_7)
	{
...
void cischeat_state::bigrun_map(address_map &map)
{
	...
	map(0x08000a, 0x08000b).w(m_soundlatch[0], FUNC(generic_latch_16_device::write));  // To sound cpu
	...
	map(0x082000, 0x082005).rw(m_tmap[0], FUNC(megasys1_tilemap_device::scroll_r), FUNC(megasys1_tilemap_device::scroll_w));
	map(0x082008, 0x08200d).rw(m_tmap[1], FUNC(megasys1_tilemap_device::scroll_r), FUNC(megasys1_tilemap_device::scroll_w));
	map(0x082100, 0x082105).rw(m_tmap[2], FUNC(megasys1_tilemap_device::scroll_r), FUNC(megasys1_tilemap_device::scroll_w));
	...
	map(0x088000, 0x08bfff).ram().share("road2_shareram"); // Sharedram with sub CPU#2
	map(0x08c000, 0x08ffff).ram().share("road1_shareram"); // Sharedram with sub CPU#1
	...
	map(0x090000, 0x093fff).ram().w(m_tmap[0], FUNC(megasys1_tilemap_device::write)).share("scroll0");  // Scroll ram 0
	map(0x094000, 0x097fff).ram().w(m_tmap[1], FUNC(megasys1_tilemap_device::write)).share("scroll1");  // Scroll ram 1
	map(0x098000, 0x09bfff).ram().w(m_tmap[2], FUNC(megasys1_tilemap_device::write)).share("scroll2");  // Scroll ram 2

	map(0x09c000, 0x09ffff).ram().w(m_palette, FUNC(palette_device::write16)).share("palette");         // Palettes
	map(0x0f0000, 0x0fffff).ram().share(m_ram);                                                         // RAM
...
void cischeat_state::cischeat_map(address_map &map)
{
	...
	map(0x082200, 0x082201).portr("IN5");    // DSW 3 (4 bits)
	map(0x082208, 0x082209).noprw();                 // watchdog reset
	map(0x082300, 0x082301).r(m_soundlatch[1], FUNC(generic_latch_16_device::read)).w(FUNC(cischeat_state::cischeat_soundlatch_w)); // From sound cpu
	map(0x082308, 0x082309).w(FUNC(cischeat_state::cischeat_comms_w));
	map(0x082400, 0x082401).w(FUNC(cischeat_state::active_layers_w));
...
/*  Only the first 0x800 bytes are tested but:
	...
	map(0x090000, 0x097fff).ram().share("road2_shareram"); // Sharedram with sub CPU#2
	map(0x098000, 0x09ffff).ram().share("road1_shareram"); // Sharedram with sub CPU#1

	/* Only writes to the first 0x40000 bytes affect the tilemaps:             */
	/* either these games support larger tilemaps or have more ram than needed */
	map(0x0a0000, 0x0a7fff).ram().w(m_tmap[0], FUNC(megasys1_tilemap_device::write)).share("scroll0");  // Scroll ram 0
	map(0x0a8000, 0x0affff).ram().w(m_tmap[1], FUNC(megasys1_tilemap_device::write)).share("scroll1");  // Scroll ram 1
	map(0x0b0000, 0x0b7fff).ram().w(m_tmap[2], FUNC(megasys1_tilemap_device::write)).share("scroll2");  // Scroll ram 2
...
void cischeat_state::f1gpstar_map(address_map &map)
{
	...
	map(0x080008, 0x080009).r(m_soundlatch[1], FUNC(generic_latch_16_device::read));     // From sound cpu
	map(0x080008, 0x080009).w(m_soundlatch[0], FUNC(generic_latch_16_device::write));    // To sound cpu
...
void wildplt_state::machine_start()
{
	cischeat_state::machine_start();

	save_item(NAME(m_wildplt_output));
}

// ad stick read select
u16 wildplt_state::wildplt_xy_r()
{
	switch (m_ip_select)
	{
		case 1: return m_io_y[1]->read() | (m_io_x[1]->read() << 8);
		case 2: return m_io_y[0]->read() | (m_io_x[0]->read() << 8);
	}
...
u16 wildplt_state::wildplt_mux_r()
{
	u16 split_in = 0xffff;
	switch (m_wildplt_output & 0xc)
	{
//      case 0: return m_io_in[1]->read();
		case 4: split_in = m_io_in1_sub[0]->read(); break;
		case 8: split_in = m_io_in1_sub[1]->read(); break;
	}

	return split_in & m_io_in1_common->read();
}

void wildplt_state::wildplt_mux_w(u16 data)
{
	m_wildplt_output = data & 0xc;
}


// Same as f1gpstr2, but vregs are slightly different:
void wildplt_state::wildplt_map(address_map &map)
{
	map(0x000000, 0x07ffff).rom();                                                                     // ROM
	map(0x080000, 0x080001).portr("IN0").w(FUNC(wildplt_state::f1gpstr2_io_w));    // DSW 1 & 2
	map(0x080004, 0x080005).r(FUNC(wildplt_state::wildplt_mux_r)).w(FUNC(wildplt_state::wildplt_mux_w)); // Buttons
	map(0x080008, 0x080009).r(m_soundlatch[1], FUNC(generic_latch_16_device::read));     // From sound cpu
	map(0x080008, 0x080009).w(m_soundlatch[0], FUNC(generic_latch_16_device::write));    // To sound cpu
	map(0x08000c, 0x08000d).w(FUNC(wildplt_state::sprite_dma_w)); // 1000, 3000
	map(0x080010, 0x080011).r(FUNC(wildplt_state::wildplt_xy_r)).w(FUNC(wildplt_state::ip_select_w)); // X, Y
	map(0x080014, 0x080015).nopw();
	map(0x080018, 0x080019).rw(FUNC(wildplt_state::f1gpstr2_ioready_r), FUNC(wildplt_state::f1gpstar_soundint_w));

	map(0x081000, 0x081fff).ram().share("io_shareram");
	...
	map(0x082108, 0x082109).noprw();                 // ? written with 0 only
	map(0x082208, 0x082209).noprw();                 // watchdog reset
	map(0x082308, 0x082309).nopr().w(FUNC(wildplt_state::f1gpstar_comms_w));
	map(0x082400, 0x082401).w(FUNC(wildplt_state::active_layers_w));
...
void cischeat_state::bigrun_map2(address_map &map)
{
	...
	map(0x040000, 0x043fff).ram().share("road1_shareram");  // Shared RAM (with Main CPU)
	map(0x080000, 0x0807ff).ram().share(m_roadram[0]);      // Road RAM
...
{
	map(0x000000, 0x03ffff).rom();                          // ROM
	map(0x040000, 0x043fff).ram().share("road2_shareram");  // Shared RAM (with Main CPU)
	map(0x080000, 0x0807ff).ram().share(m_roadram[1]);      // Road RAM
	map(0x0c0000, 0x0c3fff).ram();                          // RAM
}
...
{
	map(0x000000, 0x03ffff).rom();                          // ROM
	map(0x040000, 0x047fff).ram().share("road1_shareram");  // Shared RAM (with Main CPU)
	map(0x080000, 0x0807ff).ram().share(m_roadram[0]);      // Road RAM
	map(0x0c0000, 0x0c3fff).ram();                          // RAM
	map(0x100000, 0x100001).nopw();                         // watchdog
	map(0x200000, 0x23ffff).rom().region("road1cpu", 0x40000);  // ROM
}

void cischeat_state::cischeat_map3(address_map &map)
{
	map(0x000000, 0x03ffff).rom();                          // ROM
	map(0x040000, 0x047fff).ram().share("road2_shareram");  // Shared RAM (with Main CPU)
	map(0x080000, 0x0807ff).ram().share(m_roadram[1]);      // Road RAM
	map(0x0c0000, 0x0c3fff).ram();                          // RAM
	map(0x100000, 0x100001).nopw();                         // watchdog
	map(0x200000, 0x23ffff).rom().region("road2cpu", 0x40000);  // ROM
}
...
{
	map(0x000000, 0x03ffff).rom();                         // ROM
	map(0x080000, 0x087fff).ram().share("road1_shareram"); // Shared RAM (with Main CPU)
	map(0x100000, 0x1007ff).ram().share(m_roadram[0]);     // Road RAM
...
{
	map(0x000000, 0x03ffff).rom();                          // ROM
	map(0x080000, 0x087fff).ram().share("road2_shareram");  // Shared RAM (with Main CPU)
	map(0x100000, 0x1007ff).ram().share(m_roadram[1]);      // Road RAM
	map(0x180000, 0x183fff).ram();                          // RAM
	map(0x200000, 0x200001).nopw();                         // watchdog
}
...
void cischeat_state::bigrun_soundbank_w(offs_t offset, u16 data, u16 mem_mask)
{
	if (ACCESSING_BITS_0_7)
	{
		m_oki[0]->set_rom_bank(BIT(data, 0));
		m_oki[1]->set_rom_bank(BIT(data, 4));
	}
}
...
{
	map(0x000000, 0x03ffff).rom();                                                 // ROM
	map(0x040000, 0x040001).r(m_soundlatch[0], FUNC(generic_latch_16_device::read)).w(FUNC(cischeat_state::bigrun_soundbank_w));    // From Main CPU
...
void cischeat_state::cischeat_soundbank_w(offs_t offset, u16 data, u16 mem_mask)
{
	if (ACCESSING_BITS_0_7)
		m_oki[Which]->set_rom_bank(BIT(data, 0));
}

void cischeat_state::cischeat_sound_map(address_map &map)
{
	map(0x000000, 0x03ffff).rom();                                                 // ROM
	map(0x040002, 0x040003).w(FUNC(cischeat_state::cischeat_soundbank_w<0>));               // Sample Banking
	map(0x040004, 0x040005).w(FUNC(cischeat_state::cischeat_soundbank_w<1>));               // Sample Banking
	map(0x060002, 0x060003).w(m_soundlatch[1], FUNC(generic_latch_16_device::write));                          // To Main CPU
	map(0x060004, 0x060005).r(m_soundlatch[0], FUNC(generic_latch_16_device::read));                             // From Main CPU
	map(0x080000, 0x080003).rw("ymsnd", FUNC(ym2151_device::read), FUNC(ym2151_device::write)).umask16(0x00ff);
	map(0x0a0000, 0x0a0003).rw(m_oki[0], FUNC(okim6295_device::read), FUNC(okim6295_device::write)).umask16(0x00ff);
	map(0x0c0000, 0x0c0003).rw(m_oki[1], FUNC(okim6295_device::read), FUNC(okim6295_device::write)).umask16(0x00ff);
	map(0x0f0000, 0x0fffff).ram();                                                 // RAM
...
{
	map(0x000000, 0x03ffff).rom();                                                 // ROM
	map(0x040004, 0x040005).w(FUNC(cischeat_state::cischeat_soundbank_w<0>));               // Sample Banking   (cischeat: 40002)
	map(0x040008, 0x040009).w(FUNC(cischeat_state::cischeat_soundbank_w<1>));               // Sample Banking   (cischeat: 40004)
	map(0x060000, 0x060001).r(m_soundlatch[0], FUNC(generic_latch_16_device::read)).w(m_soundlatch[1], FUNC(generic_latch_16_device::write));   // From Main CPU    (cischeat: 60004)
	map(0x080000, 0x080003).rw("ymsnd", FUNC(ym2151_device::read), FUNC(ym2151_device::write)).umask16(0x00ff);
	map(0x0a0000, 0x0a0003).rw(m_oki[0], FUNC(okim6295_device::read), FUNC(okim6295_device::write)).umask16(0x00ff);
	map(0x0c0000, 0x0c0003).rw(m_oki[1], FUNC(okim6295_device::read), FUNC(okim6295_device::write)).umask16(0x00ff);
...
{
	map(0x000000, 0x03ffff).rom(); // ROM
	map(0x040004, 0x040005).w(FUNC(cischeat_state::cischeat_soundbank_w<0>));                   // Sample Banking
	map(0x040008, 0x040009).w(FUNC(cischeat_state::cischeat_soundbank_w<1>));                   // Sample Banking
...
{
	map(0x000000, 0x07ffff).rom();                                  // ROM
	map(0x080000, 0x080fff).ram().share("io_shareram");
	map(0x100000, 0x100001).writeonly().share(m_f1gpstr2_ioready);  //
	map(0x180000, 0x183fff).ram();                                  // RAM
...
void cischeat_state::bigrun(machine_config &config)
{
	...
	m_maincpu->set_addrmap(AS_PROGRAM, &cischeat_state::bigrun_map);
	TIMER(config, "scantimer").configure_scanline(FUNC(cischeat_state::bigrun_scanline), "screen", 0, 1);

	M68000(config, m_road1cpu, 10000000);
	m_road1cpu->set_addrmap(AS_PROGRAM, &cischeat_state::bigrun_map2);
	m_road1cpu->set_vblank_int("screen", FUNC(cischeat_state::irq4_line_hold));

	M68000(config, m_road2cpu, 10000000);
	m_road2cpu->set_addrmap(AS_PROGRAM, &cischeat_state::bigrun_map3);
	m_road2cpu->set_vblank_int("screen", FUNC(cischeat_state::irq4_line_hold));
...
//  m_soundcpu->set_periodic_int(FUNC(cischeat_state::irq4_line_hold), attotime::from_hz(16*30));
	...
	JALECO_ZOOMSPR_BIGRUN(config, m_spritegen, m_palette, gfx_bigrun);
	...
	JALECO_ROAD(config, m_road[0], m_palette, 0x2000/2);
	JALECO_ROAD(config, m_road[1], m_palette, 0x1800/2);
	...
	OKIM6295(config, m_oki[0], 4000000, okim6295_device::PIN7_HIGH); // clock frequency & pin 7 not verified
	m_oki[0]->add_route(ALL_OUTPUTS, "speaker", 0.25, 0);
	m_oki[0]->add_route(ALL_OUTPUTS, "speaker", 0.25, 1);

	OKIM6295(config, m_oki[1], 4000000, okim6295_device::PIN7_HIGH); // clock frequency & pin 7 not verified
	m_oki[1]->add_route(ALL_OUTPUTS, "speaker", 0.25, 0);
	m_oki[1]->add_route(ALL_OUTPUTS, "speaker", 0.25, 1);
}
...
void cischeat_state::cischeat(machine_config &config)
{
	...
	/* basic machine hardware */
	m_maincpu->set_addrmap(AS_PROGRAM, &cischeat_state::cischeat_map);

	m_road1cpu->set_addrmap(AS_PROGRAM, &cischeat_state::cischeat_map2);

	m_road2cpu->set_addrmap(AS_PROGRAM, &cischeat_state::cischeat_map3);
	...
	JALECO_ZOOMSPR(config.replace(), m_spritegen, m_palette, gfx_cischeat);
	...
	m_road[0]->set_colorbase(0x3800/2);
	m_road[1]->set_colorbase(0x4800/2);
}
...
void cischeat_state::f1gpstar(machine_config &config)
{
	...
	m_maincpu->set_addrmap(AS_PROGRAM, &cischeat_state::f1gpstar_map);

	m_road1cpu->set_clock(12000000);
	m_road1cpu->set_addrmap(AS_PROGRAM, &cischeat_state::f1gpstar_map2);

	m_road2cpu->set_clock(12000000);
	m_road2cpu->set_addrmap(AS_PROGRAM, &cischeat_state::f1gpstar_map3);
	...
	JALECO_ZOOM_ROAD(config.replace(), m_road[0], m_palette, 0x3800/2);
	JALECO_ZOOM_ROAD(config.replace(), m_road[1], m_palette, 0x4800/2);
...
void cischeat_state::f1gpstr2(machine_config &config)
{
	...
	m_maincpu->set_addrmap(AS_PROGRAM, &cischeat_state::f1gpstr2_map);

	m_soundcpu->set_addrmap(AS_PROGRAM, &cischeat_state::f1gpstr2_sound_map);

	M68000(config, m_iocpu, 10000000);
	m_iocpu->set_addrmap(AS_PROGRAM, &cischeat_state::f1gpstr2_io_map);
...
{
	f1gpstr2(config);
	m_maincpu->set_addrmap(AS_PROGRAM, &wildplt_state::wildplt_map);
}
...
{
	u8       *src = memregion(region)->base();
	const u8 *end = src + memregion(region)->bytes();
...
ROM_START( cischeat )
	...
	ROM_COPY( "road1cpu", 0x40000, 0x80000, 0x40000 )
	ROM_COPY( "road2cpu", 0x40000, 0xc0000, 0x40000 )

	...
	m_write_break_cb(*this)
{
	...
	all_caps = false;                   // Set by user switch
}
				...
				if(key_state)
				{
					switch(column_counter)
					{
						...
						default:
							...
							new_keystroke(~((column_counter-1)*8 + row_counter + 1), (shift||shift_lock), control);
					}

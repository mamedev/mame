...
DEFINE_DEVICE_TYPE(K1515XM031_KBD, k1515xm031_kbd_device, "uknc_kbd", "UKNC keyboard")
DEFINE_DEVICE_TYPE(K1515XM031_TMR, k1515xm031_tmr_device, "uknc_timer", "UKNC timer")
...
INPUT_PORTS_START(ms7007)
	   ...
	   PORT_BIT( 0x02, IP_ACTIVE_HIGH, IPT_KEYBOARD ) PORT_NAME("Keypad -") PORT_CODE(KEYCODE_SLASH_PAD) PORT_CHAR(UCHAR_MAMEKEY(SLASH_PAD))
	   PORT_BIT( 0x04, IP_ACTIVE_HIGH, IPT_UNUSED )
	   PORT_BIT( 0x08, IP_ACTIVE_HIGH, IPT_UNUSED )
	   PORT_BIT( 0x10, IP_ACTIVE_HIGH, IPT_KEYBOARD ) PORT_CODE(KEYCODE_LSHIFT) PORT_CODE(KEYCODE_RSHIFT) PORT_CHAR(UCHAR_SHIFT_1)
	   ...
	   PORT_BIT( 0x10, IP_ACTIVE_HIGH, IPT_KEYBOARD ) PORT_CODE(KEYCODE_MINUS_PAD) PORT_CHAR('^') PORT_CHAR('~')
	   ...
	   PORT_BIT( 0x20, IP_ACTIVE_HIGH, IPT_KEYBOARD ) PORT_NAME("Keypad +") PORT_CODE(KEYCODE_NUMLOCK) PORT_CHAR(UCHAR_MAMEKEY(NUMLOCK))
	   PORT_BIT( 0x40, IP_ACTIVE_HIGH, IPT_KEYBOARD ) PORT_NAME("ISP (Go)") PORT_CODE(KEYCODE_F8) PORT_CHAR(UCHAR_MAMEKEY(F8))
	   PORT_BIT( 0x80, IP_ACTIVE_HIGH, IPT_KEYBOARD ) PORT_NAME("SBROS (Reset)") PORT_CODE(KEYCODE_F9) PORT_CHAR(UCHAR_MAMEKEY(F9))
	   ...
	   PORT_BIT( 0x20, IP_ACTIVE_HIGH, IPT_KEYBOARD ) PORT_CODE(KEYCODE_BACKSPACE) PORT_CHAR(8)
	   PORT_BIT( 0x40, IP_ACTIVE_HIGH, IPT_KEYBOARD ) PORT_NAME("UST (Setup)") PORT_CODE(KEYCODE_F7) PORT_CHAR(UCHAR_MAMEKEY(F7))
	   PORT_BIT( 0x80, IP_ACTIVE_HIGH, IPT_KEYBOARD ) PORT_NAME("POM (Help)") PORT_CODE(KEYCODE_F6) PORT_CHAR(UCHAR_MAMEKEY(F6))
	   ...
	   PORT_BIT( 0x40, IP_ACTIVE_HIGH, IPT_KEYBOARD ) PORT_CODE(KEYCODE_ENTER) PORT_CHAR('\r')
	   ...
	   PORT_BIT( 0x40, IP_ACTIVE_HIGH, IPT_KEYBOARD ) PORT_CODE(KEYCODE_EQUALS) PORT_CHAR('_')
...
ioport_constructor k1515xm031_kbd_device::device_input_ports() const
{
	return INPUT_PORTS_NAME(ms7007);
}

//-------------------------------------------------
//  k1515xm031_kbd_device - constructor
//-------------------------------------------------

k1515xm031_kbd_device::k1515xm031_kbd_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock)
	: device_t(mconfig, K1515XM031_KBD, tag, owner, clock)
...
void k1515xm031_kbd_device::device_start()
{
...
void k1515xm031_kbd_device::device_reset()
{
	m_csr = 0;
...
int k1515xm031_kbd_device::z80daisy_irq_state()
{
...
int k1515xm031_kbd_device::z80daisy_irq_ack()
{
	int vec = -1;
...
uint16_t k1515xm031_kbd_device::read(offs_t offset)
{
	uint16_t data = 0;
...
void k1515xm031_kbd_device::write(offs_t offset, uint16_t data, uint16_t mem_mask)
{
	if (offset == 0)
	{
		if ((data & CSR_IE) == 0)
...
void k1515xm031_kbd_device::key_make(uint8_t row, uint8_t column)
{
	m_buf = column * 16 + row + 4;
	LOG("%11.6f MAKE  c %d r %d = %04x\n", machine().time().as_double(), column, row, m_buf);
	m_csr |= CSR_DONE;
	raise_virq(m_write_virq, m_csr, CSR_IE, m_rxrdy);
	stop_processing();
}

void k1515xm031_kbd_device::key_break(uint8_t row, uint8_t column)
{
	m_buf = 128 + row + 4;
	LOG("%11.6f break c %d r %d = %04x\n", machine().time().as_double(), column, row, m_buf);
	m_csr |= CSR_DONE;
	raise_virq(m_write_virq, m_csr, CSR_IE, m_rxrdy);
	stop_processing();
}
...
//-------------------------------------------------
//  k1515xm031_tmr_device - constructor
//-------------------------------------------------

k1515xm031_tmr_device::k1515xm031_tmr_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock)
	: device_t(mconfig, K1515XM031_TMR, tag, owner, clock)
	, device_z80daisy_interface(mconfig, *this)
	, m_write_virq(*this)
{
}
...
void k1515xm031_tmr_device::device_start()
{
	save_item(NAME(m_csr));
	save_item(NAME(m_buf));
	...
	m_timer = timer_alloc(FUNC(k1515xm031_tmr_device::timer_tick), this);
...
void k1515xm031_tmr_device::device_reset()
{
...
int k1515xm031_tmr_device::z80daisy_irq_state()
{
	if (m_rxrdy == ASSERT_LINE)
		return Z80_DAISY_INT;
	else
...
int k1515xm031_tmr_device::z80daisy_irq_ack()
{
	int vec = -1;
...
uint16_t k1515xm031_tmr_device::read(offs_t offset)
{
	uint16_t data = 0;
...
void k1515xm031_tmr_device::write(offs_t offset, uint16_t data, uint16_t mem_mask)
{
...
TIMER_CALLBACK_MEMBER(k1515xm031_tmr_device::timer_tick)
{
	if (m_counter)
	{
		if (--m_counter == 0)

...
/***************************************************************************
    ...
    hw revisions (this driver implements rev B):
...
static constexpr int PK11_SLICES = 300;

typedef struct
{
	uint32_t addr[PK11_SLICES];
	uint16_t mode[PK11_SLICES];
	int border[PK11_SLICES];
} pk11_scanline;

static std::map<int, std::string> syscall_map =
{
	// syscalls
	{0174176, "GET4K"},
	{0174200, "GET8K"},
	...
	{0177144, "MDCSR"}, // ram disk (md.sys, mdc.prc)
	{0177146, "MDBUF"},
	{0177150, "MDSIZ"},
	{0176500, "RCSR"},  // serial port, vector 300 (dlc.prc)
	{0176502, "RBUF"},
	{0176504, "TCSR"},
	{0176506, "TBUF"},
	{0177530, "WCSR"},  // window manager (wm.prc)
	{0177524, "WMCSR"},
	{0177564, "DCSR"},  // console, vector 64
	{0177566, "DBUF"},
	{0177174, "HOLER"}, // calculator (calc.prc)
	{0174060, "FILCSR"},// filer (fil.prc)
	{0174062, "FMCSR"},
	{0174064, "FCCSR"},
	{0177600, "GCSR"},  // graphics display (gt.prc)
	{0176200, "MXX"},   // mouse (msxc.prc)
	{0176202, "MYY"},
	{0176204, "MCSR"},
	{0176206, "MCALL"},
	{0176240, "SNCSR"}, // sound (snc.prc)
	{0176242, "SNBUF"},
	...
	{0176220, "COCO"},
	{0176216, "COCS"},
	{0177514, "LPCSR"}, // printer (lp.sys, lpc.prc)
};
...
public:
	...
	void pk11(machine_config &config) ATTR_COLD;
...
INPUT_PORTS_START(pk11)
	...
	PORT_START("X1")
	PORT_BIT( 0x001F, IP_ACTIVE_LOW, IPT_UNUSED )
	PORT_BIT( 0x0020, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_NAME("Keypad -") PORT_CODE(KEYCODE_SLASH_PAD) PORT_CHAR(UCHAR_MAMEKEY(SLASH_PAD))
	PORT_BIT( 0x0040, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_TAB) PORT_CHAR('\t')
	PORT_BIT( 0x0080, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_J) PORT_CHAR('j') PORT_CHAR('J')
	...
	PORT_BIT( 0x0800, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_E) PORT_CHAR('e') PORT_CHAR('E')
	...
	PORT_BIT( 0x0040, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_LCONTROL) PORT_CODE(KEYCODE_RCONTROL) PORT_CHAR(UCHAR_SHIFT_2)
	PORT_BIT( 0x0080, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_F) PORT_CHAR('f') PORT_CHAR('F')
	PORT_BIT( 0x0100, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_C) PORT_CHAR('c') PORT_CHAR('C')
	PORT_BIT( 0x0200, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_U) PORT_CHAR('u') PORT_CHAR('U')
	PORT_BIT( 0x0400, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_K) PORT_CHAR('k') PORT_CHAR('K')
	PORT_BIT( 0x0800, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_P) PORT_CHAR('p') PORT_CHAR('P')
	PORT_BIT( 0x1000, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_N) PORT_CHAR('n') PORT_CHAR('N')
	PORT_BIT( 0x2000, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_G) PORT_CHAR('g') PORT_CHAR('G')
	PORT_BIT( 0x4000, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_L) PORT_CHAR('l') PORT_CHAR('L')
	...
	PORT_BIT( 0x0080, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_Q) PORT_CHAR('q') PORT_CHAR('Q')
	PORT_BIT( 0x0100, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_Y) PORT_CHAR('y') PORT_CHAR('Y')
	PORT_BIT( 0x0200, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_W) PORT_CHAR('w') PORT_CHAR('W')
	PORT_BIT( 0x0400, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_A) PORT_CHAR('a') PORT_CHAR('A')
	PORT_BIT( 0x0800, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_I) PORT_CHAR('i') PORT_CHAR('I')
	PORT_BIT( 0x1000, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_R) PORT_CHAR('r') PORT_CHAR('R')
	PORT_BIT( 0x2000, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_O) PORT_CHAR('o') PORT_CHAR('O')
	PORT_BIT( 0x4000, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_B) PORT_CHAR('b') PORT_CHAR('B')
	PORT_BIT( 0x8000, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_TILDE) PORT_CHAR('@') PORT_CHAR('`')
	...
	PORT_BIT( 0x0080, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_NAME("Lock") PORT_CODE(KEYCODE_CAPSLOCK) PORT_CHAR(UCHAR_MAMEKEY(CAPSLOCK))
	PORT_BIT( 0x0100, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_MINUS_PAD) PORT_CHAR('^') PORT_CHAR('~')
	PORT_BIT( 0x0200, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_S) PORT_CHAR('s') PORT_CHAR('S')
	PORT_BIT( 0x0400, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_M) PORT_CHAR('m') PORT_CHAR('M')
	PORT_BIT( 0x0800, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_SPACE) PORT_CHAR(' ')
	PORT_BIT( 0x1000, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_T) PORT_CHAR('t') PORT_CHAR('T')
	PORT_BIT( 0x2000, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_X) PORT_CHAR('x') PORT_CHAR('X')
	PORT_BIT( 0x4000, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_LEFT) PORT_CHAR(UCHAR_MAMEKEY(LEFT))
	PORT_BIT( 0x8000, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_COMMA) PORT_CHAR(',') PORT_CHAR('<')

	PORT_START("X5")
	PORT_BIT( 0x001F, IP_ACTIVE_LOW, IPT_UNUSED )
	PORT_BIT( 0x0020, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_7_PAD) PORT_CHAR(UCHAR_MAMEKEY(7_PAD))
	PORT_BIT( 0x0040, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_0_PAD) PORT_CHAR(UCHAR_MAMEKEY(0_PAD))
	PORT_BIT( 0x0080, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_1_PAD) PORT_CHAR(UCHAR_MAMEKEY(1_PAD))
	PORT_BIT( 0x0100, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_4_PAD) PORT_CHAR(UCHAR_MAMEKEY(4_PAD))
	PORT_BIT( 0x0200, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_NAME("Keypad +") PORT_CODE(KEYCODE_NUMLOCK) PORT_CHAR(UCHAR_MAMEKEY(NUMLOCK))
	PORT_BIT( 0x0400, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_BACKSPACE) PORT_CHAR(8)
	PORT_BIT( 0x0800, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_RIGHT) PORT_CHAR(UCHAR_MAMEKEY(RIGHT))
	PORT_BIT( 0x1000, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_DOWN) PORT_CHAR(UCHAR_MAMEKEY(DOWN))
	...
	PORT_START("X6")
	PORT_BIT( 0x001F, IP_ACTIVE_LOW, IPT_UNUSED )
	PORT_BIT( 0x0020, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_8_PAD) PORT_CHAR(UCHAR_MAMEKEY(8_PAD))
	PORT_BIT( 0x0040, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_DEL_PAD) PORT_CHAR(UCHAR_MAMEKEY(DEL_PAD))
	PORT_BIT( 0x0080, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_2_PAD) PORT_CHAR(UCHAR_MAMEKEY(2_PAD))
	PORT_BIT( 0x0100, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_5_PAD) PORT_CHAR(UCHAR_MAMEKEY(5_PAD))
	PORT_BIT( 0x0200, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_NAME("ISP (Go)") PORT_CODE(KEYCODE_F8) PORT_CHAR(UCHAR_MAMEKEY(F8))
	PORT_BIT( 0x0400, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_NAME("UST (Setup)") PORT_CODE(KEYCODE_F7) PORT_CHAR(UCHAR_MAMEKEY(F7))
	PORT_BIT( 0x0800, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_ENTER) PORT_CHAR('\r')
	PORT_BIT( 0x1000, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_UP) PORT_CHAR(UCHAR_MAMEKEY(UP))
	PORT_BIT( 0x2000, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_EQUALS) PORT_CHAR('_')
	PORT_BIT( 0x4000, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_H) PORT_CHAR('h') PORT_CHAR('H')
	PORT_BIT( 0x8000, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_Z) PORT_CHAR('z') PORT_CHAR('Z')

	PORT_START("X7")
	PORT_BIT( 0x001F, IP_ACTIVE_LOW, IPT_UNUSED )
	PORT_BIT( 0x0020, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_9_PAD) PORT_CHAR(UCHAR_MAMEKEY(9_PAD))
	PORT_BIT( 0x0040, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_ENTER_PAD) PORT_CHAR(UCHAR_MAMEKEY(ENTER_PAD))
	PORT_BIT( 0x0080, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_3_PAD) PORT_CHAR(UCHAR_MAMEKEY(3_PAD))
	PORT_BIT( 0x0100, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_6_PAD) PORT_CHAR(UCHAR_MAMEKEY(6_PAD))
	PORT_BIT( 0x0200, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_NAME("SBROS (Reset)") PORT_CODE(KEYCODE_F9) PORT_CHAR(UCHAR_MAMEKEY(F9))
	PORT_BIT( 0x0400, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_NAME("POM (Help)") PORT_CODE(KEYCODE_F6) PORT_CHAR(UCHAR_MAMEKEY(F6))
	PORT_BIT( 0x0800, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_QUOTE) PORT_CHAR(':') PORT_CHAR('*')
	PORT_BIT( 0x1000, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_SLASH) PORT_CHAR('/') PORT_CHAR('?')
	PORT_BIT( 0x2000, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_MINUS) PORT_CHAR('-') PORT_CHAR('=')
	PORT_BIT( 0x4000, IP_ACTIVE_LOW, IPT_KEYBOARD ) PORT_CODE(KEYCODE_0) PORT_CHAR('0')
...
void pk11_state::machine_start()
{
	...
	for (int i = 0; i < 2; i++)
	{
		const char name[2] = { static_cast<char>('0' + i), 0 };
...
{
	const uint16_t data = m_ur[offset];
	LOGMMU("UR%d R == %06o\n", offset, data);
	return data;
}
...
{
	const uint16_t data = m_hr[offset];
	LOGMMU("HR%d R == %06o\n", offset, data);
	return data;
}
...
{
	uint16_t data = 0; // FIXME
	if (((addr << 1) >= m_ram->size() || BIT(reg, 3)) && !machine().side_effects_disabled())
		BUS_ERROR;
	else
		data = m_p_ram[addr];
	return data;
...
{
	const uint16_t ur = BIT(offset, 12, 3);
	const uint32_t addr = (offset & 07777) + (BIT(m_ur[ur], 4, 10) << 11);
...
{
	offset += 020000;
	const uint16_t hr = BIT(offset, 12, 3);
	const uint32_t addr = (offset & 07777) + (BIT(m_hr[hr], 4, 10) << 11);
...
{
	const uint16_t a = 0170000 + (offset << 1);
	const uint16_t data = m_p_ram[offset & 03777];
...
{
	const uint16_t data = m_buf[m_buf_ptr];
...
{
	const uint16_t raw = bitswap<16>(m_io_keyboard[m_digit]->read(),
		0, 0, 0, 0, 4, 15, 14, 13, 12, 11, 10, 9, 8, 7, 6, 5);

	return bitswap<8>(raw, 1, 0, 9, 7, 6, 4, 3, 2) &
 ...
 *   512 entries -- 1,2,4bpp palettes (two sets, 4 palettes per mode):
 *     vm1, vm2, vm40, vm41 modes in each set (vm41 unused in set 1)
 *   512 entries -- 8bpp palettes (two palettes)
 *
 * VDPTAS points to 2-level display list (300 dwords)
 *   list entry format: 19:0 = top 20 bits of physical address,
 *     25:20 and 31 = mode bits, 30:26 = slice length
...
{
	const uint32_t paladdr = vdpword(4 >> 1);
	uint32_t tasaddr = vdpword(8 >> 1) + 4; // skip first 2 invisible entries
	...
	{
		const uint16_t hi = m_p_ram[paladdr + ((c >> 1) & 0x7f) + (c & ~0xff)];
		const uint16_t lo = m_p_ram[paladdr + ((c >> 1) & 0x7f) + (c & ~0xff) + 0x80];
		uint16_t rgb;
		if (c & 1)
			rgb = (hi & 0xff00) | (lo >> 8);
		else
	...
	for (int i = 0; i < PK11_DISP_VERT; i++)
	{
		...
		for (int j = 0, bar = PK11_SLICES; bar > 0;)
		{
...
uint32_t pk11_state::screen_update(screen_device &screen, bitmap_rgb32 &bitmap, const rectangle &cliprect)
{
	...
	for (int row = 0; row < PK11_DISP_VERT; row++)
	{
		...
		for (int g = 0; g < PK11_SLICES; g++)
		{
			// 0: 1bpp, 1: 2bpp, 2: 4bpp, 3: 4/8bpp
			const int vn = BIT(scanline->mode[g], 6, 2);
			// vd 0,1: fetch 52 bytes per scanline, 2: 104, 3: 208 bytes FIXME vd 0
			const int vd = BIT(scanline->mode[g], 8, 2);
			const int pb = (BIT(scanline->mode[g], 15) << 4) | (vn << 2) | BIT(scanline->mode[g], 4, 2);
			const int wide = vd ? (1 << (vd - 1)) : 1;
			const int zoom = vd ? (1 << (3 - vd)) : 4;
...
//          else TODO
			...
			{
				const uint16_t gfx = m_p_ram[scanline->addr[g] + w];
				uint32_t color;
...
{
	K1801VM2(config, m_maincpu, 16_MHz_XTAL / 2); // CPU is a 1806VM2 (no internal divider by 2)
	...
	// debug console, active if power-on memory test fails
	...
	m_pit8253_1->set_clk<0>(16_MHz_XTAL / 8);
	m_pit8253_1->out_handler<0>().set(m_pit8253_2, FUNC(pit8253_device::write_gate0));
	m_pit8253_1->set_clk<1>(16_MHz_XTAL / 8);
	m_pit8253_1->out_handler<1>().set(m_pit8253_2, FUNC(pit8253_device::write_gate1));
	m_pit8253_1->out_handler<1>().append([this] (int state) { m_upd8251->write_txc(state); m_upd8251->write_rxc(state); });
	m_pit8253_1->set_clk<2>(16_MHz_XTAL / 8);
	m_pit8253_1->out_handler<2>().set(m_pit8253_2, FUNC(pit8253_device::write_gate2));

	PIT8253(config, m_pit8253_2);
	m_pit8253_2->set_clk<0>(16_MHz_XTAL / 8);
	m_pit8253_2->set_clk<1>(16_MHz_XTAL / 8);
	m_pit8253_2->set_clk<2>(16_MHz_XTAL / 8);
	...
	I8279(config, m_kdc, 16_MHz_XTAL / 8);
	m_kdc->out_irq_callback().set(m_pic8259, FUNC(pic8259_device::ir4_w));
	m_kdc->out_sl_callback().set([this](uint8_t data) { m_digit = data & 7; });
	m_kdc->in_rl_callback().set(FUNC(pk11_state::kbd_r));
	...
	WD1010(config, m_hdc, 20_MHz_XTAL / 4); // FIXME -- WRCLK produced by PAL
	...
	m_screen->set_raw(16_MHz_XTAL, PK11_TOTAL_HORZ, PK11_HORZ_START,

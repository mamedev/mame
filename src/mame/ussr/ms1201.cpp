...
/***************************************************************************
    ...
    Elektronika MS 1201 series of single-board computers and DVK desktops.

    Firmware is similar to DEC LSI-11 ODT.
    ...
    Paper tape: "177550L" (loads absolute loader from tape), then
    ...
    Paper tape: "B LA" (copies absolute loader from ROM), "P" (loads tape).
...
public:
	...
	void ms120101(machine_config &config) ATTR_COLD;
	void dvk1(machine_config &config) ATTR_COLD;
	void dvk2(machine_config &config) ATTR_COLD;
...
public:
	...
	void ms120102(machine_config &config) ATTR_COLD;
	void dvk3(machine_config &config) ATTR_COLD;
	void dvk3m(machine_config &config) ATTR_COLD;
...
{
	// FIXME move to machine_start? see https://github.com/1801BM1/k1801/tree/master/030
	m_odt_map = 2;
...
void ms1201_base_state::ms1201_base(machine_config &config)
{
	...
	QBUS_SLOT(config, "qbus" ":2", qbus_cards, nullptr); // FIXME RX11 device should be here
	QBUS_SLOT(config, "qbus" ":3", qbus_cards, nullptr);
	QBUS_SLOT(config, "qbus" ":4", qbus_cards, nullptr);

	K1801VP065(config, m_dl11, 4.608_MHz_XTAL);
	m_dl11->set_rxc(9600);
	m_dl11->set_txc(9600);
...
void ms1201_01_state::ms120101(machine_config &config)
{
	...
	K1801VM1(config, m_maincpu, 4'000'000); // clocked by RC circuit
	...
	K1801VM1_TIMER(config, m_timer, 4'000'000);
...
void ms1201_02_state::ms120102(machine_config &config)
{
	...
	K1801VM2(config, m_maincpu, 8'000'000 / 2); // clocked by VCO; /2 -- internal divider in the CPU
...
COMP( 1983, ms120101, 0,      0,     ms120101, ms1201, ms1201_01_state, empty_init, "USSR",  "MS 1201.01",  MACHINE_NOT_WORKING | MACHINE_NO_SOUND_HW)
COMP( 1985, ms120102, 0,      0,     ms120102, ms1201, ms1201_02_state, empty_init, "USSR",  "MS 1201.02",  MACHINE_NOT_WORKING | MACHINE_NO_SOUND_HW)
// ie15, rom 013 (basic), no storage FIXME: years are inaccurate
COMP( 1984, dvk1,   ms120101, 0,     dvk1,     ms1201, ms1201_01_state, empty_init, "USSR",  "DVK-1",       MACHINE_NOT_WORKING | MACHINE_NO_SOUND_HW)
// ie15, kgd, rom 093? (focal), no storage, lan
//MP( 1984, dvk1msh, ms120101, 0,    dvk1,     ms1201, ms1201_01_state, empty_init, "USSR",  "DVK-1MSH",    MACHINE_NOT_WORKING | MACHINE_NO_SOUND_HW)
// ie15, dx floppy
COMP( 1984, dvk2,   ms120101, 0,     dvk2,     ms1201, ms1201_01_state, empty_init, "USSR",  "DVK-2",       MACHINE_NOT_WORKING | MACHINE_NO_SOUND_HW)
// ksm, kgd, mx floppy
COMP( 1985, dvk3,   ms120102, 0,     dvk3,     ms1201, ms1201_02_state, empty_init, "USSR",  "DVK-3",       MACHINE_NOT_WORKING | MACHINE_NO_SOUND_HW)
// kcgd, my floppy, dw disk
COMP( 1985, dvk3m,  ms120102, 0,     dvk3m,    ms1201, ms1201_02_state, empty_init, "USSR",  "DVK-3M",      MACHINE_NOT_WORKING | MACHINE_NO_SOUND_HW)
// ksd, mx floppy
//MP( 1985, dvk3m2, ms120102, 0,     dvk3ms,   ms1201, ms1201_02_state, empty_init, "USSR",  "DVK-3M2",     MACHINE_NOT_WORKING | MACHINE_NO_SOUND_HW)

...
UKNC (Educational Computer by Scientific Centre) a/k/a Elektronika MS-0511.

RAM = 192K (CPU 1 = 128K, CPU 2 = 64K), ROM = 32K.  Graphics 640x288 pixels.

To do:

- sound
- printer (IFSP protocol)
- cassette I/O
- expansion carts and cards
- hardware breakpoint device
- softlist
...
public:
	...
	void uknc(machine_config &config) ATTR_COLD;
...
private:
	...
	required_device<k1515xm031_kbd_device> m_keyboard;
	required_device<k1515xm031_tmr_device> m_timer;
...
void uknc_state::uknc_sub_mem(address_map &map)
{
	...
	map(0177700, 0177707).rw(m_keyboard, FUNC(k1515xm031_kbd_device::read), FUNC(k1515xm031_kbd_device::write));
	map(0177710, 0177715).rw(m_timer, FUNC(k1515xm031_tmr_device::read), FUNC(k1515xm031_tmr_device::write));
...
{
	m_odt_map = 1;
	m_view_main.select(1);
	bcsr_w(0, 01401, 0177777);
	scsr_w(0, 0, 0177777);
}
...
void uknc_state::sprite_w(offs_t offset, uint16_t data, uint16_t mem_mask)
{
	...
	case 3: // 177024
		...
		{
			uint8_t vdata;
			const uint8_t fg = BIT(m_video.foreground, 0);

			vdata = (BIT(data, 0) ? fg : BIT(m_video.background, 0));
			vdata |= (BIT(data, 1) ? fg : BIT(m_video.background, 4)) << 1;
			vdata |= (BIT(data, 2) ? fg : BIT(m_video.background, 8)) << 2;
			vdata |= (BIT(data, 3) ? fg : BIT(m_video.background, 12)) << 3;
			vdata |= (BIT(data, 4) ? fg : BIT(m_video.background, 16)) << 4;
			vdata |= (BIT(data, 5) ? fg : BIT(m_video.background, 20)) << 5;
			vdata |= (BIT(data, 6) ? fg : BIT(m_video.background, 24)) << 6;
			vdata |= (BIT(data, 7) ? fg : BIT(m_video.background, 28)) << 7;
			m_subram[m_video.vram_addr] = vdata;
		}
		if (!BIT(m_video.plane_mask, 1))
		{
			uint8_t vdata;
			const uint8_t fg = BIT(m_video.foreground, 1);

			vdata = (BIT(data, 0) ? fg : BIT(m_video.background, 1));
			vdata |= (BIT(data, 1) ? fg : BIT(m_video.background, 5)) << 1;
			vdata |= (BIT(data, 2) ? fg : BIT(m_video.background, 9)) << 2;
			vdata |= (BIT(data, 3) ? fg : BIT(m_video.background, 13)) << 3;
			vdata |= (BIT(data, 4) ? fg : BIT(m_video.background, 17)) << 4;
			vdata |= (BIT(data, 5) ? fg : BIT(m_video.background, 21)) << 5;
			vdata |= (BIT(data, 6) ? fg : BIT(m_video.background, 25)) << 6;
			vdata |= (BIT(data, 7) ? fg : BIT(m_video.background, 29)) << 7;
		...
		{
			uint8_t vdata;
			const uint8_t fg = BIT(m_video.foreground, 2);

			vdata = (BIT(data, 0) ? fg : BIT(m_video.background, 2));
			vdata |= (BIT(data, 1) ? fg : BIT(m_video.background, 6)) << 1;
			vdata |= (BIT(data, 2) ? fg : BIT(m_video.background, 10)) << 2;
			vdata |= (BIT(data, 3) ? fg : BIT(m_video.background, 14)) << 3;
			vdata |= (BIT(data, 4) ? fg : BIT(m_video.background, 18)) << 4;
			vdata |= (BIT(data, 5) ? fg : BIT(m_video.background, 22)) << 5;
			vdata |= (BIT(data, 6) ? fg : BIT(m_video.background, 26)) << 6;
			vdata |= (BIT(data, 7) ? fg : BIT(m_video.background, 30)) << 7;
...
/*
 *  00  R   EXT - ext data in (reads as 1 if OUT2 is not set)
 *  01  RW  OUT1 - cassette data out
 *  02  RW  OUT2 - ext data interrupt enable.  1: enable
 *  03  RW  OUT3 - NC
 *  04  RW  IO1 - maincpu USER/HALT mode. 0: USER, 1: HALT
 *  05  RW  IO2 - maincpu DCLO pin
 *  06  -
 *  07  RW  sound data out, if 08-12 == 0
 *  12:08   sound frequency matrix
 *  13  RW  reserved
 *  14  -
 *  15  RW  OUT4 - maincpu ACLO pin
 */
uint16_t uknc_state::scsr_r()
{
	const uint16_t ext_data = (m_cassette->input() < 0.0) | !BIT(m_scsr, 2);

	return (m_scsr & ~1) | ext_data;
}
...
void uknc_state::scsr_w(offs_t offset, uint16_t data, uint16_t mem_mask)
{
	...
	m_maincpu->set_input_line(t11_device::HLT_LINE, BIT(m_scsr, 4) ? ASSERT_LINE : CLEAR_LINE);
...
{
	const int bank1 = bitswap<2>(data, 4, 0);
	const int bank2 = (data >> 5) & 7;
...
void uknc_state::draw_scanline(uint32_t *p, uknc_scanline *scanline)
{
	...
	const int wide = (m_video.control >> 20) & 3;
	...
	for (int i = 0; i < (80 >> wide); i++)
	{
		...
		{
			const int pen_selected = BIT(bit_plane_0, pixel_x) |
...
{
	m_subcpu->set_input_line(t11_device::CP2_LINE, state & !BIT(m_bcsr, 8));
	m_maincpu->set_input_line(t11_device::CP2_LINE, state & !BIT(m_bcsr, 9));
	m_video.yrgb = m_yrgb->read();
}

TIMER_DEVICE_CALLBACK_MEMBER(uknc_state::scanline_callback)
{
	const int y = m_screen->vpos();
...
{
	K1801VM2(config, m_maincpu, 16_MHz_XTAL / 4); // external clock is /2 + internal divider /2
	...
	K1801VM2(config, m_subcpu, 25_MHz_XTAL / 4); // SEMZ hardware revision (Kvant used 12.5 MHz XTAL)
	...
	m_screen->set_raw(25_MHz_XTAL / 2, UKNC_TOTAL_HORZ, UKNC_HORZ_START, UKNC_HORZ_START+UKNC_DISP_HORZ,
	...
	K1801VP065(config, m_s2, 4.608_MHz_XTAL);
	m_s2->set_rxc(9600);
	m_s2->set_txc(9600);
	m_s2->set_rxvec(0370);
	m_s2->set_txvec(0374);

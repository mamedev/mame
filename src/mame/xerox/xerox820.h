...
public:
	...
	void xerox820ii_common(machine_config &config) ATTR_COLD; // shared 820-II hardware (no disk drives)
	void xerox820ii_sasi(machine_config &config, bool rgd5 = false) ATTR_COLD; // add the SA1403D SASI host adapter (rgd5 = the 5.25" drive complement)
	...
	void xerox1685s(machine_config &config) ATTR_COLD;
	void xerox820iilp(machine_config &config) ATTR_COLD;   // 16/8 low profile, 5.25" rigid disk unit (rgd5 box)
...
protected:
	...
	void add_8086(machine_config &config) ATTR_COLD; // the 16/8 add-in 8086 board

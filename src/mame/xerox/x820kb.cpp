...
TIMER_CALLBACK_MEMBER(xerox_820_keyboard_device::announce)
{
	...
	// which calls $F0BF to enable keyboard input); every other value falls
	// through to the FIFO enqueue at $F167. So the greeting MUST be 0x9E --
	// 0x80 (or anything else) gets enqueued as a phantom character, putting
	// the input stream permanently one byte behind (the bug Dave saw).
	// Emit the keyboard's power-on "available" greeting (0x9E, the code the
	// monitor's ISR matches at $F14D to open its input gate). Drive it onto the
	// bus, pulse KBSTB low (strobe low -> PIO samples 0x9E) then high (rising ->
	// IRQ), matching the order a real key follows so the ISR latches 0x9E.
	m_bus = 0x9e;

...
void nes_vt369_soc_device::nes_vt369_map(address_map &map)
{
	...
	map(0x4000, 0x4013).w(m_apu, FUNC(nes_apu_vt_device::write));

	map(0x4014, 0x4014).w(FUNC(nes_vt369_soc_device::vt_dma_w));
	map(0x4015, 0x4015).r(m_apu, FUNC(nes_apu_vt_device::status_r)).lw8(NAME([this](u8 data) { m_apu->write(0x15, data); })); // PSG status / first control register
	map(0x4016, 0x4016).rw(FUNC(nes_vt369_soc_device::in0_r), FUNC(nes_vt369_soc_device::in0_w));
	map(0x4017, 0x4017).r(FUNC(nes_vt369_soc_device::in1_r)).lw8(NAME([this](u8 data) { m_apu->write(0x17, data); }));

...
private:
	...
	static inline _PixelType dest_assemble_rgb(uint32_t r, uint32_t g, uint32_t b) { return (r << _DstShiftR) | (g << _DstShiftG) | (b << _DstShiftB); }
	static inline _PixelType dest_rgb_to_pixel(uint32_t r, uint32_t g, uint32_t b) { return dest_assemble_rgb(r >> _SrcShiftR, g >> _SrcShiftG, b >> _SrcShiftB); }

	// source 32-bit pixels are in MAME standardized format
	static inline uint32_t source32_r(uint32_t pixel) { return (pixel >> (16 + _SrcShiftR)) & (0xff >> _SrcShiftR); }
	static inline uint32_t source32_g(uint32_t pixel) { return (pixel >> ( 8 + _SrcShiftG)) & (0xff >> _SrcShiftG); }
	static inline uint32_t source32_b(uint32_t pixel) { return (pixel >> ( 0 + _SrcShiftB)) & (0xff >> _SrcShiftB); }

	// destination pixel masks are based on the template parameters as well
	static inline uint32_t dest_r(_PixelType pixel) { return (pixel >> _DstShiftR) & (0xff >> _SrcShiftR); }
	static inline uint32_t dest_g(_PixelType pixel) { return (pixel >> _DstShiftG) & (0xff >> _SrcShiftG); }
	static inline uint32_t dest_b(_PixelType pixel) { return (pixel >> _DstShiftB) & (0xff >> _SrcShiftB); }

	// generic conversion with special optimization for destinations in the standard format
	static inline _PixelType source32_to_dest(uint32_t pixel)
	{
	...
	static inline uint32_t clamp16_shift8(uint32_t x)
	{
	...
	static inline uint32_t get_texel_palette16(const render_texinfo &texture, int32_t curu, int32_t curv)
	{
		...
		if (_BilinearFilter)
		{
			...
			int32_t v0 = curv >> 16;
			int32_t v1 = texture.rowpixels;
			if (v0 < 0) v0 = v1 = 0;
			else if (v0 + 1 >= texture.height) v0 = texture.height - 1, v1 = 0;

			const uint16_t *texbase = reinterpret_cast<const uint16_t *>(texture.base);
			texbase += v0 * texture.rowpixels + u0;

			uint32_t pix00 = palbase[texbase[0]];
			uint32_t pix01 = palbase[texbase[u1]];
			uint32_t pix10 = palbase[texbase[v1]];
			uint32_t pix11 = palbase[texbase[u1 + v1]];
			return rgbaint_t::bilinear_filter(pix00, pix01, pix10, pix11, curu >> 8, curv >> 8);
		}
		else
		{
			const uint16_t *texbase = reinterpret_cast<const uint16_t *>(texture.base) + (curv >> 16) * texture.rowpixels + (curu >> 16);
	...
	static inline uint32_t get_texel_palette16a(const render_texinfo &texture, int32_t curu, int32_t curv)
	{
		const rgb_t *palbase = texture.palette;
		if (_BilinearFilter)
		{
	...
	static inline uint32_t get_texel_yuy16(const render_texinfo &texture, int32_t curu, int32_t curv)
	{
		if (_BilinearFilter)
		{
			int32_t u0 = curu >> 16;
			...
			uint32_t pix00, pix01, pix10, pix11;
			if ((curu & 0x10000) == 0)
			{
				uint32_t cbcr = ((texbase[0] & 0xff) << 8) | ((texbase[1] & 0xff) << 16);
		...
		{
			const uint16_t *texbase = reinterpret_cast<const uint16_t *>(texture.base) + (curv >> 16) * texture.rowpixels + (curu >> 17) * 2;
	...
	static inline uint32_t get_texel_rgb32(const render_texinfo &texture, int32_t curu, int32_t curv)
	{
		if (_BilinearFilter)
		{
			int32_t u0 = curu >> 16;
			...
			const uint32_t *texbase = reinterpret_cast<const uint32_t *>(texture.base);
			texbase += v0 * texture.rowpixels + u0;
		...
		{
			const uint32_t *texbase = reinterpret_cast<const uint32_t *>(texture.base) + (curv >> 16) * texture.rowpixels + (curu >> 16);
			return texbase[0];
		}
	}
	...
	static inline uint32_t get_texel_argb32(const render_texinfo &texture, int32_t curu, int32_t curv)
	{
		if (_BilinearFilter)
		{
			int32_t u0 = curu >> 16;
	...
	static inline void draw_aa_pixel(_PixelType *dstdata, uint32_t pitch, int x, int y, uint32_t col)
	{
		_PixelType *dest = dstdata + y * pitch + x;
		uint32_t dpix = _NoDestRead ? 0 : *dest;
	...
	static void draw_line(const render_primitive &prim, _PixelType *dstdata, int32_t width, int32_t height, uint32_t pitch)
	{
		// internal tables
		...
		uint32_t col = rgb_t(int(255.0f * prim.color.r * prim.color.a), int(255.0f * prim.color.g * prim.color.a), int(255.0f * prim.color.b * prim.color.a));
	...
	static void draw_rect(const render_primitive &prim, _PixelType *dstdata, int32_t width, int32_t height, uint32_t pitch)
	{
	...
	static void draw_quad_palette16_none(const render_primitive &prim, _PixelType *dstdata, uint32_t pitch, quad_setup_data &setup)
	{
	...
	static void draw_quad_palette16_add(const render_primitive &prim, _PixelType *dstdata, uint32_t pitch, quad_setup_data &setup)
	{
		int32_t dudx = setup.dudx;
		int32_t dvdx = setup.dvdx;
		int32_t endx = setup.endx;
	...
	static void draw_quad_palettea16_alpha(const render_primitive &prim, _PixelType *dstdata, uint32_t pitch, quad_setup_data &setup)
	{
		int32_t dudx = setup.dudx;
		int32_t dvdx = setup.dvdx;
		int32_t endx = setup.endx;
	...
	static void draw_quad_yuy16_none(const render_primitive &prim, _PixelType *dstdata, uint32_t pitch, quad_setup_data &setup)
	{
		int32_t dudx = setup.dudx;
		int32_t dvdx = setup.dvdx;
		int32_t endx = setup.endx;
	...
	static void draw_quad_yuy16_add(const render_primitive &prim, _PixelType *dstdata, uint32_t pitch, quad_setup_data &setup)
	{
		int32_t dudx = setup.dudx;
		int32_t dvdx = setup.dvdx;
		int32_t endx = setup.endx;
	...
	static void draw_quad_rgb32(const render_primitive &prim, _PixelType *dstdata, uint32_t pitch, quad_setup_data &setup)
	{
		...
		if (prim.color.r >= 1.0f && prim.color.g >= 1.0f && prim.color.b >= 1.0f && is_opaque(prim.color.a))
		{
			...
			for (int32_t y = setup.starty; y < setup.endy; y++)
			{
				...
				else
				{
					...
					{
						uint32_t pix = get_texel_rgb32(prim.texture, curu, curv);
						uint32_t r = palbase[(pix >> 16) & 0xff] >> _SrcShiftR;
						uint32_t g = palbase[(pix >> 8) & 0xff] >> _SrcShiftG;
						uint32_t b = palbase[(pix >> 0) & 0xff] >> _SrcShiftB;
		...
		else if (is_opaque(prim.color.a))
		{
			...
			for (int32_t y = setup.starty; y < setup.endy; y++)
			{
				...
				else
				{
					...
					{
						uint32_t pix = get_texel_rgb32(prim.texture, curu, curv);
						uint32_t r = (palbase[(pix >> 16) & 0xff] * sr) >> (8 + _SrcShiftR);
						uint32_t g = (palbase[(pix >> 8) & 0xff] * sg) >> (8 + _SrcShiftG);
						uint32_t b = (palbase[(pix >> 0) & 0xff] * sb) >> (8 + _SrcShiftB);
			...
			for (int32_t y = setup.starty; y < setup.endy; y++)
			{
				...
				else
				{
					...
					for (int32_t x = setup.startx; x < endx; x++)
					{
						uint32_t pix = get_texel_rgb32(prim.texture, curu, curv);
						uint32_t dpix = _NoDestRead ? 0 : *dest;
						uint32_t r = ((palbase[(pix >> 16) & 0xff] >> _SrcShiftR) * sr + dest_r(dpix) * invsa) >> 8;
						uint32_t g = ((palbase[(pix >> 8) & 0xff] >> _SrcShiftG) * sg + dest_g(dpix) * invsa) >> 8;
						uint32_t b = ((palbase[(pix >> 0) & 0xff] >> _SrcShiftB) * sb + dest_b(dpix) * invsa) >> 8;
	...
	static void draw_quad_rgb32_add(const render_primitive &prim, _PixelType *dstdata, uint32_t pitch, quad_setup_data &setup)
	{
		const rgb_t *palbase = prim.texture.palette;
		int32_t dudx = setup.dudx;
		int32_t dvdx = setup.dvdx;
			...
			for (int32_t y = setup.starty; y < setup.endy; y++)
			{
				...
				else
				{
					...
					for (int32_t x = setup.startx; x < endx; x++)
					{
						uint32_t pix = get_texel_argb32(prim.texture, curu, curv);
						uint32_t dpix = _NoDestRead ? 0 : *dest;
						uint32_t r = (palbase[(pix >> 16) & 0xff] >> _SrcShiftR) + dest_r(dpix);
						uint32_t g = (palbase[(pix >> 8) & 0xff] >> _SrcShiftG) + dest_g(dpix);
						uint32_t b = (palbase[(pix >> 0) & 0xff] >> _SrcShiftB) + dest_b(dpix);
						r = (r | -(r >> (8 - _SrcShiftR))) & (0xff >> _SrcShiftR);
						g = (g | -(g >> (8 - _SrcShiftG))) & (0xff >> _SrcShiftG);
						b = (b | -(b >> (8 - _SrcShiftB))) & (0xff >> _SrcShiftB);
						*dest++ = dest_assemble_rgb(r, g, b);
			...
			for (int32_t y = setup.starty; y < setup.endy; y++)
			{
				...
				else
				{
					...
					for (int32_t x = setup.startx; x < endx; x++)
					{
						uint32_t pix = get_texel_argb32(prim.texture, curu, curv);
						uint32_t dpix = _NoDestRead ? 0 : *dest;
						uint32_t r = ((palbase[(pix >> 16) & 0xff] * sr * sa) >> (16 + _SrcShiftR)) + dest_r(dpix);
						uint32_t g = ((palbase[(pix >> 8) & 0xff] * sr * sa) >> (16 + _SrcShiftR)) + dest_g(dpix);
						uint32_t b = ((palbase[(pix >> 0) & 0xff] * sr * sa) >> (16 + _SrcShiftR)) + dest_b(dpix);
						r = (r | -(r >> (8 - _SrcShiftR))) & (0xff >> _SrcShiftR);
						g = (g | -(g >> (8 - _SrcShiftG))) & (0xff >> _SrcShiftG);
						b = (b | -(b >> (8 - _SrcShiftB))) & (0xff >> _SrcShiftB);
						*dest++ = dest_assemble_rgb(r, g, b);
	...
	static void draw_quad_argb32_alpha(const render_primitive &prim, _PixelType *dstdata, uint32_t pitch, quad_setup_data &setup)
	{
		const rgb_t *palbase = prim.texture.palette;
		int32_t dudx = setup.dudx;
		int32_t dvdx = setup.dvdx;
				...
				else
				{
					...
					for (int32_t x = setup.startx; x < endx; x++)
					{
						...
						if (ta != 0)
						{
							uint32_t dpix = _NoDestRead ? 0 : *dest;
							uint32_t invta = 0x100 - ta;
							uint32_t r = ((palbase[(pix >> 16) & 0xff] >> _SrcShiftR) * ta + dest_r(dpix) * invta) >> 8;
							uint32_t g = ((palbase[(pix >> 8) & 0xff] >> _SrcShiftG) * ta + dest_g(dpix) * invta) >> 8;
							uint32_t b = ((palbase[(pix >> 0) & 0xff] >> _SrcShiftB) * ta + dest_b(dpix) * invta) >> 8;
				...
				else
				{
					...
					for (int32_t x = setup.startx; x < endx; x++)
					{
						...
						if (ta != 0)
						{
							uint32_t dpix = _NoDestRead ? 0 : *dest;
							uint32_t invsta = (0x10000 - ta) << 8;
							uint32_t r = ((palbase[(pix >> 16) & 0xff] >> _SrcShiftR) * sr * ta + dest_r(dpix) * invsta) >> 24;
							uint32_t g = ((palbase[(pix >> 8) & 0xff] >> _SrcShiftG) * sg * ta + dest_g(dpix) * invsta) >> 24;
							uint32_t b = ((palbase[(pix >> 0) & 0xff] >> _SrcShiftB) * sb * ta + dest_b(dpix) * invsta) >> 24;
	...
	static void draw_quad_argb32_multiply(const render_primitive &prim, _PixelType *dstdata, uint32_t pitch, quad_setup_data &setup)
	{
		const rgb_t *palbase = prim.texture.palette;
		int32_t dudx = setup.dudx;
		int32_t dvdx = setup.dvdx;
			...
			for (int32_t y = setup.starty; y < setup.endy; y++)
			{
				...
				if (palbase == nullptr)
				{
					...
					for (int32_t x = setup.startx; x < endx; x++)
					{
						uint32_t pix = get_texel_argb32(prim.texture, curu, curv);
						uint32_t dpix = _NoDestRead ? 0 : *dest;
						uint32_t r = (source32_r(pix) * dest_r(dpix)) >> (8 - _SrcShiftR);
						uint32_t g = (source32_g(pix) * dest_g(dpix)) >> (8 - _SrcShiftG);
						uint32_t b = (source32_b(pix) * dest_b(dpix)) >> (8 - _SrcShiftB);
			...
			for (int32_t y = setup.starty; y < setup.endy; y++)
			{
				...
				if (palbase == nullptr)
				{
					...
					for (int32_t x = setup.startx; x < endx; x++)
					{
						uint32_t pix = get_texel_argb32(prim.texture, curu, curv);
						uint32_t dpix = _NoDestRead ? 0 : *dest;
						uint32_t r = (source32_r(pix) * sr * dest_r(dpix)) >> (16 - _SrcShiftR);
						uint32_t g = (source32_g(pix) * sg * dest_g(dpix)) >> (16 - _SrcShiftG);
						uint32_t b = (source32_b(pix) * sb * dest_b(dpix)) >> (16 - _SrcShiftB);
	...
	static void draw_quad_argb32_add(const render_primitive &prim, _PixelType *dstdata, uint32_t pitch, quad_setup_data &setup)
	{
		const rgb_t *palbase = prim.texture.palette;
		int32_t dudx = setup.dudx;
		int32_t dvdx = setup.dvdx;
			...
			for (int32_t y = setup.starty; y < setup.endy; y++)
			{
				...
				else
				{
					...
					for (int32_t x = setup.startx; x < endx; x++)
					{
						...
						{
							uint32_t dpix = _NoDestRead ? 0 : *dest;
							uint32_t r = ((palbase[(pix >> 16) & 0xff] * ta) >> (8 + _SrcShiftR)) + dest_r(dpix);
							uint32_t g = ((palbase[(pix >> 8) & 0xff] * ta) >> (8 + _SrcShiftG)) + dest_g(dpix);
							uint32_t b = ((palbase[(pix >> 0) & 0xff] * ta) >> (8 + _SrcShiftB)) + dest_b(dpix);
							r = (r | -(r >> (8 - _SrcShiftR))) & (0xff >> _SrcShiftR);
							g = (g | -(g >> (8 - _SrcShiftG))) & (0xff >> _SrcShiftG);
							b = (b | -(b >> (8 - _SrcShiftB))) & (0xff >> _SrcShiftB);
							*dest = dest_assemble_rgb(r, g, b);
			...
			for (int32_t y = setup.starty; y < setup.endy; y++)
			{
				...
				else
				{
					...
					for (int32_t x = setup.startx; x < endx; x++)
					{
						...
						{
							uint32_t dpix = _NoDestRead ? 0 : *dest;
							uint32_t r = ((palbase[(pix >> 16) & 0xff] * sr * ta) >> (24 + _SrcShiftR)) + dest_r(dpix);
							uint32_t g = ((palbase[(pix >> 8) & 0xff] * sr * ta) >> (24 + _SrcShiftR)) + dest_g(dpix);
							uint32_t b = ((palbase[(pix >> 0) & 0xff] * sr * ta) >> (24 + _SrcShiftR)) + dest_b(dpix);
							r = (r | -(r >> (8 - _SrcShiftR))) & (0xff >> _SrcShiftR);
							g = (g | -(g >> (8 - _SrcShiftG))) & (0xff >> _SrcShiftG);
							b = (b | -(b >> (8 - _SrcShiftB))) & (0xff >> _SrcShiftB);
							*dest = dest_assemble_rgb(r, g, b);
	...
	static void setup_and_draw_textured_quad(const render_primitive &prim, _PixelType *dstdata, int32_t width, int32_t height, uint32_t pitch)
	{
...
public:
	static void draw_primitives(const render_primitive_list &primlist, void *dstdata, uint32_t width, uint32_t height, uint32_t pitch)
	{

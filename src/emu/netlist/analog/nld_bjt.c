...
NETLIB_UPDATE(QBJT_switch)
{
    if (!m_RB.m_P.net().isRailNet())
        m_RB.m_P.net_analog().schedule_solve();   // Basis
    else if (!m_RB.m_N.net().isRailNet())
        m_RB.m_N.net_analog().schedule_solve();   // Emitter
    else if (!m_RC.m_P.net().isRailNet())
        m_RC.m_P.net_analog().schedule_solve();   // Collector
}
...
NETLIB_UPDATE(QBJT_EB)
{
	if (!m_D_EB.m_P.net().isRailNet())
		m_D_EB.m_P.net_analog().schedule_solve();   // Basis
	else if (!m_D_EB.m_N.net().isRailNet())
		m_D_EB.m_N.net_analog().schedule_solve();   // Emitter
	else
		m_D_CB.m_N.net_analog().schedule_solve();   // Collector
}

...
template<> devcb_line_dispatch_device<2>::devcb_line_dispatch_device(const machine_config &mconfig, std::string tag, device_t *owner, UINT32 clock) :
	device_t(mconfig, DEVCB_LINE_DISPATCH_2, "Line dispatcher (2 slots)", tag, owner, clock, "devcb_line_dispatch", __FILE__)
{
	init_fwd();
}

template<> devcb_line_dispatch_device<3>::devcb_line_dispatch_device(const machine_config &mconfig, std::string tag, device_t *owner, UINT32 clock) :
	device_t(mconfig, DEVCB_LINE_DISPATCH_3, "Line dispatcher (3 slots)", tag, owner, clock, "devcb_line_dispatch", __FILE__)
{
	init_fwd();
}

template<> devcb_line_dispatch_device<4>::devcb_line_dispatch_device(const machine_config &mconfig, std::string tag, device_t *owner, UINT32 clock) :
	device_t(mconfig, DEVCB_LINE_DISPATCH_4, "Line dispatcher (4 slots)", tag, owner, clock, "devcb_line_dispatch", __FILE__)
{
	init_fwd();
}

template<> devcb_line_dispatch_device<5>::devcb_line_dispatch_device(const machine_config &mconfig, std::string tag, device_t *owner, UINT32 clock) :
	device_t(mconfig, DEVCB_LINE_DISPATCH_5, "Line dispatcher (5 slots)", tag, owner, clock, "devcb_line_dispatch", __FILE__)
{
	init_fwd();
}

template<> devcb_line_dispatch_device<6>::devcb_line_dispatch_device(const machine_config &mconfig, std::string tag, device_t *owner, UINT32 clock) :

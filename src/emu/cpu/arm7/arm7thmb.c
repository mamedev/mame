...
const void tg00_0(arm_state *arm, UINT32 pc, UINT32 insn) /* Shift left */
{
	UINT32 rs, rd, rrs;
	INT32 offs;
...
const void tg00_1(arm_state *arm, UINT32 pc, UINT32 insn) /* Shift right */
{
	UINT32 rs, rd, rrs;
	INT32 offs;
...
const void tg01_10(arm_state *arm, UINT32 pc, UINT32 insn)  /* ADD Rd, Rs, Rn */
{
	UINT32 rn, rs, rd;


	rn = GET_REGISTER(arm, (insn & THUMB_ADDSUB_RNIMM) >> THUMB_ADDSUB_RNIMM_SHIFT);
	rs = GET_REGISTER(arm, (insn & THUMB_ADDSUB_RS) >> THUMB_ADDSUB_RS_SHIFT);
	rd = (insn & THUMB_ADDSUB_RD) >> THUMB_ADDSUB_RD_SHIFT;
...
const void tg01_11(arm_state *arm, UINT32 pc, UINT32 insn) /* SUB Rd, Rs, Rn */
{
	UINT32 rn, rs, rd;
...
const void tg01_12(arm_state *arm, UINT32 pc, UINT32 insn) /* ADD Rd, Rs, #imm */
{
	UINT32 rs, rd, imm;
...
const void tg01_13(arm_state *arm, UINT32 pc, UINT32 insn) /* SUB Rd, Rs, #imm */
{
	UINT32 rs, rd, imm;
...
const void tg02_1(arm_state *arm, UINT32 pc, UINT32 insn)
{
	...
	rn = GET_REGISTER(arm, (insn & THUMB_INSN_IMM_RD) >> THUMB_INSN_IMM_RD_SHIFT);
	op2 = insn & THUMB_INSN_IMM;
	rd = rn - op2;
...
const void tg03_0(arm_state *arm, UINT32 pc, UINT32 insn) /* ADD Rd, #Offset8 */
{
	UINT32 rn, rd, op2;
	...
	SET_REGISTER(arm, (insn & THUMB_INSN_IMM_RD) >> THUMB_INSN_IMM_RD_SHIFT, rd);
	HandleThumbALUAddFlags(rd, rn, op2);
}

const void tg03_1(arm_state *arm, UINT32 pc, UINT32 insn) /* SUB Rd, #Offset8 */
{
	UINT32 rn, rd, op2;
...
const void tg04_00_00(arm_state *arm, UINT32 pc, UINT32 insn) /* AND Rd, Rs */
{
	UINT32 rs, rd;
...
const void tg04_00_01(arm_state *arm, UINT32 pc, UINT32 insn) /* EOR Rd, Rs */
{
	UINT32 rs, rd;
...
const void tg04_00_02(arm_state *arm, UINT32 pc, UINT32 insn) /* LSL Rd, Rs */
{
	UINT32 rs, rd, rrd;
	INT32 offs;
	...
	rs = (insn & THUMB_ADDSUB_RS) >> THUMB_ADDSUB_RS_SHIFT;
	rd = (insn & THUMB_ADDSUB_RD) >> THUMB_ADDSUB_RD_SHIFT;
	rrd = GET_REGISTER(arm, rd);
	offs = GET_REGISTER(arm, rs) & 0x000000ff;
	if (offs > 0)
	{
		if (offs < 32)
		{
...
const void tg04_00_03(arm_state *arm, UINT32 pc, UINT32 insn) /* LSR Rd, Rs */
{
	UINT32 rs, rd, rrd;
	INT32 offs;
...
const void tg04_00_04(arm_state *arm, UINT32 pc, UINT32 insn) /* ASR Rd, Rs */
{
	UINT32 rs, rd, rrs, rrd;

	rs = (insn & THUMB_ADDSUB_RS) >> THUMB_ADDSUB_RS_SHIFT;
	rd = (insn & THUMB_ADDSUB_RD) >> THUMB_ADDSUB_RD_SHIFT;
	rrs = GET_REGISTER(arm, rs)&0xff;
	rrd = GET_REGISTER(arm, rd);
	if (rrs != 0)
	{
		if (rrs >= 32)
...
const void tg04_00_05(arm_state *arm, UINT32 pc, UINT32 insn) /* ADC Rd, Rs */
{
	UINT32 rn, rs, rd, op2;
	...
	rn=GET_REGISTER(arm, rd) + GET_REGISTER(arm, rs) + op2;
	HandleThumbALUAddFlags(rn, GET_REGISTER(arm, rd), (GET_REGISTER(arm, rs))); // ?
	SET_REGISTER(arm, rd, rn);

}

const void tg04_00_06(arm_state *arm, UINT32 pc, UINT32 insn)  /* SBC Rd, Rs */
{
	UINT32 rn, rs, rd, op2;
...
const void tg04_00_07(arm_state *arm, UINT32 pc, UINT32 insn) /* ROR Rd, Rs */
{
	...
	rd = (insn & THUMB_ADDSUB_RD) >> THUMB_ADDSUB_RD_SHIFT;
	rrd = GET_REGISTER(arm, rd);
	imm = GET_REGISTER(arm, rs) & 0x0000001f;
...
const void tg04_00_08(arm_state *arm, UINT32 pc, UINT32 insn) /* TST Rd, Rs */
{
	UINT32 rs, rd;
...
const void tg04_00_09(arm_state *arm, UINT32 pc, UINT32 insn) /* NEG Rd, Rs */
{
	UINT32 rn, rs, rd, rrs;
...
const void tg04_00_0a(arm_state *arm, UINT32 pc, UINT32 insn) /* CMP Rd, Rs */
{
	UINT32 rn, rs, rd;

	rs = (insn & THUMB_ADDSUB_RS) >> THUMB_ADDSUB_RS_SHIFT;
	rd = (insn & THUMB_ADDSUB_RD) >> THUMB_ADDSUB_RD_SHIFT;
	rn = GET_REGISTER(arm, rd) - GET_REGISTER(arm, rs);
	HandleThumbALUSubFlags(rn, GET_REGISTER(arm, rd), GET_REGISTER(arm, rs));

}


const void tg04_00_0b(arm_state *arm, UINT32 pc, UINT32 insn) /* CMN Rd, Rs - check flags, add dasm */
{
	UINT32 rn, rs, rd;
...
const void tg04_00_0c(arm_state *arm, UINT32 pc, UINT32 insn) /* ORR Rd, Rs */
{
	UINT32 rs, rd;
...
const void tg04_00_0d(arm_state *arm, UINT32 pc, UINT32 insn) /* MUL Rd, Rs */
{
	UINT32 rn, rs, rd;
...
const void tg04_00_0e(arm_state *arm, UINT32 pc, UINT32 insn) /* BIC Rd, Rs */
{
	UINT32 rs, rd;
...
const void tg04_00_0f(arm_state *arm, UINT32 pc, UINT32 insn) /* MVN Rd, Rs */
{
	UINT32 rs, rd, op2;

	rs = (insn & THUMB_ADDSUB_RS) >> THUMB_ADDSUB_RS_SHIFT;
	rd = (insn & THUMB_ADDSUB_RD) >> THUMB_ADDSUB_RD_SHIFT;
	op2 = GET_REGISTER(arm, rs);
	SET_REGISTER(arm, rd, ~op2);
	SET_CPSR(GET_CPSR & ~(Z_MASK | N_MASK));
	SET_CPSR(GET_CPSR | HandleALUNZFlags(GET_REGISTER(arm, rd)));
	R15 += 2;
...
const void tg04_01_01(arm_state *arm, UINT32 pc, UINT32 insn) /* ADD Rd, HRs */
{
	UINT32 rs, rd;
	rs = (insn & THUMB_HIREG_RS) >> THUMB_HIREG_RS_SHIFT;
	rd = insn & THUMB_HIREG_RD;
...
const void tg04_01_02(arm_state *arm, UINT32 pc, UINT32 insn) /* ADD HRd, Rs */
{
	UINT32 rs, rd;
	rs = (insn & THUMB_HIREG_RS) >> THUMB_HIREG_RS_SHIFT;
	rd = insn & THUMB_HIREG_RD;
...
const void tg04_01_03(arm_state *arm, UINT32 pc, UINT32 insn) /* Add HRd, HRs */
{
	UINT32 rs, rd;
	rs = (insn & THUMB_HIREG_RS) >> THUMB_HIREG_RS_SHIFT;
	rd = insn & THUMB_HIREG_RD;
...
const void tg04_01_10(arm_state *arm, UINT32 pc, UINT32 insn)  /* CMP Rd, Rs */
{
	UINT32 rn, rs, rd;

	rs = GET_REGISTER(arm, ((insn & THUMB_HIREG_RS) >> THUMB_HIREG_RS_SHIFT));
	rd = GET_REGISTER(arm, insn & THUMB_HIREG_RD);
	rn = rd - rs;
	HandleThumbALUSubFlags(rn, rd, rs);

}

const void tg04_01_11(arm_state *arm, UINT32 pc, UINT32 insn) /* CMP Rd, Hs */
{
	UINT32 rn, rs, rd;

	rs = GET_REGISTER(arm, ((insn & THUMB_HIREG_RS) >> THUMB_HIREG_RS_SHIFT) + 8);
	rd = GET_REGISTER(arm, insn & THUMB_HIREG_RD);
	rn = rd - rs;
	HandleThumbALUSubFlags(rn, rd, rs);
...
const void tg04_01_12(arm_state *arm, UINT32 pc, UINT32 insn) /* CMP Hd, Rs */
{
	UINT32 rn, rs, rd;

	rs = GET_REGISTER(arm, ((insn & THUMB_HIREG_RS) >> THUMB_HIREG_RS_SHIFT));
	rd = GET_REGISTER(arm, (insn & THUMB_HIREG_RD) + 8);
	rn = rd - rs;
	HandleThumbALUSubFlags(rn, rd, rs);

}

const void tg04_01_13(arm_state *arm, UINT32 pc, UINT32 insn) /* CMP Hd, Hs */
{
	UINT32 rn, rs, rd;
...
const void tg04_01_20(arm_state *arm, UINT32 pc, UINT32 insn) /* MOV Rd, Rs (undefined) */
{
	UINT32 rs, rd;
...
const void tg04_01_21(arm_state *arm, UINT32 pc, UINT32 insn) /* MOV Rd, Hs */
{
	UINT32 rs, rd;
	...
	if (rs == 7)
	{
		SET_REGISTER(arm, rd, GET_REGISTER(arm, rs + 8) + 4);
	}
	else
	{
...
const void tg04_01_22(arm_state *arm, UINT32 pc, UINT32 insn) /* MOV Hd, Rs */
{
	UINT32 rs, rd;
...
const void tg04_01_23(arm_state *arm, UINT32 pc, UINT32 insn) /* MOV Hd, Hs */
{
	UINT32 rs, rd;
...
const void tg04_01_31(arm_state *arm, UINT32 pc, UINT32 insn)
{
	...
	addr = GET_REGISTER(arm, ((insn & THUMB_HIREG_RS) >> THUMB_HIREG_RS_SHIFT) + 8);
...
const void tg04_0203(arm_state *arm, UINT32 pc, UINT32 insn)
{
	...
	SET_REGISTER(arm, (insn & THUMB_INSN_IMM_RD) >> THUMB_INSN_IMM_RD_SHIFT, readword);
	R15 += 2;
}

/* LDR* STR* group */

const void tg05_0(arm_state *arm, UINT32 pc, UINT32 insn)  /* STR Rd, [Rn, Rm] */
{
	UINT32 addr;
	UINT32 rm, rn, rd;
	...
	addr = GET_REGISTER(arm, rn) + GET_REGISTER(arm, rm);
	WRITE32(addr, GET_REGISTER(arm, rd));
	R15 += 2;

}

const void tg05_1(arm_state *arm, UINT32 pc, UINT32 insn)  /* STRH Rd, [Rn, Rm] */
{
	UINT32 addr;
	UINT32 rm, rn, rd;
...
const void tg05_2(arm_state *arm, UINT32 pc, UINT32 insn)  /* STRB Rd, [Rn, Rm] */
{
	UINT32 addr;
	UINT32 rm, rn, rd;
...
const void tg05_3(arm_state *arm, UINT32 pc, UINT32 insn)  /* LDSB Rd, [Rn, Rm] todo, add dasm */
{
	UINT32 addr;
	UINT32 rm, rn, rd, op2;
...
const void tg05_4(arm_state *arm, UINT32 pc, UINT32 insn)  /* LDR Rd, [Rn, Rm] */
{
	UINT32 addr;
	UINT32 rm, rn, rd, op2;
...
const void tg05_5(arm_state *arm, UINT32 pc, UINT32 insn)  /* LDRH Rd, [Rn, Rm] */
{
	UINT32 addr;
	UINT32 rm, rn, rd, op2;
...
const void tg05_6(arm_state *arm, UINT32 pc, UINT32 insn)  /* LDRB Rd, [Rn, Rm] */
{
	UINT32 addr;
	UINT32 rm, rn, rd, op2;
...
const void tg05_7(arm_state *arm, UINT32 pc, UINT32 insn)  /* LDSH Rd, [Rn, Rm] */
{
	UINT32 addr;
	UINT32 rm, rn, rd, op2;
...
const void tg06_0(arm_state *arm, UINT32 pc, UINT32 insn) /* Store */
{
	UINT32 rn, rd;
	INT32 offs;
...
const void tg06_1(arm_state *arm, UINT32 pc, UINT32 insn) /* Load */
{
	UINT32 rn, rd;
	INT32 offs;
...
const void tg07_0(arm_state *arm, UINT32 pc, UINT32 insn) /* Store */
{
	UINT32 rn, rd;
	INT32 offs;
...
const void tg07_1(arm_state *arm, UINT32 pc, UINT32 insn)  /* Load */
{
	UINT32 rn, rd;
	INT32 offs;
...
const void tg08_0(arm_state *arm, UINT32 pc, UINT32 insn) /* Store */
{
	UINT32 rs, rd, imm;
...
const void tg08_1(arm_state *arm, UINT32 pc, UINT32 insn) /* Load */
{
	UINT32 rs, rd, imm;
...
const void tg09_0(arm_state *arm, UINT32 pc, UINT32 insn) /* Store */
{
	UINT32 rd;
	INT32 offs;
...
const void tg09_1(arm_state *arm, UINT32 pc, UINT32 insn) /* Load */
{
	UINT32 readword;
	UINT32 rd;
	INT32 offs;
	...
	rd = (insn & THUMB_STACKOP_RD) >> THUMB_STACKOP_RD_SHIFT;
	offs = (UINT8)(insn & THUMB_INSN_IMM);
	readword = READ32(GET_REGISTER(arm, 13) + ((UINT32)offs << 2));
...
const void tg0a_0(arm_state *arm, UINT32 pc, UINT32 insn)  /* ADD Rd, PC, #nn */
{
	UINT32 rd;
	INT32 offs;
...
const void tg0a_1(arm_state *arm, UINT32 pc, UINT32 insn) /* ADD Rd, SP, #nn */
{
	UINT32 rd;
	INT32 offs;
...
const void tg0b_0(arm_state *arm, UINT32 pc, UINT32 insn) /* ADD SP, #imm */
{
	UINT32 addr;
	...
	SET_REGISTER(arm, 13, GET_REGISTER(arm, 13) + ((insn & THUMB_INSN_IMM_S) ? -(addr << 2) : (addr << 2)));
	R15 += 2;
...
const void tg0b_4(arm_state *arm, UINT32 pc, UINT32 insn) /* PUSH {Rlist} */
{
	INT32 offs;
...
const void tg0b_5(arm_state *arm, UINT32 pc, UINT32 insn) /* PUSH {Rlist}{LR} */
{
	INT32 offs;
...
const void tg0b_c(arm_state *arm, UINT32 pc, UINT32 insn) /* POP {Rlist} */
{
	INT32 offs;
...
const void tg0b_d(arm_state *arm, UINT32 pc, UINT32 insn) /* POP {Rlist}{PC} */
{
	INT32 offs;
...
const void tg0c_0(arm_state *arm, UINT32 pc, UINT32 insn) /* Store */
{
	UINT32 rd;
	INT32 offs;
	...
	ld_st_address = GET_REGISTER(arm, rd);
...
const void tg0c_1(arm_state *arm, UINT32 pc, UINT32 insn) /* Load */
{
	UINT32 rd;
	INT32 offs;
...
const void tg0d_0(arm_state *arm, UINT32 pc, UINT32 insn) // COND_EQ:
{
	INT32 offs;
...
const void tg0d_1(arm_state *arm, UINT32 pc, UINT32 insn) // COND_NE:
{
	INT32 offs;
...
const void tg0d_2(arm_state *arm, UINT32 pc, UINT32 insn) // COND_CS:
{
	INT32 offs;
...
const void tg0d_3(arm_state *arm, UINT32 pc, UINT32 insn) // COND_CC:
{
	INT32 offs;
...
const void tg0d_4(arm_state *arm, UINT32 pc, UINT32 insn) // COND_MI:
{
	INT32 offs;
...
const void tg0d_5(arm_state *arm, UINT32 pc, UINT32 insn) // COND_PL:
{
	INT32 offs;
...
const void tg0d_6(arm_state *arm, UINT32 pc, UINT32 insn) // COND_VS:
{
	INT32 offs;
...
const void tg0d_7(arm_state *arm, UINT32 pc, UINT32 insn) // COND_VC:
{
	INT32 offs;
...
const void tg0d_8(arm_state *arm, UINT32 pc, UINT32 insn) // COND_HI:
{
	INT32 offs;
...
const void tg0d_9(arm_state *arm, UINT32 pc, UINT32 insn) // COND_LS:
{
	INT32 offs;
...
const void tg0d_a(arm_state *arm, UINT32 pc, UINT32 insn) // COND_GE:
{
	INT32 offs;
...
const void tg0d_b(arm_state *arm, UINT32 pc, UINT32 insn) // COND_LT:
{
	INT32 offs;
...
const void tg0d_c(arm_state *arm, UINT32 pc, UINT32 insn) // COND_GT:
{
	INT32 offs;
...
const void tg0d_d(arm_state *arm, UINT32 pc, UINT32 insn) // COND_LE:
{
	INT32 offs;
...
const void tg0d_e(arm_state *arm, UINT32 pc, UINT32 insn) // COND_AL:
{
//  INT32 offs;
...
const void tg0d_f(arm_state *arm, UINT32 pc, UINT32 insn) // COND_NV:   // SWI (this is sort of a "hole" in the opcode encoding)
{
//  INT32 offs;
...
const void tg0f_1(arm_state *arm, UINT32 pc, UINT32 insn) /* BL */
{
	UINT32 addr;

	addr = GET_REGISTER(arm, 14) & ~1;
	addr += (insn & THUMB_BLOP_OFFS) << 1;

...
debug_disasm_buffer::debug_data_buffer::debug_data_buffer(device_disasm_interface &intf) : m_intf(intf)
{
	m_space = nullptr;
	m_back = nullptr;
	m_opcode = true;
...
void debug_disasm_buffer::debug_data_buffer::fill(offs_t lstart, offs_t size) const
{
	...
	if(m_page_mask) {
		if((lstart ^ lend) & ~m_page_mask) {
			...
			lend = lstart | m_page_mask;
		}
	}

	if(!m_buffer.empty() && (lend > lstart || m_wrapped)) {
		if(m_wrapped) {
	...
	if(m_buffer.empty()) {
		...
		offs_t size = m_pc_delta_to_bytes(lend - lstart);
...
void debug_disasm_buffer::debug_data_buffer::setup_methods()
{
	...
	if(m_space) {
		...
		if(m_intf.disasm_interface_flags() & device_disasm_interface::DASMINTF_NONLINEAR_PC) {	
			switch(shift) {
			case -1:
		...
		assert(!(m_intf.disasm_interface_flags() & device_disasm_interface::DASMINTF_NONLINEAR_PC));
	...
	if(m_intf.disasm_interface_flags() & device_disasm_interface::DASMINTF_NONLINEAR_PC) {
		...
		case -1:
			...
			m_do_r16 = [this](offs_t pc) -> u16 {
				offs_t lpc = m_intf.disasm_pc_real_to_linear(pc);
				fill(lpc, 1);
	...
	if(m_intf.disasm_interface_flags() & device_disasm_interface::DASMINTF_NONLINEAR_PC) {
		...
		m_next_pc_wrap = [this](offs_t pc, offs_t size) {
			...
			return m_intf.disasm_pc_linear_to_real(lpce);
		};
	} else if(m_intf.disasm_interface_flags() & device_disasm_interface::DASMINTF_PAGED) {
		m_next_pc_wrap = [this](offs_t pc, offs_t size) {
...
{
	m_dintf = dynamic_cast<device_disasm_interface *>(&device);
	m_mintf = dynamic_cast<device_memory_interface *>(&device);

	m_flags = m_dintf->disasm_interface_flags();

	if(m_flags & device_disasm_interface::DASMINTF_INTERNAL_DECRYPTION) {
		m_buf_raw.set_source(m_mintf->space(AS_PROGRAM));
		m_buf_opcodes.set_source(m_buf_raw, true);
		if((m_flags & device_disasm_interface::DASMINTF_SPLIT_DECRYPTION) == device_disasm_interface::DASMINTF_SPLIT_DECRYPTION)
	...
	if(m_flags & device_disasm_interface::DASMINTF_NONLINEAR_PC) {
		...
		m_next_pc = [this](offs_t pc, offs_t size) {
			offs_t lpc = m_dintf->disasm_pc_real_to_linear(pc);
			offs_t lpce = lpc + size;
			if((lpc ^ lpce) & ~m_page_mask)
				lpce = (lpc | m_page_mask) + 1;
			lpce &= m_pc_mask;
			return m_dintf->disasm_pc_linear_to_real(lpce);
		};
		m_next_pc_wrap = [this](offs_t pc, offs_t size) {
			offs_t lpc = m_dintf->disasm_pc_real_to_linear(pc);
			offs_t lpce = (lpc & ~m_page_mask) | ((lpc + size) & m_page_mask);
	...
	} else if(m_flags & device_disasm_interface::DASMINTF_PAGED) {
		m_next_pc = [this](offs_t pc, offs_t size) {
			offs_t pce = pc + size;
	...
	if((m_flags & device_disasm_interface::DASMINTF_PAGED2LEVEL) == device_disasm_interface::DASMINTF_PAGED2LEVEL) {
		int bits1 = m_dintf->disasm_page_address_bits();
		int bits2 = m_dintf->disasm_page2_address_bits();
...
void debug_disasm_buffer::disassemble(offs_t pc, std::string &instruction, offs_t &next_pc, offs_t &size, u32 &info) const
{
	...
	instruction = out.str();
	size = result & DASMFLAG_LENGTHMASK;

...
bool debug_view_disasm::generate_with_pc(debug_disasm_buffer &buffer, offs_t pc)
{
	...
	device_disasm_interface &m_intf = dynamic_cast<device_disasm_interface &>(*source.device());
	if(m_intf.disasm_interface_flags() & device_disasm_interface::DASMINTF_NONLINEAR_PC) {
		...
		while(m_intf.disasm_pc_real_to_linear(address) < lpc) {	
			std::string dasm;
			offs_t size;
			offs_t next_address;
			u32 info;
			buffer.disassemble(address, dasm, next_address, size, info);
			m_dasm.emplace_back(address, size, dasm);
			if(m_intf.disasm_pc_real_to_linear(address) > m_intf.disasm_pc_real_to_linear(next_address))
				return false;
			address = next_address;
		}

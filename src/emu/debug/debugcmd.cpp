...
void debugger_commands::execute_wpset(int spacenum, const std::vector<std::string_view> &params)
{
	...
	address_space *tspace;
	if (!mintf->translate(spacenum, type == read_or_write::READ ? device_memory_interface::TR_READ : device_memory_interface::TR_WRITE, address, tspace))
	{
	...
	// set the watchpoint
	int const wpnum = debug->watchpoint_set(*tspace, type, address, length, (condition.is_empty()) ? nullptr : condition.original_string(), action);
...
{
	offs_t offset, endoffset, length;
	device_memory_interface *mintf;
...
{
	offs_t offset, endoffset, length = 0;
	device_memory_interface *mintf;

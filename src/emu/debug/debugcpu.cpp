...
u32 device_debug::compute_opcode_crc32(offs_t pc) const
{
	...
	buffer.data_get(pc, dasmresult & DASMFLAG_LENGTHMASK, true, opbuf);
...
void device_debug::prepare_for_step_overout(offs_t pc)
{
	...
	if ((dasmresult & DASMFLAG_SUPPORTED) != 0 && (dasmresult & DASMFLAG_STEP_OVER) != 0)
	{
		...
		pc = buffer.next_pc_wrap(pc, dasmresult & DASMFLAG_LENGTHMASK);

		// if we need to skip additional instructions, advance as requested
		while (extraskip-- > 0) {
			u32 result = buffer.disassemble_info(pc);
			pc += buffer.next_pc_wrap(pc, result & DASMFLAG_LENGTHMASK);
		}
		m_stepaddr = pc;
	}
...
void device_debug::tracer::update(offs_t pc)
{
	...
	{
		int extraskip = (dasmresult & DASMFLAG_OVERINSTMASK) >> DASMFLAG_OVERINSTSHIFT;
		offs_t trace_over_target = buffer.next_pc_wrap(pc, dasmresult & DASMFLAG_LENGTHMASK);

		// if we need to skip additional instructions, advance as requested
		while (extraskip-- > 0)
			trace_over_target = buffer.next_pc_wrap(trace_over_target, buffer.disassemble_info(trace_over_target) & DASMFLAG_LENGTHMASK);

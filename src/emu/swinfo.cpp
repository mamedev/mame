...
public:
	...
	softlist_parser(parsed_software_list &list, std::ostringstream &errors);
...
{
	// if filter is null, assume compatible
	if (filter == nullptr)
...
software_info::software_info(parsed_software_list &list, std::string &&name, std::string &&parent, const char *supported)
	: m_next(nullptr),
		m_list(list),
...
//**************************************************************************
//  PARSED SOFTWARE LIST
//**************************************************************************

//-------------------------------------------------
//  parsed_software_list - constructor
//-------------------------------------------------

parsed_software_list::parsed_software_list(const char *path, const char *list_name, bool report_not_found)
	: m_file(path, OPEN_FLAG_READ)
{
	// attempt to open the file
	osd_file::error filerr = m_file.open(list_name, ".xml");
	...
	else if (report_not_found)
		m_errors = string_format("Error opening file: %s\n", filename());
}


//-------------------------------------------------
//  parsed_software_list - destructor
//-------------------------------------------------

parsed_software_list::~parsed_software_list()
{
}
...
const software_info *parsed_software_list::find(const char *look_for) const
{
	assert(look_for != nullptr);
...
void parsed_software_list::internal_validity_check(validity_checker &valid)
{
	...
	for (const software_info &swinfo : get_info())
	{
		...
		for (const software_part &part : swinfo.parts())
		{
			...
			const char *requirement = part.feature("requirement");
			if (requirement != nullptr)
			{
				if (strchr(requirement, ':') == nullptr && find(requirement) == nullptr)
					osd_printf_error("%s: %s has a part (%s) whose requirement (%s) is not in the list\n", filename(), swinfo.shortname().c_str(), part.name().c_str(), requirement);
			}
		}
	}
}
...
softlist_parser::softlist_parser(parsed_software_list &list, std::ostringstream &errors)
	: m_list(list),
		m_errors(errors),
		m_done(false),

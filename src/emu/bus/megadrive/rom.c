...
{
	if (offset < 0x400000/2)
		return m_rom[(((m_bank * 0x10000) + (offset << 1)) & (m_rom.count() - 1))/2];
	else
...
#define MD_LION3_ADDR(a)  (((offset << 1) | (m_bank << 15)) & (m_rom.count() - 1))/2
...
#define MD_POKESTAD_ADDR(a)  (((offset << 1) | (m_bank << 15)) & (m_rom.count() - 1))/2

...
READ16_MEMBER( dmac_device::register_read )
{
	...
	case 0x48:
		data = m_scsi_read_handler(0);

		if (VERBOSE)
			logerror("%s('%s'): read scsi status %04x [mask = %04x]\n", shortname(), basetag(), data, mem_mask);

		break;

	case 0x49:
		data = m_scsi_read_handler(1);

		if (VERBOSE)
			logerror("%s('%s'): read scsi scmd %04x [mask = %04x]\n", shortname(), basetag(), data, mem_mask);
...
WRITE16_MEMBER( dmac_device::register_write )
{
	...
	case 0x48:
		if (VERBOSE)
			logerror("%s('%s'): write scsi sasr %04x [mask = %04x]\n", shortname(), basetag(), data, mem_mask);

		m_scsi_write_handler(0, data, 0xff);
		break;

	case 0x49:
		if (VERBOSE)
			logerror("%s('%s'): write scsi scmd %04x [mask = %04x]\n", shortname(), basetag(), data, mem_mask);

		m_scsi_write_handler(1, data, 0xff);
		break;

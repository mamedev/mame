#ifndef __AES_128_ECB_H__
#define __AES_128_ECB_H__
...
void AES_Encrypt(AES_CTX *ctx, const unsigned char *in_data, unsigned int in_size, unsigned char *out_data) {
	...
	for (offset = 0; offset < in_size; offset += AES_BLOCK_SIZE) {
		// Initial round
...
void AES_CTX_Free(AES_CTX *ctx) {
	for (unsigned char index = 0; index < 44; index++) {
		ctx->roundkey[index] = 0;
	}
}

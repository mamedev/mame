#ifndef __AES_192_ECB_H__
#define __AES_192_ECB_H__
...
void AES_EncryptInit(AES_CTX *ctx, const unsigned char *key) {
	...
	for (unsigned char index = 6; index < 52; index += 6) {
		ctx->roundkey[index] = ctx->roundkey[index - 6] ^
			...
			(Te4[(ctx->roundkey[index - 1] >> 24) & 0xff] & 0x000000ff) ^ rcon[index / 6 - 1];
		ctx->roundkey[index + 1] = ctx->roundkey[index - 5] ^ ctx->roundkey[index];
		ctx->roundkey[index + 2] = ctx->roundkey[index - 4] ^ ctx->roundkey[index + 1];
		ctx->roundkey[index + 3] = ctx->roundkey[index - 3] ^ ctx->roundkey[index + 2];

		if (index == 48) {
			break;
		}

		ctx->roundkey[index + 4] = ctx->roundkey[index - 2] ^ ctx->roundkey[index + 3];
		ctx->roundkey[index + 5] = ctx->roundkey[index - 1] ^ ctx->roundkey[index + 4];
	}
}
...
void AES_DecryptInit(AES_CTX *ctx, const unsigned char *key) {
	...
	for (unsigned char i = 0, j = 48; i < j; i += 4, j -= 4) {
    ...
    for (unsigned char index = 4; index < 48; index += 4) {
        ctx->roundkey[index] =
...
void AES_Encrypt(const AES_CTX *ctx, const unsigned char in_data[AES_BLOCK_SIZE], unsigned char out_data[AES_BLOCK_SIZE]) {
    ...
    // Final round
    s0 = (Te4[(t0 >> 24) & 0xff] & 0xff000000) ^ (Te4[(t1 >> 16) & 0xff] & 0x00ff0000) ^ (Te4[(t2 >>  8) & 0xff] & 0x0000ff00) ^ (Te4[(t3 >>  0) & 0xff] & 0x000000ff) ^ ctx->roundkey[48];
	s1 = (Te4[(t1 >> 24) & 0xff] & 0xff000000) ^ (Te4[(t2 >> 16) & 0xff] & 0x00ff0000) ^ (Te4[(t3 >>  8) & 0xff] & 0x0000ff00) ^ (Te4[(t0 >>  0) & 0xff] & 0x000000ff) ^ ctx->roundkey[49];
	s2 = (Te4[(t2 >> 24) & 0xff] & 0xff000000) ^ (Te4[(t3 >> 16) & 0xff] & 0x00ff0000) ^ (Te4[(t0 >>  8) & 0xff] & 0x0000ff00) ^ (Te4[(t1 >>  0) & 0xff] & 0x000000ff) ^ ctx->roundkey[50];
	s3 = (Te4[(t3 >> 24) & 0xff] & 0xff000000) ^ (Te4[(t0 >> 16) & 0xff] & 0x00ff0000) ^ (Te4[(t1 >>  8) & 0xff] & 0x0000ff00) ^ (Te4[(t2 >>  0) & 0xff] & 0x000000ff) ^ ctx->roundkey[51];
...
void AES_Decrypt(const AES_CTX *ctx, const unsigned char out_data[AES_BLOCK_SIZE], unsigned char in_data[AES_BLOCK_SIZE]) {
    unsigned int s0, s1, s2, s3, t0, t1, t2, t3;

    s0 = GETU32(out_data + 0) ^ ctx->roundkey[0];
    s1 = GETU32(out_data + 4) ^ ctx->roundkey[1];
    s2 = GETU32(out_data + 8) ^ ctx->roundkey[2];
    s3 = GETU32(out_data + 12) ^ ctx->roundkey[3];
    ...
    // Final round
    s0 = (Td4[(t0 >> 24) & 0xff] & 0xff000000) ^ (Td4[(t3 >> 16) & 0xff] & 0x00ff0000) ^ (Td4[(t2 >>  8) & 0xff] & 0x0000ff00) ^ (Td4[(t1 >>  0) & 0xff] & 0x000000ff) ^ ctx->roundkey[48];
    s1 = (Td4[(t1 >> 24) & 0xff] & 0xff000000) ^ (Td4[(t0 >> 16) & 0xff] & 0x00ff0000) ^ (Td4[(t3 >>  8) & 0xff] & 0x0000ff00) ^ (Td4[(t2 >>  0) & 0xff] & 0x000000ff) ^ ctx->roundkey[49];
    s2 = (Td4[(t2 >> 24) & 0xff] & 0xff000000) ^ (Td4[(t1 >> 16) & 0xff] & 0x00ff0000) ^ (Td4[(t0 >>  8) & 0xff] & 0x0000ff00) ^ (Td4[(t3 >>  0) & 0xff] & 0x000000ff) ^ ctx->roundkey[50];
    s3 = (Td4[(t3 >> 24) & 0xff] & 0xff000000) ^ (Td4[(t2 >> 16) & 0xff] & 0x00ff0000) ^ (Td4[(t1 >>  8) & 0xff] & 0x0000ff00) ^ (Td4[(t0 >>  0) & 0xff] & 0x000000ff) ^ ctx->roundkey[51];

    // Map the decrypted state to a byte array block
    PUTU32(in_data, s0);
    PUTU32(in_data + 4, s1);
    PUTU32(in_data + 8, s2);
    PUTU32(in_data + 12, s3);
}

void AES_CTX_Free(AES_CTX *ctx) {
	for (unsigned char index = 0; index < 52; index++) {

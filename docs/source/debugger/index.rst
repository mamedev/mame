...
To facilitate source-level debugging in such a situation, where code may be swapped in
and out of a range of logical addresses, you may issue debugger commands to let MAME
know when that swapping occurs. If you automate the issuing of these commands
with a LUA script, the end result is an intelligent debugging experience
...
as the argument to :ref:`-src_debug_info <mame-commandline-srcdbginfo>`
and you're good. Even in cases when you have multiple assembly
files which you assemble to obj files, and then link together, you would still specify
that single combined MDI file generated by the linker.
...
a single MDI file path to :ref:`-src_debug_info <mame-commandline-srcdbginfo>`,
...
files instead: ``-src_debug_info c:\project\BlocksA.mdi;c:\project\BlocksB.mdi;c:\project\BlocksC.mdi``.

From the debugger console window, you can use the
:ref:`sdlist <debugger-command-sdlist>`, :ref:`sdenable <debugger-command-sdenable>`,
and :ref:`sddisable <debugger-command-sddisable>` commands to choose which
subset of MDIs to enable at any given time.  These commands dynamically adjust which source
...
addresses corresponding to the emulated computer's MMU's hardware registers, and then issues the appropriate 
:ref:`sdenable <debugger-command-sdenable>` and
:ref:`sddisable <debugger-command-sddisable>` commands to ensure only
...
When using the MAME debugger tracing feature, original source will automatically appear next to the
disassembly when available. So long as source-level debugging is enabled, nothing extra needs to be
done for this to work.  Once the :ref:`trace <debugger-command-trace>` command initiates tracing,
for each line traced, MAME will first perform the specified trace actions
(e.g., :ref:`tracelog <debugger-command-tracelog>` command), then print the corresponding
disassembly, and finally print the original source line corresponding with the traced address.
If an instruction is traced for which there is no corresponding source, then only the trace action
and disassembly are traced as usual.

As with the source-level debugging view in the debugger GUI, tabs from the source are first
converted into spaces.  This may be customized with the
:ref:`src_debug_spaces_per_tab <mame-commandline-srcdbgtabs>` option.

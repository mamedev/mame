...
# NOTE: The genie support for xcode is not as fleshed out as other generators, so there
#       are a few issues:
#
#       The generator does not support custom build tasks, so xcode will fail to build
#       and find autogenerated files like `drivlist.cpp`. To workaround this issue
#       build first with the generic makefile generator.
#
#       There seem to be issues with relative path handling: the generated xcode project
#       will clobber the repository with some build files placed incorrectly.
#
#       Additionally, the path of `src/mame/mame.cpp` is wrongly computed, and this script
#       works around it with the `sed` hack.

.PHONY: xcode
xcode: generate
	$(SILENT) $(GENIE) $(PARAMS) $(TARGET_PARAMS) xcode15
	$(SILENT) sed -I bak "s/path = \"\(\.\.\/\)\{4\}src\";/path = \"..\/..\/\..\/..\/\..\/src\";/" build/projects/sdl/mame/xcode15/mame.xcodeproj/project.pbxproj
